...
def assess_ml_quality(dataset: pd.DataFrame, scores: pd.DataFrame, timeframe: str) -> dict[str, Any]:
    ...
    forbidden_feature_columns = [
        column
        for column in ALLOWED_FEATURE_COLUMNS_V2_8
        if any(term in column.casefold() for term in FORBIDDEN_FEATURE_TERMS_V2_8)
    ]
    forbidden_output_columns = [
        column
        for column in scores.columns
        if any(term in column.casefold() for term in FORBIDDEN_OUTPUT_TERMS_V2_8)
    ]
    errors: list[str] = []
    if forbidden_feature_columns:
        errors.append(f"V2.8 forbidden feature columns for {timeframe}: {forbidden_feature_columns}")
    if forbidden_output_columns:
        errors.append(f"V2.8 forbidden output columns for {timeframe}: {forbidden_output_columns}")
    if TARGET_NAME != "up_down_flat_h1":
        errors.append("V2.8 target name invalid")
    split_valid = _split_temporal_order_valid(used)
    if len(used) > 0 and not split_valid:
        errors.append(f"V2.8 split temporal order invalid for {timeframe}")
    return {
