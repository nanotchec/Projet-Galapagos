...
def main() -> None:
    ...
    with tempfile.TemporaryDirectory(prefix="galapagos_v2_8_smoke_") as tmp:
        tmp_path = Path(tmp)
    ...
    Path("reports/zip_smoke_test_v2_8.json").write_text(json.dumps(payload, indent=2, ensure_ascii=False) + "\n", encoding="utf-8")
    Path("reports/zip_smoke_test_v2_8.md").write_text(
        "# Smoke ZIP V2.8\n\n"
