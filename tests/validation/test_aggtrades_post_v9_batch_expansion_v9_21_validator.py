...
from galapagos.data.aggtrades_post_v9_batch_expansion_v9_21_validation import (
    validate_manifest_payload_v9_21,
    validate_markdown_v9_21,
    validate_batch_scope_v9_21,
    validate_report_payload_v9_21,
    validate_source_design_v9_21,
)
...
def _report() -> dict:
    ...
    return {
        ...
        "batch_window": {"start": "2024-06-11", "end": "2024-08-09", "max_downloads": 60, "days_requested": 60, "requested_dates": []},
        "collection_result": {
        ...
        "v9_21_decision": {
            ...
            "next_recommendation": "V9.22 - AggTrades Post-V9 Multi-Batch Completion Plan.",
            "collection_executed": True,
        },
        "collection_executed": True,
        "network_used": True,
...
def test_validator_accepts_valid_batch_collect_report_v9_21() -> None:
    assert validate_report_payload_v9_21(_report()) == []


def test_validator_rejects_private_source_host_v9_21() -> None:
    source = _source_design()
    source["host"] = "api.binance.com"

    errors = validate_source_design_v9_21(source)

    assert "V9.21 source host must be data.binance.vision" in errors


def test_validator_rejects_collect_without_recorded_limit_v9_21() -> None:
    report = _report()
    report["batch_window"]["max_downloads"] = None

    errors = validate_batch_scope_v9_21(report)

    assert "V9.21 collect mode must record max_downloads" in errors


def test_validator_rejects_more_than_sixty_batch_days_v9_21() -> None:
    report = _report()
    ...
    assert "V9.21 batch must not exceed 60 requested days" in errors


def test_validator_rejects_full_collection_claim_v9_21() -> None:
    report = _report()
    report["complete_collection_reached"] = True

    errors = validate_batch_scope_v9_21(report)

    assert "V9.21 must not mark full collection complete" in errors


def test_validator_rejects_feature_creation_v9_21() -> None:
    report = _report()
    report["features_created"] = True

    errors = validate_report_payload_v9_21(report)

    assert "V9.21 must keep features_created=false" in errors


def test_validator_rejects_wrong_download_scope_v9_21() -> None:
    report = _report()
    report["safety_flags"]["new_data_download_scope"] = "full_window"

    errors = validate_report_payload_v9_21(report)

    assert "V9.21 collect must limit download scope to public_historical_aggtrades_batch_expansion_only" in errors


def test_validator_rejects_manifest_sidecar_field_v9_21() -> None:
    report = _report()
    ...
    errors = validate_manifest_payload_v9_21(manifest, report)

    assert "V9.21 manifest must not contain sidecar or ZIP hash fields" in errors


def test_validator_rejects_markdown_forbidden_metric_v9_21() -> None:
    ...
    errors = validate_markdown_v9_21(text)
