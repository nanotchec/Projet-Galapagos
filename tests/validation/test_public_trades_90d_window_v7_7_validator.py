...
from galapagos.data.public_trades.ninety_day_window import safety_flags_v7_7
...
def test_validator_v7_7_accepts_valid_public_trades(tmp_path: Path) -> None:
    _write_fixture(tmp_path)

    result = validate_public_trades_90d_window_v7_7(tmp_path)

    assert result["passed"] is True
    assert result["errors"] == []


def test_validator_v7_7_rejects_missing_raw_zip(tmp_path: Path) -> None:
...
def test_validator_v7_7_rejects_wrong_raw_checksum(tmp_path: Path) -> None:
    paths = _write_fixture(tmp_path)
...
def test_validator_v7_7_rejects_duplicate_aggregate_trade_id_even_with_synced_checksum(tmp_path: Path) -> None:
    paths = _write_fixture(tmp_path)
...
def test_validator_v7_7_rejects_non_monotonic_aggregate_trade_id_even_with_synced_checksum(tmp_path: Path) -> None:
    paths = _write_fixture(tmp_path)
    frame = pd.read_parquet(paths["partition_by_date"]["2023-03-25"], engine="pyarrow")
...
def test_validator_v7_7_rejects_non_positive_price_even_with_synced_checksum(tmp_path: Path) -> None:
    paths = _write_fixture(tmp_path)
    frame = pd.read_parquet(paths["partition_by_date"]["2023-03-25"], engine="pyarrow")
...
def test_validator_v7_7_rejects_non_positive_quantity_even_with_synced_checksum(tmp_path: Path) -> None:
    paths = _write_fixture(tmp_path)
    frame = pd.read_parquet(paths["partition_by_date"]["2023-03-25"], engine="pyarrow")
...
def test_validator_v7_7_rejects_extra_signal_column_even_with_synced_checksum(tmp_path: Path) -> None:
    paths = _write_fixture(tmp_path)
    frame = pd.read_parquet(paths["partition_by_date"]["2023-03-25"], engine="pyarrow")
    frame["signal"] = "none"
    _sync_partition(paths, "2023-03-25", frame)
...
def test_validator_v7_7_rejects_extra_order_column_even_with_synced_checksum(tmp_path: Path) -> None:
    paths = _write_fixture(tmp_path)
    frame = pd.read_parquet(paths["partition_by_date"]["2023-03-25"], engine="pyarrow")
    frame["order"] = "none"
    _sync_partition(paths, "2023-03-25", frame)
...
def test_validator_v7_7_rejects_extra_pnl_column_even_with_synced_checksum(tmp_path: Path) -> None:
    paths = _write_fixture(tmp_path)
    frame = pd.read_parquet(paths["partition_by_date"]["2023-03-25"], engine="pyarrow")
    frame["pnl"] = 0.0
    _sync_partition(paths, "2023-03-25", frame)
...
def test_validator_v7_7_rejects_report_json_lie(tmp_path: Path) -> None:
    paths = _write_fixture(tmp_path)
...
def test_validator_v7_7_rejects_manifest_unexpected_key(tmp_path: Path) -> None:
    paths = _write_fixture(tmp_path)
    manifest = _load_json(paths["manifest"])
...
def test_validator_v7_7_rejects_report_unexpected_key(tmp_path: Path) -> None:
    paths = _write_fixture(tmp_path)
    report = _load_json(paths["report"])
...
def test_validator_v7_7_rejects_markdown_strategy_validated_claim(tmp_path: Path) -> None:
    paths = _write_fixture(tmp_path)
...
def test_validator_v7_7_rejects_safety_flag_trading_true(tmp_path: Path) -> None:
    paths = _write_fixture(tmp_path)
    manifest = _load_json(paths["manifest"])
...
def test_validator_v7_7_rejects_features_v7_7_directory_created(tmp_path: Path) -> None:
    _write_fixture(tmp_path)
    forbidden = tmp_path / "data/research/v7_7/features/feature.parquet"
    forbidden.parent.mkdir(parents=True)
    forbidden.write_text("x", encoding="utf-8")

    result = validate_public_trades_90d_window_v7_7(tmp_path)

    assert _contains(result["errors"], "Forbidden V7.7 artifact detected")


def test_validator_v7_7_rejects_labels_v7_7_directory_created(tmp_path: Path) -> None:
    _write_fixture(tmp_path)
    forbidden = tmp_path / "data/research/v7_7/labels/labels.parquet"
    forbidden.parent.mkdir(parents=True)
    forbidden.write_text("x", encoding="utf-8")
...
def test_validator_v7_7_rejects_dataset_ml_v7_7_directory_created(tmp_path: Path) -> None:
    _write_fixture(tmp_path)
    forbidden = tmp_path / "data/research/v7_7/datasets/dataset.parquet"
    forbidden.parent.mkdir(parents=True)
    forbidden.write_text("x", encoding="utf-8")
...
def test_validator_v7_7_rejects_backtest_report_created(tmp_path: Path) -> None:
    _write_fixture(tmp_path)
...
def _write_fixture(root: Path) -> dict:
    ...
    for offset, date_key in enumerate(_dates()):
        ...
        output = output_partition_path_v7_7(root, "2023-03-25", "2023-06-22", date_key)
    ...
    paths = {
        ...
        "manifest": root / MANIFEST_PATH_V7_7,
        "report": root / REPORT_JSON_PATH_V7_7,
        "report_md": root / REPORT_MD_PATH_V7_7,
        "doc": root / DOC_PATH_V7_7,
    }
    _write_manifest_and_report(paths, manifest)
    paths["report_md"].parent.mkdir(parents=True, exist_ok=True)
    paths["report_md"].write_text("V7.7 ne valide aucune strategie.\n", encoding="utf-8")
    paths["doc"].parent.mkdir(parents=True, exist_ok=True)
    paths["doc"].write_text("V7.7 ne produit aucun backtest.\n", encoding="utf-8")
    return paths
...
def _manifest_payload(root: Path, raw_files: dict[str, dict], partitions: dict[str, dict]) -> dict:
    ...
    quality = assess_ninety_day_agg_trade_partitions_v7_7(root, partitions, expected_days=90, missing_dates=[])
    return {
        "version": "V7.7",
        "status": "PASS",
...
def _dates() -> list[str]:
    ...
    return [(start + timedelta(days=offset)).isoformat() for offset in range(90)]
