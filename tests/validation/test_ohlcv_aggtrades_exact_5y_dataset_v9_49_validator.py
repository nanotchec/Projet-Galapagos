...
from galapagos.datasets.ohlcv_aggtrades_exact_5y_dataset_v9_49_schemas import EXPECTED_ROWS, SAFETY_FLAGS, SELECTED_PRIMARY_LABEL
...
def test_validator_accepts_complete_created_report(tmp_path: Path) -> None:
    _write_report_bundle(tmp_path, decision="combined_features_5y_dataset_created_with_warnings")

    result = validate_v9_49_report(tmp_path)

    assert result["passed"] is True
    assert result["decision"] == "combined_features_5y_dataset_created_with_warnings"
    assert result["dataset_created"] is True


def test_validator_rejects_network_usage(tmp_path: Path) -> None:
    report = _write_report_bundle(tmp_path, decision="combined_features_5y_dataset_created")
    report["network_used"] = True
    report["safety_flags"]["network_used"] = True
    _write_json(tmp_path / "reports/datasets/ohlcv_aggtrades_exact_5y_dataset_v9_49.json", report)
...
def test_validator_audit_lite_does_not_require_full_dataset_outputs(tmp_path: Path) -> None:
    report = _write_report_bundle(tmp_path, decision="combined_features_5y_dataset_created")
    for item in report["outputs"].values():
        (tmp_path / item["path"]).unlink()

    result = validate_v9_49_report(tmp_path, audit_lite=True)
...
def _write_report_bundle(tmp_path: Path, *, decision: str) -> dict:
    ...
    for timeframe in EXPECTED_ROWS:
        path = tmp_path / f"data/research/v9_49/datasets/{timeframe}/dataset.parquet"
    ...
    manifest = {
        ...
        "report_path": "reports/datasets/ohlcv_aggtrades_exact_5y_dataset_v9_49.json",
    }
    _write_json(tmp_path / "reports/datasets/ohlcv_aggtrades_exact_5y_dataset_v9_49.json", report)
    _write_json(tmp_path / "reports/manifests/ohlcv_aggtrades_exact_5y_dataset_v9_49_manifest.json", manifest)
    return report
