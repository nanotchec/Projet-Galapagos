...
from galapagos.data.aggtrades_post_v9_batch_collection_v9_20_validation import (
    validate_manifest_payload_v9_20,
    validate_markdown_v9_20,
    validate_batch_scope_v9_20,
    validate_report_payload_v9_20,
    validate_source_design_v9_20,
)
...
def _report() -> dict:
    ...
    return {
        ...
        "batch_window": {"start": "2024-05-12", "end": "2024-06-10", "max_downloads": 30, "days_requested": 30, "requested_dates": []},
        "collection_result": {
        ...
        "v9_20_decision": {
            ...
            "next_recommendation": "V9.21 - AggTrades Post-V9 Batch Collection Expansion.",
            "collection_executed": True,
        },
        "collection_executed": True,
        "network_used": True,
...
def test_validator_accepts_valid_batch_collect_report_v9_20() -> None:
    assert validate_report_payload_v9_20(_report()) == []


def test_validator_rejects_private_source_host_v9_20() -> None:
    source = _source_design()
    source["host"] = "api.binance.com"

    errors = validate_source_design_v9_20(source)

    assert "V9.20 source host must be data.binance.vision" in errors


def test_validator_rejects_collect_without_recorded_limit_v9_20() -> None:
    report = _report()
    report["batch_window"]["max_downloads"] = None

    errors = validate_batch_scope_v9_20(report)

    assert "V9.20 collect mode must record max_downloads" in errors


def test_validator_rejects_more_than_thirty_batch_days_v9_20() -> None:
    report = _report()
    report["batch_window"]["days_requested"] = 31
    report["batch_validation"]["summary"]["days_requested"] = 31

    errors = validate_batch_scope_v9_20(report)

    assert "V9.20 batch must not exceed 30 requested days" in errors


def test_validator_rejects_full_collection_claim_v9_20() -> None:
    report = _report()
    report["complete_collection_reached"] = True

    errors = validate_batch_scope_v9_20(report)

    assert "V9.20 must not mark full collection complete" in errors


def test_validator_rejects_feature_creation_v9_20() -> None:
    report = _report()
    report["features_created"] = True

    errors = validate_report_payload_v9_20(report)

    assert "V9.20 must keep features_created=false" in errors


def test_validator_rejects_wrong_download_scope_v9_20() -> None:
    report = _report()
    report["safety_flags"]["new_data_download_scope"] = "full_window"

    errors = validate_report_payload_v9_20(report)

    assert "V9.20 collect must limit download scope to public_historical_aggtrades_batch_only" in errors


def test_validator_rejects_manifest_sidecar_field_v9_20() -> None:
    report = _report()
    ...
    errors = validate_manifest_payload_v9_20(manifest, report)

    assert "V9.20 manifest must not contain sidecar or ZIP hash fields" in errors


def test_validator_rejects_markdown_forbidden_metric_v9_20() -> None:
    ...
    errors = validate_markdown_v9_20(text)
