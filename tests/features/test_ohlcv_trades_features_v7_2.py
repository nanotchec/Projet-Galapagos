...
@pytest.fixture(scope="session")
def v7_2_manifest(project_root: Path) -> dict:
    return json.loads((project_root / MANIFEST_PATH_V7_2).read_text(encoding="utf-8"))


@pytest.fixture(scope="session")
def feature_paths(project_root: Path) -> dict[str, Path]:
    return {
        timeframe: output_path(project_root, timeframe, WINDOW_START_V7_2, WINDOW_END_V7_2)
...
def test_ohlcv_trades_features_row_count_matches_expected_bars(feature_paths: dict[str, Path]) -> None:
    for timeframe, path in feature_paths.items():
        assert pq.ParquetFile(path).metadata.num_rows == EXPECTED_ROWS_V7_2[timeframe]


def test_ohlcv_trades_features_strict_columns(feature_paths: dict[str, Path]) -> None:
    for path in feature_paths.values():
        assert pq.ParquetFile(path).schema.names == OHLCV_TRADES_FEATURE_COLUMNS_V7_2


def test_ohlcv_trades_aggregation_has_positive_counts(frame_1m: pd.DataFrame) -> None:
...
def test_ohlcv_trades_taker_buy_ratios_bounded(frame_1m: pd.DataFrame) -> None:
...
def test_ohlcv_trades_imbalance_bounded(frame_1m: pd.DataFrame) -> None:
    for column in ["taker_imbalance_count", "taker_imbalance_quantity", "taker_imbalance_quote"]:
        clean = frame_1m[column].dropna()
        assert ((clean >= -1.0) & (clean <= 1.0)).all()


def test_ohlcv_trades_feature_available_ts_not_before_available_ts(feature_paths: dict[str, Path]) -> None:
    for path in feature_paths.values():
...
def test_ohlcv_trades_decision_ts_not_before_feature_available_ts(feature_paths: dict[str, Path]) -> None:
    for path in feature_paths.values():
...
def test_ohlcv_trades_no_forbidden_feature_columns(frame_1h: pd.DataFrame) -> None:
...
def test_ohlcv_trades_volume_consistency_metrics_present(v7_2_manifest: dict) -> None:
    for timeframe in TIMEFRAMES_V7_2:
        quality = v7_2_manifest["quality"][timeframe]
...
def test_ohlcv_trades_source_hashes_match_inputs(
    project_root: Path,
    feature_paths: dict[str, Path],
    v7_2_manifest: dict,
) -> None:
    v5_manifest = load_v5_0_ohlcv_manifest(project_root)
    trades_manifest_sha = sha256_file(project_root / "reports/manifests/public_trades_expanded_window_v7_1_manifest.json")
    for timeframe, path in feature_paths.items():
        ...
        assert v7_2_manifest["input_ohlcv"][timeframe]["sha256"] == expected_ohlcv_sha
