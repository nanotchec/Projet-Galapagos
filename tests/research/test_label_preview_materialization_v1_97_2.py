...
from galapagos.research.label_preview_materialization.label_preview_builder import ALLOWED_ROOT, EXPECTED_FILES, LabelPreviewBuilder
...
def _write(path: Path, payload: dict) -> None:
    ...
    if path.suffix == ".json" and "label_approval_decision_v1_96_1" not in str(path):
        path.with_suffix(".md").write_text(f"# {path.stem}\n", encoding="utf-8")


def _ensure_placeholder_reports(work: Path) -> None:
    reader = FeaturePreviewReader(work)
    LabelPreviewBuilder(work).write(reader.read_feature_payloads())
    physical = LabelPreviewPhysicalAuditor(work).audit()
    feature = reader.audit_feature_preview()
    seed = reader.audit_seed()
    summary = {
        "version": "V1.97.2",
        "version_suffix": V_SUFFIX,
        "corrective_for_version": "V1.97.1",
        "previous_validated_version": "V1.96.1",
        "final_verdict": "V1_97_2_LABEL_AVAILABILITY_CAUSALITY_FIX_PASSED",
    ...
    for rel in [
        ...
        f"reports/research/label_preview_materialization_file_audit_{V_SUFFIX}.json",
        f"reports/research/label_preview_materialization_semantic_audit_{V_SUFFIX}.json",
        f"reports/research/label_preview_materialization_timestamp_audit_{V_SUFFIX}.json",
        f"reports/research/label_preview_materialization_safety_check_{V_SUFFIX}.json",
        f"reports/research/label_preview_materialization_consistency_check_{V_SUFFIX}.json",
        f"reports/research/{V_SUFFIX}_recommendation.json",
        "reports/current/latest_metrics.json",
        ...
        _write(work / rel, summary)
    _write(work / f"reports/release_zip_{V_SUFFIX}.json", {"version": "V1.97.2", "release_zip_created": True, "final_zip_created": True, "release_ready_for_external_review": True, "clean_zip_ready_for_external_review": True, "final_audit_passed": True, "final_smoke_passed": True, "blocking_reason": None})
    _write(work / f"reports/zip_audit_{V_SUFFIX}.json", {"version": "V1.97.2", "clean_zip_ready_for_external_review": True, "audit_zip_project_state_version": "V1.97.2", "audit_zip_version_parse_correct": True, "global_json_finiteness_passed": True, "missing_required_files": [], "forbidden_count": 0})
    _write(work / f"reports/zip_smoke_test_{V_SUFFIX}.json", {"version": "V1.97.2", "smoke_test_passed": True, "smoke_failed_count": 0, "smoke_passed_count": 3, "smoke_commands_count": 3, "smoke_commands_not_empty": True, "bounded_smoke_for_v1_97_2": True, "real_orders_possible": False, "codex_cli_called": False, "holdout_executed": False})
    _write(work / "reports/research/label_approval_decision_v1_96_1.json", {"version": "V1.96.1", "human_approval_granted": True, "v1_97_authorized": True})
    (work / "docs").mkdir(parents=True, exist_ok=True)
    (work / f"docs/code_review_{V_SUFFIX}.md").write_text("# Code Review V1.97.2\n", encoding="utf-8")
    (work / f"docs/label_preview_materialization_{V_SUFFIX}.md").write_text("# Label Preview V1.97.2\n", encoding="utf-8")
    (work / "reports/REPORT_INDEX.md").parent.mkdir(parents=True, exist_ok=True)
    (work / "reports/REPORT_INDEX.md").write_text("V1.97.2 v1_97_2\n", encoding="utf-8")
    test_target = work / f"tests/research/test_label_preview_materialization_{V_SUFFIX}.py"
...
def _recompute_label_manifest(work: Path) -> None:
    import hashlib

    manifest_path = work / ALLOWED_ROOT / "label_preview_manifest.json"
    manifest = _load(manifest_path)
    checksums = {}
    for name in EXPECTED_FILES:
        if name == "label_preview_manifest.json":
            continue
        checksums[name] = hashlib.sha256((work / ALLOWED_ROOT / name).read_bytes()).hexdigest()
    manifest["label_preview_file_checksums"] = checksums
    _write(manifest_path, manifest)


def _mutate_label_row(work: Path, mutation) -> None:
    rows_path = work / ALLOWED_ROOT / "label_preview_rows.json"
    payload = _load(rows_path)
    mutation(payload["rows"][0])
    _write(rows_path, payload)
    _recompute_label_manifest(work)
...
def test_label_available_ts_computed_from_each_source_decision_ts(tmp_path: Path) -> None:
    work = _copy_tree(tmp_path)
    rows = _load(work / ALLOWED_ROOT / "label_preview_rows.json")["rows"]
    observed = [row["label_available_ts"] for row in rows]
    assert observed == [
        "2026-01-01T00:05:01Z",
        "2026-01-01T01:05:01Z",
        "2026-01-01T02:05:01Z",
    ]


def test_rejects_label_available_ts_before_source_decision_ts_even_with_recomputed_manifest(tmp_path: Path) -> None:
    work = _copy_tree(tmp_path)
    _mutate_label_row(work, lambda row: row.update({"label_available_ts": "2025-12-31T23:59:59Z"}))
    assert validate_report_set(work, V_SUFFIX)


def test_rejects_label_available_ts_equal_to_source_decision_ts_even_with_recomputed_manifest(tmp_path: Path) -> None:
    work = _copy_tree(tmp_path)
    _mutate_label_row(work, lambda row: row.update({"label_available_ts": row["source_decision_ts"]}))
    assert validate_report_set(work, V_SUFFIX)


def test_rejects_label_available_ts_before_decision_plus_horizon_even_with_recomputed_manifest(tmp_path: Path) -> None:
    work = _copy_tree(tmp_path)
    _mutate_label_row(work, lambda row: row.update({"label_available_ts": "2026-01-01T00:04:59Z"}))
    assert validate_report_set(work, V_SUFFIX)


def test_rejects_missing_source_decision_ts(tmp_path: Path) -> None:
    work = _copy_tree(tmp_path)
    _mutate_label_row(work, lambda row: row.pop("source_decision_ts"))
    assert validate_report_set(work, V_SUFFIX)


def test_rejects_missing_label_available_ts(tmp_path: Path) -> None:
    work = _copy_tree(tmp_path)
    _mutate_label_row(work, lambda row: row.pop("label_available_ts"))
    assert validate_report_set(work, V_SUFFIX)


def test_rejects_missing_horizon_seconds(tmp_path: Path) -> None:
    work = _copy_tree(tmp_path)
    _mutate_label_row(work, lambda row: row.pop("horizon_seconds"))
    assert validate_report_set(work, V_SUFFIX)


def test_rejects_invalid_source_decision_ts(tmp_path: Path) -> None:
    work = _copy_tree(tmp_path)
    _mutate_label_row(work, lambda row: row.update({"source_decision_ts": "invalid"}))
    assert validate_report_set(work, V_SUFFIX)


def test_rejects_invalid_label_available_ts(tmp_path: Path) -> None:
    work = _copy_tree(tmp_path)
    _mutate_label_row(work, lambda row: row.update({"label_available_ts": "invalid"}))
    assert validate_report_set(work, V_SUFFIX)


def test_reports_label_timestamp_violation_details(tmp_path: Path) -> None:
    work = _copy_tree(tmp_path)
    _mutate_label_row(work, lambda row: row.update({"label_available_ts": row["source_decision_ts"]}))
    audit = LabelPreviewPhysicalAuditor(work).audit()
    assert audit["label_timestamp_violations_count"] >= 1
    first = audit["label_timestamp_violations"][0]
    assert {"row_index", "field_pair", "left_value", "right_value", "rule"} <= set(first)


def test_validator_rejects_label_timestamp_order_valid_false(tmp_path: Path) -> None:
    work = _copy_tree(tmp_path)
    _mutate_summary(work, "label_timestamp_order_valid", False)
    assert validate_report_set(work, V_SUFFIX)


def test_validator_rejects_label_timestamp_violations_detected_true(tmp_path: Path) -> None:
    work = _copy_tree(tmp_path)
    _mutate_summary(work, "label_timestamp_violations_detected", True)
    assert validate_report_set(work, V_SUFFIX)


def test_validator_rejects_label_timestamp_violations_count_positive(tmp_path: Path) -> None:
    work = _copy_tree(tmp_path)
    _mutate_summary(work, "label_timestamp_violations_count", 1)
    assert validate_report_set(work, V_SUFFIX)
...
def test_validator_rejects_fast_tests_false(tmp_path: Path) -> None:
    work = _copy_tree(tmp_path)
    _mutate_summary(work, "fast_tests_for_v1_97_2", False)
    assert validate_report_set(work, V_SUFFIX)
...
def test_v1_97_2_tests_do_not_copy_entire_reports_research() -> None:
...
def test_report_index_references_v1_97_2(tmp_path: Path) -> None:
    work = _copy_tree(tmp_path)
    content = (work / "reports/REPORT_INDEX.md").read_text(encoding="utf-8")
    assert "V1.97.2" in content
    assert "v1_97_2" in content


def test_smoke_v1_97_2_runs_validator_import_and_summary_presence() -> None:
    content = (ROOT / "scripts/smoke_test_clean_zip.py").read_text(encoding="utf-8")
    assert "validate_label_preview_materialization_v1_97_2_reports.py" in content
    assert "galapagos.research.label_preview_materialization" in content
    assert "label_preview_materialization_summary_v1_97_2.json" in content


def test_cross_file_alignment_summary_latest_metrics_project_state(tmp_path: Path) -> None:
    work = _copy_tree(tmp_path)
    summary = _load(work / f"reports/research/label_preview_materialization_summary_{V_SUFFIX}.json")
    latest = _load(work / "reports/current/latest_metrics.json")
    project = _load(work / "reports/PROJECT_STATE.json")
...
def test_no_pass_only_tests_in_v1_97_2() -> None:
...
def test_no_assert_true_or_true_in_v1_97_2() -> None:
    tree = ast.parse(TEST_FILE.read_text(encoding="utf-8"))
