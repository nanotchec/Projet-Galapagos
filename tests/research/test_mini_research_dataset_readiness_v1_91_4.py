...
def mock_reports(tmp_path):
    ...
    base_payload = {
        "version": V,
        "final_verdict": "V1_91_4_FAST_BOUNDED_SMOKE_PASSED",
    ...
    write_j(f"reports/zip_smoke_test_{v}.json", {
        ...
        "smoke_runs_full_pytest_suite": False, "smoke_calls_smoke_script": False, "smoke_runs_audit_clean_zip_full_scan": False
    })
    
    (tmp_path / "reports/REPORT_INDEX.md").write_text(f"V1.91.4 v1_91_4", encoding="utf-8")
...
def test_smoke_v1_91_4_fast_path_exists(mock_reports):
    path = PROJECT_ROOT / "scripts/smoke_test_clean_zip.py"
    content = path.read_text()
    assert "_fast_smoke_v1_91_4" in content

def test_smoke_v1_91_4_uses_only_three_commands(mock_reports):
    path = mock_reports / "reports/zip_smoke_test_v1_91_4.json"
    data = json.loads(path.read_text())
    data["smoke_commands_count"] = 4
    path.write_text(json.dumps(data))
    errors = validate_report_set(mock_reports, version_suffix="v1_91_4")
    assert any("smoke_passed_count mismatch" in e for e in errors)

def test_smoke_v1_91_4_does_not_run_pytest(mock_reports):
    path = mock_reports / "reports/zip_smoke_test_v1_91_4.json"
    data = json.loads(path.read_text())
    data["smoke_runs_full_pytest_suite"] = True
    path.write_text(json.dumps(data))
    errors = validate_report_set(mock_reports, version_suffix="v1_91_4")
    assert any("smoke_runs_full_pytest_suite != false" in e for e in errors)

def test_smoke_v1_91_4_does_not_run_audit_clean_zip(mock_reports):
    path = mock_reports / "reports/zip_smoke_test_v1_91_4.json"
    data = json.loads(path.read_text())
    data["smoke_runs_audit_clean_zip_full_scan"] = True
    path.write_text(json.dumps(data))
    errors = validate_report_set(mock_reports, version_suffix="v1_91_4")
    assert any("smoke_runs_audit_clean_zip_full_scan != false" in e for e in errors)

def test_smoke_v1_91_4_does_not_call_smoke_recursively(mock_reports):
    path = mock_reports / "reports/zip_smoke_test_v1_91_4.json"
    data = json.loads(path.read_text())
    data["smoke_calls_smoke_script"] = True
    path.write_text(json.dumps(data))
    errors = validate_report_set(mock_reports, version_suffix="v1_91_4")
    assert any("smoke_calls_smoke_script != false" in e for e in errors)

def test_smoke_v1_91_4_reports_timeout_false(mock_reports):
    path = mock_reports / "reports/zip_smoke_test_v1_91_4.json"
    data = json.loads(path.read_text())
...
def test_no_pass_only_tests_in_v1_91_4():
    path = Path(__file__)
...
def test_no_assert_true_or_true_in_v1_91_4():
    path = Path(__file__)
    tree = ast.parse(path.read_text(encoding="utf-8"))
    for node in ast.walk(tree):
...
def test_run_script_does_not_generate_assert_true_stub():
    path = PROJECT_ROOT / "scripts/run_mini_research_dataset_readiness_v1_91_4.py"
...
def test_validator_rejects_tautology_in_test_file(mock_reports):
    path = mock_reports / "tests/research/test_mini_research_dataset_readiness_v1_91_4.py"
    path.parent.mkdir(parents=True, exist_ok=True)
...
def test_validator_rejects_dataset_seed_max_files_above_5_in_design_report(mock_reports):
    path = mock_reports / "reports/research/mini_research_dataset_seed_design_v1_91_4.json"
    data = json.loads(path.read_text())
...
def test_validator_rejects_available_ts_policy_false_in_anti_leakage_report(mock_reports):
    path = mock_reports / "reports/research/mini_research_dataset_anti_leakage_plan_v1_91_4.json"
    data = json.loads(path.read_text())
...
def test_validator_rejects_zip_audit_project_state_version_mismatch(mock_reports):
    path = mock_reports / "reports/zip_audit_v1_91_4.json"
    data = json.loads(path.read_text())
    data["audit_zip_project_state_version"] = "V1.91.3"
    path.write_text(json.dumps(data))
    errors = validate_report_set(mock_reports, version_suffix="v1_91_4")
...
def test_report_index_references_v1_91_4(mock_reports):
    (mock_reports / "reports/REPORT_INDEX.md").write_text("V1.91.3", encoding="utf-8")
    errors = validate_report_set(mock_reports, version_suffix="v1_91_4")
    assert any("REPORT_INDEX does not reference V1.91.4" in e for e in errors)

def test_smoke_v1_91_4_runs_validator_import_and_summary_presence(mock_reports):
    errors = validate_report_set(mock_reports, version_suffix="v1_91_4")
