...
TEST_FILE = ROOT / "tests/research/test_training_dataset_readiness_v1_98_2.py"
...
def _baseline_repo(tmp_path: Path) -> Path:
    ...
    for rel in [
        "reports/research/training_dataset_readiness_summary_v1_98_2.json",
        "reports/research/training_dataset_feature_preview_review_v1_98_2.json",
        "reports/research/training_dataset_label_preview_review_v1_98_2.json",
        "reports/research/training_dataset_alignment_dryrun_v1_98_2.json",
        "reports/research/training_dataset_policy_v1_98_2.json",
        "reports/research/training_dataset_approval_decision_v1_98_2.json",
        "reports/research/training_dataset_readiness_safety_check_v1_98_2.json",
        "reports/research/training_dataset_readiness_consistency_check_v1_98_2.json",
        "reports/current/latest_metrics.json",
        "reports/PROJECT_STATE.json",
    ]:
        _write_json(work / rel, summary)
    _write_json(work / "reports/release_zip_v1_98_2.json", {"version": "V1.98.2", "release_zip_created": True, "final_zip_created": True, "release_ready_for_external_review": True, "clean_zip_ready_for_external_review": True, "final_audit_passed": True, "final_smoke_passed": True, "blocking_reason": None})
    _write_json(work / "reports/zip_audit_v1_98_2.json", {"version": "V1.98.2", "clean_zip_ready_for_external_review": True, "audit_zip_project_state_version": "V1.98.2", "audit_zip_version_parse_correct": True, "global_json_finiteness_passed": True, "missing_required_files": [], "forbidden_count": 0})
    _write_json(work / "reports/zip_smoke_test_v1_98_2.json", {"version": "V1.98.2", "smoke_test_passed": True, "smoke_failed_count": 0, "smoke_passed_count": 3, "smoke_commands_count": 3, "smoke_commands_not_empty": True, "bounded_smoke_for_v1_98_2": True, "real_orders_possible": False, "codex_cli_called": False, "holdout_executed": False})
    (work / "docs").mkdir(parents=True, exist_ok=True)
    (work / "docs/code_review_v1_98_2.md").write_text("# Code Review V1.98.2\n", encoding="utf-8")
    (work / "docs/training_dataset_readiness_v1_98_2.md").write_text("# Training Dataset Readiness V1.98.2\n", encoding="utf-8")
    test_target = work / "tests/research/test_training_dataset_readiness_v1_98_2.py"
    test_target.parent.mkdir(parents=True, exist_ok=True)
    shutil.copy2(TEST_FILE, test_target)
    (work / "reports/REPORT_INDEX.md").parent.mkdir(parents=True, exist_ok=True)
    (work / "reports/REPORT_INDEX.md").write_text("v1_98_2\n", encoding="utf-8")
    return work
...
def test_rejects_missing_feature_preview_file(tmp_path: Path) -> None:
    ...
    assert validate_report_set(work, "v1_98_2")
...
def test_alignment_dryrun_created_in_reports_only(tmp_path: Path) -> None:
    work = _baseline_repo(tmp_path)
    payload = json.loads((work / "reports/research/training_dataset_alignment_dryrun_v1_98_2.json").read_text())
...
def test_alignment_pairs_limited_to_10(tmp_path: Path) -> None:
    work = _baseline_repo(tmp_path)
    payload = json.loads((work / "reports/research/training_dataset_readiness_summary_v1_98_2.json").read_text())
...
def test_alignment_rejects_labels_available_at_feature_decision_ts(tmp_path: Path) -> None:
    work = _baseline_repo(tmp_path)
    path = work / "reports/research/training_dataset_readiness_summary_v1_98_2.json"
...
def _mutate_alignment(work: Path, field: str, value: object) -> None:
    path = work / "reports/research/training_dataset_alignment_dryrun_v1_98_2.json"
    payload = json.loads(path.read_text())
    payload[field] = value
    path.write_text(json.dumps(payload, indent=2), encoding="utf-8")


def _mutate_policy(work: Path, field: str, value: object) -> None:
    path = work / "reports/research/training_dataset_policy_v1_98_2.json"
    payload = json.loads(path.read_text())
    payload[field] = value
    path.write_text(json.dumps(payload, indent=2), encoding="utf-8")


def test_validator_rejects_alignment_labels_available_at_feature_decision_ts_true(tmp_path: Path) -> None:
    work = _baseline_repo(tmp_path)
    _mutate_alignment(work, "labels_available_at_feature_decision_ts", True)
    assert validate_report_set(work, "v1_98_2")


def test_validator_rejects_alignment_physical_join_created_true(tmp_path: Path) -> None:
    work = _baseline_repo(tmp_path)
    _mutate_alignment(work, "physical_feature_label_join_created", True)
    assert validate_report_set(work, "v1_98_2")


def test_validator_rejects_alignment_training_dataset_created_true(tmp_path: Path) -> None:
    work = _baseline_repo(tmp_path)
    _mutate_alignment(work, "training_dataset_created", True)
    assert validate_report_set(work, "v1_98_2")


def test_validator_rejects_alignment_data_write_allowed_true(tmp_path: Path) -> None:
    work = _baseline_repo(tmp_path)
    _mutate_alignment(work, "alignment_dry_run_data_write_allowed", True)
    assert validate_report_set(work, "v1_98_2")


def test_validator_rejects_alignment_pairs_above_10(tmp_path: Path) -> None:
    work = _baseline_repo(tmp_path)
    _mutate_alignment(work, "alignment_pairs_count", 11)
    assert validate_report_set(work, "v1_98_2")


def test_validator_rejects_alignment_preview_rows_above_10(tmp_path: Path) -> None:
    work = _baseline_repo(tmp_path)
    _mutate_alignment(work, "alignment_preview_rows", [{"reports_only": True, "physical_join_created": False} for _ in range(11)])
    assert validate_report_set(work, "v1_98_2")


def test_validator_rejects_alignment_row_reports_only_false(tmp_path: Path) -> None:
    work = _baseline_repo(tmp_path)
    _mutate_alignment(work, "alignment_preview_rows", [{"reports_only": False, "physical_join_created": False}])
    assert validate_report_set(work, "v1_98_2")


def test_validator_rejects_alignment_row_physical_join_created_true(tmp_path: Path) -> None:
    work = _baseline_repo(tmp_path)
    _mutate_alignment(work, "alignment_preview_rows", [{"reports_only": True, "physical_join_created": True}])
    assert validate_report_set(work, "v1_98_2")


def test_validator_rejects_alignment_row_usable_for_training_true(tmp_path: Path) -> None:
    work = _baseline_repo(tmp_path)
    _mutate_alignment(work, "alignment_preview_rows", [{"reports_only": True, "physical_join_created": False, "usable_for_training_in_v1_98_2": True}])
    assert validate_report_set(work, "v1_98_2")


def test_validator_rejects_policy_future_training_dataset_max_files_above_5(tmp_path: Path) -> None:
    work = _baseline_repo(tmp_path)
    _mutate_policy(work, "future_training_dataset_max_files", 6)
    assert validate_report_set(work, "v1_98_2")


def test_validator_rejects_policy_future_training_dataset_max_bytes_above_75000(tmp_path: Path) -> None:
    work = _baseline_repo(tmp_path)
    _mutate_policy(work, "future_training_dataset_max_bytes", 75001)
    assert validate_report_set(work, "v1_98_2")


def test_validator_rejects_policy_purge_policy_false(tmp_path: Path) -> None:
    work = _baseline_repo(tmp_path)
    _mutate_policy(work, "purge_policy_defined", False)
    assert validate_report_set(work, "v1_98_2")


def test_validator_rejects_policy_embargo_policy_false(tmp_path: Path) -> None:
    work = _baseline_repo(tmp_path)
    _mutate_policy(work, "embargo_policy_defined", False)
    assert validate_report_set(work, "v1_98_2")


def test_validator_rejects_policy_temporal_split_policy_false(tmp_path: Path) -> None:
    work = _baseline_repo(tmp_path)
    _mutate_policy(work, "temporal_split_policy_defined", False)
    assert validate_report_set(work, "v1_98_2")


def test_validator_rejects_policy_no_random_shuffle_policy_false(tmp_path: Path) -> None:
    work = _baseline_repo(tmp_path)
    _mutate_policy(work, "no_random_shuffle_policy_defined", False)
    assert validate_report_set(work, "v1_98_2")


def test_validator_rejects_policy_label_availability_policy_false(tmp_path: Path) -> None:
    work = _baseline_repo(tmp_path)
    _mutate_policy(work, "label_availability_policy_defined", False)
    assert validate_report_set(work, "v1_98_2")


def test_validator_rejects_policy_wrong_allowed_root(tmp_path: Path) -> None:
    work = _baseline_repo(tmp_path)
    _mutate_policy(work, "future_training_dataset_allowed_root", "data/research/not_allowed/")
    assert validate_report_set(work, "v1_98_2")


def test_validator_rejects_policy_allowed_extensions_not_json(tmp_path: Path) -> None:
    work = _baseline_repo(tmp_path)
    _mutate_policy(work, "future_training_dataset_allowed_extensions", [".json", ".csv"])
    assert validate_report_set(work, "v1_98_2")


def test_validator_rejects_policy_no_ml_false(tmp_path: Path) -> None:
    work = _baseline_repo(tmp_path)
    _mutate_policy(work, "future_training_dataset_no_ml", False)
    assert validate_report_set(work, "v1_98_2")


def test_validator_rejects_policy_no_backtest_false(tmp_path: Path) -> None:
    work = _baseline_repo(tmp_path)
    _mutate_policy(work, "future_training_dataset_no_backtest", False)
    assert validate_report_set(work, "v1_98_2")


def test_validator_rejects_policy_no_trading_false(tmp_path: Path) -> None:
    work = _baseline_repo(tmp_path)
    _mutate_policy(work, "future_training_dataset_no_trading", False)
    assert validate_report_set(work, "v1_98_2")


def test_validator_rejects_strict_alignment_report_validation_false(tmp_path: Path) -> None:
    work = _baseline_repo(tmp_path)
    _mutate_summary(work, "strict_alignment_report_validation", False)
    assert validate_report_set(work, "v1_98_2")


def test_validator_rejects_strict_training_dataset_policy_validation_false(tmp_path: Path) -> None:
    work = _baseline_repo(tmp_path)
    _mutate_summary(work, "strict_training_dataset_policy_validation", False)
    assert validate_report_set(work, "v1_98_2")
...
def test_validator_rejects_release_final_smoke_false(tmp_path: Path) -> None:
    work = _baseline_repo(tmp_path)
    path = work / "reports/release_zip_v1_98_2.json"
    payload = json.loads(path.read_text())
...
def test_validator_rejects_zip_audit_project_state_version_mismatch(tmp_path: Path) -> None:
    work = _baseline_repo(tmp_path)
    path = work / "reports/zip_audit_v1_98_2.json"
    payload = json.loads(path.read_text())
...
def test_validator_rejects_zip_smoke_failed_count_positive(tmp_path: Path) -> None:
    work = _baseline_repo(tmp_path)
    path = work / "reports/zip_smoke_test_v1_98_2.json"
    payload = json.loads(path.read_text())
...
def test_report_index_references_v1_98_2(tmp_path: Path) -> None:
    work = _baseline_repo(tmp_path)
    assert "v1_98_2" in (work / "reports/REPORT_INDEX.md").read_text(encoding="utf-8")


def test_smoke_v1_98_2_runs_validator_import_and_summary_presence() -> None:
    smoke = ROOT / "scripts/smoke_test_clean_zip.py"
    text = smoke.read_text(encoding="utf-8")
    assert "validate_training_dataset_readiness_v1_98_2_reports.py" in text
    assert "training_dataset_readiness_summary_v1_98_2.json" in text
...
def test_no_pass_only_tests_in_v1_98_2() -> None:
...
def test_no_assert_true_or_true_in_v1_98_2() -> None:
    tree = ast.parse(TEST_FILE.read_text(encoding="utf-8"))
