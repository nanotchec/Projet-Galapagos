...
def mock_reports(tmp_path):
    ...
    base_payload = {
        "version": V,
        "final_verdict": "V1_93_3_NO_PASS_AND_STRONG_NO_TAUTOLOGY_TESTS_PASSED",
    ...
    (tmp_path / "reports/REPORT_INDEX.md").write_text("V1.93.3 v1_93_3", encoding="utf-8")
...
def test_validator_accepts_valid_v1_93_3(mock_reports):
    errors = validate_report_set(mock_reports, version_suffix="v1_93_3")
    assert len(errors) == 0
...
def test_validator_rejects_new_data_files_created_true(mock_reports):
    path = mock_reports / "reports/research/mini_research_dataset_post_review_summary_v1_93_3.json"
...
def test_report_index_references_v1_93_3(mock_reports):
    (mock_reports / "reports/REPORT_INDEX.md").write_text("V1.93.2", encoding="utf-8")
    errors = validate_report_set(mock_reports, version_suffix="v1_93_3")
    assert any("REPORT_INDEX does not reference V1.93.3" in e for e in errors)

def test_smoke_v1_93_3_fast_path_exists(mock_reports):
    path = PROJECT_ROOT / "scripts/smoke_test_clean_zip.py"
    content = path.read_text()
    assert "_fast_smoke_v1_93_3" in content

def test_no_pass_anywhere_in_v1_93_3_tests():
    path = Path(__file__)
...
def test_no_assert_true_or_true_in_v1_93_3():
    path = Path(__file__)
    content = path.read_text(encoding="utf-8")
    tree = ast.parse(content)
    for node in ast.walk(tree):
...
def test_no_tautological_assertions_in_v1_93_3():
    path = Path(__file__)
    content = path.read_text(encoding="utf-8")
    tree = ast.parse(content)
