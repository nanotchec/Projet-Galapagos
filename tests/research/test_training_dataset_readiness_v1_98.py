...
TEST_FILE = ROOT / "tests/research/test_training_dataset_readiness_v1_98.py"
...
def _baseline_repo(tmp_path: Path) -> Path:
    ...
    summary = {
        "version": "V1.98",
        "version_suffix": "v1_98",
        "final_verdict": "V1_98_FEATURE_LABEL_ALIGNMENT_READINESS_PASSED",
    ...
    _write_json(work / "reports/release_zip_v1_98.json", {"version": "V1.98", "release_zip_created": True, "final_zip_created": True, "release_ready_for_external_review": True, "clean_zip_ready_for_external_review": True, "final_audit_passed": True, "final_smoke_passed": True, "blocking_reason": None})
    _write_json(work / "reports/zip_audit_v1_98.json", {"version": "V1.98", "clean_zip_ready_for_external_review": True, "audit_zip_project_state_version": "V1.98", "audit_zip_version_parse_correct": True, "global_json_finiteness_passed": True, "missing_required_files": [], "forbidden_count": 0})
    _write_json(work / "reports/zip_smoke_test_v1_98.json", {"version": "V1.98", "smoke_test_passed": True, "smoke_failed_count": 0, "smoke_passed_count": 3, "smoke_commands_count": 3, "smoke_commands_not_empty": True, "bounded_smoke_for_v1_98": True, "real_orders_possible": False, "codex_cli_called": False, "holdout_executed": False})
    (work / "docs").mkdir(parents=True, exist_ok=True)
    (work / "docs/code_review_v1_98.md").write_text("# Code Review V1.98\n", encoding="utf-8")
    (work / "docs/training_dataset_readiness_v1_98.md").write_text("# Training Dataset Readiness V1.98\n", encoding="utf-8")
    test_target = work / "tests/research/test_training_dataset_readiness_v1_98.py"
    test_target.parent.mkdir(parents=True, exist_ok=True)
    shutil.copy2(TEST_FILE, test_target)
    (work / "reports/REPORT_INDEX.md").parent.mkdir(parents=True, exist_ok=True)
    (work / "reports/REPORT_INDEX.md").write_text("v1_98\n", encoding="utf-8")
    return work
...
def test_rejects_missing_feature_preview_file(tmp_path: Path) -> None:
    ...
    assert validate_report_set(work, "v1_98")
...
def test_alignment_dryrun_created_in_reports_only(tmp_path: Path) -> None:
    work = _baseline_repo(tmp_path)
    payload = json.loads((work / "reports/research/training_dataset_alignment_dryrun_v1_98.json").read_text())
...
def test_alignment_pairs_limited_to_10(tmp_path: Path) -> None:
    work = _baseline_repo(tmp_path)
    payload = json.loads((work / "reports/research/training_dataset_readiness_summary_v1_98.json").read_text())
...
def test_alignment_rejects_labels_available_at_feature_decision_ts(tmp_path: Path) -> None:
    work = _baseline_repo(tmp_path)
    path = work / "reports/research/training_dataset_readiness_summary_v1_98.json"
...
def test_validator_rejects_release_final_smoke_false(tmp_path: Path) -> None:
    work = _baseline_repo(tmp_path)
    path = work / "reports/release_zip_v1_98.json"
    payload = json.loads(path.read_text())
...
def test_validator_rejects_zip_audit_project_state_version_mismatch(tmp_path: Path) -> None:
    work = _baseline_repo(tmp_path)
    path = work / "reports/zip_audit_v1_98.json"
    payload = json.loads(path.read_text())
    payload["audit_zip_project_state_version"] = "V1.97.1"
    path.write_text(json.dumps(payload, indent=2), encoding="utf-8")
    assert validate_report_set(work, "v1_98")
...
def test_validator_rejects_zip_smoke_failed_count_positive(tmp_path: Path) -> None:
    work = _baseline_repo(tmp_path)
    path = work / "reports/zip_smoke_test_v1_98.json"
    payload = json.loads(path.read_text())
...
def test_report_index_references_v1_98(tmp_path: Path) -> None:
    work = _baseline_repo(tmp_path)
    assert "v1_98" in (work / "reports/REPORT_INDEX.md").read_text(encoding="utf-8")


def test_smoke_v1_98_runs_validator_import_and_summary_presence() -> None:
    smoke = ROOT / "scripts/smoke_test_clean_zip.py"
    text = smoke.read_text(encoding="utf-8")
    assert "validate_training_dataset_readiness_v1_98_reports.py" in text
    assert "training_dataset_readiness_summary_v1_98.json" in text
...
def test_no_pass_only_tests_in_v1_98() -> None:
...
def test_no_assert_true_or_true_in_v1_98() -> None:
    tree = ast.parse(TEST_FILE.read_text(encoding="utf-8"))
