...
from galapagos.ml.ohlcv_trades_robustness import (
    ...
    compute_ohlcv_trades_vs_references_comparison_v7_5,
    compute_ohlcv_trades_baseline_delta_v7_5,
    compute_ohlcv_trades_split_stability_v7_5,
    compute_ohlcv_trades_timeframe_stability_v7_5,
    compute_ohlcv_trades_walk_forward_stability_v7_5,
    run_ohlcv_trades_ml_robustness_v7_5,
    scan_ohlcv_trades_feature_leakage_v7_5,
    scan_ohlcv_trades_metric_forbidden_terms_v7_5,
)


def test_baseline_delta_contains_majority_and_random_comparisons_v7_5() -> None:
    delta = compute_ohlcv_trades_baseline_delta_v7_5(_metrics())
...
def test_split_stability_computes_train_validation_test_gaps_v7_5() -> None:
    stability = compute_ohlcv_trades_split_stability_v7_5(_metrics())
...
def test_timeframe_stability_contains_all_timeframes_v7_5() -> None:
    stability = compute_ohlcv_trades_timeframe_stability_v7_5(_metrics())
...
def test_walk_forward_stability_contains_groups_v7_5() -> None:
    stability = compute_ohlcv_trades_walk_forward_stability_v7_5(_walk_forward_metrics())
    logistic = stability["1m.logistic_regression"]
...
def test_ohlcv_trades_vs_references_comparison_is_descriptive_v7_5(tmp_path: Path) -> None:
    ...
    comparison = compute_ohlcv_trades_vs_references_comparison_v7_5(
        tmp_path,
        {
...
def test_label_shuffle_falsification_uses_seed_123_v7_5() -> None:
    assert LABEL_SHUFFLE_RANDOM_SEED_V7_5 == 123


def test_feature_leakage_scan_rejects_future_label_split_features_v7_5() -> None:
    scan = scan_ohlcv_trades_feature_leakage_v7_5(
...
def test_feature_leakage_scan_allows_causal_trade_features_v7_5() -> None:
    scan = scan_ohlcv_trades_feature_leakage_v7_5(["agg_trade_count", "taker_buy_ratio_quantity"])

    assert scan["feature_leakage_detected"] is False
    assert scan["forbidden_feature_columns_present"] == []


def test_metric_forbidden_scan_rejects_trading_metrics_v7_5() -> None:
    scan = scan_ohlcv_trades_metric_forbidden_terms_v7_5({"model": {"sharpe": 1.0, "drawdown": 0.2}})

    assert scan["metric_forbidden_terms_detected"] is True
    assert scan["forbidden_terms_present"] == ["sharpe", "drawdown"]


def test_no_robust_edge_claimed_by_default_v7_5() -> None:
    source = inspect.getsource(run_ohlcv_trades_ml_robustness_v7_5)
...
def test_ohlcv_trades_not_validated_for_trading_by_default_v7_5() -> None:
    source = inspect.getsource(run_ohlcv_trades_ml_robustness_v7_5)
