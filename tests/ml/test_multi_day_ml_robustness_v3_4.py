...
from galapagos.ml.robustness import (
    ...
    compute_baseline_delta_v3_4,
    compute_split_stability_v3_4,
    compute_timeframe_stability_v3_4,
    run_multi_day_ml_robustness_v3_4,
    scan_feature_leakage_v3_4,
    scan_metric_forbidden_terms_v3_4,
)


def test_baseline_delta_contains_majority_and_random_comparisons() -> None:
    delta = compute_baseline_delta_v3_4(_metrics())
...
def test_split_stability_computes_train_validation_test_gaps() -> None:
    stability = compute_split_stability_v3_4(_metrics())
...
def test_timeframe_stability_contains_all_timeframes() -> None:
    stability = compute_timeframe_stability_v3_4(_metrics())
...
def test_label_shuffle_falsification_uses_seed_123() -> None:
    assert LABEL_SHUFFLE_RANDOM_SEED_V3_4 == 123


def test_feature_leakage_scan_rejects_future_label_split_features() -> None:
    scan = scan_feature_leakage_v3_4(["return_1", "future_log_return_h1", "label_valid_h1", "split"])
...
def test_metric_forbidden_scan_rejects_trading_metrics() -> None:
    scan = scan_metric_forbidden_terms_v3_4({"model": {"sharpe": 1.0, "drawdown": 0.2}})

    assert scan["metric_forbidden_terms_detected"] is True
    assert scan["forbidden_terms_present"] == ["sharpe", "drawdown"]


def test_no_robust_edge_claimed_by_default() -> None:
    source = inspect.getsource(run_multi_day_ml_robustness_v3_4)
