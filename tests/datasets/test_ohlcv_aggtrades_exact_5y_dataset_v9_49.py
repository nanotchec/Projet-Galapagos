...
from galapagos.datasets.ohlcv_aggtrades_exact_5y_dataset_v9_49 import (
    assemble_dataset_frame_v9_49,
    split_series_v9_49,
)
from galapagos.datasets.ohlcv_aggtrades_exact_5y_dataset_v9_49_schemas import FEATURE_COLUMNS, SELECTED_PRIMARY_LABEL


def test_split_series_is_temporal_and_non_shuffled() -> None:
    split = split_series_v9_49(10).tolist()
...
def test_assemble_dataset_uses_selected_h1_target_not_h4_label_valid() -> None:
    ...
    dataset = assemble_dataset_frame_v9_49(features, labels, "1h", "test_run")
...
def _feature_frame(rows: int) -> pd.DataFrame:
    ...
    data = {
        ...
        "combined_feature_schema_version": ["test_features"] * rows,
        "warmup_row": [False] * rows,
        "zero_trade_bucket": [False] * rows,
        "feature_null_count": [0] * rows,
        "feature_error_count": [0] * rows,
        "combined_feature_null_count": [0] * rows,
        "combined_feature_error_count": [0] * rows,
        "row_valid_for_combined_features": [True] * rows,
        "combined_feature_invalid_reason": [""] * rows,
    }
