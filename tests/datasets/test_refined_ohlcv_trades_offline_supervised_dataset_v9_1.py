...
def manifests() -> tuple[dict, dict]:
    return load_v9_0_feature_manifest(ROOT), load_v5_2_label_manifest(ROOT)


def test_refined_ohlcv_trades_dataset_row_count_matches_features_and_filtered_labels(manifests: tuple[dict, dict]) -> None:
    ...
    for timeframe in TIMEFRAMES_V9_1:
        ...
        assert _parquet_rows(dataset_path) == EXPECTED_ROWS_V9_1[timeframe]


def test_refined_ohlcv_trades_dataset_strict_columns() -> None:
    for timeframe in TIMEFRAMES_V9_1:
        assert _parquet_columns(dataset_output_path(ROOT, timeframe)) == DATASET_COLUMNS_V9_1
        assert _parquet_columns(split_output_path(ROOT, timeframe)) == SPLIT_COLUMNS_V9_1


def test_refined_ohlcv_trades_dataset_join_keys_match_sources(manifests: tuple[dict, dict]) -> None:
    feature_manifest, label_manifest = manifests
    features = _read_columns(input_feature_path(ROOT, "15m", feature_manifest), JOIN_KEYS)
    labels = filter_labels_to_v9_1_window(_read_columns(input_label_path(ROOT, "15m", label_manifest), JOIN_KEYS))
...
def test_refined_ohlcv_trades_dataset_feature_values_match_source(manifests: tuple[dict, dict]) -> None:
    feature_manifest, _label_manifest = manifests
    columns = ["open", "close", "agg_trade_count", "taker_buy_ratio_quantity", "trade_flow_pressure", "warmup_row"]
...
def test_refined_ohlcv_trades_dataset_label_values_match_source(manifests: tuple[dict, dict]) -> None:
    _feature_manifest, label_manifest = manifests
    columns = ["future_close_h1", "future_log_return_h3", "up_down_flat_h5", "tail_row"]
    labels = filter_labels_to_v9_1_window(_read_columns(input_label_path(ROOT, "1h", label_manifest), [*columns, "event_ts"]))
    dataset = _read_columns(dataset_output_path(ROOT, "1h"), columns)
    pd.testing.assert_frame_equal(dataset, labels[columns], check_dtype=False, check_exact=False, atol=1e-12, rtol=1e-12)


def test_refined_ohlcv_trades_dataset_contains_only_refined_trade_feature_columns() -> None:
    columns = _parquet_columns(dataset_output_path(ROOT, "1h"))
    expected_refined_columns = [
        "agg_trade_count",
    ...
    dropped_or_review_columns = ["agg_trade_price_std", "taker_buy_quantity", "is_weekend_utc"]
    for feature_column in expected_refined_columns:
        assert feature_column in columns
        assert feature_column in REFINED_OHLCV_TRADES_DATASET_FEATURE_COLUMNS_V9_1
    for feature_column in dropped_or_review_columns:
        assert feature_column not in columns


def test_refined_ohlcv_trades_split_counts_sum_to_rows() -> None:
    for timeframe in TIMEFRAMES_V9_1:
        ...
        assert sum(counts.values()) == EXPECTED_ROWS_V9_1[timeframe]
        assert counts == EXPECTED_SPLIT_COUNTS_V9_1[timeframe]


def test_refined_ohlcv_trades_split_temporal_order() -> None:
...
def test_refined_ohlcv_trades_walk_forward_groups_present() -> None:
...
def test_refined_ohlcv_trades_feature_available_ts_not_after_decision_ts() -> None:
    for timeframe in TIMEFRAMES_V9_1:
...
def test_refined_ohlcv_trades_label_available_ts_after_decision_ts_for_valid_labels() -> None:
...
def test_refined_ohlcv_trades_no_forbidden_dataset_columns() -> None:
    forbidden_exact = set(FORBIDDEN_DATASET_COLUMNS_EXACT_V9_1)
    for timeframe in TIMEFRAMES_V9_1:
...
def _filtered_labels(path: Path) -> pd.DataFrame:
    return filter_labels_to_v9_1_window(pd.read_parquet(path, engine="pyarrow"))
