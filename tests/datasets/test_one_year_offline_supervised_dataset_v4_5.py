...
def frames() -> dict[str, tuple[pd.DataFrame, pd.DataFrame, pd.DataFrame, pd.DataFrame]]:
    ...
    for timeframe in EXPECTED_ROWS_V4_5:
        ...
        splits = read_parquet(get_split_v4_5_path(ROOT, timeframe))
        loaded[timeframe] = (features, labels, dataset, splits)
    return loaded


def test_one_year_dataset_row_count_matches_features_and_labels(
    ...
    for timeframe, (features, labels, dataset, _splits) in frames.items():
        assert len(dataset) == len(features) == len(labels) == EXPECTED_ROWS_V4_5[timeframe]


def test_one_year_dataset_strict_columns(frames: dict[str, tuple[pd.DataFrame, pd.DataFrame, pd.DataFrame, pd.DataFrame]]) -> None:
    for _features, _labels, dataset, _splits in frames.values():
        assert list(dataset.columns) == DATASET_COLUMNS_V4_5


def test_one_year_dataset_join_keys_match_sources(
    frames: dict[str, tuple[pd.DataFrame, pd.DataFrame, pd.DataFrame, pd.DataFrame]]
) -> None:
...
def test_one_year_dataset_feature_values_match_source(
    frames: dict[str, tuple[pd.DataFrame, pd.DataFrame, pd.DataFrame, pd.DataFrame]]
) -> None:
...
def test_one_year_dataset_label_values_match_source(
    frames: dict[str, tuple[pd.DataFrame, pd.DataFrame, pd.DataFrame, pd.DataFrame]]
) -> None:
...
def test_one_year_split_counts_sum_to_rows(frames: dict[str, tuple[pd.DataFrame, pd.DataFrame, pd.DataFrame, pd.DataFrame]]) -> None:
...
def test_one_year_split_temporal_order(frames: dict[str, tuple[pd.DataFrame, pd.DataFrame, pd.DataFrame, pd.DataFrame]]) -> None:
...
def test_one_year_feature_available_ts_not_after_decision_ts(
    frames: dict[str, tuple[pd.DataFrame, pd.DataFrame, pd.DataFrame, pd.DataFrame]]
) -> None:
...
def test_one_year_label_available_ts_after_decision_ts_for_valid_labels(
    frames: dict[str, tuple[pd.DataFrame, pd.DataFrame, pd.DataFrame, pd.DataFrame]]
) -> None:
    _features, _labels, dataset, _splits = frames["1m"]
...
def test_one_year_no_forbidden_dataset_columns(
    frames: dict[str, tuple[pd.DataFrame, pd.DataFrame, pd.DataFrame, pd.DataFrame]]
) -> None:
