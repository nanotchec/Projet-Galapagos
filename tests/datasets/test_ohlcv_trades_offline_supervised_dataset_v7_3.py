...
def manifests() -> tuple[dict, dict]:
    return load_v7_2_feature_manifest(ROOT), load_v5_2_label_manifest(ROOT)


def test_ohlcv_trades_dataset_row_count_matches_features_and_filtered_labels(manifests: tuple[dict, dict]) -> None:
    ...
    for timeframe in TIMEFRAMES_V7_3:
        ...
        assert _parquet_rows(dataset_path) == EXPECTED_ROWS_V7_3[timeframe]


def test_ohlcv_trades_dataset_strict_columns() -> None:
    for timeframe in TIMEFRAMES_V7_3:
        assert _parquet_columns(dataset_output_path(ROOT, timeframe)) == DATASET_COLUMNS_V7_3
        assert _parquet_columns(split_output_path(ROOT, timeframe)) == SPLIT_COLUMNS_V7_3


def test_ohlcv_trades_dataset_join_keys_match_sources(manifests: tuple[dict, dict]) -> None:
    feature_manifest, label_manifest = manifests
    features = _read_columns(input_feature_path(ROOT, "15m", feature_manifest), JOIN_KEYS)
    labels = filter_labels_to_v7_3_window(_read_columns(input_label_path(ROOT, "15m", label_manifest), JOIN_KEYS))
...
def test_ohlcv_trades_dataset_feature_values_match_source(manifests: tuple[dict, dict]) -> None:
...
def test_ohlcv_trades_dataset_label_values_match_source(manifests: tuple[dict, dict]) -> None:
    _feature_manifest, label_manifest = manifests
    columns = ["future_close_h1", "future_log_return_h3", "up_down_flat_h5", "tail_row"]
    labels = filter_labels_to_v7_3_window(_read_columns(input_label_path(ROOT, "1h", label_manifest), [*columns, "event_ts"]))
    dataset = _read_columns(dataset_output_path(ROOT, "1h"), columns)
    pd.testing.assert_frame_equal(dataset, labels[columns], check_dtype=False, check_exact=False, atol=1e-12, rtol=1e-12)


def test_ohlcv_trades_dataset_contains_trade_feature_columns() -> None:
    ...
    for feature_column in expected_trade_columns:
        ...
        assert feature_column in OHLCV_TRADES_DATASET_FEATURE_COLUMNS_V7_3


def test_ohlcv_trades_split_counts_sum_to_rows() -> None:
    for timeframe in TIMEFRAMES_V7_3:
        ...
        assert sum(counts.values()) == EXPECTED_ROWS_V7_3[timeframe]
        assert counts == EXPECTED_SPLIT_COUNTS_V7_3[timeframe]


def test_ohlcv_trades_split_temporal_order() -> None:
...
def test_ohlcv_trades_walk_forward_groups_present() -> None:
    splits = _read_columns(split_output_path(ROOT, "1h"), ["walk_forward_group"])
    assert splits["walk_forward_group"].notna().all()
    assert splits["walk_forward_group"].str.startswith("wf_window_").all()
    assert set(splits["walk_forward_group"].unique()) == {
        "wf_window_01",
        "wf_window_02",
        "wf_window_03",
        "wf_window_04",
        "wf_window_05_partial",
    }


def test_ohlcv_trades_feature_available_ts_not_after_decision_ts() -> None:
    for timeframe in TIMEFRAMES_V7_3:
...
def test_ohlcv_trades_label_available_ts_after_decision_ts_for_valid_labels() -> None:
...
def test_ohlcv_trades_no_forbidden_dataset_columns() -> None:
    forbidden_exact = set(FORBIDDEN_DATASET_COLUMNS_EXACT_V7_3)
    for timeframe in TIMEFRAMES_V7_3:
...
def _filtered_labels(path: Path) -> pd.DataFrame:
    return filter_labels_to_v7_3_window(pd.read_parquet(path, engine="pyarrow"))
