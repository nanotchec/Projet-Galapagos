...
from galapagos.data.public_trades.config import MANIFEST_PATH_V7_1
...
def _manifest() -> dict:
    return json.loads((ROOT / MANIFEST_PATH_V7_1).read_text(encoding="utf-8"))
...
def test_public_trades_v7_1_window_has_30_days() -> None:
    discovery = _manifest()["discovery"]

    assert discovery["window_start"] == "2023-03-25"
    assert discovery["window_end"] == "2023-04-23"
    assert discovery["total_days"] == 30
    assert discovery["documented_gaps_allowed"] is False


def test_public_trades_v7_1_schema_strict() -> None:
    for _date_key, frame in _partition_frames():
        assert list(frame.columns) == AGG_TRADE_COLUMNS_V7_1


def test_public_trades_v7_1_price_quantity_positive() -> None:
    for _date_key, frame in _partition_frames():
        assert (frame["price"] > 0).all()
        assert (frame["quantity"] > 0).all()


def test_public_trades_v7_1_trade_ids_monotonic() -> None:
...
def test_public_trades_v7_1_no_duplicate_aggregate_trade_ids() -> None:
    previous_last_id = None
    for _date_key, frame in _partition_frames():
...
def test_public_trades_v7_1_timestamps_utc() -> None:
...
def test_public_trades_v7_1_no_forbidden_columns() -> None:
    for _date_key, frame in _partition_frames():
