...
from galapagos.data.aggtrades_post_v9_batch_collection_v9_20 import (
    MAX_BATCH_DOWNLOADS,
    build_batch_day_plan_v9_20,
    build_source_design_v9_20,
    collect_batch_public_aggtrades_v9_20,
    date_range_v9_20,
    decide_v9_20,
    safety_flags_for_batch_v9_20,
    summarize_batch_validation_v9_20,
    validate_batch_day_v9_20,
    validate_batch_request_v9_20,
)


def test_batch_window_is_limited_to_thirty_days_v9_20() -> None:
    batch_dates = date_range_v9_20("2024-05-12", "2024-06-10")

    assert len(batch_dates) == MAX_BATCH_DOWNLOADS
    assert batch_dates[0] == "2024-05-12"
    assert batch_dates[-1] == "2024-06-10"


def test_collect_mode_requires_explicit_download_limit_v9_20() -> None:
    batch_dates = date_range_v9_20("2024-05-12", "2024-06-10")

    with pytest.raises(ValueError, match="requires --max-downloads"):
        validate_batch_request_v9_20("collect", batch_dates, None)
    with pytest.raises(ValueError, match="1 <= --max-downloads <= 30"):
        validate_batch_request_v9_20("collect", batch_dates, 31)


def test_source_design_is_public_archive_without_auth_v9_20() -> None:
    source = build_source_design_v9_20()
...
def test_day_plan_detects_raw_and_silver_batch_outputs_v9_20(tmp_path: Path) -> None:
    ...
    plan = build_batch_day_plan_v9_20(tmp_path, [day_value])
...
def test_validate_batch_day_accepts_small_valid_normalized_sample_v9_20(tmp_path: Path) -> None:
    day_value = "2024-05-05"
    _write_raw_zip(tmp_path, day_value)
    _write_silver_parquet(tmp_path, day_value)

    result = validate_batch_day_v9_20(tmp_path, day_value)
...
def test_validate_batch_day_rejects_duplicate_aggregate_trade_id_v9_20(tmp_path: Path) -> None:
    day_value = "2024-05-05"
    _write_raw_zip(tmp_path, day_value)
...
def test_decision_success_requires_complete_quality_batch_v9_20() -> None:
    collection_result = {
    ...
    assert decision["decision"] == "aggtrades_post_v9_batch_collection_success"
    assert "V9.21" in decision["next_recommendation"]
    assert decision["complete_collection_reached"] is False


def test_safety_flags_for_collect_are_public_archive_only_v9_20() -> None:
    flags = safety_flags_for_batch_v9_20(
        {
            "collection_executed": True,
            "network_used": True,
    ...
    assert flags["new_data_download_scope"] == "public_historical_aggtrades_batch_only"
    assert flags["ingestion_scope"] == "public_aggtrades_bronze_silver_batch_only"
...
def test_collect_batch_refuses_unbounded_execution_v9_20(tmp_path: Path) -> None:
    batch_dates = date_range_v9_20("2024-05-12", "2024-06-10")

    with pytest.raises(ValueError, match="requires --max-downloads"):
        collect_batch_public_aggtrades_v9_20(tmp_path, batch_dates, max_downloads=None)


def test_collect_batch_skips_existing_complete_day_v9_20(tmp_path: Path) -> None:
    day_value = "2024-05-05"
    _write_raw_zip(tmp_path, day_value)
    _write_silver_parquet(tmp_path, day_value)

    result = collect_batch_public_aggtrades_v9_20(tmp_path, [day_value], max_downloads=1)
...
def test_batch_summary_never_claims_full_future_coverage_v9_20() -> None:
    summary = summarize_batch_validation_v9_20(
        ["2024-05-05"],
        [{"status": "day_missing"}],
        ...
        2.0,
        {
            "v9_19_days_present_and_complete": True,
