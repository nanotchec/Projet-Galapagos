...
from galapagos.data.aggtrades_post_v9_batch2_collection_v9_23 import (
    MAX_BATCH_DOWNLOADS,
    build_batch_day_plan_v9_23,
    build_local_file_coverage_v9_23,
    build_reported_cumulative_coverage_v9_23,
    build_source_design_v9_23,
    collect_batch_public_aggtrades_v9_23,
    date_range_v9_23,
    decide_v9_23,
    safety_flags_for_batch_v9_23,
    summarize_batch_validation_v9_23,
    validate_batch_day_v9_23,
    validate_batch_request_v9_23,
)


def test_batch_window_is_limited_to_sixty_days_v9_23() -> None:
    batch_dates = date_range_v9_23("2024-08-10", "2024-10-08")

    assert len(batch_dates) == MAX_BATCH_DOWNLOADS
    assert batch_dates[0] == "2024-08-10"
    assert batch_dates[-1] == "2024-10-08"


def test_collect_mode_requires_explicit_download_limit_v9_23() -> None:
    batch_dates = date_range_v9_23("2024-08-10", "2024-10-08")

    with pytest.raises(ValueError, match="requires --max-downloads"):
        validate_batch_request_v9_23("collect", batch_dates, None)
    with pytest.raises(ValueError, match="1 <= --max-downloads <= 60"):
        validate_batch_request_v9_23("collect", batch_dates, 61)


def test_source_design_is_public_archive_without_auth_v9_23() -> None:
    source = build_source_design_v9_23()
...
def test_day_plan_detects_raw_and_silver_batch_outputs_v9_23(tmp_path: Path) -> None:
    ...
    plan = build_batch_day_plan_v9_23(tmp_path, [day_value])
...
def test_validate_batch_day_accepts_small_valid_normalized_sample_v9_23(tmp_path: Path) -> None:
    day_value = "2024-05-05"
    _write_raw_zip(tmp_path, day_value)
    _write_silver_parquet(tmp_path, day_value)

    result = validate_batch_day_v9_23(tmp_path, day_value)
...
def test_validate_batch_day_rejects_duplicate_aggregate_trade_id_v9_23(tmp_path: Path) -> None:
    day_value = "2024-05-05"
    _write_raw_zip(tmp_path, day_value)
...
def test_decision_success_requires_complete_quality_batch_v9_23() -> None:
    collection_result = {
    ...
    assert decision["decision"] == "aggtrades_post_v9_batch2_collection_success"
    assert "V9.24" in decision["next_recommendation"]
    assert decision["complete_collection_reached"] is False


def test_safety_flags_for_collect_are_public_archive_only_v9_23() -> None:
    flags = safety_flags_for_batch_v9_23(
        {
            "collection_executed": True,
            "network_used": True,
    ...
    assert flags["new_data_download_scope"] == "public_historical_aggtrades_batch2_only"
    assert flags["ingestion_scope"] == "public_aggtrades_bronze_silver_batch2_only"
...
def test_collect_batch_refuses_unbounded_execution_v9_23(tmp_path: Path) -> None:
    batch_dates = date_range_v9_23("2024-08-10", "2024-10-08")

    with pytest.raises(ValueError, match="requires --max-downloads"):
        collect_batch_public_aggtrades_v9_23(tmp_path, batch_dates, max_downloads=None)


def test_collect_batch_skips_existing_complete_day_v9_23(tmp_path: Path) -> None:
    day_value = "2024-05-05"
    _write_raw_zip(tmp_path, day_value)
    _write_silver_parquet(tmp_path, day_value)

    result = collect_batch_public_aggtrades_v9_23(tmp_path, [day_value], max_downloads=1)
...
def test_batch_summary_never_claims_full_future_coverage_v9_23() -> None:
    summary = summarize_batch_validation_v9_23(
        ["2024-05-05"],
        [{"status": "day_missing"}],
...
def test_reported_and_local_coverage_are_distinct_v9_23(tmp_path: Path) -> None:
    day_value = "2024-05-05"
    _write_raw_zip(tmp_path, day_value)
    _write_silver_parquet(tmp_path, day_value)
    summary = {
        ...
        "days_requested": 60,
        "quality_status": "PASS",
        "coverage_after_batch_start": "2024-08-10",
        "coverage_after_batch_end": "2024-10-08",
    }

    reported = build_reported_cumulative_coverage_v9_23(
        {
            "v9_22_multi_batch_plan": {
                "payload": {
                    "current_coverage": {
    ...
    local = build_local_file_coverage_v9_23(tmp_path, "2024-05-05", "2024-05-06")

    assert reported["reported_cumulative_coverage_start"] == "2024-05-05"
    assert reported["reported_cumulative_coverage_end"] == "2024-10-08"
