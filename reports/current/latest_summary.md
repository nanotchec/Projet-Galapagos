# Résumé courant — V2.8.4 candidate

La dernière version validée est **V2.7.2**. La version candidate est **V2.8.4**, en statut `pending_external_audit`.

V2.8 a ajouté un laboratoire ML offline borné sur les datasets supervisés V2.7 validés. V2.8.1 a corrigé le packaging release/audit et le garde-fou d'artefacts interdits. V2.8.2 a réduit le runtime du validateur. V2.8.3 restait refusée pour le test nominal et le smoke. V2.8.4 finalise le test nominal sans deuxième validation complète et rend le smoke ZIP robuste par appels directs aux validateurs.
