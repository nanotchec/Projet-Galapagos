# Script Portability Audit V1.81.7

```json
{
  "scripts_portable_without_manual_pythonpath": true,
  "validator_script_portable_without_manual_pythonpath": true,
  "run_script_portable_without_manual_pythonpath": true,
  "details": {
    "scripts/run_microstructure_data_contract_approval_intake_corrective_v1_81_5.py": {
      "script": "scripts/run_microstructure_data_contract_approval_intake_corrective_v1_81_5.py",
      "portable": true,
      "exit_code": 0,
      "stderr_preview": ""
    },
    "scripts/validate_microstructure_data_contract_approval_intake_corrective_v1_81_5_reports.py": {
      "script": "scripts/validate_microstructure_data_contract_approval_intake_corrective_v1_81_5_reports.py",
      "portable": true,
      "exit_code": 0,
      "stderr_preview": ""
    }
  }
}
```
