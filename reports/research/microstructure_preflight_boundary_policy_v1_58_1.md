# Preflight Boundary Policy (V1.58.1)

- **version_scope**: V1.59+
- **controlled_preflight_allowed**: True
- **controlled_preflight_network_policy**: STRICTLY_DISABLED_BY_DEFAULT
- **real_collection_approved**: False
- **external_api_calls_allowed**: False
- **maximum_requests_allowed**: 0
- **prohibited_actions**: ['real_collection', 'network_enablement_without_separate_approval', 'market_data_file_creation_outside_fixtures', 'real_trading', 'paper_live', 'preregistration']
- **version**: V1.58.1
- **current_version**: V1.58.1