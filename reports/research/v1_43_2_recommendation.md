# V1 43 Recommendation V1.43.2

Status: COMPLETE

### Summary
```json
{
  "version": "V1.43.2",
  "recommended_next_step": "research regime-aware raw/alpha feature set with stability constraints, keeping model outputs diagnostic-only",
  "evidence_classification": "DIAGNOSTIC_ONLY",
  "recommended_raw_feature_families_for_v1_44": [
    "alpha_score_or_model_output"
  ],
  "recommended_alpha_feature_families_for_v1_44": [
    "alpha_score_or_model_output"
  ],
  "diagnostic_only_model_output_features": [
    "predicted_probability",
    "calibrated_probability_rebuilt",
    "calibrated_probability",
    "predicted_probability_calibrated"
  ],
  "no_new_filter": true,
  "no_strategy_validated": true,
  "no_preregistration_yet": true,
  "no_paper_live": true,
  "no_money_deployment": true,
  "no_real_trading": true,
  "holdout_executed": false,
  "codex_cli_called": false
}
```