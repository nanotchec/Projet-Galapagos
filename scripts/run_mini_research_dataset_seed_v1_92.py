...
def main() -> None:
    ...
    summary = _base_payload(pytest_result=pytest_result, file_audit=file_audit)
...
def _pytest_result() -> dict[str, Any]:
    test_path = PROJECT_ROOT / "tests/research/test_mini_research_dataset_seed_v1_92.py"
...
def _base_payload(*, pytest_result: dict[str, Any], file_audit: dict[str, Any]) -> dict[str, Any]:
    return {
        ...
        "report_index_references_v1_92": True,
...
def _update_state(payload: dict[str, Any]) -> None:
    ...
    _write_md(
        ...
        [
            "- Version : V1.92",
            "- Verdict : V1_92_MINI_RESEARCH_DATASET_SEED_ULTRA_BOUNDED_PASSED",
    ...
    _write_md(
        ...
        "Latest Summary V1.92",
        [
            "V1.92 cree un mini research dataset seed ultra-borne.",
...
def _write_docs() -> None:
    _write_md(
        PROJECT_ROOT / "docs/code_review_v1_92.md",
        "Code Review V1.92",
        [
            "V1.92 limite l'ecriture data au dossier autorise data/research/dataset_seed/v1_92/.",
            "Les fichiers V1.84, V1.87 et V1.90 sont lus en lecture seule avec verification de hashes.",
            "Le seed exclut labels, targets, predictions, champs de performance future, EV, MFE et MAE.",
            "Le systeme ne peut pas passer d'ordre reel.",
        ],
    )
    _write_md(
        PROJECT_ROOT / "docs/mini_research_dataset_seed_v1_92.md",
        "Mini Research Dataset Seed V1.92",
        [
...
def _write_index() -> None:
    ...
    if "V1.92: Mini Research Dataset Seed" not in content:
        section = (
...
def _write_release_placeholders(summary: dict[str, Any]) -> None:
    release = {
        ...
        "release_zip_path": "projet-galapagos-v1.92-clean.zip",
        "blocking_reason": None,
        "required_reports_present": True,
    ...
    for name, payload in [
        ("release_zip_v1_92", release),
        ("zip_audit_v1_92", audit),
        ("zip_smoke_test_v1_92", smoke),
    ]:
        write_research_report(
            name=name,
            payload={**payload, "dataset_seed_created": summary["dataset_seed_created"]},
            title=name.replace("_", " ").title(),
            lines=["Rapport de packaging V1.92 coherent avec l'etat projet."],
            output_dir="reports",
        )
