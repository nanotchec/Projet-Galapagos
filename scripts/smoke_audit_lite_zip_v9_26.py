...
def smoke_zip(zip_path: Path) -> dict[str, Any]:
    errors: list[str] = []
    with tempfile.TemporaryDirectory(prefix="galapagos_v9_26_smoke_") as tmp:
        ...
        errors.extend(_run_v9_26_tests(extract_root, env))
...
def _import_modules(extract_root: Path, env: dict[str, str]) -> list[str]:
    ...
    for module in [
        ...
        "galapagos.data.aggtrades_post_v9_storage_resume_campaign_v9_26",
        "galapagos.data.aggtrades_post_v9_storage_resume_campaign_v9_26_validation",
    ]:
...
def _run_v9_26_tests(extract_root: Path, env: dict[str, str]) -> list[str]:
    completed = _run(
        [
            "python",
            "-m",
    ...
    if completed["returncode"] != 0:
        return [f"V9.26 tests failed: {completed['stdout'][-1000:]} {completed['stderr'][-1000:]}"]
    return []


def _run_self_audit(extract_root: Path, zip_path: Path, env: dict[str, str]) -> list[str]:
    completed = _run(["python", "scripts/audit_audit_lite_zip_v9_26.py", "--zip", str(zip_path)], extract_root, env, AUDIT_TIMEOUT_SECONDS)
    if completed["returncode"] != 0:
...
def _check_reports(extract_root: Path) -> list[str]:
    ...
    if report.get("version") != VERSION:
        errors.append("V9.26 report version mismatch")
    if report.get("decision") != "resume_collection_not_executed_storage_blocker":
        errors.append("V9.26 smoke expects storage blocker decision")
    if summary.get("days_downloaded_total") != 0 or summary.get("days_normalized_total") != 0:
        errors.append("V9.26 smoke expects no download and no ingestion")
    if report.get("safety_flags", {}).get("network_used") is not False:
        errors.append("V9.26 smoke expects no network")
    return errors
...
def _write_reports(result: dict[str, Any]) -> None:
    ...
    (report_dir / "zip_smoke_v9_26.json").write_text(json.dumps(result, indent=2, ensure_ascii=False) + "\n", encoding="utf-8")
    (report_dir / "zip_smoke_v9_26.md").write_text(
        "# Smoke ZIP V9.26\n\n"
        ...
        f"- Timeout tests V9.26 : `{TEST_TIMEOUT_SECONDS}` secondes.\n"
