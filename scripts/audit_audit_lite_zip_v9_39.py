...
VERSION = "V9.39"
ZIP_NAME = "projet-galapagos-v9.39-audit-lite.zip"

REQUIRED_FILES = [
    "scripts/_bootstrap.py",
    "src/galapagos/datasets/ohlcv_aggtrades_5y_dataset_v9_39.py",
    "src/galapagos/datasets/ohlcv_aggtrades_5y_dataset_v9_39_validation.py",
    "src/galapagos/datasets/ohlcv_aggtrades_5y_dataset_v9_39_schemas.py",
    "src/galapagos/datasets/ohlcv_aggtrades_5y_dataset_v9_39_datacard.py",
    "scripts/run_ohlcv_aggtrades_5y_dataset_v9_39.py",
    "scripts/validate_ohlcv_aggtrades_5y_dataset_v9_39.py",
    "scripts/release_audit_lite_zip_v9_39.py",
    "scripts/audit_audit_lite_zip_v9_39.py",
    "scripts/smoke_audit_lite_zip_v9_39.py",
    "tests/datasets/test_ohlcv_aggtrades_5y_dataset_v9_39.py",
    "tests/validation/test_ohlcv_aggtrades_5y_dataset_v9_39_validator.py",
    "reports/manifests/ohlcv_aggtrades_5y_dataset_v9_39_manifest.json",
    "reports/datasets/ohlcv_aggtrades_5y_dataset_v9_39.json",
    "reports/datasets/ohlcv_aggtrades_5y_dataset_v9_39.md",
    "reports/datasets/ohlcv_aggtrades_5y_dataset_v9_39_datacard.md",
    "docs/ohlcv_aggtrades_5y_dataset_v9_39.md",
    "reports/audit_lite/v9_39_command_results.json",
    "reports/audit_lite/v9_39_command_results.md",
    "reports/audit_lite/v9_39_full_local_validation_attestation.json",
    "reports/audit_lite/v9_39_full_local_validation_attestation.md",
    "reports/audit_lite/v9_39_artifact_inventory.json",
    "reports/audit_lite/v9_39_artifact_inventory.md",
    "reports/audit_lite/zip_size_report_v9_39.json",
    "reports/audit_lite/zip_size_report_v9_39.md",
    "reports/audit_lite/zip_audit_v9_39.json",
    "reports/audit_lite/zip_audit_v9_39.md",
    "reports/audit_lite/zip_smoke_v9_39.json",
    "reports/audit_lite/zip_smoke_v9_39.md",
...
def audit_zip(zip_path: Path) -> list[str]:
    ...
    errors: list[str] = []
    with tempfile.TemporaryDirectory(prefix="galapagos_v9_39_audit_") as tmp:
...
def _check_inventory(extract_root: Path, names: list[str]) -> list[str]:
    errors: list[str] = []
    inventory = _read_json(extract_root / "reports/audit_lite/v9_39_artifact_inventory.json")
    size_report = _read_json(extract_root / "reports/audit_lite/zip_size_report_v9_39.json")
...
def _check_reports(extract_root: Path) -> list[str]:
    errors: list[str] = []
    report = _read_json(extract_root / "reports/datasets/ohlcv_aggtrades_5y_dataset_v9_39.json")
    manifest = _read_json(extract_root / "reports/manifests/ohlcv_aggtrades_5y_dataset_v9_39_manifest.json")
    attestation = _read_json(extract_root / "reports/audit_lite/v9_39_full_local_validation_attestation.json")
    if report.get("version") != VERSION or report.get("source_version") != "V9.38":
        errors.append("V9.39 report version/source mismatch")
    if manifest.get("decision") != report.get("decision") or manifest.get("safety_flags") != report.get("safety_flags"):
        errors.append("V9.39 manifest mismatch")
    if report.get("decision") != "ohlcv_aggtrades_5y_dataset_blocked_by_missing_labels":
        errors.append("audit-lite V9.39 expects missing-label blocker decision")
    if report.get("dataset_created") is not False:
        errors.append("V9.39 must not create a dataset when labels are missing")
...
def _check_command_results(extract_root: Path) -> list[str]:
    payload = _read_json(extract_root / "reports/audit_lite/v9_39_command_results.json")
    ...
    for needle in ["pytest --collect-only -q", "test_ohlcv_aggtrades_5y_dataset_v9_39.py", "test_ohlcv_aggtrades_5y_dataset_v9_39_validator.py", "run_ohlcv_aggtrades_5y_dataset_v9_39.py", "validate_ohlcv_aggtrades_5y_dataset_v9_39.py", "release_audit_lite_zip_v9_39.py"]:
...
def _check_state(extract_root: Path) -> list[str]:
    ...
    errors: list[str] = []
    if metrics.get("last_validated_version") != "V9.38":
    ...
    if metrics.get("direction") != "ohlcv_aggtrades_5y_dataset":
...
def _write_reports(result: dict[str, Any]) -> None:
    ...
    (report_dir / "zip_audit_v9_39.json").write_text(json.dumps(result, indent=2, ensure_ascii=False) + "\n", encoding="utf-8")
    (report_dir / "zip_audit_v9_39.md").write_text("# Audit ZIP V9.39\n\n" f"- Resultat : `{'PASS' if result['passed'] else 'FAIL'}`.\n" f"- Erreurs : `{result['errors']}`.\n" "- Aucun sidecar et aucune empreinte ZIP.\n", encoding="utf-8")
