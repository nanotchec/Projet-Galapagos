...
VERSION = "V5.1"
REPORT_JSON = Path("reports/audit_lite/zip_audit_v5_1.json")
REPORT_MD = Path("reports/audit_lite/zip_audit_v5_1.md")
REQUIRED_FILES = {
    "README.md",
    "pyproject.toml",
    "reports/manifests/max_history_causal_feature_store_v5_1_manifest.json",
    "reports/features/max_history_causal_feature_store_v5_1.json",
    "reports/features/max_history_causal_feature_store_v5_1.md",
    "reports/audit_lite/v5_1_artifact_inventory.json",
    "reports/audit_lite/v5_1_artifact_inventory.md",
    "reports/audit_lite/v5_1_parquet_summary.json",
    "reports/audit_lite/v5_1_full_local_validation_attestation.json",
    "reports/audit_lite/v5_1_full_local_validation_attestation.md",
    "reports/PROJECT_STATE.json",
    "reports/current/latest_summary.md",
    "docs/max_history_causal_feature_store_v5_1.md",
...
def audit_zip(zip_path: Path) -> dict[str, Any]:
    ...
    with tempfile.TemporaryDirectory(prefix="galapagos-v5-1-audit-") as tmp:
        ...
        attestation = _read_json(extract_root / "reports/audit_lite/v5_1_full_local_validation_attestation.json")
        if manifest != report:
            errors.append("V5.1 manifest/report mismatch in ZIP")
        if manifest.get("version") != VERSION or manifest.get("status") != "PASS":
            errors.append("V5.1 manifest version/status mismatch in ZIP")
        ...
        if len(inventory.get("full_parquet_excluded", [])) != 4:
            errors.append("V5.1 full feature parquet exclusion count mismatch")
        ...
        for flag in [
            ...
            if attestation.get(flag) is not True:
                errors.append(f"V5.1 attestation flag must be true: {flag}")
...
def _render_markdown(result: dict[str, Any]) -> str:
    ...
    return f"""# Audit ZIP audit-lite V5.1
