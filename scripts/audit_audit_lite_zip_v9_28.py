...
REQUIRED_FILES = [
    ...
    "scripts/run_aggtrades_post_v9_bad_day_repair_v9_28.py",
    "scripts/validate_aggtrades_post_v9_bad_day_repair_v9_28.py",
    "scripts/release_audit_lite_zip_v9_28.py",
    "scripts/audit_audit_lite_zip_v9_28.py",
    "scripts/smoke_audit_lite_zip_v9_28.py",
    "src/galapagos/data/aggtrades_post_v9_bad_day_repair_v9_28.py",
    "src/galapagos/data/aggtrades_post_v9_bad_day_repair_v9_28_validation.py",
    "tests/data/test_aggtrades_post_v9_bad_day_repair_v9_28.py",
    "tests/validation/test_aggtrades_post_v9_bad_day_repair_v9_28_validator.py",
    "reports/manifests/aggtrades_post_v9_bad_day_repair_v9_28_manifest.json",
    "reports/data/aggtrades_post_v9_bad_day_repair_v9_28.json",
    "reports/data/aggtrades_post_v9_bad_day_repair_v9_28.md",
    "docs/aggtrades_post_v9_bad_day_repair_v9_28.md",
    "reports/audit_lite/v9_28_command_results.json",
    "reports/audit_lite/v9_28_command_results.md",
    "reports/audit_lite/v9_28_full_local_validation_attestation.json",
    "reports/audit_lite/v9_28_full_local_validation_attestation.md",
    "reports/audit_lite/v9_28_artifact_inventory.json",
    "reports/audit_lite/v9_28_artifact_inventory.md",
    "reports/audit_lite/zip_size_report_v9_28.json",
    "reports/audit_lite/zip_size_report_v9_28.md",
    "reports/audit_lite/zip_audit_v9_28.json",
    "reports/audit_lite/zip_audit_v9_28.md",
    "reports/audit_lite/zip_smoke_v9_28.json",
    "reports/audit_lite/zip_smoke_v9_28.md",
    "reports/data/aggtrades_post_v9_storage_recheck_resume_v9_27.json",
    "reports/data/aggtrades_post_v9_storage_recheck_batch06_v9_27.json",
...
def audit_zip(zip_path: Path) -> list[str]:
    ...
    errors: list[str] = []
    with tempfile.TemporaryDirectory(prefix="galapagos_v9_28_audit_") as tmp:
...
def _check_inventory(extract_root: Path, names: list[str]) -> list[str]:
    errors: list[str] = []
    inventory = _read_json(extract_root / "reports/audit_lite/v9_28_artifact_inventory.json")
    size_report = _read_json(extract_root / "reports/audit_lite/zip_size_report_v9_28.json")
...
def _check_reports(extract_root: Path) -> list[str]:
    errors: list[str] = []
    report = _read_json(extract_root / "reports/data/aggtrades_post_v9_bad_day_repair_v9_28.json")
    manifest = _read_json(extract_root / "reports/manifests/aggtrades_post_v9_bad_day_repair_v9_28_manifest.json")
    attestation = _read_json(extract_root / "reports/audit_lite/v9_28_full_local_validation_attestation.json")
    summary = report.get("v9_28_summary", {})
    if report.get("version") != VERSION or report.get("source_version") != "V9.27":
        errors.append("V9.28 report version/source mismatch")
    ...
    if manifest.get("decision") != report.get("decision") or manifest.get("safety_flags") != report.get("safety_flags"):
        errors.append("V9.28 manifest mismatch")
    if summary.get("repair_applied") is True and summary.get("duplicate_conflict_count") != 0:
        errors.append("V9.28 repair cannot be applied to conflicting duplicates")
    if report.get("decision") == "bad_day_repaired_and_remaining_window_completed":
        if summary.get("local_file_coverage_start") != "2024-05-05" or summary.get("local_file_coverage_end") != "2026-05-05":
            errors.append("V9.28 completed decision must cover full target window")
        if summary.get("complete_collection_reached") is not True or summary.get("future_full_coverage_complete") is not True:
            errors.append("V9.28 completed decision must set full completion flags")
    if report.get("decision") != "bad_day_repaired_and_remaining_window_completed" and summary.get("complete_collection_reached") is True:
        errors.append("V9.28 non-completed decision cannot claim full completion")
...
def _check_flags(report: dict[str, Any], attestation: dict[str, Any]) -> list[str]:
    ...
    for key in ["no_trading", "no_paper_live", "no_orders", "no_backtest", "no_walk_forward", "no_strategy", "no_actionable_signal", "no_persistent_model", "no_destructive_cleanup", "no_sidecars", "no_zip_fingerprints"]:
    ...
    if flags.get("network_used"):
        if flags.get("network_scope") != "public_archive_read_only":
            errors.append("collection must use public archive read-only scope")
    return errors


def _check_command_results(extract_root: Path) -> list[str]:
    payload = _read_json(extract_root / "reports/audit_lite/v9_28_command_results.json")
    ...
    for needle in [
        ...
        "test_aggtrades_post_v9_bad_day_repair_v9_28.py",
        "test_aggtrades_post_v9_bad_day_repair_v9_28_validator.py",
        "run_aggtrades_post_v9_bad_day_repair_v9_28.py",
        "validate_aggtrades_post_v9_bad_day_repair_v9_28.py",
        "release_audit_lite_zip_v9_28.py",
    ]:
...
def _check_state(extract_root: Path) -> list[str]:
    ...
    for name, payload in {"PROJECT_STATE": state, "latest_metrics": metrics}.items():
        if payload.get("last_validated_version") != "V9.27":
        ...
        if payload.get("direction") != "aggtrades_post_v9_bad_day_repair_final_completion":
...
def _write_reports(result: dict[str, Any]) -> None:
    ...
    (report_dir / "zip_audit_v9_28.json").write_text(json.dumps(result, indent=2, ensure_ascii=False) + "\n", encoding="utf-8")
    (report_dir / "zip_audit_v9_28.md").write_text(
        "# Audit ZIP V9.28\n\n"
