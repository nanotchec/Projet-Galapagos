...
def main() -> None:
    ...
    with tempfile.TemporaryDirectory(prefix="galapagos_v3_3_1_smoke_") as tmp:
        tmp_path = Path(tmp)
    ...
    Path("reports/zip_smoke_test_v3_3_1.json").write_text(json.dumps(payload, indent=2, ensure_ascii=False) + "\n", encoding="utf-8")
    Path("reports/zip_smoke_test_v3_3_1.md").write_text(
        "# Smoke ZIP V3.3.1\n\n"
...
def _validate_state_manifest_and_report(root: Path) -> list[str]:
    ...
    if manifest.get("version") != ARTIFACT_VERSION or report.get("version") != ARTIFACT_VERSION:
    ...
    if project_state.get("candidate_version") != VERSION and VERSION not in latest_summary:
        errors.append("PROJECT_STATE/latest_summary must identify V3.3.1 as candidate")
    ...
    if "pending_external_audit" not in latest_summary and project_state.get("candidate_status") != "pending_external_audit":
        errors.append("V3.3.1 must remain pending_external_audit")
