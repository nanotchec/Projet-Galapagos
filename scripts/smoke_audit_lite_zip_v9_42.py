...
SAMPLE_CHECK_TIMEOUT_SECONDS = 30
...
def smoke_zip(zip_path: Path) -> dict[str, Any]:
    ...
    with tempfile.TemporaryDirectory(prefix="galapagos_v9_42_smoke_") as tmp:
        ...
        pyarrow_errors = _check_pyarrow_available(extract_root, env)
        errors.extend(pyarrow_errors)
        errors.extend(_import_modules(extract_root, env))
        errors.extend(_run_pytest_collect(extract_root, env))
        errors.extend(_run_v9_42_tests(extract_root, env))
        errors.extend(_run_audit_lite_validator(extract_root, env))
        if not pyarrow_errors:
            errors.extend(_check_sample_parquets(extract_root, env))
...
def _check_packaging_names(names: list[str]) -> list[str]:
    ...
    if not any(name.startswith("data/audit_samples/v9_42/") for name in names):
        errors.append("ZIP must include V9.42 audit samples")
    return errors
...
def _check_pyarrow_available(extract_root: Path, env: dict[str, str]) -> list[str]:
    ...
    if completed["returncode"] != 0:
        return ["pyarrow is required for V9.42 sample parquet smoke checks but is not importable"]
    return []
...
def _import_modules(extract_root: Path, env: dict[str, str]) -> list[str]:
    errors: list[str] = []
    for module in [
        "galapagos.datasets.ohlcv_aggtrades_5y_dataset_validation_v9_42",
        "galapagos.datasets.ohlcv_aggtrades_5y_dataset_validation_v9_42_validation",
    ]:
...
def _run_pytest_collect(extract_root: Path, env: dict[str, str]) -> list[str]:
    completed = _run(["python", "-m", "pytest", "--collect-only", "-q", "tests/datasets/test_ohlcv_aggtrades_5y_dataset_validation_v9_42.py", "tests/validation/test_ohlcv_aggtrades_5y_dataset_validation_v9_42_validator.py"], extract_root, env, PYTEST_TIMEOUT_SECONDS)
    if completed["returncode"] != 0:
        return [f"pytest collect-only failed: {completed['stdout'][-1000:]} {completed['stderr'][-1000:]}"]
    return []


def _run_v9_42_tests(extract_root: Path, env: dict[str, str]) -> list[str]:
    completed = _run(["python", "-m", "pytest", "-q", "tests/datasets/test_ohlcv_aggtrades_5y_dataset_validation_v9_42.py", "tests/validation/test_ohlcv_aggtrades_5y_dataset_validation_v9_42_validator.py"], extract_root, env, TEST_TIMEOUT_SECONDS)
    if completed["returncode"] != 0:
        return [f"V9.42 tests failed: {completed['stdout'][-1000:]} {completed['stderr'][-1000:]}"]
    return []


def _run_audit_lite_validator(extract_root: Path, env: dict[str, str]) -> list[str]:
    completed = _run(["python", "scripts/validate_ohlcv_aggtrades_5y_dataset_validation_v9_42.py", "--mode", "audit-lite"], extract_root, env, AUDIT_TIMEOUT_SECONDS)
    if completed["returncode"] != 0:
        return [f"V9.42 audit-lite validator failed: {completed['stdout'][-1000:]} {completed['stderr'][-1000:]}"]
    return []


def _check_sample_parquets(extract_root: Path, env: dict[str, str]) -> list[str]:
    script = """
import json
from pathlib import Path
import pandas as pd
payload=json.loads(Path('reports/datasets/ohlcv_aggtrades_5y_dataset_validation_v9_42_samples.json').read_text())
for timeframe,item in payload['samples'].items():
    frame=pd.read_parquet(item['path'])
    assert len(frame) == item['rows']
    assert 'row_valid_for_dataset' in frame.columns
    assert 'up_down_flat_volnorm_h1_5y' in frame.columns
print('sample parquet checks passed')
"""
    completed = _run(["python", "-c", script], extract_root, env, SAMPLE_CHECK_TIMEOUT_SECONDS)
    if completed["returncode"] != 0:
        return [f"sample parquet checks failed: {completed['stdout'][-1000:]} {completed['stderr'][-1000:]}"]
    return []


def _run_self_audit(extract_root: Path, zip_path: Path, env: dict[str, str]) -> list[str]:
    completed = _run(["python", "scripts/audit_audit_lite_zip_v9_42.py", "--zip", str(zip_path)], extract_root, env, AUDIT_TIMEOUT_SECONDS)
    if completed["returncode"] != 0:
