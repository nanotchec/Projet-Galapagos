...
def smoke_zip(zip_path: Path) -> dict[str, Any]:
    errors: list[str] = []
    with tempfile.TemporaryDirectory(prefix="galapagos_v9_30_smoke_") as tmp:
        ...
        errors.extend(_run_v9_30_tests(extract_root, env))
...
def _import_modules(extract_root: Path, env: dict[str, str]) -> list[str]:
    ...
    for module in [
        ...
        "galapagos.data.aggtrades_post_v9_bad_day_repair_v9_28",
        "galapagos.data.aggtrades_5y_extension_plan_v9_30",
        "galapagos.data.aggtrades_5y_extension_plan_v9_30_validation",
    ]:
...
def _run_v9_30_tests(extract_root: Path, env: dict[str, str]) -> list[str]:
    completed = _run(
        [
            "python",
            "-m",
    ...
    if completed["returncode"] != 0:
        return [f"V9.30 tests failed: {completed['stdout'][-1000:]} {completed['stderr'][-1000:]}"]
    return []
...
def _check_reports(extract_root: Path) -> list[str]:
    ...
    if report.get("version") != VERSION:
        errors.append("V9.30 report version mismatch")
    if report.get("decision", "").startswith("aggtrades_5y_extension_plan_ready"):
        if report.get("target_5y_window_start") != "2021-05-05" or report.get("target_5y_window_end") != "2026-05-05":
            errors.append("V9.30 smoke expects target 5Y window")
        if report.get("extension_days_needed") != 1096:
            errors.append("V9.30 smoke expects 1096 extension days")
        if report.get("network_used") is not False or report.get("new_data_downloaded") is not False:
            errors.append("V9.30 smoke expects no network and no new download")
    else:
        if report.get("ingestion_executed") is True:
            errors.append("V9.30 non-ready smoke cannot claim ingestion")
    return errors


def _run_self_audit(extract_root: Path, zip_path: Path, env: dict[str, str]) -> list[str]:
    completed = _run(["python", "scripts/audit_audit_lite_zip_v9_30.py", "--zip", str(zip_path)], extract_root, env, AUDIT_TIMEOUT_SECONDS)
    if completed["returncode"] != 0:
...
def _write_reports(result: dict[str, Any]) -> None:
    ...
    (report_dir / "zip_smoke_v9_30.json").write_text(json.dumps(result, indent=2, ensure_ascii=False) + "\n", encoding="utf-8")
    (report_dir / "zip_smoke_v9_30.md").write_text(
        "# Smoke ZIP V9.30\n\n"
        ...
        f"- Timeout tests V9.30 : `{TEST_TIMEOUT_SECONDS}` secondes.\n"
