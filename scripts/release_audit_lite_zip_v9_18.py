...
VERSION = "V9.18"
ZIP_NAME = "projet-galapagos-v9.18-audit-lite.zip"
ROOT = Path(".").resolve()
...
REPORT_PATHS = [
    ...
    Path("docs/aggtrades_post_v9_collection_v9_18.md"),
]

AUDIT_PATHS = [
    Path("reports/audit_lite/v9_18_command_results.json"),
    Path("reports/audit_lite/v9_18_command_results.md"),
    Path("reports/audit_lite/v9_18_full_local_validation_attestation.json"),
    Path("reports/audit_lite/v9_18_full_local_validation_attestation.md"),
    Path("reports/audit_lite/v9_18_artifact_inventory.json"),
    Path("reports/audit_lite/v9_18_artifact_inventory.md"),
    Path("reports/audit_lite/zip_size_report_v9_18.json"),
    Path("reports/audit_lite/zip_size_report_v9_18.md"),
    Path("reports/audit_lite/zip_audit_v9_18.json"),
    Path("reports/audit_lite/zip_audit_v9_18.md"),
    Path("reports/audit_lite/zip_smoke_v9_18.json"),
    Path("reports/audit_lite/zip_smoke_v9_18.md"),
]
...
SCRIPT_PATHS = [
    ...
    Path("scripts/run_aggtrades_post_v9_collection_v9_18.py"),
    Path("scripts/validate_aggtrades_post_v9_collection_v9_18.py"),
    Path("scripts/release_audit_lite_zip_v9_18.py"),
    Path("scripts/audit_audit_lite_zip_v9_18.py"),
    Path("scripts/smoke_audit_lite_zip_v9_18.py"),
]

TEST_PATHS = [
    Path("tests/data/test_aggtrades_post_v9_collection_v9_18.py"),
    Path("tests/validation/test_aggtrades_post_v9_collection_v9_18_validator.py"),
]

CODE_PATHS = [
    Path("src/galapagos/__init__.py"),
    Path("src/galapagos/data/aggtrades_post_v9_collection_v9_18.py"),
    Path("src/galapagos/data/aggtrades_post_v9_collection_v9_18_validation.py"),
]
...
def main() -> int:
    _ensure_inputs()
    report = _read_json(Path("reports/data/aggtrades_post_v9_collection_v9_18.json"))
...
def _ensure_inputs() -> None:
    ...
    if missing:
        raise FileNotFoundError(f"missing V9.18 audit-lite inputs: {missing}")


def _ensure_placeholders() -> None:
    placeholders = {
        "reports/audit_lite/v9_18_command_results.json": {"version": VERSION, "status": "PENDING_CAPTURE", "commands": [], "sidecars_created": False, "zip_fingerprints_created": False},
        "reports/audit_lite/zip_audit_v9_18.json": {"version": VERSION, "passed": False, "errors": [], "status": "PENDING_RUN"},
        "reports/audit_lite/zip_smoke_v9_18.json": {"version": VERSION, "passed": False, "errors": [], "status": "PENDING_RUN"},
    }
    ...
    for raw_path, text in {
        "reports/audit_lite/v9_18_command_results.md": "# Commandes V9.18\n\nRapport en attente de capture finale.\n",
        "reports/audit_lite/zip_audit_v9_18.md": "# Audit ZIP V9.18\n\nRapport en attente d'execution.\n",
        "reports/audit_lite/zip_smoke_v9_18.md": "# Smoke ZIP V9.18\n\nRapport en attente d'execution.\n",
    }.items():
        path = Path(raw_path)
        if not (ROOT / path).exists():
...
def _write_attestation(report: dict[str, Any], zip_bytes_estimate: int | None) -> None:
    command_results = _read_optional_json(Path("reports/audit_lite/v9_18_command_results.json"))
    commands = command_results.get("commands", [])
    payload = {
        "version": VERSION,
        "validation_scope": "aggtrades_post_v9_collection_pack_dry_run_plus_audit_lite_packaging",
        ...
        "validator_passed": _commands_passed(commands, ["validate_aggtrades_post_v9_collection_v9_18.py"]),
        "audit_lite_passed": _commands_passed(commands, ["audit_audit_lite_zip_v9_18.py"]),
        "smoke_audit_lite_passed": _commands_passed(commands, ["smoke_audit_lite_zip_v9_18.py"]),
    ...
    _write_json(Path("reports/audit_lite/v9_18_full_local_validation_attestation.json"), payload)
    _write_text(
        Path("reports/audit_lite/v9_18_full_local_validation_attestation.md"),
        "# Attestation full locale V9.18\n\n"
        f"- Decision : `{report['v9_18_decision']['decision']}`.\n"
        f"- Mode : `{report['mode']}`.\n"
        "- Collecte executee : `False` en dry-run.\n"
        "- Aucun reseau, telechargement ou ingestion executee en dry-run.\n"
...
def _write_inventory(paths: list[Path], zip_bytes_estimate: int | None) -> None:
    ...
    _write_json(Path("reports/audit_lite/v9_18_artifact_inventory.json"), payload)
    _write_text(
        Path("reports/audit_lite/v9_18_artifact_inventory.md"),
        "# Inventaire audit-lite V9.18\n\n"
        f"- ZIP : `{ZIP_NAME}`.\n"
        f"- Fichiers inclus : `{len(paths)}`.\n"
        f"- Taille ZIP estimee non autoritative : `{zip_bytes_estimate}`.\n"
        "- Exclusions : data/raw, data/silver, data/research, data/gold, backtests, strategies, ordres, execution, modeles, caches, secrets, `Icon`, sidecars SHA256 et empreintes ZIP.\n",
    )
...
def _write_size_report(paths: list[Path], zip_bytes_estimate: int | None) -> None:
    ...
    _write_json(Path("reports/audit_lite/zip_size_report_v9_18.json"), payload)
    _write_text(Path("reports/audit_lite/zip_size_report_v9_18.md"), f"# Taille ZIP V9.18\n\n- ZIP : `{ZIP_NAME}`.\n- Taille bytes estimee non autoritative : `{zip_bytes_estimate}`.\n- Fichiers inclus : `{len(paths)}`.\n")
