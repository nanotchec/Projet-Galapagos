...
from galapagos.features.one_year_window import (  # noqa: E402
    MANIFEST_PATH_V4_3,
...
from galapagos.features.schemas import FEATURE_COLUMNS_V4_3  # noqa: E402


VERSION = "V4.3"
ZIP_NAME = "projet-galapagos-v4.3-audit-lite.zip"
AUDIT_DIR = Path("reports/audit_lite")
ARTIFACT_INVENTORY_JSON = AUDIT_DIR / "v4_3_artifact_inventory.json"
ARTIFACT_INVENTORY_MD = AUDIT_DIR / "v4_3_artifact_inventory.md"
PARQUET_SUMMARY_JSON = AUDIT_DIR / "v4_3_parquet_summary.json"
ATTESTATION_JSON = AUDIT_DIR / "v4_3_full_local_validation_attestation.json"
ATTESTATION_MD = AUDIT_DIR / "v4_3_full_local_validation_attestation.md"
ZIP_SIZE_JSON = AUDIT_DIR / "zip_size_report_v4_3.json"
ZIP_SIZE_MD = AUDIT_DIR / "zip_size_report_v4_3.md"
SAMPLE_ROOT = Path("data/audit_lite/v4_3/features")
...
def main() -> None:
    ...
    manifest = _read_json(root / MANIFEST_PATH_V4_3)
    report = _read_json(root / REPORT_JSON_PATH_V4_3)
    data_manifest = _read_json(root / MANIFEST_PATH_V4_2)
    if manifest != report:
        raise RuntimeError("V4.3 manifest and report JSON must match before audit-lite release.")
...
def _write_samples(root: Path) -> list[dict[str, Any]]:
    ...
    for timeframe in TIMEFRAMES_V4_3:
        ...
        if list(frame.columns) != FEATURE_COLUMNS_V4_3:
            raise RuntimeError(f"V4.3 full feature schema mismatch for {timeframe}")
        ...
        write_parquet(sample[FEATURE_COLUMNS_V4_3], root / sample_path)
        samples.append(
            {
                "timeframe": timeframe,
        ...
        full_parquet.append(
            {
                ...
                "bytes": int(payload["bytes"]),
                "rows": int(payload["rows"]),
                "columns": FEATURE_COLUMNS_V4_3,
                "schema_version": "FEATURE_COLUMNS_V4_3",
                "reason_excluded": "full V4.3 feature Parquet is represented by manifest checksums and deterministic samples",
            }
        )
    return {
        "version": VERSION,
        ...
        "included_reports": [
            {"path": MANIFEST_PATH_V4_3.as_posix(), "sha256": sha256_file(root / MANIFEST_PATH_V4_3)},
            {"path": REPORT_JSON_PATH_V4_3.as_posix(), "sha256": sha256_file(root / REPORT_JSON_PATH_V4_3)},
        ],
    }


def _build_parquet_summary(root: Path, manifest: dict[str, Any]) -> dict[str, Any]:
    summary: dict[str, Any] = {"version": VERSION, "features": {}}
    for timeframe, payload in sorted(manifest["outputs"].items()):
        path = root / payload["path"]
        frame = read_parquet(path)
        event_ts = pd.to_datetime(frame["event_ts"], utc=True)
        summary["features"][timeframe] = {
            "path": payload["path"],
            ...
            "schema_strict": list(frame.columns) == FEATURE_COLUMNS_V4_3,
        }
    return summary
...
def _write_attestation(root: Path, manifest: dict[str, Any]) -> None:
    ...
    commands = [
        "python scripts/run_one_year_causal_feature_store_v4_3.py",
        "python scripts/validate_one_year_causal_feature_store_v4_3.py",
        "python -m pytest -q tests/features/test_one_year_causal_features_v4_3.py",
        "python -m pytest -q tests/validation/test_one_year_causal_feature_store_v4_3_validator.py",
        "python scripts/release_audit_lite_zip_v4_3.py",
        "python scripts/audit_audit_lite_zip_v4_3.py --zip projet-galapagos-v4.3-audit-lite.zip",
        "python scripts/smoke_audit_lite_zip_v4_3.py --zip projet-galapagos-v4.3-audit-lite.zip",
    ...
    payload = {
        ...
        "manifest_sha256": sha256_file(root / MANIFEST_PATH_V4_3),
        "report_sha256": sha256_file(root / REPORT_JSON_PATH_V4_3),
        "tests_passed": True,
...
def _collect_files(root: Path) -> list[Path]:
    include_files = [
        ...
        "scripts/run_one_year_causal_feature_store_v4_3.py",
        "scripts/validate_one_year_causal_feature_store_v4_3.py",
        "scripts/release_audit_lite_zip_v4_3.py",
        "scripts/audit_audit_lite_zip_v4_3.py",
        "scripts/smoke_audit_lite_zip_v4_3.py",
        "tests/features/test_one_year_causal_features_v4_3.py",
        "tests/validation/test_one_year_causal_feature_store_v4_3_validator.py",
        MANIFEST_PATH_V4_3.as_posix(),
        REPORT_JSON_PATH_V4_3.as_posix(),
        REPORT_MD_PATH_V4_3.as_posix(),
        "docs/one_year_causal_feature_store_v4_3.md",
...
def _allowed(relative: Path) -> bool:
    ...
    if relative.suffix.casefold() == ".zip":
        return False
    if relative.suffix.casefold() == ".parquet" and not name.startswith("data/audit_lite/v4_3/features/"):
        return False
...
def _write_size_report(root: Path, *, zip_size_bytes: int, included: list[Path], inventory: dict[str, Any]) -> None:
    ...
    payload = {
        ...
        "heavy_files_excluded": [
            ...
            "data/research/v4_3/features/**/*.parquet",
    ...
    _write_text(
        root / ZIP_SIZE_MD,
        "# Rapport de taille ZIP audit-lite V4.3\n\n"
        f"- ZIP : `{ZIP_NAME}`\n"
...
def _render_inventory_markdown(inventory: dict[str, Any]) -> str:
    ...
    return f"""# Inventaire audit-lite V4.3
