...
REPORT_PATHS = [
    ...
    Path("docs/aggtrades_post_v9_batch_collection_v9_20.md"),
]

AUDIT_PATHS = [
    Path("reports/audit_lite/v9_20_command_results.json"),
    Path("reports/audit_lite/v9_20_command_results.md"),
    Path("reports/audit_lite/v9_20_full_local_validation_attestation.json"),
    Path("reports/audit_lite/v9_20_full_local_validation_attestation.md"),
    Path("reports/audit_lite/v9_20_artifact_inventory.json"),
    Path("reports/audit_lite/v9_20_artifact_inventory.md"),
    Path("reports/audit_lite/zip_size_report_v9_20.json"),
    Path("reports/audit_lite/zip_size_report_v9_20.md"),
    Path("reports/audit_lite/zip_audit_v9_20.json"),
    Path("reports/audit_lite/zip_audit_v9_20.md"),
    Path("reports/audit_lite/zip_smoke_v9_20.json"),
    Path("reports/audit_lite/zip_smoke_v9_20.md"),
]
...
SCRIPT_PATHS = [
    ...
    Path("scripts/run_aggtrades_post_v9_batch_collection_v9_20.py"),
    Path("scripts/validate_aggtrades_post_v9_batch_collection_v9_20.py"),
    Path("scripts/release_audit_lite_zip_v9_20.py"),
    Path("scripts/audit_audit_lite_zip_v9_20.py"),
    Path("scripts/smoke_audit_lite_zip_v9_20.py"),
]

TEST_PATHS = [
    Path("tests/data/test_aggtrades_post_v9_batch_collection_v9_20.py"),
    Path("tests/validation/test_aggtrades_post_v9_batch_collection_v9_20_validator.py"),
]
...
CODE_PATHS = [
    ...
    Path("src/galapagos/data/aggtrades_post_v9_batch_collection_v9_20.py"),
    Path("src/galapagos/data/aggtrades_post_v9_batch_collection_v9_20_validation.py"),
]
...
def main() -> int:
    _ensure_inputs()
    report = _read_json(Path("reports/data/aggtrades_post_v9_batch_collection_v9_20.json"))
...
def _ensure_inputs() -> None:
    ...
    if missing:
        raise FileNotFoundError(f"missing V9.20 audit-lite inputs: {missing}")


def _ensure_placeholders() -> None:
    placeholders = {
        "reports/audit_lite/v9_20_command_results.json": {"version": VERSION, "status": "PENDING_CAPTURE", "commands": [], "sidecars_created": False, "zip_fingerprints_created": False},
        "reports/audit_lite/zip_audit_v9_20.json": {"version": VERSION, "passed": False, "errors": [], "status": "PENDING_RUN"},
        "reports/audit_lite/zip_smoke_v9_20.json": {"version": VERSION, "passed": False, "errors": [], "status": "PENDING_RUN"},
    }
    ...
    texts = {
        "reports/audit_lite/v9_20_command_results.md": "# Commandes V9.20\n\nRapport en attente de capture finale.\n",
        "reports/audit_lite/zip_audit_v9_20.md": "# Audit ZIP V9.20\n\nRapport en attente d'execution.\n",
        "reports/audit_lite/zip_smoke_v9_20.md": "# Smoke ZIP V9.20\n\nRapport en attente d'execution.\n",
    }
...
def _write_attestation(report: dict[str, Any], zip_bytes_estimate: int | None) -> None:
    command_results = _read_optional_json(Path("reports/audit_lite/v9_20_command_results.json"))
    commands = command_results.get("commands", [])
    summary = report["batch_validation"]["summary"]
    payload = {
        "version": VERSION,
        "validation_scope": "aggtrades_post_v9_controlled_batch_collection_plus_audit_lite_packaging",
        ...
        "validator_passed": _commands_passed(commands, ["validate_aggtrades_post_v9_batch_collection_v9_20.py"]),
        "audit_lite_passed": _commands_passed(commands, ["audit_audit_lite_zip_v9_20.py"]),
        "smoke_audit_lite_passed": _commands_passed(commands, ["smoke_audit_lite_zip_v9_20.py"]),
    ...
    _write_json(Path("reports/audit_lite/v9_20_full_local_validation_attestation.json"), payload)
    _write_text(
        Path("reports/audit_lite/v9_20_full_local_validation_attestation.md"),
        "# Attestation full locale V9.20\n\n"
        f"- Decision : `{report['v9_20_decision']['decision']}`.\n"
        f"- Mode : `{report['mode']}`.\n"
...
def _write_inventory(paths: list[Path], zip_bytes_estimate: int | None) -> None:
    ...
    _write_json(Path("reports/audit_lite/v9_20_artifact_inventory.json"), payload)
    _write_text(
        Path("reports/audit_lite/v9_20_artifact_inventory.md"),
        "# Inventaire audit-lite V9.20\n\n"
        f"- ZIP : `{ZIP_NAME}`.\n"
...
def _write_size_report(paths: list[Path], zip_bytes_estimate: int | None) -> None:
    ...
    _write_json(Path("reports/audit_lite/zip_size_report_v9_20.json"), payload)
    _write_text(Path("reports/audit_lite/zip_size_report_v9_20.md"), f"# Taille ZIP V9.20\n\n- ZIP : `{ZIP_NAME}`.\n- Taille bytes estimee non autoritative : `{zip_bytes_estimate}`.\n- Fichiers inclus : `{len(paths)}`.\n")
