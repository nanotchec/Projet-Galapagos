...
SAMPLE_CHECK_TIMEOUT_SECONDS = 60
...
def smoke_zip(zip_path: Path) -> dict[str, Any]:
    ...
    with tempfile.TemporaryDirectory(prefix="galapagos_v9_52_to_v9_55_smoke_") as tmp:
        ...
        errors.extend(_run_tests(extract_root, env))
        errors.extend(_run_stage_validators(extract_root, env))
    ...
    return {
        ...
        "sample_checks_passed": sample_checks_passed,
        ...
        "sample_checks_timeout_seconds": SAMPLE_CHECK_TIMEOUT_SECONDS,
...
def _import_modules(extract_root: Path, env: dict[str, str]) -> list[str]:
    errors: list[str] = []
    for module in [
        "galapagos.research.derivatives_source_readiness_v9_52",
        "galapagos.data.derivatives_funding_oi_collection_v9_53",
        "galapagos.features.derivatives_funding_oi_feature_store_v9_54",
        "galapagos.features.derivatives_funding_oi_feature_store_validation_v9_55",
    ]:
...
def _run_tests(extract_root: Path, env: dict[str, str]) -> list[str]:
    tests = [
        "tests/research/test_derivatives_source_readiness_v9_52.py",
        "tests/validation/test_derivatives_source_readiness_v9_52_validator.py",
        "tests/data/test_derivatives_funding_oi_collection_v9_53.py",
        "tests/validation/test_derivatives_funding_oi_collection_v9_53_validator.py",
        "tests/features/test_derivatives_funding_oi_feature_store_v9_54.py",
        "tests/validation/test_derivatives_funding_oi_feature_store_v9_54_validator.py",
        "tests/features/test_derivatives_funding_oi_feature_store_validation_v9_55.py",
        "tests/validation/test_derivatives_funding_oi_feature_store_validation_v9_55_validator.py",
    ]
    completed = _run(["python", "-m", "pytest", "-q", *tests], extract_root, env, TEST_TIMEOUT_SECONDS)
    return [] if completed["returncode"] == 0 else [f"tests failed: {completed['stdout'][-1000:]} {completed['stderr'][-1000:]}"]


def _run_stage_validators(extract_root: Path, env: dict[str, str]) -> list[str]:
    commands = [
        ["python", "scripts/validate_derivatives_source_readiness_v9_52.py", "--mode", "audit-lite"],
        ["python", "scripts/validate_derivatives_funding_oi_collection_v9_53.py", "--mode", "audit-lite"],
    ]
    if (extract_root / "reports/features/derivatives_funding_oi_feature_store_v9_54.json").exists():
        commands.append(["python", "scripts/validate_derivatives_funding_oi_feature_store_v9_54.py", "--mode", "audit-lite"])
    if (extract_root / "reports/features/derivatives_funding_oi_feature_store_validation_v9_55.json").exists():
        commands.append(["python", "scripts/validate_derivatives_funding_oi_feature_store_validation_v9_55.py", "--mode", "audit-lite"])
    errors: list[str] = []
    for command in commands:
        completed = _run(command, extract_root, env, AUDIT_TIMEOUT_SECONDS)
        if completed["returncode"] != 0:
            errors.append(f"validator failed {' '.join(command)}: {completed['stdout'][-1000:]} {completed['stderr'][-1000:]}")
    return errors


def _run_self_audit(extract_root: Path, zip_path: Path, env: dict[str, str]) -> list[str]:
    completed = _run(["python", "scripts/audit_audit_lite_zip_v9_52_to_v9_55.py", "--zip", str(zip_path)], extract_root, env, AUDIT_TIMEOUT_SECONDS)
...
def _write_reports(result: dict[str, Any]) -> None:
    ...
    (report_dir / "zip_smoke_v9_52_to_v9_55.json").write_text(json.dumps(result, indent=2, ensure_ascii=False) + "\n", encoding="utf-8")
    (report_dir / "zip_smoke_v9_52_to_v9_55.md").write_text(
        "# Smoke ZIP V9.52 a V9.55\n\n"
        ...
        f"- Timeout tests : `{TEST_TIMEOUT_SECONDS}` secondes.\n"
