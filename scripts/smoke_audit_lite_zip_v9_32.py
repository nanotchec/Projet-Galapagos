...
def smoke_zip(zip_path: Path) -> dict[str, Any]:
    errors: list[str] = []
    with tempfile.TemporaryDirectory(prefix="galapagos_v9_32_smoke_") as tmp:
        ...
        errors.extend(_run_v9_32_tests(extract_root, env))
...
def _import_modules(extract_root: Path, env: dict[str, str]) -> list[str]:
    ...
    for module in [
        ...
        "galapagos.data.aggtrades_5y_full_coverage_validation_v9_32",
        "galapagos.data.aggtrades_5y_full_coverage_validation_v9_32_validation",
    ]:
...
def _run_v9_32_tests(extract_root: Path, env: dict[str, str]) -> list[str]:
    completed = _run(
        [
            "python",
            "-m",
    ...
    if completed["returncode"] != 0:
        return [f"V9.32 tests failed: {completed['stdout'][-1000:]} {completed['stderr'][-1000:]}"]
    return []
...
def _check_reports(extract_root: Path) -> list[str]:
    ...
    if report.get("version") != VERSION:
        errors.append("V9.32 report version mismatch")
    if report.get("target_5y_window_start") != "2021-05-05" or report.get("target_5y_window_end") != "2026-05-05":
        errors.append("V9.32 smoke expects target 5Y window")
    if report.get("decision") in {"aggtrades_5y_full_coverage_validated", "aggtrades_5y_full_coverage_validated_with_non_blocking_warnings"}:
        if report.get("days_complete") != 1827 or report.get("complete_collection_reached") is not True:
            errors.append("V9.32 smoke expects complete 5Y coverage for validated decision")
    if report.get("features_created") is not False or report.get("ml_executed") is not False:
        errors.append("V9.32 smoke expects data-only flags")
    if report.get("network_used") is not False or report.get("new_data_downloaded") is not False or report.get("ingestion_executed") is not False:
        errors.append("V9.32 smoke expects no network, no new downloads and no ingestion")
    return errors


def _run_self_audit(extract_root: Path, zip_path: Path, env: dict[str, str]) -> list[str]:
    completed = _run(["python", "scripts/audit_audit_lite_zip_v9_32.py", "--zip", str(zip_path)], extract_root, env, AUDIT_TIMEOUT_SECONDS)
    if completed["returncode"] != 0:
...
def _write_reports(result: dict[str, Any]) -> None:
    ...
    (report_dir / "zip_smoke_v9_32.json").write_text(json.dumps(result, indent=2, ensure_ascii=False) + "\n", encoding="utf-8")
    (report_dir / "zip_smoke_v9_32.md").write_text(
        "# Smoke ZIP V9.32\n\n"
        ...
        f"- Timeout tests V9.32 : `{TEST_TIMEOUT_SECONDS}` secondes.\n"
