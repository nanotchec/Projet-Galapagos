...
REQUIRED_FILES = [
    ...
    "src/galapagos/labels/ohlcv_aggtrades_5y_label_factory_v9_40.py",
    "src/galapagos/labels/ohlcv_aggtrades_5y_label_factory_v9_40_validation.py",
    "src/galapagos/labels/ohlcv_aggtrades_5y_label_factory_v9_40_schemas.py",
    "scripts/run_ohlcv_aggtrades_5y_label_factory_v9_40.py",
    "scripts/validate_ohlcv_aggtrades_5y_label_factory_v9_40.py",
    "scripts/release_audit_lite_zip_v9_40.py",
    "scripts/audit_audit_lite_zip_v9_40.py",
    "scripts/smoke_audit_lite_zip_v9_40.py",
    "tests/labels/test_ohlcv_aggtrades_5y_label_factory_v9_40.py",
    "tests/validation/test_ohlcv_aggtrades_5y_label_factory_v9_40_validator.py",
    "reports/manifests/ohlcv_aggtrades_5y_label_factory_v9_40_manifest.json",
    "reports/labels/ohlcv_aggtrades_5y_label_factory_v9_40.json",
    "reports/labels/ohlcv_aggtrades_5y_label_factory_v9_40.md",
    "reports/labels/ohlcv_aggtrades_5y_label_factory_v9_40_datacard.md",
    "reports/labels/ohlcv_aggtrades_5y_label_distribution_v9_40.json",
    "reports/labels/ohlcv_aggtrades_5y_label_stability_v9_40.json",
    "docs/ohlcv_aggtrades_5y_label_factory_v9_40.md",
    "reports/audit_lite/v9_40_command_results.json",
    "reports/audit_lite/v9_40_command_results.md",
    "reports/audit_lite/v9_40_full_local_validation_attestation.json",
    "reports/audit_lite/v9_40_full_local_validation_attestation.md",
    "reports/audit_lite/v9_40_artifact_inventory.json",
    "reports/audit_lite/v9_40_artifact_inventory.md",
    "reports/audit_lite/zip_size_report_v9_40.json",
    "reports/audit_lite/zip_size_report_v9_40.md",
    "reports/audit_lite/zip_audit_v9_40.json",
    "reports/audit_lite/zip_audit_v9_40.md",
    "reports/audit_lite/zip_smoke_v9_40.json",
    "reports/audit_lite/zip_smoke_v9_40.md",
    "reports/datasets/ohlcv_aggtrades_5y_dataset_v9_39.json",
...
def audit_zip(zip_path: Path) -> list[str]:
    ...
    errors: list[str] = []
    with tempfile.TemporaryDirectory(prefix="galapagos_v9_40_audit_") as tmp:
...
def _check_inventory(extract_root: Path, names: list[str]) -> list[str]:
    errors: list[str] = []
    inventory = _read_json(extract_root / "reports/audit_lite/v9_40_artifact_inventory.json")
    size_report = _read_json(extract_root / "reports/audit_lite/zip_size_report_v9_40.json")
...
def _check_reports(extract_root: Path) -> list[str]:
    ...
    manifest = _read_json(extract_root / "reports/manifests/ohlcv_aggtrades_5y_label_factory_v9_40_manifest.json")
    attestation = _read_json(extract_root / "reports/audit_lite/v9_40_full_local_validation_attestation.json")
    if report.get("version") != VERSION or report.get("source_version") != "V9.39":
        errors.append("V9.40 report version/source mismatch")
    if manifest.get("decision") != report.get("decision") or manifest.get("safety_flags") != report.get("safety_flags"):
        errors.append("V9.40 manifest mismatch")
    ...
    if report.get("network_used") is not False or report.get("new_data_downloaded") is not False:
        errors.append("V9.40 must not use network or download data")
    if report.get("ml_executed") is not False or report.get("walk_forward_executed") is not False or report.get("backtest_executed") is not False:
        errors.append("V9.40 must not run ML, walk-forward or backtest")
    if report.get("leakage_guard", {}).get("status") != "PASS":
        errors.append("V9.40 leakage guard must pass")
    if report.get("labels_created") is True and report.get("selected_primary_label") in {None, "none_requires_review"}:
        errors.append("created labels require a selected primary label")
...
def _check_flags(report: dict[str, Any], attestation: dict[str, Any]) -> list[str]:
    errors: list[str] = []
    for key in ["no_trading", "no_paper_live", "no_orders", "no_backtest", "no_walk_forward", "no_ml", "no_dataset_supervised", "no_strategy", "no_actionable_signal", "no_persistent_model", "no_new_data_download", "no_destructive_cleanup", "no_sidecars", "no_zip_fingerprints"]:
...
def _check_command_results(extract_root: Path) -> list[str]:
    payload = _read_json(extract_root / "reports/audit_lite/v9_40_command_results.json")
    ...
    for needle in ["pytest --collect-only -q", "test_ohlcv_aggtrades_5y_label_factory_v9_40.py", "test_ohlcv_aggtrades_5y_label_factory_v9_40_validator.py", "run_ohlcv_aggtrades_5y_label_factory_v9_40.py", "validate_ohlcv_aggtrades_5y_label_factory_v9_40.py", "release_audit_lite_zip_v9_40.py"]:
...
def _check_state(extract_root: Path) -> list[str]:
    ...
    errors: list[str] = []
    if metrics.get("last_validated_version") != "V9.39":
    ...
    if metrics.get("direction") != "ohlcv_aggtrades_5y_label_factory":
...
def _write_reports(result: dict[str, Any]) -> None:
    ...
    (report_dir / "zip_audit_v9_40.json").write_text(json.dumps(result, indent=2, ensure_ascii=False) + "\n", encoding="utf-8")
    (report_dir / "zip_audit_v9_40.md").write_text("# Audit ZIP V9.40\n\n" f"- Resultat : `{'PASS' if result['passed'] else 'FAIL'}`.\n" f"- Erreurs : `{result['errors']}`.\n" "- Aucun sidecar et aucune empreinte ZIP.\n", encoding="utf-8")
