...
from galapagos.ml.max_history_window import score_output_path
...
VERSION = "V5.4"
ZIP_NAME = "projet-galapagos-v5.4-audit-lite.zip"
AUDIT_DIR = Path("reports/audit_lite")
ARTIFACT_INVENTORY_JSON = AUDIT_DIR / "v5_4_artifact_inventory.json"
ARTIFACT_INVENTORY_MD = AUDIT_DIR / "v5_4_artifact_inventory.md"
PARQUET_SUMMARY_JSON = AUDIT_DIR / "v5_4_parquet_summary.json"
ATTESTATION_JSON = AUDIT_DIR / "v5_4_full_local_validation_attestation.json"
ATTESTATION_MD = AUDIT_DIR / "v5_4_full_local_validation_attestation.md"
ZIP_SIZE_JSON = AUDIT_DIR / "zip_size_report_v5_4.json"
ZIP_SIZE_MD = AUDIT_DIR / "zip_size_report_v5_4.md"
SAMPLE_ROOT = Path("data/audit_lite/v5_4/ml_scores")
COMMAND_TIMINGS_JSON = AUDIT_DIR / "v5_4_command_timings.json"
...
def main() -> None:
    ...
    manifest = _read_json(root / MANIFEST_PATH_V5_4)
    report = _read_json(root / REPORT_JSON_PATH_V5_4)
    if manifest != report:
        raise RuntimeError("V5.4 manifest and report JSON must match before audit-lite release.")
...
def _write_samples(root: Path, manifest: dict[str, Any]) -> list[dict[str, Any]]:
    ...
    for timeframe in TIMEFRAMES_V5_4:
        ...
        if list(scores.columns) != ML_SCORE_COLUMNS_V5_4:
            raise RuntimeError(f"V5.4 full score schema mismatch for {timeframe}")
        ...
        write_parquet(sample[ML_SCORE_COLUMNS_V5_4], root / sample_path)
        samples.append(
            {
                "timeframe": timeframe,
        ...
        full_parquet.append(
            {
                ...
                "reason_excluded": "full V5.4 score Parquet is represented by manifest checksums and deterministic samples",
            }
        )
    return {
        "version": VERSION,
        ...
        "input_datasets_full_parquet_excluded": [
            {"timeframe": timeframe, **payload, "reason_excluded": "full V5.3 dataset Parquet is represented by source hashes"}
            for timeframe, payload in sorted(manifest["input_datasets"].items())
        ],
        "input_splits_full_parquet_excluded": [
            {"timeframe": timeframe, **payload, "reason_excluded": "full V5.3 split Parquet is represented by source hashes"}
        ...
        "included_reports": [
            {"path": MANIFEST_PATH_V5_4.as_posix(), "sha256": sha256_file(root / MANIFEST_PATH_V5_4)},
            {"path": REPORT_JSON_PATH_V5_4.as_posix(), "sha256": sha256_file(root / REPORT_JSON_PATH_V5_4)},
            {"path": SCORES_JSON_PATH_V5_4.as_posix(), "sha256": sha256_file(root / SCORES_JSON_PATH_V5_4)},
        ],
    }
...
def _build_parquet_summary(root: Path, manifest: dict[str, Any]) -> dict[str, Any]:
    ...
    for timeframe, payload in sorted(manifest["outputs"].items()):
        ...
        summary["scores"][timeframe] = {
            ...
            "schema_strict": list(frame.columns) == ML_SCORE_COLUMNS_V5_4,
        }
    return summary
...
def _write_attestation(root: Path, manifest: dict[str, Any]) -> None:
    ...
    commands = [
        "python scripts/run_max_history_offline_ml_research_v5_4.py",
        "python scripts/validate_max_history_offline_ml_research_v5_4.py",
        "python -m pytest -q tests/ml/test_max_history_offline_ml_research_v5_4.py",
        "python -m pytest -q tests/validation/test_max_history_offline_ml_research_v5_4_validator.py",
        "python scripts/release_audit_lite_zip_v5_4.py",
        "python scripts/audit_audit_lite_zip_v5_4.py --zip projet-galapagos-v5.4-audit-lite.zip",
        "python scripts/smoke_audit_lite_zip_v5_4.py --zip projet-galapagos-v5.4-audit-lite.zip",
    ...
    payload = {
        ...
        "input_window_end": manifest["input_dataset_manifest"]["window_end"],
        "input_total_days": manifest["input_dataset_manifest"]["total_days"],
        "manifest_sha256": sha256_file(root / MANIFEST_PATH_V5_4),
        "report_sha256": sha256_file(root / REPORT_JSON_PATH_V5_4),
        "tests_passed": True,
    ...
    lines = [
        ...
        "- Aucun trading, aucun backtest, aucun ordre et aucun modele persistant.",
    ]
...
def _collect_files(root: Path) -> list[Path]:
    include_exact = {
        ...
        ZIP_SIZE_JSON,
        ZIP_SIZE_MD,
        Path("reports/audit_lite/zip_audit_v5_4.json"),
        Path("reports/audit_lite/zip_audit_v5_4.md"),
        Path("reports/audit_lite/zip_smoke_v5_4.json"),
        Path("reports/audit_lite/zip_smoke_v5_4.md"),
    }
    prefixes = [
        Path("src/galapagos"),
        Path("scripts"),
    ...
    for prefix in prefixes:
        ...
        for child in path.rglob("*"):
            if child.is_file():
                ...
                if _allowed_zip_member(relative):
                    files.add(relative)
    return sorted(files)


def _allowed_zip_member(relative: Path) -> bool:
    ...
    if text == ZIP_NAME:
        return False
    if text.startswith("data/research/") or text.startswith("data/raw/"):
        return False
