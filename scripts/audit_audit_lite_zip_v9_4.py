...
REQUIRED_FILES = [
    ...
    "scripts/release_audit_lite_zip_v9_4.py",
    "scripts/audit_audit_lite_zip_v9_4.py",
    "scripts/smoke_audit_lite_zip_v9_4.py",
    ...
    "reports/audit_lite/v9_4_command_results.md",
    "reports/audit_lite/v9_4_full_local_validation_attestation.json",
    "reports/audit_lite/v9_4_full_local_validation_attestation.md",
    "reports/audit_lite/v9_4_artifact_inventory.json",
    "reports/audit_lite/v9_4_artifact_inventory.md",
    "reports/audit_lite/zip_size_report_v9_4.json",
    "reports/audit_lite/zip_size_report_v9_4.md",
    "reports/audit_lite/zip_audit_v9_4.json",
    "reports/audit_lite/zip_audit_v9_4.md",
    "reports/audit_lite/zip_smoke_v9_4.json",
    "reports/audit_lite/zip_smoke_v9_4.md",
...
def audit_zip(zip_path: Path) -> list[str]:
    ...
    errors: list[str] = []
    with tempfile.TemporaryDirectory(prefix="galapagos_v9_4_audit_") as tmp:
...
def _check_sidecars(zip_path: Path) -> list[str]:
    ...
    if final_sha not in sidecar_txt.read_text(encoding="utf-8"):
        errors.append("sidecar TXT hash mismatch")
    if payload.get("zip_bytes") != zip_path.stat().st_size:
        errors.append("sidecar ZIP size mismatch")
    return errors


def _check_inventory(extract_root: Path, names: list[str], zip_path: Path) -> list[str]:
    errors: list[str] = []
    inventory = _read_json(extract_root / "reports/audit_lite/v9_4_artifact_inventory.json")
    ...
    size_report = _read_json(extract_root / "reports/audit_lite/zip_size_report_v9_4.json")
    ...
    if size_report.get("zip_bytes") not in {zip_path.stat().st_size, inventory.get("zip_bytes")}:
        errors.append("zip size report byte size is not coherent")
    return errors
...
def _check_reports(extract_root: Path) -> list[str]:
    ...
    command_results = _read_json(extract_root / "reports/audit_lite/v9_4_command_results.json")
    attestation = _read_json(extract_root / "reports/audit_lite/v9_4_full_local_validation_attestation.json")
    ...
    if decision.get("research_decision") != "backtest_not_justified_refine_labels":
        errors.append("V9.4 research decision must remain conservative")
    ...
    if decision.get("label_shuffle_assessment", {}).get("no_clear_edge_vs_shuffled_labels_count", 0) <= 0:
        errors.append("V9.4 must preserve label shuffle warning cases")
    if command_results.get("version") != VERSION:
        errors.append("V9.4 command results version mismatch")
    if not isinstance(command_results.get("commands"), list):
        errors.append("V9.4 command results commands must be a list")
    if attestation.get("no_trading") is not True or attestation.get("no_backtest_performed") is not True:
        errors.append("V9.4 attestation safety mismatch")
    return errors
...
def _check_state_surfaces(extract_root: Path) -> list[str]:
    ...
    if VERSION not in latest_summary:
        errors.append("latest_summary must mention V9.4")
    return errors
...
def _write_reports(result: dict[str, Any]) -> None:
    ...
    (report_dir / "zip_audit_v9_4.json").write_text(json.dumps(result, indent=2, ensure_ascii=False) + "\n", encoding="utf-8")
    (report_dir / "zip_audit_v9_4.md").write_text(
        "# Audit ZIP V9.4\n\n"
