...
from galapagos.ml.advanced_ohlcv_window import score_output_path
...
VERSION = "V6.2"
ZIP_NAME = "projet-galapagos-v6.2-audit-lite.zip"
AUDIT_DIR = Path("reports/audit_lite")
ARTIFACT_INVENTORY_JSON = AUDIT_DIR / "v6_2_artifact_inventory.json"
ARTIFACT_INVENTORY_MD = AUDIT_DIR / "v6_2_artifact_inventory.md"
PARQUET_SUMMARY_JSON = AUDIT_DIR / "v6_2_parquet_summary.json"
ATTESTATION_JSON = AUDIT_DIR / "v6_2_full_local_validation_attestation.json"
ATTESTATION_MD = AUDIT_DIR / "v6_2_full_local_validation_attestation.md"
ZIP_SIZE_JSON = AUDIT_DIR / "zip_size_report_v6_2.json"
ZIP_SIZE_MD = AUDIT_DIR / "zip_size_report_v6_2.md"
COMMAND_TIMINGS_JSON = AUDIT_DIR / "v6_2_command_timings.json"
SAMPLE_ROOT = Path("data/audit_lite/v6_2/ml_scores")
SOURCE_PREFIXES = [
...
SCRIPT_EXACT = [
    ...
    Path("scripts/run_advanced_ohlcv_offline_ml_research_v6_2.py"),
    Path("scripts/validate_advanced_ohlcv_offline_ml_research_v6_2.py"),
    Path("scripts/release_audit_lite_zip_v6_2.py"),
    Path("scripts/audit_audit_lite_zip_v6_2.py"),
    Path("scripts/smoke_audit_lite_zip_v6_2.py"),
]
TEST_EXACT = [
    Path("tests/ml/test_advanced_ohlcv_offline_ml_research_v6_2.py"),
    Path("tests/validation/test_advanced_ohlcv_offline_ml_research_v6_2_validator.py"),
]
REPORT_EXACT = [
    MANIFEST_PATH_V6_2,
    REPORT_JSON_PATH_V6_2,
...
def main() -> None:
    root = Path(".").resolve()
    manifest = _read_json(root / MANIFEST_PATH_V6_2)
    report = _read_json(root / REPORT_JSON_PATH_V6_2)
    if manifest != report:
        raise RuntimeError("V6.2 manifest and report JSON must match before audit-lite release.")
...
def _write_samples(root: Path, manifest: dict[str, Any]) -> list[dict[str, Any]]:
    ...
    for timeframe in TIMEFRAMES_V6_2:
        ...
        if list(scores.columns) != ML_SCORE_COLUMNS_V6_2:
            raise RuntimeError(f"V6.2 full score schema mismatch for {timeframe}")
        ...
        write_parquet(sample[ML_SCORE_COLUMNS_V6_2], root / sample_path)
        samples.append(
            {
                "timeframe": timeframe,
...
def _build_inventory(root: Path, manifest: dict[str, Any], samples: list[dict[str, Any]]) -> dict[str, Any]:
    return {
        ...
        "input_datasets_full_parquet_excluded": [
            {"timeframe": timeframe, **payload, "reason_excluded": "full V6.1 dataset Parquet is represented by source hashes"}
            for timeframe, payload in sorted(manifest["input_datasets"].items())
        ],
        "input_splits_full_parquet_excluded": [
            {"timeframe": timeframe, **payload, "reason_excluded": "full V6.1 split Parquet is represented by source hashes"}
            for timeframe, payload in sorted(manifest["input_splits"].items())
        ],
        "full_parquet_excluded": [
            {"timeframe": timeframe, **payload, "reason_excluded": "full V6.2 score Parquet is represented by manifest checksums and deterministic samples"}
            for timeframe, payload in sorted(manifest["outputs"].items())
        ],
        "score_samples_included": samples,
        "included_reports": [
            {"path": MANIFEST_PATH_V6_2.as_posix(), "sha256": sha256_file(root / MANIFEST_PATH_V6_2)},
            {"path": REPORT_JSON_PATH_V6_2.as_posix(), "sha256": sha256_file(root / REPORT_JSON_PATH_V6_2)},
            {"path": SCORES_JSON_PATH_V6_2.as_posix(), "sha256": sha256_file(root / SCORES_JSON_PATH_V6_2)},
        ],
    }
...
def _build_parquet_summary(root: Path, manifest: dict[str, Any]) -> dict[str, Any]:
    ...
    for timeframe, payload in sorted(manifest["outputs"].items()):
        ...
        summary["scores"][timeframe] = {
            ...
            "schema_strict": list(frame.columns) == ML_SCORE_COLUMNS_V6_2,
        }
    return summary
...
def _write_attestation(root: Path, manifest: dict[str, Any]) -> None:
    ...
    commands = [
        "python scripts/run_advanced_ohlcv_offline_ml_research_v6_2.py",
        "python scripts/validate_advanced_ohlcv_offline_ml_research_v6_2.py",
        "python -m pytest -q tests/ml/test_advanced_ohlcv_offline_ml_research_v6_2.py",
        "python -m pytest -q tests/validation/test_advanced_ohlcv_offline_ml_research_v6_2_validator.py",
        "python scripts/release_audit_lite_zip_v6_2.py",
        "python scripts/audit_audit_lite_zip_v6_2.py --zip projet-galapagos-v6.2-audit-lite.zip",
        "python scripts/smoke_audit_lite_zip_v6_2.py --zip projet-galapagos-v6.2-audit-lite.zip",
    ...
    payload = {
        ...
        "input_total_days": manifest["input_dataset_manifest"]["total_days"],
        "advanced_feature_columns_count": manifest["advanced_feature_columns_count"],
        "manifest_sha256": sha256_file(root / MANIFEST_PATH_V6_2),
        "report_sha256": sha256_file(root / REPORT_JSON_PATH_V6_2),
        "tests_passed": True,
...
def _collect_files(root: Path) -> list[Path]:
    files: set[Path] = set()
    sample_exact = [SAMPLE_ROOT / f"timeframe={timeframe}" / "sample.parquet" for timeframe in TIMEFRAMES_V6_2]
...
def _allowed_member(relative: Path) -> bool:
    ...
    for prefix in FORBIDDEN_PREFIXES:
        if text.startswith(prefix) and not text.startswith("data/audit_lite/v6_2/"):
            return False
    return True
