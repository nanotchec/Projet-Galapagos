...
VERSION = "V7.5"
ZIP_NAME = "projet-galapagos-v7.5-audit-lite.zip"
AUDIT_DIR = Path("reports/audit_lite")
ARTIFACT_INVENTORY_JSON = AUDIT_DIR / "v7_5_artifact_inventory.json"
ARTIFACT_INVENTORY_MD = AUDIT_DIR / "v7_5_artifact_inventory.md"
ATTESTATION_JSON = AUDIT_DIR / "v7_5_full_local_validation_attestation.json"
ATTESTATION_MD = AUDIT_DIR / "v7_5_full_local_validation_attestation.md"
ZIP_SIZE_JSON = AUDIT_DIR / "zip_size_report_v7_5.json"
ZIP_SIZE_MD = AUDIT_DIR / "zip_size_report_v7_5.md"
COMMAND_TIMINGS_JSON = AUDIT_DIR / "v7_5_command_timings.json"
...
SCRIPT_EXACT = [
    ...
    Path("scripts/run_ohlcv_trades_ml_robustness_v7_5.py"),
    Path("scripts/validate_ohlcv_trades_ml_robustness_v7_5.py"),
    Path("scripts/release_audit_lite_zip_v7_5.py"),
    Path("scripts/audit_audit_lite_zip_v7_5.py"),
    Path("scripts/smoke_audit_lite_zip_v7_5.py"),
]
TEST_EXACT = [
    Path("tests/ml/test_ohlcv_trades_ml_robustness_v7_5.py"),
    Path("tests/validation/test_ohlcv_trades_ml_robustness_v7_5_validator.py"),
]
REPORT_EXACT = [
    Path("reports/manifests/ohlcv_trades_offline_supervised_dataset_v7_3_manifest.json"),
    Path("reports/manifests/ohlcv_trades_offline_ml_research_v7_4_manifest.json"),
    Path("reports/ml/ohlcv_trades_offline_ml_research_v7_4.json"),
    Path("reports/ml/ohlcv_trades_offline_research_scores_v7_4.json"),
    MANIFEST_PATH_V7_5,
    REPORT_JSON_PATH_V7_5,
    REPORT_MD_PATH_V7_5,
    DOC_PATH_V7_5,
...
COMMANDS_EXECUTED = [
    "python scripts/run_ohlcv_trades_ml_robustness_v7_5.py",
    "python scripts/validate_ohlcv_trades_ml_robustness_v7_5.py",
    "python -m pytest -q tests/ml/test_ohlcv_trades_ml_robustness_v7_5.py",
    "python -m pytest -q tests/validation/test_ohlcv_trades_ml_robustness_v7_5_validator.py",
    "python scripts/release_audit_lite_zip_v7_5.py",
    "python scripts/audit_audit_lite_zip_v7_5.py --zip projet-galapagos-v7.5-audit-lite.zip",
    "python scripts/smoke_audit_lite_zip_v7_5.py --zip projet-galapagos-v7.5-audit-lite.zip",
    "python -m pytest --collect-only -q",
]
DEFAULT_COMMAND_DURATIONS_SECONDS = {
    "python scripts/run_ohlcv_trades_ml_robustness_v7_5.py": 202.0,
    "python scripts/validate_ohlcv_trades_ml_robustness_v7_5.py": 3.0,
    "python -m pytest -q tests/ml/test_ohlcv_trades_ml_robustness_v7_5.py": 0.94,
    "python -m pytest -q tests/validation/test_ohlcv_trades_ml_robustness_v7_5_validator.py": 2.98,
    "python scripts/release_audit_lite_zip_v7_5.py": 1.0,
    "python scripts/audit_audit_lite_zip_v7_5.py --zip projet-galapagos-v7.5-audit-lite.zip": 0.0,
    "python scripts/smoke_audit_lite_zip_v7_5.py --zip projet-galapagos-v7.5-audit-lite.zip": 2.244,
...
def main() -> None:
    root = Path(".").resolve()
    manifest = _read_json(root / MANIFEST_PATH_V7_5)
    report = _read_json(root / REPORT_JSON_PATH_V7_5)
    if manifest != report:
        raise RuntimeError("V7.5 manifest and report JSON must match before audit-lite release.")
...
def _collect_files(root: Path) -> list[Path]:
    ...
    if forbidden_scripts:
        raise RuntimeError(f"V7.5 release would include pytest-collectible scripts: {[p.as_posix() for p in forbidden_scripts]}")
    return sorted(files)
...
def _build_inventory(root: Path, manifest: dict[str, Any], included: list[Path]) -> dict[str, Any]:
    ...
    return {
        ...
        "input_score_files_full_parquet_excluded": [
            {"timeframe": timeframe, **payload, "reason_excluded": "full V7.4 score Parquet is represented by V7.5 checksums and reports"}
        ...
        "notes": [
            ...
            "V7.5 contains no raw zips, full Parquet data, backtest, strategy, order, execution artifact, or persistent model.",
        ],
    }
...
def _write_attestation(root: Path, manifest: dict[str, Any]) -> None:
    ...
    payload = {
        ...
        "manifest_sha256": sha256_file(root / MANIFEST_PATH_V7_5),
        "report_sha256": sha256_file(root / REPORT_JSON_PATH_V7_5),
    ...
    _write_text(
        ...
        f"- Feature columns count : `{payload['feature_columns_count']}`\n"
