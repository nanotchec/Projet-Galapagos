...
def main() -> None:
    ...
    with tempfile.TemporaryDirectory(prefix="galapagos_v3_1_10_smoke_") as tmp:
        tmp_path = Path(tmp)
    ...
    payload = {
        ...
        "safety_flags_ok": not any("unsafe V3.1.10 flag active" in error for error in errors),
    ...
    Path("reports/zip_smoke_test_v3_1_10.json").write_text(
        json.dumps(payload, indent=2, ensure_ascii=False) + "\n",
        encoding="utf-8",
    )
    Path("reports/zip_smoke_test_v3_1_10.md").write_text(
        "# Smoke ZIP V3.1.10\n\n"
...
def _validate_state_manifest_and_report(root: Path) -> list[str]:
    ...
    if manifest.get("correction_version") is None or report.get("correction_version") is None:
        if VERSION not in project_state_text or VERSION not in latest_summary:
            errors.append("V3.1.10 must be documented in PROJECT_STATE/latest_summary when correction_version is absent")

    if project_state.get("candidate_version") != VERSION and VERSION not in latest_summary:
        errors.append("PROJECT_STATE/latest_summary must identify V3.1.10 as candidate")
    ...
    if "pending_external_audit" not in latest_summary and project_state.get("candidate_status") != "pending_external_audit":
        errors.append("V3.1.10 must remain pending_external_audit")
    ...
    for flag in SAFETY_FALSE_FLAGS:
        if safety.get(flag) is not False:
            errors.append(f"unsafe V3.1.10 flag active: {flag}")
