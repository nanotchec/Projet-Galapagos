...
def main() -> None:
    ...
    Path("reports/zip_smoke_test_v3_2.json").write_text(json.dumps(payload, indent=2, ensure_ascii=False) + "\n", encoding="utf-8")
    Path("reports/zip_smoke_test_v3_2.md").write_text(
        "# Smoke ZIP V3.2\n\n"
...
def _validate_state_manifest_and_report(root: Path) -> list[str]:
    ...
    if project_state.get("candidate_version") != VERSION and VERSION not in latest_summary:
        errors.append("PROJECT_STATE/latest_summary must identify V3.2 as candidate")
