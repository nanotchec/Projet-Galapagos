...
CORE_PATHS = [
    ...
    Path("src/galapagos/data/aggtrades_post_v9_full_coverage_validation_v9_29.py"),
    Path("src/galapagos/data/aggtrades_5y_full_coverage_validation_v9_32.py"),
    Path("src/galapagos/data/aggtrades_5y_full_coverage_validation_v9_32_validation.py"),
    Path("scripts/run_aggtrades_5y_full_coverage_validation_v9_32.py"),
    Path("scripts/validate_aggtrades_5y_full_coverage_validation_v9_32.py"),
    Path("scripts/release_audit_lite_zip_v9_32.py"),
    Path("scripts/audit_audit_lite_zip_v9_32.py"),
    Path("scripts/smoke_audit_lite_zip_v9_32.py"),
    Path("tests/data/test_aggtrades_5y_full_coverage_validation_v9_32.py"),
    Path("tests/validation/test_aggtrades_5y_full_coverage_validation_v9_32_validator.py"),
    ...
    Path("docs/aggtrades_5y_full_coverage_validation_v9_32.md"),
]
...
AUDIT_PATHS = [
    Path("reports/audit_lite/v9_32_command_results.json"),
    Path("reports/audit_lite/v9_32_command_results.md"),
    Path("reports/audit_lite/v9_32_full_local_validation_attestation.json"),
    Path("reports/audit_lite/v9_32_full_local_validation_attestation.md"),
    Path("reports/audit_lite/v9_32_artifact_inventory.json"),
    Path("reports/audit_lite/v9_32_artifact_inventory.md"),
    Path("reports/audit_lite/zip_size_report_v9_32.json"),
    Path("reports/audit_lite/zip_size_report_v9_32.md"),
    Path("reports/audit_lite/zip_audit_v9_32.json"),
    Path("reports/audit_lite/zip_audit_v9_32.md"),
    Path("reports/audit_lite/zip_smoke_v9_32.json"),
    Path("reports/audit_lite/zip_smoke_v9_32.md"),
]
...
def main() -> int:
    report = _read_json(Path("reports/data/aggtrades_5y_full_coverage_validation_v9_32.json"))
...
def _ensure_placeholders() -> None:
    placeholders = {
        "reports/audit_lite/v9_32_command_results.json": {"version": VERSION, "status": "PENDING_CAPTURE", "commands": [], "sidecars_created": False, "zip_fingerprints_created": False},
        "reports/audit_lite/zip_audit_v9_32.json": {"version": VERSION, "passed": False, "errors": [], "status": "PENDING_RUN"},
        "reports/audit_lite/zip_smoke_v9_32.json": {"version": VERSION, "passed": False, "errors": [], "status": "PENDING_RUN"},
    }
    texts = {
        "reports/audit_lite/v9_32_command_results.md": "# Commandes V9.32\n\nEn attente.\n",
        "reports/audit_lite/zip_audit_v9_32.md": "# Audit ZIP V9.32\n\nEn attente.\n",
        "reports/audit_lite/zip_smoke_v9_32.md": "# Smoke ZIP V9.32\n\nEn attente.\n",
    }
...
def _collect_paths() -> list[Path]:
    ...
    if missing_core:
        raise FileNotFoundError(f"missing V9.32 release inputs: {missing_core}")
...
def _write_attestation(report: dict[str, Any], zip_size: int | None) -> None:
    commands = _read_optional_json(Path("reports/audit_lite/v9_32_command_results.json")).get("commands", [])
    payload = {
        "version": VERSION,
    ...
    _write_json(Path("reports/audit_lite/v9_32_full_local_validation_attestation.json"), payload)
    _write_text(
        Path("reports/audit_lite/v9_32_full_local_validation_attestation.md"),
        "# Attestation V9.32\n\n"
        f"- Decision : `{report['decision']}`.\n"
        f"- Fenetre 5Y : `{report['target_5y_window_start']}` -> `{report['target_5y_window_end']}`.\n"
        f"- Jours complets/failed/quarantine : `{report['days_complete']}` / `{report['days_failed']}` / `{report['days_quarantined']}`.\n"
        f"- Couverture complete : `{report['complete_collection_reached']}`.\n"
        ...
        "- Aucun telechargement de nouvelles donnees, aucune nouvelle ingestion, aucune suppression destructive, aucun sidecar et aucune empreinte ZIP.\n",
    )
...
def _write_inventory(paths: list[Path], zip_size: int | None) -> None:
    ...
    _write_json(Path("reports/audit_lite/v9_32_artifact_inventory.json"), payload)
    _write_text(Path("reports/audit_lite/v9_32_artifact_inventory.md"), "# Inventaire audit-lite V9.32\n\n" f"- Fichiers inclus : `{len(paths)}`.\n" f"- ZIP : `{ZIP_NAME}`.\n")
...
def _write_size_report(paths: list[Path], zip_size: int | None) -> None:
    ...
    _write_json(Path("reports/audit_lite/zip_size_report_v9_32.json"), payload)
    _write_text(Path("reports/audit_lite/zip_size_report_v9_32.md"), "# Taille ZIP V9.32\n\n" f"- Fichiers : `{len(paths)}`.\n" f"- ZIP bytes estimate : `{zip_size}`.\n")
