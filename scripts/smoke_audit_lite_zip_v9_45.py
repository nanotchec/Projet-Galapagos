...
def smoke_zip(zip_path: Path) -> dict[str, Any]:
    ...
    with tempfile.TemporaryDirectory(prefix="galapagos_v9_45_smoke_") as tmp:
        ...
        errors.extend(_run_v9_45_tests(extract_root, env))
        errors.extend(_run_audit_lite_validator(extract_root, env))
        errors.extend(_run_self_audit(extract_root, zip_path, env))
    return {"version": VERSION, "zip": str(zip_path), "passed": not errors, "errors": errors, "packaging_checks_passed": packaging_checks_passed, "full_dataset_required": False, "import_timeout_seconds": IMPORT_TIMEOUT_SECONDS, "pytest_collect_only_timeout_seconds": PYTEST_TIMEOUT_SECONDS, "tests_timeout_seconds": TEST_TIMEOUT_SECONDS, "audit_timeout_seconds": AUDIT_TIMEOUT_SECONDS, "sidecars_expected": False, "zip_fingerprints_expected": False}
...
def _import_modules(extract_root: Path, env: dict[str, str]) -> list[str]:
    errors: list[str] = []
    for module in [
        "galapagos.features.aggtrades_exact_5y_feature_enrichment_v9_45",
        "galapagos.features.aggtrades_exact_5y_feature_enrichment_v9_45_validation",
        "galapagos.features.aggtrades_exact_5y_feature_enrichment_v9_45_schemas",
    ]:
...
def _run_pytest_collect(extract_root: Path, env: dict[str, str]) -> list[str]:
    completed = _run(["python", "-m", "pytest", "--collect-only", "-q", "tests/features/test_aggtrades_exact_5y_feature_enrichment_v9_45.py", "tests/validation/test_aggtrades_exact_5y_feature_enrichment_v9_45_validator.py"], extract_root, env, PYTEST_TIMEOUT_SECONDS)
    return [] if completed["returncode"] == 0 else [f"pytest collect-only failed: {completed['stdout'][-1000:]} {completed['stderr'][-1000:]}"]


def _run_v9_45_tests(extract_root: Path, env: dict[str, str]) -> list[str]:
    completed = _run(["python", "-m", "pytest", "-q", "tests/features/test_aggtrades_exact_5y_feature_enrichment_v9_45.py", "tests/validation/test_aggtrades_exact_5y_feature_enrichment_v9_45_validator.py"], extract_root, env, TEST_TIMEOUT_SECONDS)
    return [] if completed["returncode"] == 0 else [f"V9.45 tests failed: {completed['stdout'][-1000:]} {completed['stderr'][-1000:]}"]


def _run_audit_lite_validator(extract_root: Path, env: dict[str, str]) -> list[str]:
    completed = _run(["python", "scripts/validate_aggtrades_exact_5y_feature_enrichment_v9_45.py", "--mode", "audit-lite"], extract_root, env, AUDIT_TIMEOUT_SECONDS)
    return [] if completed["returncode"] == 0 else [f"V9.45 audit-lite validator failed: {completed['stdout'][-1000:]} {completed['stderr'][-1000:]}"]


def _run_self_audit(extract_root: Path, zip_path: Path, env: dict[str, str]) -> list[str]:
    completed = _run(["python", "scripts/audit_audit_lite_zip_v9_45.py", "--zip", str(zip_path)], extract_root, env, AUDIT_TIMEOUT_SECONDS)
