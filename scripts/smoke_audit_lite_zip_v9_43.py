...
def smoke_zip(zip_path: Path) -> dict[str, Any]:
    ...
    with tempfile.TemporaryDirectory(prefix="galapagos_v9_43_smoke_") as tmp:
        ...
        errors.extend(_run_v9_43_tests(extract_root, env))
...
def _import_modules(extract_root: Path, env: dict[str, str]) -> list[str]:
    errors: list[str] = []
    for module in [
        "galapagos.ml.ohlcv_aggtrades_5y_offline_ml_v9_43",
        "galapagos.ml.ohlcv_aggtrades_5y_offline_ml_v9_43_validation",
        "galapagos.ml.ohlcv_aggtrades_5y_offline_ml_v9_43_metrics",
        "galapagos.ml.ohlcv_aggtrades_5y_offline_ml_v9_43_quality",
    ]:
...
def _run_pytest_collect(extract_root: Path, env: dict[str, str]) -> list[str]:
    completed = _run(["python", "-m", "pytest", "--collect-only", "-q", "tests/ml/test_ohlcv_aggtrades_5y_offline_ml_v9_43.py", "tests/validation/test_ohlcv_aggtrades_5y_offline_ml_v9_43_validator.py"], extract_root, env, PYTEST_TIMEOUT_SECONDS)
    if completed["returncode"] != 0:
        return [f"pytest collect-only failed: {completed['stdout'][-1000:]} {completed['stderr'][-1000:]}"]
    return []


def _run_v9_43_tests(extract_root: Path, env: dict[str, str]) -> list[str]:
    completed = _run(["python", "-m", "pytest", "-q", "tests/ml/test_ohlcv_aggtrades_5y_offline_ml_v9_43.py", "tests/validation/test_ohlcv_aggtrades_5y_offline_ml_v9_43_validator.py"], extract_root, env, TEST_TIMEOUT_SECONDS)
    if completed["returncode"] != 0:
        return [f"V9.43 tests failed: {completed['stdout'][-1000:]} {completed['stderr'][-1000:]}"]
    return []


def _run_audit_lite_validator(extract_root: Path, env: dict[str, str]) -> list[str]:
    completed = _run(["python", "scripts/validate_ohlcv_aggtrades_5y_offline_ml_v9_43.py", "--mode", "audit-lite"], extract_root, env, AUDIT_TIMEOUT_SECONDS)
    if completed["returncode"] != 0:
        return [f"V9.43 audit-lite validator failed: {completed['stdout'][-1000:]} {completed['stderr'][-1000:]}"]
    return []


def _run_self_audit(extract_root: Path, zip_path: Path, env: dict[str, str]) -> list[str]:
    completed = _run(["python", "scripts/audit_audit_lite_zip_v9_43.py", "--zip", str(zip_path)], extract_root, env, AUDIT_TIMEOUT_SECONDS)
    if completed["returncode"] != 0:
