...
def smoke_zip(zip_path: Path) -> dict[str, Any]:
    errors: list[str] = []
    with tempfile.TemporaryDirectory(prefix="galapagos_v9_24_smoke_") as tmp:
        ...
        errors.extend(_run_v9_24_tests(extract_root, env))
...
def _import_modules(extract_root: Path, env: dict[str, str]) -> list[str]:
    ...
    for module in [
        ...
        "galapagos.data.aggtrades_post_v9_batch3_collection_v9_24_validation",
    ]:
...
def _run_v9_24_tests(extract_root: Path, env: dict[str, str]) -> list[str]:
    completed = _run(
        [
            "python",
            "-m",
    ...
    if completed["returncode"] != 0:
        return [f"V9.24 tests failed: {completed['stdout'][-1000:]} {completed['stderr'][-1000:]}"]
    return []


def _run_self_audit(extract_root: Path, zip_path: Path, env: dict[str, str]) -> list[str]:
    completed = _run(["python", "scripts/audit_audit_lite_zip_v9_24.py", "--zip", str(zip_path)], extract_root, env, AUDIT_TIMEOUT_SECONDS)
    if completed["returncode"] != 0:
...
def _check_reports(extract_root: Path) -> list[str]:
    ...
    if report.get("mode") != "collect":
        errors.append("V9.24 smoke expects collect mode")
    ...
    if batch.get("start") != "2024-10-09" or batch.get("end") != "2024-12-07" or batch.get("max_downloads") != 60:
        errors.append("V9.24 batch scope mismatch")
    if summary.get("days_requested") != 60 or summary.get("days_downloaded") != 60 or summary.get("days_normalized") != 60 or summary.get("days_complete") != 60:
        errors.append("V9.24 smoke expects 60 downloaded, normalized and complete days")
    if summary.get("days_failed") != 0 or summary.get("days_quarantined") != 0:
        errors.append("V9.24 smoke expects no failed or quarantined days")
    if summary.get("total_rows", 0) <= 0 or summary.get("raw_bytes_total", 0) <= 0 or summary.get("silver_bytes_total", 0) <= 0:
        errors.append("V9.24 smoke expects positive batch metrics")
    ...
    for key in ["collection_executed", "network_used", "new_data_downloaded", "ingestion_executed"]:
        if report.get(key) is not True:
            errors.append(f"V9.24 must keep {key}=true")
    ...
    if flags.get("network_used") is not True or flags.get("no_new_data_download") is not False or flags.get("no_ingestion_executed") is not False:
        errors.append("V9.24 must confirm public batch network, new download and ingestion")
    return errors
...
def _write_reports(result: dict[str, Any]) -> None:
    ...
    (report_dir / "zip_smoke_v9_24.json").write_text(json.dumps(result, indent=2, ensure_ascii=False) + "\n", encoding="utf-8")
    (report_dir / "zip_smoke_v9_24.md").write_text(
        "# Smoke ZIP V9.24\n\n"
        ...
        f"- Timeout tests V9.24 : `{TEST_TIMEOUT_SECONDS}` secondes.\n"
