...
CORE_PATHS = [
    ...
    Path("src/galapagos/datasets/ohlcv_aggtrades_5y_dataset_v9_41_schemas.py"),
    Path("src/galapagos/datasets/ohlcv_aggtrades_5y_dataset_validation_v9_42.py"),
    Path("src/galapagos/datasets/ohlcv_aggtrades_5y_dataset_validation_v9_42_validation.py"),
    Path("scripts/run_ohlcv_aggtrades_5y_dataset_validation_v9_42.py"),
    Path("scripts/validate_ohlcv_aggtrades_5y_dataset_validation_v9_42.py"),
    Path("scripts/release_audit_lite_zip_v9_42.py"),
    Path("scripts/audit_audit_lite_zip_v9_42.py"),
    Path("scripts/smoke_audit_lite_zip_v9_42.py"),
    Path("tests/datasets/test_ohlcv_aggtrades_5y_dataset_validation_v9_42.py"),
    Path("tests/validation/test_ohlcv_aggtrades_5y_dataset_validation_v9_42_validator.py"),
    Path("reports/manifests/ohlcv_aggtrades_5y_dataset_validation_v9_42_manifest.json"),
    ...
    Path("reports/datasets/ohlcv_aggtrades_5y_dataset_validation_v9_42_samples.json"),
    Path("docs/ohlcv_aggtrades_5y_dataset_validation_v9_42.md"),
]

PRIOR_REPORTS = [
    Path("reports/datasets/ohlcv_aggtrades_5y_dataset_v9_41.json"),
    Path("reports/datasets/ohlcv_aggtrades_5y_dataset_v9_41.md"),
    Path("reports/datasets/ohlcv_aggtrades_5y_dataset_v9_41_datacard.md"),
    Path("reports/manifests/ohlcv_aggtrades_5y_dataset_v9_41_manifest.json"),
...
AUDIT_PATHS = [
    Path("reports/audit_lite/v9_42_command_results.json"),
    Path("reports/audit_lite/v9_42_command_results.md"),
    Path("reports/audit_lite/v9_42_full_local_validation_attestation.json"),
    Path("reports/audit_lite/v9_42_full_local_validation_attestation.md"),
    Path("reports/audit_lite/v9_42_artifact_inventory.json"),
    Path("reports/audit_lite/v9_42_artifact_inventory.md"),
    Path("reports/audit_lite/zip_size_report_v9_42.json"),
    Path("reports/audit_lite/zip_size_report_v9_42.md"),
    Path("reports/audit_lite/zip_audit_v9_42.json"),
    Path("reports/audit_lite/zip_audit_v9_42.md"),
    Path("reports/audit_lite/zip_smoke_v9_42.json"),
    Path("reports/audit_lite/zip_smoke_v9_42.md"),
]
...
def main() -> int:
    report = _read_json(Path("reports/datasets/ohlcv_aggtrades_5y_dataset_validation_v9_42.json"))
    _ensure_placeholders()
    zip_size: int | None = None
    paths: list[Path] = []
    for _ in range(20):
        _write_attestation(report, zip_size)
        paths = _collect_paths(report)
...
def _ensure_placeholders() -> None:
    placeholders = {
        "reports/audit_lite/v9_42_command_results.json": {"version": VERSION, "status": "PENDING_CAPTURE", "commands": [], "sidecars_created": False, "zip_fingerprints_created": False},
        "reports/audit_lite/zip_audit_v9_42.json": {"version": VERSION, "passed": False, "errors": [], "status": "PENDING_RUN"},
        "reports/audit_lite/zip_smoke_v9_42.json": {"version": VERSION, "passed": False, "errors": [], "status": "PENDING_RUN"},
    }
    texts = {
        "reports/audit_lite/v9_42_command_results.md": "# Commandes V9.42\n\nEn attente.\n",
        "reports/audit_lite/zip_audit_v9_42.md": "# Audit ZIP V9.42\n\nEn attente.\n",
        "reports/audit_lite/zip_smoke_v9_42.md": "# Smoke ZIP V9.42\n\nEn attente.\n",
    }
...
def _collect_paths(report: dict[str, Any]) -> list[Path]:
    sample_paths = [Path(item["path"]) for item in report.get("sample_inventory", {}).values() if item.get("path")]
    explicit = [*CORE_PATHS, *PRIOR_REPORTS, *STATE_PATHS, *AUDIT_PATHS, *sample_paths]
    missing_core = [path.as_posix() for path in CORE_PATHS if not (ROOT / path).is_file()]
    if missing_core:
        raise FileNotFoundError(f"missing V9.42 release inputs: {missing_core}")
...
def _write_attestation(report: dict[str, Any], zip_size: int | None) -> None:
    commands = _read_optional_json(Path("reports/audit_lite/v9_42_command_results.json")).get("commands", [])
    payload = {
        "version": VERSION,
    ...
    _write_json(Path("reports/audit_lite/v9_42_full_local_validation_attestation.json"), payload)
    _write_text(Path("reports/audit_lite/v9_42_full_local_validation_attestation.md"), "# Attestation V9.42\n\n" f"- Decision : `{report['decision']}`.\n" f"- Mode : `{report['validation_mode']}`.\n" f"- Qualite : `{report['quality_status']}`.\n" "- Aucun reseau, telechargement, trading, paper live, ordre, backtest, walk-forward, ML, strategie, signal actionnable, modele persistant, API privee, cle API ou websocket live.\n" "- Aucune suppression destructive, aucun sidecar et aucune empreinte ZIP.\n")
...
def _write_inventory(paths: list[Path], zip_size: int | None) -> None:
    ...
    _write_json(Path("reports/audit_lite/v9_42_artifact_inventory.json"), payload)
    _write_text(Path("reports/audit_lite/v9_42_artifact_inventory.md"), "# Inventaire audit-lite V9.42\n\n" f"- Fichiers inclus : `{len(paths)}`.\n" f"- ZIP : `{ZIP_NAME}`.\n")
...
def _write_size_report(paths: list[Path], zip_size: int | None) -> None:
    ...
    _write_json(Path("reports/audit_lite/zip_size_report_v9_42.json"), payload)
    _write_text(Path("reports/audit_lite/zip_size_report_v9_42.md"), "# Taille ZIP V9.42\n\n" f"- Fichiers : `{len(paths)}`.\n" f"- ZIP bytes estimate : `{zip_size}`.\n")
