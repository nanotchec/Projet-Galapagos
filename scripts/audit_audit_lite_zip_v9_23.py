...
REQUIRED_FILES = [
    ...
    "scripts/run_aggtrades_post_v9_batch2_collection_v9_23.py",
    "scripts/validate_aggtrades_post_v9_batch2_collection_v9_23.py",
    "scripts/release_audit_lite_zip_v9_23.py",
    "scripts/audit_audit_lite_zip_v9_23.py",
    "scripts/smoke_audit_lite_zip_v9_23.py",
    "src/galapagos/data/aggtrades_post_v9_collection_v9_18.py",
    "src/galapagos/data/aggtrades_post_v9_batch2_collection_v9_23.py",
    "src/galapagos/data/aggtrades_post_v9_batch2_collection_v9_23_validation.py",
    ...
    "reports/data/aggtrades_post_v9_batch2_collection_v9_23.md",
    "docs/aggtrades_post_v9_batch2_collection_v9_23.md",
    "reports/audit_lite/v9_23_command_results.json",
    "reports/audit_lite/v9_23_command_results.md",
    "reports/audit_lite/v9_23_full_local_validation_attestation.json",
    "reports/audit_lite/v9_23_full_local_validation_attestation.md",
    "reports/audit_lite/v9_23_artifact_inventory.json",
    "reports/audit_lite/v9_23_artifact_inventory.md",
    "reports/audit_lite/zip_size_report_v9_23.json",
    "reports/audit_lite/zip_size_report_v9_23.md",
    "reports/audit_lite/zip_audit_v9_23.json",
    "reports/audit_lite/zip_audit_v9_23.md",
    "reports/audit_lite/zip_smoke_v9_23.json",
    "reports/audit_lite/zip_smoke_v9_23.md",
...
def audit_zip(zip_path: Path) -> list[str]:
    ...
    errors: list[str] = []
    with tempfile.TemporaryDirectory(prefix="galapagos_v9_23_audit_") as tmp:
...
def _check_inventory(extract_root: Path, names: list[str]) -> list[str]:
    errors: list[str] = []
    inventory = _read_json(extract_root / "reports/audit_lite/v9_23_artifact_inventory.json")
    size_report = _read_json(extract_root / "reports/audit_lite/zip_size_report_v9_23.json")
...
def _check_reports(extract_root: Path) -> list[str]:
    ...
    manifest = _read_json(extract_root / "reports/manifests/aggtrades_post_v9_batch2_collection_v9_23_manifest.json")
    attestation = _read_json(extract_root / "reports/audit_lite/v9_23_full_local_validation_attestation.json")
    if report.get("mode") != "collect":
        errors.append("V9.23 audit-lite must describe the collect run")
    ...
    if report.get("audit_lite_mode_handling", {}).get("avoid_v9_22_contradictory_regeneration") is not True:
        errors.append("V9.23 must document audit-lite no-regeneration handling")
    if report.get("complete_collection_reached") is not False:
        errors.append("V9.23 must not claim complete future coverage")
    for key in ["collection_executed", "network_used", "new_data_downloaded", "ingestion_executed"]:
        if report.get(key) is not True:
            errors.append(f"V9.23 report must keep {key}=true")
    for key in ["features_created", "labels_created", "dataset_created", "ml_executed", "walk_forward_executed", "backtest_executed"]:
        if report.get(key) is not False:
            errors.append(f"V9.23 report must keep {key}=false")
    source = report.get("source_public_target", {})
    if source.get("host") != "data.binance.vision":
        errors.append("V9.23 source host mismatch")
...
def _check_command_results(extract_root: Path) -> list[str]:
    errors: list[str] = []
    payload = _read_json(extract_root / "reports/audit_lite/v9_23_command_results.json")
    ...
    required_needles = [
        ...
        "test_aggtrades_post_v9_batch2_collection_v9_23.py",
        "test_aggtrades_post_v9_batch2_collection_v9_23_validator.py",
        "run_aggtrades_post_v9_batch2_collection_v9_23.py",
        "validate_aggtrades_post_v9_batch2_collection_v9_23.py",
        "release_audit_lite_zip_v9_23.py",
    ]
...
def _check_state(extract_root: Path) -> list[str]:
    ...
    for name, payload in {"PROJECT_STATE": state, "latest_metrics": metrics}.items():
        if payload.get("last_validated_version") != "V9.22":
        ...
        if payload.get("direction") != "aggtrades_post_v9_batch2_collection":
...
def _write_reports(result: dict[str, Any]) -> None:
    ...
    (report_dir / "zip_audit_v9_23.json").write_text(json.dumps(result, indent=2, ensure_ascii=False) + "\n", encoding="utf-8")
    (report_dir / "zip_audit_v9_23.md").write_text(
        "# Audit ZIP V9.23\n\n"
