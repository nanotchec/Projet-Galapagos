...
VERSION = "V5.6"
REPORT_JSON = Path("reports/audit_lite/zip_smoke_v5_6.json")
REPORT_MD = Path("reports/audit_lite/zip_smoke_v5_6.md")
...
def smoke_zip(zip_path: Path) -> dict[str, Any]:
    ...
    with tempfile.TemporaryDirectory(prefix="galapagos-v5-6-smoke-") as tmp:
        ...
        try:
            ...
            if not hasattr(module, "validate_research_decision_gate_v5_6"):
                errors.append("missing V5.6 validator entrypoint")
            provenance = importlib.import_module("galapagos.data.public_market.provenance")
            if not hasattr(provenance, "sha256_file"):
                errors.append("missing sha256_file helper")
        ...
        inventory = _read_json(extract_root / "reports/audit_lite/v5_6_artifact_inventory.json")
        attestation = _read_json(extract_root / "reports/audit_lite/v5_6_full_local_validation_attestation.json")
        if report.get("version") != VERSION:
            errors.append("V5.6 report version mismatch inside audit-lite ZIP")
        if report.get("status") != "PASS":
            errors.append("V5.6 report status must be PASS inside audit-lite ZIP")
        if report.get("decision_gate_type") != "research_only":
            errors.append("V5.6 decision gate must be research_only")
        ...
        for flag in ["validator_passed", "tests_passed", "audit_lite_passed", "smoke_audit_lite_passed", "no_trading", "no_backtest", "no_orders", "no_strategy"]:
            if attestation.get(flag) is not True:
                errors.append(f"V5.6 attestation flag must be true: {flag}")
        collect_result = _run_collect_only(extract_root)
        if collect_result["returncode"] != 0:
            errors.append("V5.6 audit-lite pytest collect-only failed")
...
def _render_markdown(result: dict[str, Any]) -> str:
    ...
    return f"""# Smoke ZIP audit-lite V5.6
