...
VERSION = "V5.2"
REPORT_JSON = Path("reports/audit_lite/zip_audit_v5_2.json")
REPORT_MD = Path("reports/audit_lite/zip_audit_v5_2.md")
REQUIRED_FILES = {
    "README.md",
    "pyproject.toml",
    "reports/manifests/max_history_label_factory_v5_2_manifest.json",
    "reports/labels/max_history_label_factory_v5_2.json",
    "reports/labels/max_history_label_factory_v5_2.md",
    "reports/audit_lite/v5_2_artifact_inventory.json",
    "reports/audit_lite/v5_2_artifact_inventory.md",
    "reports/audit_lite/v5_2_parquet_summary.json",
    "reports/audit_lite/v5_2_full_local_validation_attestation.json",
    "reports/audit_lite/v5_2_full_local_validation_attestation.md",
    "reports/PROJECT_STATE.json",
    "reports/current/latest_summary.md",
    "docs/max_history_label_factory_v5_2.md",
...
REQUIRED_SAMPLES = {
    "data/audit_lite/v5_2/labels/timeframe=1m/sample.parquet",
    "data/audit_lite/v5_2/labels/timeframe=5m/sample.parquet",
    "data/audit_lite/v5_2/labels/timeframe=15m/sample.parquet",
    "data/audit_lite/v5_2/labels/timeframe=1h/sample.parquet",
}
FORBIDDEN_PREFIXES = [
...
def audit_zip(zip_path: Path) -> dict[str, Any]:
    ...
    with tempfile.TemporaryDirectory(prefix="galapagos-v5-2-audit-") as tmp:
        extract_root = Path(tmp)
        with zipfile.ZipFile(zip_path) as archive:
            archive.extractall(extract_root)
        manifest = _read_json(extract_root / "reports/manifests/max_history_label_factory_v5_2_manifest.json")
        report = _read_json(extract_root / "reports/labels/max_history_label_factory_v5_2.json")
        inventory = _read_json(extract_root / "reports/audit_lite/v5_2_artifact_inventory.json")
        summary = _read_json(extract_root / "reports/audit_lite/v5_2_parquet_summary.json")
        attestation = _read_json(extract_root / "reports/audit_lite/v5_2_full_local_validation_attestation.json")
        if manifest != report:
            errors.append("V5.2 manifest/report mismatch in ZIP")
        if manifest.get("version") != VERSION or manifest.get("status") != "PASS":
            errors.append("V5.2 manifest version/status mismatch in ZIP")
        ...
        if len(inventory.get("full_parquet_excluded", [])) != 4:
            errors.append("V5.2 full label parquet exclusion count mismatch")
        if set(summary.get("labels", {})) != {"1m", "5m", "15m", "1h"}:
            errors.append("V5.2 parquet summary labels mismatch")
        ...
        for flag in [
            ...
            if attestation.get(flag) is not True:
                errors.append(f"V5.2 attestation flag must be true: {flag}")
...
def _render_markdown(result: dict[str, Any]) -> str:
    ...
    return f"""# Audit ZIP audit-lite V5.2
