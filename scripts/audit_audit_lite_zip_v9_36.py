...
REQUIRED_FILES = [
    ...
    "src/galapagos/data/ohlcv_from_aggtrades_5y_validation_v9_36.py",
    "src/galapagos/data/ohlcv_from_aggtrades_5y_validation_v9_36_validation.py",
    "scripts/run_ohlcv_from_aggtrades_5y_validation_v9_36.py",
    "scripts/validate_ohlcv_from_aggtrades_5y_validation_v9_36.py",
    "scripts/release_audit_lite_zip_v9_36.py",
    "scripts/audit_audit_lite_zip_v9_36.py",
    "scripts/smoke_audit_lite_zip_v9_36.py",
    "tests/data/test_ohlcv_from_aggtrades_5y_validation_v9_36.py",
    "tests/validation/test_ohlcv_from_aggtrades_5y_validation_v9_36_validator.py",
    ...
    "reports/data/ohlcv_from_aggtrades_5y_validation_v9_36.md",
    "reports/data/ohlcv_from_aggtrades_5y_zero_trade_buckets_v9_36.json",
    "reports/data/ohlcv_from_aggtrades_5y_parity_v9_36.json",
    "docs/ohlcv_from_aggtrades_5y_validation_v9_36.md",
    "reports/audit_lite/v9_36_command_results.json",
    "reports/audit_lite/v9_36_command_results.md",
    "reports/audit_lite/v9_36_full_local_validation_attestation.json",
    "reports/audit_lite/v9_36_full_local_validation_attestation.md",
    "reports/audit_lite/v9_36_artifact_inventory.json",
    "reports/audit_lite/v9_36_artifact_inventory.md",
    "reports/audit_lite/zip_size_report_v9_36.json",
    "reports/audit_lite/zip_size_report_v9_36.md",
    "reports/audit_lite/zip_audit_v9_36.json",
    "reports/audit_lite/zip_audit_v9_36.md",
    "reports/audit_lite/zip_smoke_v9_36.json",
    "reports/audit_lite/zip_smoke_v9_36.md",
...
def audit_zip(zip_path: Path) -> list[str]:
    ...
    errors: list[str] = []
    with tempfile.TemporaryDirectory(prefix="galapagos_v9_36_audit_") as tmp:
...
def _check_inventory(extract_root: Path, names: list[str]) -> list[str]:
    errors: list[str] = []
    inventory = _read_json(extract_root / "reports/audit_lite/v9_36_artifact_inventory.json")
    size_report = _read_json(extract_root / "reports/audit_lite/zip_size_report_v9_36.json")
...
def _check_reports(extract_root: Path) -> list[str]:
    ...
    manifest = _read_json(extract_root / "reports/manifests/ohlcv_from_aggtrades_5y_validation_v9_36_manifest.json")
    attestation = _read_json(extract_root / "reports/audit_lite/v9_36_full_local_validation_attestation.json")
    if report.get("version") != VERSION or report.get("source_version") != "V9.35":
        errors.append("V9.36 report version/source mismatch")
    if manifest.get("decision") != report.get("decision") or manifest.get("safety_flags") != report.get("safety_flags"):
        errors.append("V9.36 manifest mismatch")
    if report.get("network_used") is not False or report.get("new_data_downloaded") is not False:
        errors.append("V9.36 must not use network or download data")
    if report.get("feature_store_created") is not False or report.get("combined_feature_store_created") is not False:
        errors.append("V9.36 must not create a feature store")
    if report.get("labels_created") is not False or report.get("dataset_created") is not False or report.get("ml_executed") is not False:
        errors.append("V9.36 must not create labels, dataset or ML")
...
def _check_flags(report: dict[str, Any], attestation: dict[str, Any]) -> list[str]:
    errors: list[str] = []
    for key in ["no_trading", "no_paper_live", "no_orders", "no_backtest", "no_walk_forward", "no_ml", "no_dataset_supervised", "no_labels", "no_feature_store", "no_combined_feature_store", "no_strategy", "no_actionable_signal", "no_persistent_model", "no_new_data_download", "no_ingestion_executed", "no_data_deletion", "no_destructive_cleanup", "no_sidecars", "no_zip_fingerprints"]:
...
def _check_command_results(extract_root: Path) -> list[str]:
    payload = _read_json(extract_root / "reports/audit_lite/v9_36_command_results.json")
    ...
    for needle in ["pytest --collect-only -q", "test_ohlcv_from_aggtrades_5y_validation_v9_36.py", "test_ohlcv_from_aggtrades_5y_validation_v9_36_validator.py", "run_ohlcv_from_aggtrades_5y_validation_v9_36.py", "validate_ohlcv_from_aggtrades_5y_validation_v9_36.py", "release_audit_lite_zip_v9_36.py"]:
...
def _check_state(extract_root: Path) -> list[str]:
    ...
    errors: list[str] = []
    if metrics.get("last_validated_version") != "V9.35":
    ...
    if metrics.get("direction") != "ohlcv_from_aggtrades_5y_coverage_validation":
...
def _write_reports(result: dict[str, Any]) -> None:
    ...
    (report_dir / "zip_audit_v9_36.json").write_text(json.dumps(result, indent=2, ensure_ascii=False) + "\n", encoding="utf-8")
    (report_dir / "zip_audit_v9_36.md").write_text("# Audit ZIP V9.36\n\n" f"- Resultat : `{'PASS' if result['passed'] else 'FAIL'}`.\n" f"- Erreurs : `{result['errors']}`.\n" "- Aucun sidecar et aucune empreinte ZIP.\n", encoding="utf-8")
