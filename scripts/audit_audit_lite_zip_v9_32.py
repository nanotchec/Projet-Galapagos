...
VERSION = "V9.32"
ZIP_NAME = "projet-galapagos-v9.32-audit-lite.zip"
REQUIRED_FILES = [
    "scripts/_bootstrap.py",
    "scripts/run_aggtrades_5y_full_coverage_validation_v9_32.py",
    "scripts/validate_aggtrades_5y_full_coverage_validation_v9_32.py",
    "scripts/release_audit_lite_zip_v9_32.py",
    "scripts/audit_audit_lite_zip_v9_32.py",
    "scripts/smoke_audit_lite_zip_v9_32.py",
    "src/galapagos/data/aggtrades_post_v9_full_coverage_validation_v9_29.py",
    "src/galapagos/data/aggtrades_5y_full_coverage_validation_v9_32.py",
    "src/galapagos/data/aggtrades_5y_full_coverage_validation_v9_32_validation.py",
    "tests/data/test_aggtrades_5y_full_coverage_validation_v9_32.py",
    "tests/validation/test_aggtrades_5y_full_coverage_validation_v9_32_validator.py",
    "reports/manifests/aggtrades_5y_full_coverage_validation_v9_32_manifest.json",
    "reports/data/aggtrades_5y_full_coverage_validation_v9_32.json",
    "reports/data/aggtrades_5y_full_coverage_validation_v9_32.md",
    "docs/aggtrades_5y_full_coverage_validation_v9_32.md",
    "reports/audit_lite/v9_32_command_results.json",
    "reports/audit_lite/v9_32_command_results.md",
    "reports/audit_lite/v9_32_full_local_validation_attestation.json",
    "reports/audit_lite/v9_32_full_local_validation_attestation.md",
    "reports/audit_lite/v9_32_artifact_inventory.json",
    "reports/audit_lite/v9_32_artifact_inventory.md",
    "reports/audit_lite/zip_size_report_v9_32.json",
    "reports/audit_lite/zip_size_report_v9_32.md",
    "reports/audit_lite/zip_audit_v9_32.json",
    "reports/audit_lite/zip_audit_v9_32.md",
    "reports/audit_lite/zip_smoke_v9_32.json",
    "reports/audit_lite/zip_smoke_v9_32.md",
...
def audit_zip(zip_path: Path) -> list[str]:
    ...
    errors: list[str] = []
    with tempfile.TemporaryDirectory(prefix="galapagos_v9_32_audit_") as tmp:
...
def _check_inventory(extract_root: Path, names: list[str]) -> list[str]:
    errors: list[str] = []
    inventory = _read_json(extract_root / "reports/audit_lite/v9_32_artifact_inventory.json")
    size_report = _read_json(extract_root / "reports/audit_lite/zip_size_report_v9_32.json")
...
def _check_reports(extract_root: Path) -> list[str]:
    errors: list[str] = []
    report = _read_json(extract_root / "reports/data/aggtrades_5y_full_coverage_validation_v9_32.json")
    manifest = _read_json(extract_root / "reports/manifests/aggtrades_5y_full_coverage_validation_v9_32_manifest.json")
    attestation = _read_json(extract_root / "reports/audit_lite/v9_32_full_local_validation_attestation.json")
    if report.get("version") != VERSION or report.get("source_version") != "V9.31":
        errors.append("V9.32 report version/source mismatch")
    ...
    if manifest.get("decision") != report.get("decision") or manifest.get("safety_flags") != report.get("safety_flags"):
        errors.append("V9.32 manifest mismatch")
    if report.get("decision") in {"aggtrades_5y_full_coverage_validated", "aggtrades_5y_full_coverage_validated_with_non_blocking_warnings"}:
        for key, expected in {
            "days_expected_5y": 1827,
            "days_complete": 1827,
            "days_missing": 0,
            "days_failed": 0,
            ...
            if report.get(key) != expected or attestation.get(key) != expected:
                errors.append(f"V9.32 complete report mismatch for {key}")
...
def _check_flags(report: dict[str, Any], attestation: dict[str, Any]) -> list[str]:
    errors: list[str] = []
    for key in ["no_trading", "no_paper_live", "no_orders", "no_backtest", "no_walk_forward", "no_ml", "no_dataset_supervised", "no_strategy", "no_actionable_signal", "no_persistent_model", "no_new_data_download", "no_ingestion_executed", "no_data_deletion", "no_destructive_cleanup", "no_sidecars", "no_zip_fingerprints"]:
    ...
    if report.get("network_used") is not False or report.get("new_data_downloaded") is not False or report.get("ingestion_executed") is not False:
        errors.append("V9.32 must be read-only without network, new downloads or ingestion")
    return errors


def _check_command_results(extract_root: Path) -> list[str]:
    payload = _read_json(extract_root / "reports/audit_lite/v9_32_command_results.json")
    ...
    for needle in [
        ...
        "test_aggtrades_5y_full_coverage_validation_v9_32.py",
        "test_aggtrades_5y_full_coverage_validation_v9_32_validator.py",
        "run_aggtrades_5y_full_coverage_validation_v9_32.py",
        "validate_aggtrades_5y_full_coverage_validation_v9_32.py",
        "release_audit_lite_zip_v9_32.py",
    ]:
...
def _check_state(extract_root: Path) -> list[str]:
    ...
    errors: list[str] = []
    if metrics.get("last_validated_version") != "V9.31":
    ...
    if metrics.get("direction") != "aggtrades_5y_full_coverage_validation":
...
def _write_reports(result: dict[str, Any]) -> None:
    ...
    (report_dir / "zip_audit_v9_32.json").write_text(json.dumps(result, indent=2, ensure_ascii=False) + "\n", encoding="utf-8")
    (report_dir / "zip_audit_v9_32.md").write_text(
        "# Audit ZIP V9.32\n\n"
