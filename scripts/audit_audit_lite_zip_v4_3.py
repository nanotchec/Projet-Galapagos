...
VERSION = "V4.3"
REPORT_JSON = Path("reports/audit_lite/zip_audit_v4_3.json")
REPORT_MD = Path("reports/audit_lite/zip_audit_v4_3.md")
REQUIRED_FILES = {
    "README.md",
    "pyproject.toml",
    "reports/manifests/one_year_causal_feature_store_v4_3_manifest.json",
    "reports/features/one_year_causal_feature_store_v4_3.json",
    "reports/features/one_year_causal_feature_store_v4_3.md",
    "reports/audit_lite/v4_3_artifact_inventory.json",
    "reports/audit_lite/v4_3_artifact_inventory.md",
    "reports/audit_lite/v4_3_parquet_summary.json",
    "reports/audit_lite/v4_3_full_local_validation_attestation.json",
    "reports/audit_lite/v4_3_full_local_validation_attestation.md",
...
REQUIRED_SAMPLES = {
    "data/audit_lite/v4_3/features/timeframe=1m/sample.parquet",
    "data/audit_lite/v4_3/features/timeframe=5m/sample.parquet",
    "data/audit_lite/v4_3/features/timeframe=15m/sample.parquet",
    "data/audit_lite/v4_3/features/timeframe=1h/sample.parquet",
}
FORBIDDEN_PREFIXES = [
...
def audit_zip(zip_path: Path) -> dict[str, Any]:
    ...
    with tempfile.TemporaryDirectory(prefix="galapagos-v4-3-audit-") as tmp:
        extract_root = Path(tmp)
        with zipfile.ZipFile(zip_path) as archive:
            archive.extractall(extract_root)
        manifest = _read_json(extract_root / "reports/manifests/one_year_causal_feature_store_v4_3_manifest.json")
        report = _read_json(extract_root / "reports/features/one_year_causal_feature_store_v4_3.json")
        inventory = _read_json(extract_root / "reports/audit_lite/v4_3_artifact_inventory.json")
        summary = _read_json(extract_root / "reports/audit_lite/v4_3_parquet_summary.json")
        attestation = _read_json(extract_root / "reports/audit_lite/v4_3_full_local_validation_attestation.json")
        if manifest != report:
            errors.append("V4.3 manifest/report mismatch in ZIP")
        if manifest.get("version") != VERSION:
            errors.append("V4.3 manifest version mismatch in ZIP")
        if len(inventory.get("raw_files_excluded", [])) != 366:
            errors.append("V4.3 raw inventory must represent 366 raw zips")
        ...
        if set(summary.get("features", {})) != {"1m", "5m", "15m", "1h"}:
            errors.append("V4.3 parquet summary must include all feature timeframes")
        ...
        for flag in ["validator_passed", "tests_passed", "audit_lite_passed", "smoke_audit_lite_passed", "no_trading", "no_backtest", "no_orders"]:
            if attestation.get(flag) is not True:
                errors.append(f"V4.3 attestation flag must be true: {flag}")
...
def _render_markdown(result: dict[str, Any]) -> str:
    ...
    return f"""# Audit ZIP audit-lite V4.3
