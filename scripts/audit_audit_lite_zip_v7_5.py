...
VERSION = "V7.5"
REPORT_JSON = Path("reports/audit_lite/zip_audit_v7_5.json")
REPORT_MD = Path("reports/audit_lite/zip_audit_v7_5.md")
REQUIRED_FILES = {
    "README.md",
    "pyproject.toml",
    "reports/manifests/ohlcv_trades_offline_supervised_dataset_v7_3_manifest.json",
    "reports/manifests/ohlcv_trades_offline_ml_research_v7_4_manifest.json",
    "reports/ml/ohlcv_trades_offline_ml_research_v7_4.json",
    "reports/ml/ohlcv_trades_offline_research_scores_v7_4.json",
    "reports/manifests/ohlcv_trades_ml_robustness_v7_5_manifest.json",
    "reports/ml/ohlcv_trades_ml_robustness_v7_5.json",
    "reports/ml/ohlcv_trades_ml_robustness_v7_5.md",
    "reports/audit_lite/v7_5_artifact_inventory.json",
    "reports/audit_lite/v7_5_artifact_inventory.md",
    "reports/audit_lite/zip_size_report_v7_5.json",
    "reports/audit_lite/zip_size_report_v7_5.md",
    "reports/audit_lite/v7_5_full_local_validation_attestation.json",
    "reports/audit_lite/v7_5_full_local_validation_attestation.md",
    "reports/PROJECT_STATE.json",
    "reports/current/latest_summary.md",
    "docs/ohlcv_trades_ml_robustness_v7_5.md",
    "scripts/run_ohlcv_trades_ml_robustness_v7_5.py",
    "scripts/validate_ohlcv_trades_ml_robustness_v7_5.py",
    "scripts/release_audit_lite_zip_v7_5.py",
    "scripts/audit_audit_lite_zip_v7_5.py",
    "scripts/smoke_audit_lite_zip_v7_5.py",
    "tests/ml/test_ohlcv_trades_ml_robustness_v7_5.py",
    "tests/validation/test_ohlcv_trades_ml_robustness_v7_5_validator.py",
}
...
def audit_zip(zip_path: Path) -> dict[str, Any]:
    ...
    with tempfile.TemporaryDirectory(prefix="galapagos-v7-5-audit-") as tmp:
        extract_root = Path(tmp)
        with zipfile.ZipFile(zip_path) as archive:
            archive.extractall(extract_root)
        manifest = _read_json(extract_root / "reports/manifests/ohlcv_trades_ml_robustness_v7_5_manifest.json")
        report = _read_json(extract_root / "reports/ml/ohlcv_trades_ml_robustness_v7_5.json")
        attestation = _read_json(extract_root / "reports/audit_lite/v7_5_full_local_validation_attestation.json")
        if manifest != report:
            errors.append("V7.5 manifest/report mismatch in ZIP")
        if manifest.get("version") != VERSION:
            errors.append("V7.5 manifest version mismatch in ZIP")
        for flag in ["validator_passed", "tests_passed", "audit_lite_passed", "smoke_audit_lite_passed", "no_trading", "no_backtest", "no_orders", "no_strategy"]:
            if attestation.get(flag) is not True:
                errors.append(f"V7.5 attestation flag must be true: {flag}")
...
def _render_markdown(result: dict[str, Any]) -> str:
    ...
    return f"""# Audit ZIP audit-lite V7.5
