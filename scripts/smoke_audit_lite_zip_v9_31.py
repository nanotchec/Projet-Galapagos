...
def smoke_zip(zip_path: Path) -> dict[str, Any]:
    errors: list[str] = []
    with tempfile.TemporaryDirectory(prefix="galapagos_v9_31_smoke_") as tmp:
        ...
        errors.extend(_run_v9_31_tests(extract_root, env))
...
def _import_modules(extract_root: Path, env: dict[str, str]) -> list[str]:
    ...
    for module in [
        ...
        "galapagos.data.aggtrades_5y_extension_collection_v9_31",
        "galapagos.data.aggtrades_5y_extension_collection_v9_31_validation",
    ]:
...
def _run_v9_31_tests(extract_root: Path, env: dict[str, str]) -> list[str]:
    completed = _run(
        [
            "python",
            "-m",
    ...
    if completed["returncode"] != 0:
        return [f"V9.31 tests failed: {completed['stdout'][-1000:]} {completed['stderr'][-1000:]}"]
    return []
...
def _check_reports(extract_root: Path) -> list[str]:
    ...
    if report.get("version") != VERSION:
        errors.append("V9.31 report version mismatch")
    if report.get("extension_window_start") != "2021-05-05" or report.get("extension_window_end") != "2024-05-04":
        errors.append("V9.31 smoke expects extension window")
    if report.get("decision") == "aggtrades_5y_extension_collection_complete":
        if report.get("days_complete") != 1096 or report.get("complete_extension_reached") is not True:
            errors.append("V9.31 smoke expects complete extension for complete decision")
    if report.get("features_created") is not False or report.get("ml_executed") is not False:
        errors.append("V9.31 smoke expects data-only flags")
    return errors


def _run_self_audit(extract_root: Path, zip_path: Path, env: dict[str, str]) -> list[str]:
    completed = _run(["python", "scripts/audit_audit_lite_zip_v9_31.py", "--zip", str(zip_path)], extract_root, env, AUDIT_TIMEOUT_SECONDS)
    if completed["returncode"] != 0:
...
def _write_reports(result: dict[str, Any]) -> None:
    ...
    (report_dir / "zip_smoke_v9_31.json").write_text(json.dumps(result, indent=2, ensure_ascii=False) + "\n", encoding="utf-8")
    (report_dir / "zip_smoke_v9_31.md").write_text(
        "# Smoke ZIP V9.31\n\n"
        ...
        f"- Timeout tests V9.31 : `{TEST_TIMEOUT_SECONDS}` secondes.\n"
