...
def smoke_zip(zip_path: Path) -> dict[str, Any]:
    ...
    with tempfile.TemporaryDirectory(prefix="galapagos_v9_41_smoke_") as tmp:
        ...
        errors.extend(_run_v9_41_tests(extract_root, env))
...
def _check_pyarrow_available(extract_root: Path, env: dict[str, str]) -> list[str]:
    ...
    if completed["returncode"] != 0:
        return ["pyarrow is required for V9.41 dataset smoke checks but is not importable"]
    return []
...
def _import_modules(extract_root: Path, env: dict[str, str]) -> list[str]:
    errors: list[str] = []
    for module in [
        "galapagos.datasets.ohlcv_aggtrades_5y_dataset_v9_41",
        "galapagos.datasets.ohlcv_aggtrades_5y_dataset_v9_41_validation",
    ]:
...
def _run_v9_41_tests(extract_root: Path, env: dict[str, str]) -> list[str]:
    completed = _run(
        [
            "python",
            "-m",
    ...
    if completed["returncode"] != 0:
        return [f"V9.41 tests failed: {completed['stdout'][-1000:]} {completed['stderr'][-1000:]}"]
    return []
...
def _check_reports(extract_root: Path) -> list[str]:
    ...
    if report.get("version") != VERSION:
        errors.append("V9.41 report version mismatch")
    if report.get("dataset_created") is not True:
        errors.append("V9.41 smoke expects dataset_created=true")
    if report.get("network_used") is not False or report.get("new_data_downloaded") is not False:
        errors.append("V9.41 smoke expects no network and no downloads")
    if report.get("ml_executed") is not False or report.get("backtest_executed") is not False:
        errors.append("V9.41 smoke expects no ML or backtest")
    if report.get("leakage_guard", {}).get("status") != "PASS":
        errors.append("V9.41 smoke expects leakage guard PASS")
    return errors


def _run_self_audit(extract_root: Path, zip_path: Path, env: dict[str, str]) -> list[str]:
    completed = _run(["python", "scripts/audit_audit_lite_zip_v9_41.py", "--zip", str(zip_path)], extract_root, env, AUDIT_TIMEOUT_SECONDS)
    if completed["returncode"] != 0:
...
def _write_reports(result: dict[str, Any]) -> None:
    ...
    (report_dir / "zip_smoke_v9_41.json").write_text(json.dumps(result, indent=2, ensure_ascii=False) + "\n", encoding="utf-8")
    (report_dir / "zip_smoke_v9_41.md").write_text(
        "# Smoke ZIP V9.41\n\n"
        ...
        f"- Timeout tests V9.41 : `{TEST_TIMEOUT_SECONDS}` secondes.\n"
