...
VERSION = "V9.38"
ZIP_NAME = "projet-galapagos-v9.38-audit-lite.zip"
ROOT = Path(".").resolve()

CORE_PATHS = [
    Path("scripts/_bootstrap.py"),
    Path("src/galapagos/features/ohlcv_aggtrades_5y_feature_store_validation_v9_38.py"),
    Path("src/galapagos/features/ohlcv_aggtrades_5y_feature_store_validation_v9_38_validation.py"),
    ...
    Path("scripts/run_ohlcv_aggtrades_5y_feature_store_validation_v9_38.py"),
    Path("scripts/validate_ohlcv_aggtrades_5y_feature_store_validation_v9_38.py"),
    Path("scripts/release_audit_lite_zip_v9_38.py"),
    Path("scripts/audit_audit_lite_zip_v9_38.py"),
    Path("scripts/smoke_audit_lite_zip_v9_38.py"),
    Path("tests/features/test_ohlcv_aggtrades_5y_feature_store_validation_v9_38.py"),
    Path("tests/validation/test_ohlcv_aggtrades_5y_feature_store_validation_v9_38_validator.py"),
    Path("reports/manifests/ohlcv_aggtrades_5y_feature_store_validation_v9_38_manifest.json"),
    ...
    Path("docs/ohlcv_aggtrades_5y_feature_store_validation_v9_38.md"),
]
...
AUDIT_PATHS = [
    Path("reports/audit_lite/v9_38_command_results.json"),
    Path("reports/audit_lite/v9_38_command_results.md"),
    Path("reports/audit_lite/v9_38_full_local_validation_attestation.json"),
    Path("reports/audit_lite/v9_38_full_local_validation_attestation.md"),
    Path("reports/audit_lite/v9_38_artifact_inventory.json"),
    Path("reports/audit_lite/v9_38_artifact_inventory.md"),
    Path("reports/audit_lite/zip_size_report_v9_38.json"),
    Path("reports/audit_lite/zip_size_report_v9_38.md"),
    Path("reports/audit_lite/zip_audit_v9_38.json"),
    Path("reports/audit_lite/zip_audit_v9_38.md"),
    Path("reports/audit_lite/zip_smoke_v9_38.json"),
    Path("reports/audit_lite/zip_smoke_v9_38.md"),
]
...
def main() -> int:
    report = _read_json(Path("reports/features/ohlcv_aggtrades_5y_feature_store_validation_v9_38.json"))
...
def _ensure_placeholders() -> None:
    placeholders = {
        "reports/audit_lite/v9_38_command_results.json": {"version": VERSION, "status": "PENDING_CAPTURE", "commands": [], "sidecars_created": False, "zip_fingerprints_created": False},
        "reports/audit_lite/zip_audit_v9_38.json": {"version": VERSION, "passed": False, "errors": [], "status": "PENDING_RUN"},
        "reports/audit_lite/zip_smoke_v9_38.json": {"version": VERSION, "passed": False, "errors": [], "status": "PENDING_RUN"},
    }
    texts = {
        "reports/audit_lite/v9_38_command_results.md": "# Commandes V9.38\n\nEn attente.\n",
        "reports/audit_lite/zip_audit_v9_38.md": "# Audit ZIP V9.38\n\nEn attente.\n",
        "reports/audit_lite/zip_smoke_v9_38.md": "# Smoke ZIP V9.38\n\nEn attente.\n",
    }
...
def _collect_paths() -> list[Path]:
    ...
    if missing_core:
        raise FileNotFoundError(f"missing V9.38 release inputs: {missing_core}")
...
def _write_attestation(report: dict[str, Any], zip_size: int | None) -> None:
    commands = _read_optional_json(Path("reports/audit_lite/v9_38_command_results.json")).get("commands", [])
    payload = {
        "version": VERSION,
    ...
    _write_json(Path("reports/audit_lite/v9_38_full_local_validation_attestation.json"), payload)
    _write_text(
        Path("reports/audit_lite/v9_38_full_local_validation_attestation.md"),
        "# Attestation V9.38\n\n"
        ...
        "- Validation-only : aucun reseau, telechargement, trading, paper live, ordre, backtest, walk-forward, ML, dataset supervise, label, strategie, signal actionnable, modele persistant, API privee, cle API, client exchange authentifie ou websocket live.\n"
...
def _write_inventory(paths: list[Path], zip_size: int | None) -> None:
    ...
    _write_json(Path("reports/audit_lite/v9_38_artifact_inventory.json"), payload)
    _write_text(Path("reports/audit_lite/v9_38_artifact_inventory.md"), "# Inventaire audit-lite V9.38\n\n" f"- Fichiers inclus : `{len(paths)}`.\n" f"- ZIP : `{ZIP_NAME}`.\n")
...
def _write_size_report(paths: list[Path], zip_size: int | None) -> None:
    ...
    _write_json(Path("reports/audit_lite/zip_size_report_v9_38.json"), payload)
    _write_text(Path("reports/audit_lite/zip_size_report_v9_38.md"), "# Taille ZIP V9.38\n\n" f"- Fichiers : `{len(paths)}`.\n" f"- ZIP bytes estimate : `{zip_size}`.\n")
