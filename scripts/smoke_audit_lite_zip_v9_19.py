...
def smoke_zip(zip_path: Path) -> dict[str, Any]:
    errors: list[str] = []
    with tempfile.TemporaryDirectory(prefix="galapagos_v9_19_smoke_") as tmp:
        ...
        errors.extend(_run_v9_19_tests(extract_root, env))
...
def _import_modules(extract_root: Path, env: dict[str, str]) -> list[str]:
    ...
    for module in [
        ...
        "galapagos.data.aggtrades_post_v9_pilot_collection_v9_19",
        "galapagos.data.aggtrades_post_v9_pilot_collection_v9_19_validation",
    ]:
...
def _run_v9_19_tests(extract_root: Path, env: dict[str, str]) -> list[str]:
    completed = _run(
        [
            "python",
            "-m",
    ...
    if completed["returncode"] != 0:
        return [f"V9.19 tests failed: {completed['stdout'][-1000:]} {completed['stderr'][-1000:]}"]
    return []


def _run_self_audit(extract_root: Path, zip_path: Path, env: dict[str, str]) -> list[str]:
    completed = _run(["python", "scripts/audit_audit_lite_zip_v9_19.py", "--zip", str(zip_path)], extract_root, env, AUDIT_TIMEOUT_SECONDS)
    if completed["returncode"] != 0:
...
def _check_reports(extract_root: Path) -> list[str]:
    ...
    if report.get("mode") != "collect":
        errors.append("V9.19 smoke expects collect mode")
    ...
    if summary.get("days_requested") != 7 or summary.get("days_complete") != 7:
        errors.append("V9.19 pilot day counts mismatch")
    ...
    if flags.get("network_scope") != "public_archive_read_only":
        errors.append("V9.19 must limit network scope to public archive read-only")
    return errors
...
def _write_reports(result: dict[str, Any]) -> None:
    ...
    (report_dir / "zip_smoke_v9_19.json").write_text(json.dumps(result, indent=2, ensure_ascii=False) + "\n", encoding="utf-8")
    (report_dir / "zip_smoke_v9_19.md").write_text(
        "# Smoke ZIP V9.19\n\n"
        ...
        f"- Timeout tests V9.19 : `{TEST_TIMEOUT_SECONDS}` secondes.\n"
