...
VERSION = "V3.6"
CURRENT_VALIDATOR = "validate_expanded_causal_feature_store_v3_6"
CURRENT_VALIDATOR_SCRIPT = "scripts/validate_expanded_causal_feature_store_v3_6.py"
...
def main() -> None:
    ...
    with tempfile.TemporaryDirectory(prefix="galapagos_v3_6_smoke_") as tmp:
        tmp_path = Path(tmp)
        ...
        feature_errors, row_counts = _validate_feature_rows_and_schema(extracted_root)
    ...
    payload = {
        ...
        "feature_schema_strict": not any("FEATURE_COLUMNS_V3_6" in error for error in errors),
        "forbidden_entries_found": forbidden_entries,
        "safety_flags_ok": not any("unsafe V3.6 flag active" in error for error in errors),
        "smoke_duration_seconds": smoke_duration,
        "errors": errors,
        "warnings": warnings,
    }
    Path("reports").mkdir(exist_ok=True)
    Path("reports/zip_smoke_test_v3_6.json").write_text(json.dumps(payload, indent=2, ensure_ascii=False) + "\n", encoding="utf-8")
    Path("reports/zip_smoke_test_v3_6.md").write_text(
        "# Smoke ZIP V3.6\n\n"
...
def _validate_essential_entries(names: list[str]) -> list[str]:
    ...
    for timeframe in EXPECTED_ROWS:
        feature_entry = (
            "data/research/v3_6/features/ohlcv/source=binance_archive/market_type=spot/"
            f"symbol=BTCUSDT/timeframe={timeframe}/window=2024-01-01_2024-03-30/features.parquet"
        )
        if feature_entry not in name_set:
            errors.append(f"missing V3.6 feature parquet: {feature_entry}")
    return errors
...
def _validate_manifest_state_and_safety(root: Path) -> list[str]:
    ...
    if manifest != report:
        errors.append("V3.6 report JSON must match manifest projection")
    if manifest.get("version") != VERSION or report.get("version") != VERSION:
        errors.append("V3.6 manifest/report version mismatch")
    ...
    if project_state.get("candidate_version") != VERSION and VERSION not in latest_summary:
        errors.append("PROJECT_STATE/latest_summary must identify V3.6 as candidate")
    if project_state.get("last_validated_version") != "V3.5.2":
        errors.append("last_validated_version must be V3.5.2")
    if "pending_external_audit" not in latest_summary and project_state.get("candidate_status") != "pending_external_audit":
        errors.append("V3.6 must remain pending_external_audit")
    ...
    if safety.get("public_read_only") is not True:
        errors.append("V3.6 public_read_only must be true")
    lower_state = (project_state_text + "\n" + latest_summary).casefold()
    for claim in ["aucun trading", "aucun paper live", "aucun ordre", "aucun backtest", "aucune strategie", "aucun label", "aucun dataset ml"]:
...
def _validate_feature_rows_and_schema(root: Path) -> tuple[list[str], dict[str, int]]:
    from galapagos.data.public_market.storage import read_parquet
    from galapagos.features.expanded_window import output_path
    ...
    for timeframe, expected_rows in EXPECTED_ROWS.items():
        ...
        if row_counts[timeframe] != expected_rows:
            errors.append(f"V3.6 row count mismatch for {timeframe}: {row_counts[timeframe]} != {expected_rows}")
        if list(frame.columns) != FEATURE_COLUMNS_V3_6:
            errors.append(f"V3.6 FEATURE_COLUMNS_V3_6 schema mismatch for {timeframe}")
