...
CORE_PATHS = [
    ...
    Path("src/galapagos/data/ohlcv_5y_extension_v9_34_validation.py"),
    Path("src/galapagos/data/ohlcv_5y_extension_correction_v9_34_1.py"),
    Path("src/galapagos/data/ohlcv_5y_extension_correction_v9_34_1_validation.py"),
    Path("scripts/run_ohlcv_5y_extension_correction_v9_34_1.py"),
    Path("scripts/validate_ohlcv_5y_extension_correction_v9_34_1.py"),
    Path("scripts/release_audit_lite_zip_v9_34_1.py"),
    Path("scripts/audit_audit_lite_zip_v9_34_1.py"),
    Path("scripts/smoke_audit_lite_zip_v9_34_1.py"),
    Path("tests/data/test_ohlcv_5y_extension_correction_v9_34_1.py"),
    Path("tests/validation/test_ohlcv_5y_extension_correction_v9_34_1_validator.py"),
    ...
    Path("reports/data/ohlcv_5y_extension_correction_v9_34_1.md"),
    Path("docs/ohlcv_5y_extension_correction_v9_34_1.md"),
]

PRIOR_REPORTS = [
    Path("reports/data/ohlcv_5y_extension_v9_34.json"),
    Path("reports/data/ohlcv_5y_extension_v9_34.md"),
    Path("reports/manifests/ohlcv_5y_extension_v9_34_manifest.json"),
...
AUDIT_PATHS = [
    Path("reports/audit_lite/v9_34_1_command_results.json"),
    Path("reports/audit_lite/v9_34_1_command_results.md"),
    Path("reports/audit_lite/v9_34_1_full_local_validation_attestation.json"),
    Path("reports/audit_lite/v9_34_1_full_local_validation_attestation.md"),
    Path("reports/audit_lite/v9_34_1_artifact_inventory.json"),
    Path("reports/audit_lite/v9_34_1_artifact_inventory.md"),
    Path("reports/audit_lite/zip_size_report_v9_34_1.json"),
    Path("reports/audit_lite/zip_size_report_v9_34_1.md"),
    Path("reports/audit_lite/zip_audit_v9_34_1.json"),
    Path("reports/audit_lite/zip_audit_v9_34_1.md"),
    Path("reports/audit_lite/zip_smoke_v9_34_1.json"),
    Path("reports/audit_lite/zip_smoke_v9_34_1.md"),
]
...
def main() -> int:
    report = _read_json(Path("reports/data/ohlcv_5y_extension_correction_v9_34_1.json"))
...
def _ensure_placeholders() -> None:
    placeholders = {
        "reports/audit_lite/v9_34_1_command_results.json": {"version": VERSION, "status": "PENDING_CAPTURE", "commands": [], "sidecars_created": False, "zip_fingerprints_created": False},
        "reports/audit_lite/zip_audit_v9_34_1.json": {"version": VERSION, "passed": False, "errors": [], "status": "PENDING_RUN"},
        "reports/audit_lite/zip_smoke_v9_34_1.json": {"version": VERSION, "passed": False, "errors": [], "status": "PENDING_RUN"},
    }
    texts = {
        "reports/audit_lite/v9_34_1_command_results.md": "# Commandes V9.34.1\n\nEn attente.\n",
        "reports/audit_lite/zip_audit_v9_34_1.md": "# Audit ZIP V9.34.1\n\nEn attente.\n",
        "reports/audit_lite/zip_smoke_v9_34_1.md": "# Smoke ZIP V9.34.1\n\nEn attente.\n",
    }
...
def _collect_paths() -> list[Path]:
    dynamic_reports = sorted(Path("reports/data").glob("ohlcv_5y_extension_correction_*_v9_34_1.json"))
    explicit = [*CORE_PATHS, *dynamic_reports, *PRIOR_REPORTS, *STATE_PATHS, *AUDIT_PATHS]
    missing_core = [path.as_posix() for path in CORE_PATHS if not (ROOT / path).is_file()]
    if missing_core:
        raise FileNotFoundError(f"missing V9.34.1 release inputs: {missing_core}")
...
def _write_attestation(report: dict[str, Any], zip_size: int | None) -> None:
    commands = _read_optional_json(Path("reports/audit_lite/v9_34_1_command_results.json")).get("commands", [])
    payload = {
        "version": VERSION,
    ...
    _write_json(Path("reports/audit_lite/v9_34_1_full_local_validation_attestation.json"), payload)
    _write_text(
        Path("reports/audit_lite/v9_34_1_full_local_validation_attestation.md"),
        "# Attestation V9.34.1\n\n"
        ...
        "- Aucun trading, paper live, ordre, backtest, walk-forward, ML, dataset supervise, label, strategie, signal actionnable, feature store combine, modele persistant, API privee, cle API, client exchange authentifie ou websocket live.\n"
...
def _write_inventory(paths: list[Path], zip_size: int | None) -> None:
    ...
    _write_json(Path("reports/audit_lite/v9_34_1_artifact_inventory.json"), payload)
    _write_text(Path("reports/audit_lite/v9_34_1_artifact_inventory.md"), "# Inventaire audit-lite V9.34.1\n\n" f"- Fichiers inclus : `{len(paths)}`.\n" f"- ZIP : `{ZIP_NAME}`.\n")
...
def _write_size_report(paths: list[Path], zip_size: int | None) -> None:
    ...
    _write_json(Path("reports/audit_lite/zip_size_report_v9_34_1.json"), payload)
    _write_text(Path("reports/audit_lite/zip_size_report_v9_34_1.md"), "# Taille ZIP V9.34.1\n\n" f"- Fichiers : `{len(paths)}`.\n" f"- ZIP bytes estimate : `{zip_size}`.\n")
