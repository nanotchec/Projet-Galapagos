...
REPORT_PATHS = [
    ...
    Path("reports/labels/horizon_event_label_redesign_v9_12.md"),
    Path("reports/labels/horizon_event_label_redesign_v9_12_datacard.md"),
    Path("docs/horizon_event_label_redesign_v9_12.md"),
]

AUDIT_PATHS = [
    Path("reports/audit_lite/v9_12_command_results.json"),
    Path("reports/audit_lite/v9_12_command_results.md"),
    Path("reports/audit_lite/v9_12_full_local_validation_attestation.json"),
    Path("reports/audit_lite/v9_12_full_local_validation_attestation.md"),
    Path("reports/audit_lite/v9_12_artifact_inventory.json"),
    Path("reports/audit_lite/v9_12_artifact_inventory.md"),
    Path("reports/audit_lite/zip_size_report_v9_12.json"),
    Path("reports/audit_lite/zip_size_report_v9_12.md"),
    Path("reports/audit_lite/zip_audit_v9_12.json"),
    Path("reports/audit_lite/zip_audit_v9_12.md"),
    Path("reports/audit_lite/zip_smoke_v9_12.json"),
    Path("reports/audit_lite/zip_smoke_v9_12.md"),
]
...
INPUT_REPORTS = [
    ...
    Path("reports/manifests/label_failure_analysis_v9_11_manifest.json"),
]

SCRIPT_PATHS = [
    Path("scripts/_bootstrap.py"),
    Path("scripts/run_horizon_event_label_redesign_v9_12.py"),
    Path("scripts/validate_horizon_event_label_redesign_v9_12.py"),
    Path("scripts/release_audit_lite_zip_v9_12.py"),
    Path("scripts/audit_audit_lite_zip_v9_12.py"),
    Path("scripts/smoke_audit_lite_zip_v9_12.py"),
]

TEST_PATHS = [
    Path("tests/labels/test_horizon_event_label_redesign_v9_12.py"),
    Path("tests/validation/test_horizon_event_label_redesign_v9_12_validator.py"),
]

CODE_PATHS = [
    Path("src/galapagos/__init__.py"),
    Path("src/galapagos/labels/horizon_event_label_redesign_v9_12.py"),
    Path("src/galapagos/labels/horizon_event_label_redesign_v9_12_schemas.py"),
    Path("src/galapagos/labels/horizon_event_label_redesign_v9_12_validation.py"),
...
def main() -> int:
    _ensure_inputs()
    report = _read_json(Path("reports/labels/horizon_event_label_redesign_v9_12.json"))
    _write_state_surfaces(report)
    _ensure_placeholders()
    _write_samples(report)
...
def _ensure_inputs() -> None:
    ...
    if missing:
        raise FileNotFoundError(f"missing V9.12 audit-lite inputs: {missing}")


def _ensure_placeholders() -> None:
    placeholders = {
        "reports/audit_lite/v9_12_command_results.json": {"version": VERSION, "status": "PENDING_CAPTURE", "commands": [], "sidecars_created": False, "zip_fingerprints_created": False},
        "reports/audit_lite/zip_audit_v9_12.json": {"version": VERSION, "passed": False, "errors": [], "status": "PENDING_RUN"},
        "reports/audit_lite/zip_smoke_v9_12.json": {"version": VERSION, "passed": False, "errors": [], "status": "PENDING_RUN"},
    }
    ...
    md_placeholders = {
        "reports/audit_lite/v9_12_command_results.md": "# Commandes V9.12\n\nRapport en attente de capture finale.\n",
        "reports/audit_lite/zip_audit_v9_12.md": "# Audit ZIP V9.12\n\nRapport en attente d'execution.\n",
        "reports/audit_lite/zip_smoke_v9_12.md": "# Smoke ZIP V9.12\n\nRapport en attente d'execution.\n",
    }
    for raw_path, text in md_placeholders.items():
        path = Path(raw_path)
        if not (ROOT / path).exists():
            _write_text(path, text)


def _write_samples(report: dict[str, Any]) -> None:
    for timeframe, output in report.get("outputs", {}).items():
        source = ROOT / output.get("path", "")
        if not source.is_file():
            continue
        sample_dir = ROOT / "data/audit_lite/v9_12/labels" / f"timeframe={timeframe}"
        sample_dir.mkdir(parents=True, exist_ok=True)
        sample = pd.read_parquet(source, engine="pyarrow").head(200)
        sample.to_parquet(sample_dir / "labels_sample.parquet", index=False, engine="pyarrow")


def _write_attestation(report: dict[str, Any], zip_bytes: int | None) -> None:
    command_results = _read_optional_json(Path("reports/audit_lite/v9_12_command_results.json"))
    ...
    payload = {
        ...
        "validator_passed": _commands_passed(commands, ["validate_horizon_event_label_redesign_v9_12.py"]),
        "audit_lite_passed": _commands_passed(commands, ["audit_audit_lite_zip_v9_12.py"]),
        "smoke_audit_lite_passed": _commands_passed(commands, ["smoke_audit_lite_zip_v9_12.py"]),
        "v9_12_decision": report["v9_12_decision"]["decision"],
        "recommended_candidate": report["recommended_candidate"]["target_name"],
        **report["safety_flags"],
    }
    _write_json(Path("reports/audit_lite/v9_12_full_local_validation_attestation.json"), payload)
    _write_text(
        Path("reports/audit_lite/v9_12_full_local_validation_attestation.md"),
        "# Attestation full locale V9.12\n\n"
        f"- Decision : `{report['v9_12_decision']['decision']}`.\n"
...
def _collect_paths() -> list[Path]:
    sample_paths = sorted(Path("data/audit_lite/v9_12").rglob("*.parquet")) if (ROOT / "data/audit_lite/v9_12").exists() else []
...
def _write_inventory(paths: list[Path], zip_bytes: int | None) -> None:
    ...
    _write_json(Path("reports/audit_lite/v9_12_artifact_inventory.json"), payload)
    _write_text(
        Path("reports/audit_lite/v9_12_artifact_inventory.md"),
        "# Inventaire audit-lite V9.12\n\n"
        f"- ZIP : `{ZIP_NAME}`.\n"
...
def _write_size_report(paths: list[Path], zip_bytes: int | None) -> None:
    ...
    _write_json(Path("reports/audit_lite/zip_size_report_v9_12.json"), payload)
    _write_text(Path("reports/audit_lite/zip_size_report_v9_12.md"), f"# Taille ZIP V9.12\n\n- ZIP : `{ZIP_NAME}`.\n- Taille bytes : `{zip_bytes}`.\n- Fichiers inclus : `{len(paths)}`.\n")


def _write_state_surfaces(report: dict[str, Any]) -> None:
    metrics = {
        "last_validated_version": LAST_VALIDATED_VERSION,
        "candidate_version": VERSION,
        "candidate_status": "pending_external_audit",
        "direction": "horizon_extension_event_based_label_redesign",
        "v9_12_decision": report["v9_12_decision"]["decision"],
        "recommended_candidate": report["recommended_candidate"]["target_name"],
        **report["safety_flags"],
    }
