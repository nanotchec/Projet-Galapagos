...
VERSION = "V9.47"
ZIP_NAME = "projet-galapagos-v9.47-audit-lite.zip"
ROOT = Path(".").resolve()
...
CORE_PATHS = [
    ...
    Path("src/galapagos/features/ohlcv_aggtrades_exact_5y_feature_store_v9_47_validation.py"),
    ...
    Path("scripts/run_ohlcv_aggtrades_exact_5y_feature_store_v9_47.py"),
    Path("scripts/validate_ohlcv_aggtrades_exact_5y_feature_store_v9_47.py"),
    Path("scripts/release_audit_lite_zip_v9_47.py"),
    Path("scripts/audit_audit_lite_zip_v9_47.py"),
    Path("scripts/smoke_audit_lite_zip_v9_47.py"),
    Path("tests/features/test_ohlcv_aggtrades_exact_5y_feature_store_v9_47.py"),
    Path("tests/validation/test_ohlcv_aggtrades_exact_5y_feature_store_v9_47_validator.py"),
    ...
    Path("docs/ohlcv_aggtrades_exact_5y_feature_store_v9_47.md"),
]
PRIOR_REPORTS = [
    ...
    Path("reports/features/ohlcv_aggtrades_5y_feature_store_validation_v9_38.md"),
...
AUDIT_PATHS = [
    Path("reports/audit_lite/v9_47_command_results.json"),
    Path("reports/audit_lite/v9_47_command_results.md"),
    Path("reports/audit_lite/v9_47_full_local_validation_attestation.json"),
    Path("reports/audit_lite/v9_47_full_local_validation_attestation.md"),
    Path("reports/audit_lite/v9_47_artifact_inventory.json"),
    Path("reports/audit_lite/v9_47_artifact_inventory.md"),
    Path("reports/audit_lite/zip_size_report_v9_47.json"),
    Path("reports/audit_lite/zip_size_report_v9_47.md"),
    Path("reports/audit_lite/zip_audit_v9_47.json"),
    Path("reports/audit_lite/zip_audit_v9_47.md"),
    Path("reports/audit_lite/zip_smoke_v9_47.json"),
    Path("reports/audit_lite/zip_smoke_v9_47.md"),
]
...
def main() -> int:
    report = _read_json(Path("reports/features/ohlcv_aggtrades_exact_5y_feature_store_v9_47.json"))
...
def _ensure_placeholders() -> None:
    placeholders = {
        "reports/audit_lite/v9_47_command_results.json": {"version": VERSION, "status": "PENDING_CAPTURE", "commands": [], "sidecars_created": False, "zip_fingerprints_created": False},
        "reports/audit_lite/zip_audit_v9_47.json": {"version": VERSION, "passed": False, "errors": [], "status": "PENDING_RUN"},
        "reports/audit_lite/zip_smoke_v9_47.json": {"version": VERSION, "passed": False, "errors": [], "status": "PENDING_RUN"},
    }
    texts = {
        "reports/audit_lite/v9_47_command_results.md": "# Commandes V9.47\n\nEn attente.\n",
        "reports/audit_lite/zip_audit_v9_47.md": "# Audit ZIP V9.47\n\nEn attente.\n",
        "reports/audit_lite/zip_smoke_v9_47.md": "# Smoke ZIP V9.47\n\nEn attente.\n",
    }
...
def _collect_paths() -> list[Path]:
    ...
    if missing:
        raise FileNotFoundError(f"missing V9.47 release inputs: {missing}")
...
def _write_attestation(report: dict[str, Any], zip_size: int | None) -> None:
    commands = _read_optional_json(Path("reports/audit_lite/v9_47_command_results.json")).get("commands", [])
    payload = {
        "version": VERSION,
        ...
        "feature_store_combined_created": report["feature_store_created"],
    ...
    _write_json(Path("reports/audit_lite/v9_47_full_local_validation_attestation.json"), payload)
    _write_text(Path("reports/audit_lite/v9_47_full_local_validation_attestation.md"), "# Attestation V9.47\n\n" f"- Decision : `{report['decision']}`.\n" f"- Qualite : `{report['quality_status']}`.\n" f"- Coverage : `{report['coverage_status']}`.\n" "- Feature-store-only combine, sans label, dataset supervise, ML, modele persistant, backtest ou walk-forward.\n" "- Aucun trading, paper live, ordre, strategie, signal actionnable, API privee, cle API, reseau ou telechargement.\n" "- Aucune suppression destructive, aucun sidecar et aucune empreinte ZIP.\n")
...
def _write_inventory(paths: list[Path], zip_size: int | None) -> None:
    ...
    _write_json(Path("reports/audit_lite/v9_47_artifact_inventory.json"), payload)
    _write_text(Path("reports/audit_lite/v9_47_artifact_inventory.md"), "# Inventaire audit-lite V9.47\n\n" f"- Fichiers inclus : `{len(paths)}`.\n" f"- ZIP : `{ZIP_NAME}`.\n")
...
def _write_size_report(paths: list[Path], zip_size: int | None) -> None:
    ...
    _write_json(Path("reports/audit_lite/zip_size_report_v9_47.json"), payload)
    _write_text(Path("reports/audit_lite/zip_size_report_v9_47.md"), "# Taille ZIP V9.47\n\n" f"- Fichiers : `{len(paths)}`.\n" f"- ZIP bytes estimate : `{zip_size}`.\n")
