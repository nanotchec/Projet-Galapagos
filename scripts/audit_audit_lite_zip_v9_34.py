...
REQUIRED_FILES = [
    ...
    "scripts/run_ohlcv_5y_extension_v9_34.py",
    "scripts/validate_ohlcv_5y_extension_v9_34.py",
    "scripts/release_audit_lite_zip_v9_34.py",
    "scripts/audit_audit_lite_zip_v9_34.py",
    "scripts/smoke_audit_lite_zip_v9_34.py",
    "tests/data/test_ohlcv_5y_extension_v9_34.py",
    "tests/validation/test_ohlcv_5y_extension_v9_34_validator.py",
    ...
    "docs/ohlcv_5y_extension_v9_34.md",
    "reports/audit_lite/v9_34_command_results.json",
    "reports/audit_lite/v9_34_command_results.md",
    "reports/audit_lite/v9_34_full_local_validation_attestation.json",
    "reports/audit_lite/v9_34_full_local_validation_attestation.md",
    "reports/audit_lite/v9_34_artifact_inventory.json",
    "reports/audit_lite/v9_34_artifact_inventory.md",
    "reports/audit_lite/zip_size_report_v9_34.json",
    "reports/audit_lite/zip_size_report_v9_34.md",
    "reports/audit_lite/zip_audit_v9_34.json",
    "reports/audit_lite/zip_audit_v9_34.md",
    "reports/audit_lite/zip_smoke_v9_34.json",
    "reports/audit_lite/zip_smoke_v9_34.md",
...
def audit_zip(zip_path: Path) -> list[str]:
    ...
    errors: list[str] = []
    with tempfile.TemporaryDirectory(prefix="galapagos_v9_34_audit_") as tmp:
...
def _check_inventory(extract_root: Path, names: list[str]) -> list[str]:
    errors: list[str] = []
    inventory = _read_json(extract_root / "reports/audit_lite/v9_34_artifact_inventory.json")
    size_report = _read_json(extract_root / "reports/audit_lite/zip_size_report_v9_34.json")
...
def _check_reports(extract_root: Path) -> list[str]:
    errors: list[str] = []
    report = _read_json(extract_root / "reports/data/ohlcv_5y_extension_v9_34.json")
    manifest = _read_json(extract_root / "reports/manifests/ohlcv_5y_extension_v9_34_manifest.json")
    attestation = _read_json(extract_root / "reports/audit_lite/v9_34_full_local_validation_attestation.json")
    if report.get("version") != VERSION or report.get("source_version") != "V9.33":
        errors.append("V9.34 report version/source mismatch")
    if manifest.get("decision") != report.get("decision") or manifest.get("safety_flags") != report.get("safety_flags"):
        errors.append("V9.34 manifest mismatch")
    if report.get("labels_created") is not False or report.get("dataset_created") is not False or report.get("ml_executed") is not False:
        errors.append("V9.34 must not create labels, dataset or ML")
...
def _check_flags(report: dict[str, Any], attestation: dict[str, Any]) -> list[str]:
    errors: list[str] = []
    for key in ["no_trading", "no_paper_live", "no_orders", "no_backtest", "no_walk_forward", "no_ml", "no_dataset_supervised", "no_labels", "no_strategy", "no_actionable_signal", "no_persistent_model", "no_destructive_cleanup", "no_sidecars", "no_zip_fingerprints"]:
...
def _check_command_results(extract_root: Path) -> list[str]:
    payload = _read_json(extract_root / "reports/audit_lite/v9_34_command_results.json")
    ...
    report = _read_json(extract_root / "reports/data/ohlcv_5y_extension_v9_34.json")
    for needle in ["pytest --collect-only -q", "test_ohlcv_5y_extension_v9_34.py", "test_ohlcv_5y_extension_v9_34_validator.py", "validate_ohlcv_5y_extension_v9_34.py", "release_audit_lite_zip_v9_34.py"]:
    ...
    run_results = [item for item in commands if "run_ohlcv_5y_extension_v9_34.py" in item.get("command", "")]
    if not run_results:
        errors.append("missing command result for run_ohlcv_5y_extension_v9_34.py")
        ...
        if not any(item.get("returncode") in {0, 1} for item in run_results):
            errors.append("failed V9.34 run command must be captured with returncode 0 or 1")
    elif not any(item.get("returncode") == 0 for item in run_results):
        errors.append("missing successful command result for run_ohlcv_5y_extension_v9_34.py")
    if payload.get("sidecars_created") is not False or payload.get("zip_fingerprints_created") is not False:
...
def _check_state(extract_root: Path) -> list[str]:
    ...
    if metrics.get("direction") != "ohlcv_5y_extension":
...
def _write_reports(result: dict[str, Any]) -> None:
    ...
    (report_dir / "zip_audit_v9_34.json").write_text(json.dumps(result, indent=2, ensure_ascii=False) + "\n", encoding="utf-8")
    (report_dir / "zip_audit_v9_34.md").write_text("# Audit ZIP V9.34\n\n" f"- Resultat : `{'PASS' if result['passed'] else 'FAIL'}`.\n" f"- Erreurs : `{result['errors']}`.\n" "- Aucun sidecar et aucune empreinte ZIP.\n", encoding="utf-8")
