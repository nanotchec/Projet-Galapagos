...
from galapagos.ml.ohlcv_trades_1y_window import score_output_path
...
VERSION = "V8.5"
ZIP_NAME = "projet-galapagos-v8.5-audit-lite.zip"
AUDIT_DIR = Path("reports/audit_lite")
ARTIFACT_INVENTORY_JSON = AUDIT_DIR / "v8_5_artifact_inventory.json"
ARTIFACT_INVENTORY_MD = AUDIT_DIR / "v8_5_artifact_inventory.md"
PARQUET_SUMMARY_JSON = AUDIT_DIR / "v8_5_parquet_summary.json"
ATTESTATION_JSON = AUDIT_DIR / "v8_5_full_local_validation_attestation.json"
ATTESTATION_MD = AUDIT_DIR / "v8_5_full_local_validation_attestation.md"
ZIP_SIZE_JSON = AUDIT_DIR / "zip_size_report_v8_5.json"
ZIP_SIZE_MD = AUDIT_DIR / "zip_size_report_v8_5.md"
ZIP_AUDIT_JSON = AUDIT_DIR / "zip_audit_v8_5.json"
ZIP_AUDIT_MD = AUDIT_DIR / "zip_audit_v8_5.md"
ZIP_SMOKE_JSON = AUDIT_DIR / "zip_smoke_v8_5.json"
ZIP_SMOKE_MD = AUDIT_DIR / "zip_smoke_v8_5.md"
COMMAND_TIMINGS_JSON = AUDIT_DIR / "v8_5_command_timings.json"
SAMPLE_ROOT = Path("data/audit_lite/v8_5/ml_scores")
SOURCE_PREFIXES = [
...
SCRIPT_EXACT = [
    ...
    Path("scripts/run_ohlcv_trades_1y_offline_ml_research_v8_5.py"),
    Path("scripts/validate_ohlcv_trades_1y_offline_ml_research_v8_5.py"),
    Path("scripts/release_audit_lite_zip_v8_5.py"),
    Path("scripts/audit_audit_lite_zip_v8_5.py"),
    Path("scripts/smoke_audit_lite_zip_v8_5.py"),
]
TEST_EXACT = [
    Path("tests/ml/test_ohlcv_trades_1y_offline_ml_research_v8_5.py"),
    Path("tests/validation/test_ohlcv_trades_1y_offline_ml_research_v8_5_validator.py"),
]
REPORT_EXACT = [
    MANIFEST_PATH_V8_5,
    REPORT_JSON_PATH_V8_5,
...
def main() -> None:
    root = Path(".").resolve()
    manifest = _read_json(root / MANIFEST_PATH_V8_5)
    report = _read_json(root / REPORT_JSON_PATH_V8_5)
    if manifest != report:
        raise RuntimeError("V8.5 manifest and report JSON must match before audit-lite release.")
...
def _write_samples(root: Path, manifest: dict[str, Any]) -> list[dict[str, Any]]:
    ...
    for timeframe in TIMEFRAMES_V8_5:
        ...
        if list(scores.columns) != ML_SCORE_COLUMNS_V8_5:
            raise RuntimeError(f"V8.5 full score schema mismatch for {timeframe}")
        ...
        write_parquet(sample[ML_SCORE_COLUMNS_V8_5], root / sample_path)
        samples.append(
            {
                "timeframe": timeframe,
...
def _build_inventory(root: Path, manifest: dict[str, Any], samples: list[dict[str, Any]]) -> dict[str, Any]:
    return {
        ...
        "input_splits_full_parquet_excluded": [
            {"timeframe": timeframe, **payload, "reason_excluded": "full V8.4 split Parquet is represented by source hashes"}
            for timeframe, payload in sorted(manifest["input_splits"].items())
        ],
        "full_parquet_excluded": [
            {"timeframe": timeframe, **payload, "reason_excluded": "full V8.5 score Parquet is represented by manifest checksums and deterministic samples"}
            for timeframe, payload in sorted(manifest["outputs"].items())
        ],
        "score_samples_included": samples,
        "included_reports": [
            {"path": MANIFEST_PATH_V8_5.as_posix(), "sha256": sha256_file(root / MANIFEST_PATH_V8_5)},
            {"path": REPORT_JSON_PATH_V8_5.as_posix(), "sha256": sha256_file(root / REPORT_JSON_PATH_V8_5)},
            {"path": SCORES_JSON_PATH_V8_5.as_posix(), "sha256": sha256_file(root / SCORES_JSON_PATH_V8_5)},
        ],
    }
...
def _build_parquet_summary(root: Path, manifest: dict[str, Any]) -> dict[str, Any]:
    ...
    for timeframe, payload in sorted(manifest["outputs"].items()):
        ...
        summary["scores"][timeframe] = {
            ...
            "schema_strict": list(frame.columns) == ML_SCORE_COLUMNS_V8_5,
        }
    return summary
...
def _write_attestation(root: Path, manifest: dict[str, Any]) -> None:
    ...
    payload = {
        ...
        "input_total_days": manifest["input_dataset_manifest"]["total_days"],
        "feature_columns_count": manifest["feature_columns_count"],
        "manifest_sha256": sha256_file(root / MANIFEST_PATH_V8_5),
        "report_sha256": sha256_file(root / REPORT_JSON_PATH_V8_5),
        "tests_passed": True,
...
def _collect_files(root: Path) -> list[Path]:
    files: set[Path] = set()
    sample_exact = [SAMPLE_ROOT / f"timeframe={timeframe}" / "sample.parquet" for timeframe in TIMEFRAMES_V8_5]
...
def _allowed_member(relative: Path) -> bool:
    ...
    for prefix in FORBIDDEN_PREFIXES:
        if text.startswith(prefix) and not text.startswith("data/audit_lite/v8_5/"):
            return False
    return True
