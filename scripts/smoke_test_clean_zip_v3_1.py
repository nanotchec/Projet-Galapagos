...
def main() -> None:
    ...
    errors: list[str] = []
    validators_run = 0
    with tempfile.TemporaryDirectory(prefix="galapagos_v3_1_smoke_") as tmp:
        tmp_path = Path(tmp)
        ...
        try:
            ...
            for flag in ["trading_enabled", "backtest_enabled", "orders_enabled", "strategy_enabled", "execution_enabled", "dataset_enabled", "ml_enabled"]:
                if safety.get(flag) is not False:
                    errors.append(f"unsafe V3.1 flag active: {flag}")
    ...
    Path("reports/zip_smoke_test_v3_1.json").write_text(json.dumps(payload, indent=2, ensure_ascii=False) + "\n", encoding="utf-8")
    Path("reports/zip_smoke_test_v3_1.md").write_text(
        "# Smoke ZIP V3.1\n\n"
