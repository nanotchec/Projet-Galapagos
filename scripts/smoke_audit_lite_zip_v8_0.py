...
VERSION = "V8.0"
SMOKE_JSON = Path("reports/audit_lite/zip_smoke_v8_0.json")
SMOKE_MD = Path("reports/audit_lite/zip_smoke_v8_0.md")
...
def smoke_zip(zip_path: Path) -> dict[str, Any]:
    ...
    with tempfile.TemporaryDirectory(prefix="galapagos_v8_0_smoke_") as tmp:
        target = Path(tmp)
...
def _check_required_files(root: Path) -> list[str]:
    required = [
        "reports/manifests/ohlcv_trades_90d_offline_ml_research_v8_0_manifest.json",
        "reports/manifests/ohlcv_trades_90d_ml_robustness_v8_0_manifest.json",
        "reports/ml/ohlcv_trades_90d_offline_ml_research_v8_0.json",
        "reports/ml/ohlcv_trades_90d_ml_robustness_v8_0.json",
        "reports/audit_lite/v8_0_full_local_validation_attestation.json",
        "docs/ohlcv_trades_90d_offline_ml_research_v8_0.md",
        "docs/ohlcv_trades_90d_ml_robustness_v8_0.md",
        "scripts/release_audit_lite_zip_v8_0.py",
        "scripts/audit_audit_lite_zip_v8_0.py",
        "scripts/smoke_audit_lite_zip_v8_0.py",
        "tests/ml/test_ohlcv_trades_90d_offline_ml_research_v8_0.py",
        "tests/ml/test_ohlcv_trades_90d_ml_robustness_v8_0.py",
        "tests/validation/test_ohlcv_trades_90d_offline_ml_research_v8_0_validator.py",
        "tests/validation/test_ohlcv_trades_90d_ml_robustness_v8_0_validator.py",
    ]
...
def _check_imports(root: Path) -> list[str]:
    ...
    errors: list[str] = []
    for module in [
        "galapagos.ml.ohlcv_trades_90d_window",
        "galapagos.ml.ohlcv_trades_90d_window_metrics",
        "galapagos.ml.ohlcv_trades_90d_window_quality",
        "galapagos.ml.ohlcv_trades_90d_window_validation",
        "galapagos.ml.ohlcv_trades_90d_robustness",
        "galapagos.ml.ohlcv_trades_90d_robustness_validation",
...
def _check_samples(root: Path) -> list[str]:
    ...
    expected_columns = schema_module.ML_SCORE_COLUMNS_V8_0
    forbidden = {"signal", "trading_signal", "order", "strategy", "pnl", "profit"}
    for timeframe in ["1m", "5m", "15m", "1h"]:
        path = root / "data" / "audit_lite" / "v8_0" / "ml_scores" / f"timeframe={timeframe}" / "sample.parquet"
        if not path.exists():
...
def _check_reports(root: Path) -> list[str]:
    errors: list[str] = []
    manifest_path = root / "reports/manifests/ohlcv_trades_90d_offline_ml_research_v8_0_manifest.json"
    report_path = root / "reports/ml/ohlcv_trades_90d_offline_ml_research_v8_0.json"
    robustness_manifest_path = root / "reports/manifests/ohlcv_trades_90d_ml_robustness_v8_0_manifest.json"
    robustness_report_path = root / "reports/ml/ohlcv_trades_90d_ml_robustness_v8_0.json"
    ...
    if robustness_manifest_path.exists() and robustness_report_path.exists():
        ...
        if robustness_manifest != robustness_report:
            errors.append("smoke robustness manifest/report mismatch")
        findings = robustness_manifest.get("findings", {})
        for key in [
            "robust_edge_claimed",
            "strategy_validated",
            "backtest_performed",
            ...
            if findings.get(key) is not False:
                errors.append(f"smoke robustness finding must be false: {key}")
    return errors
