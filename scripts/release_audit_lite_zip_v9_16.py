...
REPORT_PATHS = [
    ...
    Path("reports/research_decisions/derivatives_window_extension_v9_16.md"),
    Path("docs/derivatives_window_extension_v9_16.md"),
]

AUDIT_PATHS = [
    Path("reports/audit_lite/v9_16_command_results.json"),
    Path("reports/audit_lite/v9_16_command_results.md"),
    Path("reports/audit_lite/v9_16_full_local_validation_attestation.json"),
    Path("reports/audit_lite/v9_16_full_local_validation_attestation.md"),
    Path("reports/audit_lite/v9_16_artifact_inventory.json"),
    Path("reports/audit_lite/v9_16_artifact_inventory.md"),
    Path("reports/audit_lite/zip_size_report_v9_16.json"),
    Path("reports/audit_lite/zip_size_report_v9_16.md"),
    Path("reports/audit_lite/zip_audit_v9_16.json"),
    Path("reports/audit_lite/zip_audit_v9_16.md"),
    Path("reports/audit_lite/zip_smoke_v9_16.json"),
    Path("reports/audit_lite/zip_smoke_v9_16.md"),
]
...
SCRIPT_PATHS = [
    ...
    Path("scripts/run_derivatives_window_extension_v9_16.py"),
    Path("scripts/validate_derivatives_window_extension_v9_16.py"),
    Path("scripts/release_audit_lite_zip_v9_16.py"),
    Path("scripts/audit_audit_lite_zip_v9_16.py"),
    Path("scripts/smoke_audit_lite_zip_v9_16.py"),
]

TEST_PATHS = [
    Path("tests/research/test_derivatives_window_extension_v9_16.py"),
    Path("tests/validation/test_derivatives_window_extension_v9_16_validator.py"),
]

CODE_PATHS = [
    Path("src/galapagos/__init__.py"),
    Path("src/galapagos/research/derivatives_window_extension_v9_16.py"),
    Path("src/galapagos/research/derivatives_window_extension_v9_16_validation.py"),
]
...
def main() -> int:
    _ensure_inputs()
    report = _read_json(Path("reports/research_decisions/derivatives_window_extension_v9_16.json"))
...
def _ensure_inputs() -> None:
    ...
    if missing:
        raise FileNotFoundError(f"missing V9.16 audit-lite inputs: {missing}")


def _ensure_placeholders() -> None:
    placeholders = {
        "reports/audit_lite/v9_16_command_results.json": {"version": VERSION, "status": "PENDING_CAPTURE", "commands": [], "sidecars_created": False, "zip_fingerprints_created": False},
        "reports/audit_lite/zip_audit_v9_16.json": {"version": VERSION, "passed": False, "errors": [], "status": "PENDING_RUN"},
        "reports/audit_lite/zip_smoke_v9_16.json": {"version": VERSION, "passed": False, "errors": [], "status": "PENDING_RUN"},
    }
    ...
    for raw_path, text in {
        "reports/audit_lite/v9_16_command_results.md": "# Commandes V9.16\n\nRapport en attente de capture finale.\n",
        "reports/audit_lite/zip_audit_v9_16.md": "# Audit ZIP V9.16\n\nRapport en attente d'execution.\n",
        "reports/audit_lite/zip_smoke_v9_16.md": "# Smoke ZIP V9.16\n\nRapport en attente d'execution.\n",
    }.items():
        path = Path(raw_path)
        if not (ROOT / path).exists():
...
def _write_attestation(report: dict[str, Any], zip_bytes_estimate: int | None) -> None:
    command_results = _read_optional_json(Path("reports/audit_lite/v9_16_command_results.json"))
    ...
    payload = {
        ...
        "validator_passed": _commands_passed(commands, ["validate_derivatives_window_extension_v9_16.py"]),
        "audit_lite_passed": _commands_passed(commands, ["audit_audit_lite_zip_v9_16.py"]),
        "smoke_audit_lite_passed": _commands_passed(commands, ["smoke_audit_lite_zip_v9_16.py"]),
    ...
    _write_json(Path("reports/audit_lite/v9_16_full_local_validation_attestation.json"), payload)
    _write_text(
        Path("reports/audit_lite/v9_16_full_local_validation_attestation.md"),
        "# Attestation full locale V9.16\n\n"
        f"- Decision : `{report['v9_16_decision']['decision']}`.\n"
        f"- Features creees : `{report['features_created']}`.\n"
...
def _write_inventory(paths: list[Path], zip_bytes_estimate: int | None) -> None:
    ...
    _write_json(Path("reports/audit_lite/v9_16_artifact_inventory.json"), payload)
    _write_text(Path("reports/audit_lite/v9_16_artifact_inventory.md"), f"# Inventaire audit-lite V9.16\n\n- ZIP : `{ZIP_NAME}`.\n- Fichiers inclus : `{len(paths)}`.\n- Taille ZIP estimee non autoritative : `{zip_bytes_estimate}`.\n- Exclusions : data/research, data/silver, data/gold, backtests, strategies, ordres, execution, modeles, caches, secrets, `Icon`, sidecars SHA256 et empreintes ZIP.\n")
...
def _write_size_report(paths: list[Path], zip_bytes_estimate: int | None) -> None:
    ...
    _write_json(Path("reports/audit_lite/zip_size_report_v9_16.json"), payload)
    _write_text(Path("reports/audit_lite/zip_size_report_v9_16.md"), f"# Taille ZIP V9.16\n\n- ZIP : `{ZIP_NAME}`.\n- Taille bytes estimee non autoritative : `{zip_bytes_estimate}`.\n- Fichiers inclus : `{len(paths)}`.\n")
