...
VERSION = "V6.4"
REPORT_JSON = Path("reports/audit_lite/zip_audit_v6_4.json")
REPORT_MD = Path("reports/audit_lite/zip_audit_v6_4.md")
REQUIRED_FILES = {
    "README.md",
    "pyproject.toml",
    ...
    "reports/research_decisions/v6_4_research_decision_gate.json",
    "reports/research_decisions/v6_4_research_decision_gate.md",
    "docs/research_decision_gate_v6_4.md",
    "reports/audit_lite/v6_4_artifact_inventory.json",
    "reports/audit_lite/v6_4_artifact_inventory.md",
    "reports/audit_lite/zip_size_report_v6_4.json",
    "reports/audit_lite/zip_size_report_v6_4.md",
    "reports/audit_lite/v6_4_full_local_validation_attestation.json",
    "reports/audit_lite/v6_4_full_local_validation_attestation.md",
    "reports/PROJECT_STATE.json",
    "reports/PROJECT_STATE.md",
    "reports/current/latest_metrics.json",
    "reports/current/latest_metrics.md",
    "reports/current/latest_summary.md",
    "scripts/validate_research_decision_gate_v6_4.py",
    "scripts/release_audit_lite_zip_v6_4.py",
    "scripts/audit_audit_lite_zip_v6_4.py",
    "scripts/smoke_audit_lite_zip_v6_4.py",
    "tests/validation/test_research_decision_gate_v6_4.py",
}
...
def audit_zip(zip_path: Path) -> dict[str, Any]:
    ...
    with tempfile.TemporaryDirectory(prefix="galapagos-v6-4-audit-") as tmp:
        extract_root = Path(tmp)
        with zipfile.ZipFile(zip_path) as archive:
            archive.extractall(extract_root)
        report = _read_json(extract_root / "reports/research_decisions/v6_4_research_decision_gate.json")
        attestation = _read_json(extract_root / "reports/audit_lite/v6_4_full_local_validation_attestation.json")
        if report.get("version") != VERSION or report.get("status") != "PASS":
            errors.append("V6.4 decision report version/status mismatch in ZIP")
        if report.get("decision_gate_type") != "research_only":
            errors.append("V6.4 decision gate must be research_only")
        if report.get("recommended_next_step") != "A. Ajouter les trades publics historiques.":
            errors.append("V6.4 recommended_next_step must be public trades historical ingestion")
        roadmap = report.get("roadmap", [])
        if not roadmap or roadmap[0] != "V7.0 — Public Trades Historical Ingestion":
            errors.append("V6.4 roadmap must start with Public Trades Historical Ingestion")
        ...
        for flag in ["validator_passed", "tests_passed", "audit_lite_passed", "smoke_audit_lite_passed", "no_trading", "no_backtest", "no_orders", "no_strategy"]:
            if attestation.get(flag) is not True:
                errors.append(f"V6.4 attestation flag must be true: {flag}")
...
def _render_markdown(result: dict[str, Any]) -> str:
    ...
    return f"""# Audit ZIP audit-lite V6.4
