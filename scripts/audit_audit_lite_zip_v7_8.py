...
from galapagos.features.ohlcv_trades_90d import TIMEFRAMES_V7_8
from galapagos.features.ohlcv_trades_90d_schemas import OHLCV_TRADES_FEATURE_COLUMNS_V7_8


VERSION = "V7.8"
AUDIT_JSON = Path("reports/audit_lite/zip_audit_v7_8.json")
AUDIT_MD = Path("reports/audit_lite/zip_audit_v7_8.md")
REQUIRED_SOURCE_PREFIXES = ["src/galapagos/features/"]
REQUIRED_ENTRIES = {
    "README.md",
    "pyproject.toml",
    "reports/manifests/ohlcv_trades_90d_feature_store_v7_8_manifest.json",
    "reports/features/ohlcv_trades_90d_feature_store_v7_8.json",
    "reports/features/ohlcv_trades_90d_feature_store_v7_8.md",
    "docs/ohlcv_trades_90d_feature_store_v7_8.md",
    "reports/audit_lite/v7_8_artifact_inventory.json",
    "reports/audit_lite/v7_8_artifact_inventory.md",
    "reports/audit_lite/v7_8_parquet_summary.json",
    "reports/audit_lite/v7_8_full_local_validation_attestation.json",
    "reports/audit_lite/v7_8_full_local_validation_attestation.md",
    "reports/audit_lite/zip_size_report_v7_8.json",
    "reports/audit_lite/zip_size_report_v7_8.md",
    "reports/PROJECT_STATE.json",
    "reports/PROJECT_STATE.md",
    "reports/current/latest_metrics.json",
    "reports/current/latest_metrics.md",
    "reports/current/latest_summary.md",
    "scripts/run_ohlcv_trades_90d_feature_store_v7_8.py",
    "scripts/validate_ohlcv_trades_90d_feature_store_v7_8.py",
    "scripts/release_audit_lite_zip_v7_8.py",
    "scripts/audit_audit_lite_zip_v7_8.py",
    "scripts/smoke_audit_lite_zip_v7_8.py",
    "tests/features/test_ohlcv_trades_90d_features_v7_8.py",
    "tests/validation/test_ohlcv_trades_90d_feature_store_v7_8_validator.py",
}
SAMPLE_ENTRIES = {
    f"data/audit_lite/v7_8/features/timeframe={timeframe}/sample.parquet" for timeframe in TIMEFRAMES_V7_8
}
FORBIDDEN_PREFIXES = (
    "data/raw/",
    "data/research/",
...
def audit_zip(zip_path: Path) -> dict[str, Any]:
    ...
    with tempfile.TemporaryDirectory(prefix="galapagos-v7-8-audit-") as tmp:
        root = Path(tmp)
        with zipfile.ZipFile(zip_path) as archive:
            archive.extractall(root)
        manifest = _read_json(root / "reports/manifests/ohlcv_trades_90d_feature_store_v7_8_manifest.json")
        report = _read_json(root / "reports/features/ohlcv_trades_90d_feature_store_v7_8.json")
        if manifest != report:
            errors.append("V7.8 manifest/report mismatch in ZIP")
        if manifest.get("version") != VERSION or manifest.get("status") != "PASS":
            errors.append("V7.8 manifest version/status mismatch in ZIP")
        ...
        for flag in ["trading_enabled", "orders_enabled", "paper_live_enabled", "backtest_enabled", "strategy_enabled", "execution_enabled", "ml_enabled", "labels_enabled", "dataset_enabled"]:
            if safety.get(flag) is not False:
                errors.append(f"V7.8 safety flag must be false: {flag}")
