from __future__ import annotations

import json
from pathlib import Path

from _bootstrap import bootstrap_src_path

bootstrap_src_path()

from galapagos.datasets.refined_ohlcv_trades_window import run_refined_ohlcv_trades_offline_supervised_dataset_v9_1


def main() -> int:
    manifest = run_refined_ohlcv_trades_offline_supervised_dataset_v9_1(Path("."))
    print(
        json.dumps(
            {
                "version": manifest["version"],
                "status": manifest["status"],
                "feature_columns_count": manifest["feature_columns_count"],
                "outputs": {timeframe: payload["rows"] for timeframe, payload in manifest["outputs"].items()},
                "splits": {timeframe: payload["rows"] for timeframe, payload in manifest["splits"].items()},
                "no_backtest": not manifest["safety"]["backtest_enabled"],
                "no_strategy": not manifest["safety"]["strategy_enabled"],
                "no_orders": not manifest["safety"]["orders_enabled"],
                "no_trading": not manifest["safety"]["trading_enabled"],
            },
            indent=2,
            ensure_ascii=False,
        )
    )
    return 0 if manifest["status"] == "PASS" else 1


if __name__ == "__main__":
    raise SystemExit(main())
