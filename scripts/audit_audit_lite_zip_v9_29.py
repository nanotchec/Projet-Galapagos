...
VERSION = "V9.29"
ZIP_NAME = "projet-galapagos-v9.29-audit-lite.zip"
REQUIRED_FILES = [
    "scripts/_bootstrap.py",
    "scripts/run_aggtrades_post_v9_full_coverage_validation_v9_29.py",
    "scripts/validate_aggtrades_post_v9_full_coverage_validation_v9_29.py",
    "scripts/release_audit_lite_zip_v9_29.py",
    "scripts/audit_audit_lite_zip_v9_29.py",
    "scripts/smoke_audit_lite_zip_v9_29.py",
    "src/galapagos/data/aggtrades_post_v9_full_coverage_validation_v9_29.py",
    "src/galapagos/data/aggtrades_post_v9_full_coverage_validation_v9_29_validation.py",
    "tests/data/test_aggtrades_post_v9_full_coverage_validation_v9_29.py",
    "tests/validation/test_aggtrades_post_v9_full_coverage_validation_v9_29_validator.py",
    "reports/manifests/aggtrades_post_v9_full_coverage_validation_v9_29_manifest.json",
    "reports/data/aggtrades_post_v9_full_coverage_validation_v9_29.json",
    "reports/data/aggtrades_post_v9_full_coverage_validation_v9_29.md",
    "docs/aggtrades_post_v9_full_coverage_validation_v9_29.md",
    "reports/audit_lite/v9_29_command_results.json",
    "reports/audit_lite/v9_29_command_results.md",
    "reports/audit_lite/v9_29_full_local_validation_attestation.json",
    "reports/audit_lite/v9_29_full_local_validation_attestation.md",
    "reports/audit_lite/v9_29_artifact_inventory.json",
    "reports/audit_lite/v9_29_artifact_inventory.md",
    "reports/audit_lite/zip_size_report_v9_29.json",
    "reports/audit_lite/zip_size_report_v9_29.md",
    "reports/audit_lite/zip_audit_v9_29.json",
    "reports/audit_lite/zip_audit_v9_29.md",
    "reports/audit_lite/zip_smoke_v9_29.json",
    "reports/audit_lite/zip_smoke_v9_29.md",
    "reports/data/aggtrades_post_v9_bad_day_repair_v9_28.json",
    "reports/data/aggtrades_post_v9_final_tail_collection_v9_28.json",
...
def audit_zip(zip_path: Path) -> list[str]:
    ...
    errors: list[str] = []
    with tempfile.TemporaryDirectory(prefix="galapagos_v9_29_audit_") as tmp:
...
def _check_inventory(extract_root: Path, names: list[str]) -> list[str]:
    errors: list[str] = []
    inventory = _read_json(extract_root / "reports/audit_lite/v9_29_artifact_inventory.json")
    size_report = _read_json(extract_root / "reports/audit_lite/zip_size_report_v9_29.json")
...
def _check_reports(extract_root: Path) -> list[str]:
    errors: list[str] = []
    report = _read_json(extract_root / "reports/data/aggtrades_post_v9_full_coverage_validation_v9_29.json")
    manifest = _read_json(extract_root / "reports/manifests/aggtrades_post_v9_full_coverage_validation_v9_29_manifest.json")
    attestation = _read_json(extract_root / "reports/audit_lite/v9_29_full_local_validation_attestation.json")
    if report.get("version") != VERSION or report.get("source_version") != "V9.28":
        errors.append("V9.29 report version/source mismatch")
    ...
    if manifest.get("decision") != report.get("decision") or manifest.get("safety_flags") != report.get("safety_flags"):
        errors.append("V9.29 manifest mismatch")
    if report.get("decision", "").startswith("aggtrades_full_coverage_validated"):
        for key, expected in {
            ...
            if report.get(key) != expected or attestation.get(key) != expected:
                errors.append(f"V9.29 validated report mismatch for {key}")
        if report.get("quality_status") != "PASS":
            errors.append("V9.29 validated report requires quality_status=PASS")
        if report.get("quarantine_blocking") is True:
            errors.append("V9.29 validated report cannot have blocking quarantine")
...
def _check_flags(report: dict[str, Any], attestation: dict[str, Any]) -> list[str]:
    errors: list[str] = []
    for key in ["no_trading", "no_paper_live", "no_orders", "no_backtest", "no_walk_forward", "no_ml", "no_dataset_supervised", "no_strategy", "no_actionable_signal", "no_persistent_model", "no_data_deletion", "no_destructive_cleanup", "no_sidecars", "no_zip_fingerprints"]:
    ...
    for key in ["no_new_data_download", "no_ingestion_executed"]:
        if report.get("safety_flags", {}).get(key) is not True or attestation.get(key) is not True:
            errors.append(f"must confirm {key}=true")
    return errors
...
def _check_command_results(extract_root: Path) -> list[str]:
    payload = _read_json(extract_root / "reports/audit_lite/v9_29_command_results.json")
    ...
    for needle in [
        ...
        "test_aggtrades_post_v9_full_coverage_validation_v9_29.py",
        "test_aggtrades_post_v9_full_coverage_validation_v9_29_validator.py",
        "run_aggtrades_post_v9_full_coverage_validation_v9_29.py",
        "validate_aggtrades_post_v9_full_coverage_validation_v9_29.py",
        "release_audit_lite_zip_v9_29.py",
    ]:
...
def _check_state(extract_root: Path) -> list[str]:
    ...
    errors: list[str] = []
    if metrics.get("last_validated_version") != "V9.28":
    ...
    if metrics.get("network_used") is not False or metrics.get("no_new_data_download") is not True:
        errors.append("latest_metrics must keep V9.29 read-only/no-network flags")
    return errors
...
def _write_reports(result: dict[str, Any]) -> None:
    ...
    (report_dir / "zip_audit_v9_29.json").write_text(json.dumps(result, indent=2, ensure_ascii=False) + "\n", encoding="utf-8")
    (report_dir / "zip_audit_v9_29.md").write_text(
        "# Audit ZIP V9.29\n\n"
