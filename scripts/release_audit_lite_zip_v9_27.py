...
CORE_PATHS = [
    ...
    Path("src/galapagos/data/aggtrades_post_v9_storage_recheck_resume_v9_27_validation.py"),
    Path("scripts/run_aggtrades_post_v9_storage_recheck_resume_v9_27.py"),
    Path("scripts/validate_aggtrades_post_v9_storage_recheck_resume_v9_27.py"),
    Path("scripts/release_audit_lite_zip_v9_27.py"),
    Path("scripts/audit_audit_lite_zip_v9_27.py"),
    Path("scripts/smoke_audit_lite_zip_v9_27.py"),
    Path("tests/data/test_aggtrades_post_v9_storage_recheck_resume_v9_27.py"),
    Path("tests/validation/test_aggtrades_post_v9_storage_recheck_resume_v9_27_validator.py"),
    ...
    Path("docs/aggtrades_post_v9_storage_recheck_resume_v9_27.md"),
]
...
AUDIT_PATHS = [
    Path("reports/audit_lite/v9_27_command_results.json"),
    Path("reports/audit_lite/v9_27_command_results.md"),
    Path("reports/audit_lite/v9_27_full_local_validation_attestation.json"),
    Path("reports/audit_lite/v9_27_full_local_validation_attestation.md"),
    Path("reports/audit_lite/v9_27_artifact_inventory.json"),
    Path("reports/audit_lite/v9_27_artifact_inventory.md"),
    Path("reports/audit_lite/zip_size_report_v9_27.json"),
    Path("reports/audit_lite/zip_size_report_v9_27.md"),
    Path("reports/audit_lite/zip_audit_v9_27.json"),
    Path("reports/audit_lite/zip_audit_v9_27.md"),
    Path("reports/audit_lite/zip_smoke_v9_27.json"),
    Path("reports/audit_lite/zip_smoke_v9_27.md"),
]
...
def main() -> int:
    report = _read_json(Path("reports/data/aggtrades_post_v9_storage_recheck_resume_v9_27.json"))
...
def _ensure_placeholders() -> None:
    placeholders = {
        "reports/audit_lite/v9_27_command_results.json": {"version": VERSION, "status": "PENDING_CAPTURE", "commands": [], "sidecars_created": False, "zip_fingerprints_created": False},
        "reports/audit_lite/zip_audit_v9_27.json": {"version": VERSION, "passed": False, "errors": [], "status": "PENDING_RUN"},
        "reports/audit_lite/zip_smoke_v9_27.json": {"version": VERSION, "passed": False, "errors": [], "status": "PENDING_RUN"},
    }
    ...
    texts = {
        "reports/audit_lite/v9_27_command_results.md": "# Commandes V9.27\n\nEn attente.\n",
        "reports/audit_lite/zip_audit_v9_27.md": "# Audit ZIP V9.27\n\nEn attente.\n",
        "reports/audit_lite/zip_smoke_v9_27.md": "# Smoke ZIP V9.27\n\nEn attente.\n",
    }
...
def _collect_paths() -> list[Path]:
    batch_reports = sorted(Path("reports/data").glob("aggtrades_post_v9_storage_recheck_batch*_v9_27.json"))
    ...
    if missing:
        raise FileNotFoundError(f"missing V9.27 release inputs: {missing}")
...
def _write_attestation(report: dict[str, Any], zip_size: int | None) -> None:
    commands = _read_optional_json(Path("reports/audit_lite/v9_27_command_results.json")).get("commands", [])
    payload = {
        "version": VERSION,
        ...
        **report["storage_recheck_summary"],
    ...
    _write_json(Path("reports/audit_lite/v9_27_full_local_validation_attestation.json"), payload)
    _write_text(
        Path("reports/audit_lite/v9_27_full_local_validation_attestation.md"),
        "# Attestation V9.27\n\n"
        f"- Decision : `{report['decision']}`.\n"
        f"- Couverture locale : `{report['storage_recheck_summary']['local_file_coverage_start']}` -> `{report['storage_recheck_summary']['local_file_coverage_end']}`.\n"
...
def _write_inventory(paths: list[Path], zip_size: int | None) -> None:
    ...
    _write_json(Path("reports/audit_lite/v9_27_artifact_inventory.json"), payload)
    _write_text(Path("reports/audit_lite/v9_27_artifact_inventory.md"), f"# Inventaire V9.27\n\n- Fichiers inclus : `{len(paths)}`.\n- `zip_bytes_is_authoritative=false`.\n")
...
def _write_size_report(paths: list[Path], zip_size: int | None) -> None:
    ...
    _write_json(Path("reports/audit_lite/zip_size_report_v9_27.json"), payload)
    _write_text(Path("reports/audit_lite/zip_size_report_v9_27.md"), f"# Taille ZIP V9.27\n\n- ZIP bytes estimate : `{zip_size}`.\n- `zip_bytes_is_authoritative=false`.\n")
