...
VERSION = "V5.2"
ZIP_NAME = "projet-galapagos-v5.2-audit-lite.zip"
AUDIT_DIR = Path("reports/audit_lite")
ARTIFACT_INVENTORY_JSON = AUDIT_DIR / "v5_2_artifact_inventory.json"
ARTIFACT_INVENTORY_MD = AUDIT_DIR / "v5_2_artifact_inventory.md"
PARQUET_SUMMARY_JSON = AUDIT_DIR / "v5_2_parquet_summary.json"
ATTESTATION_JSON = AUDIT_DIR / "v5_2_full_local_validation_attestation.json"
ATTESTATION_MD = AUDIT_DIR / "v5_2_full_local_validation_attestation.md"
ZIP_SIZE_JSON = AUDIT_DIR / "zip_size_report_v5_2.json"
ZIP_SIZE_MD = AUDIT_DIR / "zip_size_report_v5_2.md"
SAMPLE_ROOT = Path("data/audit_lite/v5_2/labels")
...
def main() -> None:
    ...
    manifest = _read_json(root / MANIFEST_PATH_V5_2)
    report = _read_json(root / REPORT_JSON_PATH_V5_2)
    if manifest != report:
        raise RuntimeError("V5.2 manifest and report JSON must match before audit-lite release.")
...
def _write_samples(root: Path, manifest: dict[str, Any]) -> list[dict[str, Any]]:
    ...
    for timeframe in TIMEFRAMES_V5_2:
        ...
        if list(frame.columns) != LABEL_COLUMNS_V5_2:
            raise RuntimeError(f"V5.2 full label schema mismatch for {timeframe}")
        ...
        write_parquet(sample[LABEL_COLUMNS_V5_2], root / sample_path)
        samples.append(
            {
                "timeframe": timeframe,
...
def _build_artifact_inventory(root: Path, manifest: dict[str, Any], samples: list[dict[str, Any]]) -> dict[str, Any]:
    input_ohlcv = [
        {
            ...
            "reason_excluded": "full V5.0 OHLCV Parquet is represented by the validated V5.0 manifest and V5.2 source hashes",
        }
    ...
    full_parquet = [
        {
            ...
            "rows": int(payload["rows"]),
            "columns": LABEL_COLUMNS_V5_2,
            "schema_version": "LABEL_COLUMNS_V5_2",
            "reason_excluded": "full V5.2 label Parquet is represented by manifest checksums and deterministic audit-lite samples",
        }
    ...
    return {
        ...
        "included_reports": [
            {"path": MANIFEST_PATH_V5_2.as_posix(), "sha256": sha256_file(root / MANIFEST_PATH_V5_2)},
            {"path": REPORT_JSON_PATH_V5_2.as_posix(), "sha256": sha256_file(root / REPORT_JSON_PATH_V5_2)},
        ],
    }
...
def _build_parquet_summary(root: Path, manifest: dict[str, Any]) -> dict[str, Any]:
    ...
    for timeframe, payload in sorted(manifest["outputs"].items()):
        ...
        summary["labels"][timeframe] = {
            ...
            "schema_strict": list(frame.columns) == LABEL_COLUMNS_V5_2,
        }
    return summary
...
def _write_attestation(root: Path, manifest: dict[str, Any]) -> None:
    ...
    commands = [
        "python scripts/run_max_history_label_factory_v5_2.py",
        "python scripts/validate_max_history_label_factory_v5_2.py",
        "python -m pytest -q tests/labels/test_max_history_forward_labels_v5_2.py",
        "python -m pytest -q tests/validation/test_max_history_label_factory_v5_2_validator.py",
        "python scripts/release_audit_lite_zip_v5_2.py",
        "python scripts/audit_audit_lite_zip_v5_2.py --zip projet-galapagos-v5.2-audit-lite.zip",
        "python scripts/smoke_audit_lite_zip_v5_2.py --zip projet-galapagos-v5.2-audit-lite.zip",
    ...
    payload = {
        ...
        "input_window_end": manifest["input_ohlcv_manifest"]["window_end"],
        "input_total_days": manifest["input_ohlcv_manifest"]["total_days"],
        "manifest_sha256": sha256_file(root / MANIFEST_PATH_V5_2),
        "report_sha256": sha256_file(root / REPORT_JSON_PATH_V5_2),
        "tests_passed": True,
...
def _read_optional_timings(root: Path, commands: list[str]) -> dict[str, float]:
    timing_path = root / "reports/audit_lite/v5_2_command_timings.json"
...
def _collect_files(root: Path) -> list[Path]:
    include_files = [
        ...
        "scripts/run_max_history_label_factory_v5_2.py",
        "scripts/validate_max_history_label_factory_v5_2.py",
        "scripts/release_audit_lite_zip_v5_2.py",
        "scripts/audit_audit_lite_zip_v5_2.py",
        "scripts/smoke_audit_lite_zip_v5_2.py",
        "tests/labels/test_max_history_forward_labels_v5_2.py",
        "tests/validation/test_max_history_label_factory_v5_2_validator.py",
        MANIFEST_PATH_V5_2.as_posix(),
        REPORT_JSON_PATH_V5_2.as_posix(),
        REPORT_MD_PATH_V5_2.as_posix(),
        DOC_PATH_V5_2.as_posix(),
...
def _allowed(relative: Path) -> bool:
    ...
    if relative.suffix.casefold() == ".zip":
        return False
    if relative.suffix.casefold() == ".parquet" and not name.startswith("data/audit_lite/v5_2/labels/"):
        return False
...
def _write_size_report(root: Path, *, zip_size_bytes: int, included: list[Path], inventory: dict[str, Any]) -> None:
    ...
    _write_text(
        root / ZIP_SIZE_MD,
        "# Rapport de taille ZIP audit-lite V5.2\n\n"
        f"- ZIP : `{ZIP_NAME}`\n"
...
def _render_inventory_markdown(inventory: dict[str, Any], manifest: dict[str, Any]) -> str:
    ...
    return f"""# Inventaire audit-lite V5.2

- Raw zips exclus : `{inventory['raw_zips_excluded']}`
- Parquet labels complets exclus : `{len(inventory['full_parquet_excluded'])}`
...
## Parquet labels complets exclus
