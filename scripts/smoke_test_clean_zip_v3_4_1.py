...
FORBIDDEN_PREFIXES = [
    ...
    "data/research/v3_4_1/backtests/",
    "data/research/v3_4_1/strategies/",
]
...
def main() -> None:
    ...
    with tempfile.TemporaryDirectory(prefix="galapagos_v3_4_1_smoke_") as tmp:
        tmp_path = Path(tmp)
    ...
    payload = {
        ...
        "safety_flags_ok": not any("unsafe V3.4.1 flag active" in error for error in errors),
        "smoke_duration_seconds": smoke_duration,
        "errors": errors,
        "warnings": warnings,
    }
    Path("reports").mkdir(exist_ok=True)
    Path("reports/zip_smoke_test_v3_4_1.json").write_text(json.dumps(payload, indent=2, ensure_ascii=False) + "\n", encoding="utf-8")
    Path("reports/zip_smoke_test_v3_4_1.md").write_text(
        "# Smoke ZIP V3.4.1\n\n"
...
def _validate_state_manifest_and_report(root: Path) -> list[str]:
    ...
    if manifest.get("version") != ROBUSTNESS_VERSION or report.get("version") != ROBUSTNESS_VERSION:
        errors.append("V3.4 manifest/report version mismatch")
    if manifest.get("status") != "PASS":
        errors.append("V3.4.1 manifest status must be PASS")
    ...
    for key in ["robust_edge_claimed", "strategy_validated", "backtest_performed", "actionable_signal_produced"]:
        if findings.get(key) is not False:
            errors.append(f"unsafe V3.4.1 finding active: {key}")
    if project_state.get("candidate_version") != VERSION and VERSION not in latest_summary:
        errors.append("PROJECT_STATE/latest_summary must identify V3.4.1 as candidate")
    ...
    if "pending_external_audit" not in latest_summary and project_state.get("candidate_status") != "pending_external_audit":
        errors.append("V3.4.1 must remain pending_external_audit")
    safety = manifest.get("safety", {})
    for flag in SAFETY_FALSE_FLAGS:
        if safety.get(flag) is not False:
            errors.append(f"unsafe V3.4.1 flag active: {flag}")
    if safety.get("ml_enabled") is not True or safety.get("labels_enabled") is not True or safety.get("dataset_enabled") is not True:
        errors.append("ml_enabled, labels_enabled and dataset_enabled must be true for V3.4.1 audit context")
