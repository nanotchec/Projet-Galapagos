...
VERSION = "V8.7"
AUDIT_JSON = Path("reports/audit_lite/zip_audit_v8_7.json")
AUDIT_MD = Path("reports/audit_lite/zip_audit_v8_7.md")
...
REQUIRED_ENTRIES = {
    ...
    "reports/ml/strict_walk_forward_validation_v8_7.md",
    "reports/ml/strict_walk_forward_scores_v8_7.json",
    "reports/ml/strict_walk_forward_scores_v8_7.md",
    "reports/audit_lite/v8_7_full_local_validation_attestation.json",
    "reports/audit_lite/v8_7_full_local_validation_attestation.md",
    "reports/audit_lite/v8_7_artifact_inventory.json",
    "reports/audit_lite/v8_7_artifact_inventory.md",
    "reports/audit_lite/v8_7_parquet_summary.json",
    "reports/audit_lite/zip_size_report_v8_7.json",
    "reports/audit_lite/zip_size_report_v8_7.md",
    "docs/strict_walk_forward_validation_v8_7.md",
    "scripts/run_strict_walk_forward_validation_v8_7.py",
    "scripts/validate_strict_walk_forward_validation_v8_7.py",
...
def audit_zip(zip_path: Path) -> dict[str, Any]:
    ...
    sample_entries = [entry for entry in entries if entry.startswith("data/audit_lite/v8_7/") and entry.endswith(".parquet")]
    if len(sample_entries) < 8:
        errors.append("missing V8.7 score/fold parquet samples")
...
def _is_forbidden_artifact(entry: str) -> bool:
    ...
    if entry.startswith(FORBIDDEN_PREFIXES):
        return True
    if entry.startswith("data/audit_lite/v8_7/") and entry.endswith(".parquet"):
        return False
