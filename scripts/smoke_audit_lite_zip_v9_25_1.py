...
def smoke_zip(zip_path: Path) -> dict[str, Any]:
    errors: list[str] = []
    with tempfile.TemporaryDirectory(prefix="galapagos_v9_25_1_smoke_") as tmp:
        ...
        errors.extend(_run_v9_25_1_tests(extract_root, env))
...
def _import_modules(extract_root: Path, env: dict[str, str]) -> list[str]:
    ...
    for module in [
        ...
        "galapagos.data.aggtrades_post_v9_resume_campaign_v9_25_1",
        "galapagos.data.aggtrades_post_v9_resume_campaign_v9_25_1_validation",
    ]:
...
def _run_v9_25_1_tests(extract_root: Path, env: dict[str, str]) -> list[str]:
    completed = _run(
        [
            "python",
            "-m",
    ...
    if completed["returncode"] != 0:
        return [f"V9.25.1 tests failed: {completed['stdout'][-1000:]} {completed['stderr'][-1000:]}"]
    return []


def _run_self_audit(extract_root: Path, zip_path: Path, env: dict[str, str]) -> list[str]:
    completed = _run(
        ["python", "scripts/audit_audit_lite_zip_v9_25_1.py", "--zip", str(zip_path)],
        extract_root,
        env,
    ...
    if report.get("collection_executed"):
        for key in ["total_rows_new", "raw_bytes_new", "silver_bytes_new"]:
            if summary.get(key, 0) <= 0:
                errors.append(f"V9.25.1 smoke expects positive {key}")
    flags = report.get("safety_flags", {})
    if flags.get("api_key_used") is not False or flags.get("private_endpoint_used") is not False:
        errors.append("V9.25.1 must confirm no API key and no private endpoint")
    if flags.get("exchange_auth_used") is not False or flags.get("websocket_live_used") is not False:
        errors.append("V9.25.1 must confirm no exchange auth and no websocket live")
    if flags.get("no_data_deletion") is not True or flags.get("no_destructive_cleanup") is not True:
        errors.append("V9.25.1 must confirm no deletion and no destructive cleanup")
    return errors
...
def _write_reports(result: dict[str, Any]) -> None:
    ...
    (report_dir / "zip_smoke_v9_25_1.json").write_text(json.dumps(result, indent=2, ensure_ascii=False) + "\n", encoding="utf-8")
    (report_dir / "zip_smoke_v9_25_1.md").write_text(
        "# Smoke ZIP V9.25.1\n\n"
        ...
        f"- Timeout tests V9.25.1 : `{TEST_TIMEOUT_SECONDS}` secondes.\n"
