...
def main() -> None:
    ...
    errors: list[str] = []
    commands = []
    with tempfile.TemporaryDirectory(prefix="galapagos_v2_4_3_smoke_") as tmp:
        tmp_path = Path(tmp)
        ...
        commands.append(
            [
                ...
                (
                    ...
                    "assert manifest['correction_version']=='V2.4.3'\n"
                    "assert report['correction_version']=='V2.4.3'\n"
    ...
    payload = {
        ...
        "bounded_smoke_for_v2_4_3": True,
        "errors": errors,
    }
    Path("reports").mkdir(exist_ok=True)
    Path("reports/zip_smoke_test_v2_4_3.json").write_text(json.dumps(payload, indent=2, ensure_ascii=False) + "\n", encoding="utf-8")
    Path("reports/zip_smoke_test_v2_4_3.md").write_text(
        "# Smoke ZIP V2.4.3\n\n"
