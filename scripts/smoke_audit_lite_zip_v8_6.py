...
VERSION = "V8.6"
SMOKE_JSON = Path("reports/audit_lite/zip_smoke_v8_6.json")
SMOKE_MD = Path("reports/audit_lite/zip_smoke_v8_6.md")
...
def smoke_zip(zip_path: Path) -> dict[str, Any]:
    ...
    with tempfile.TemporaryDirectory(prefix="galapagos_v8_6_smoke_") as tmp:
        target = Path(tmp)
...
def _check_forbidden_files(root: Path) -> list[str]:
    ...
    for path in root.rglob("*"):
        ...
        if path.suffix.casefold() in {".pkl", ".pickle", ".joblib", ".onnx", ".pt", ".pth", ".ckpt", ".parquet"}:
...
def _check_imports(root: Path) -> list[str]:
    ...
    errors: list[str] = []
    for module in [
        "galapagos.ml.ohlcv_trades_1y_robustness",
        "galapagos.ml.ohlcv_trades_1y_robustness_validation",
...
def _check_reports(root: Path) -> list[str]:
    errors: list[str] = []
    manifest_path = root / "reports/manifests/ohlcv_trades_1y_ml_robustness_v8_6_manifest.json"
    report_path = root / "reports/ml/ohlcv_trades_1y_ml_robustness_v8_6.json"
    decision_path = root / "reports/research_decisions/v8_6_research_decision_gate.json"
    ...
    if decision_path.exists():
        ...
        if decision.get("decision_gate_type") != "research_only":
            errors.append("smoke decision gate must be research_only")
        if any(value is not False for value in decision.get("claims", {}).values()):
            errors.append("smoke decision claims must all be false")
    return errors
