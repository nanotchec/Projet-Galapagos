...
def smoke_zip(zip_path: Path) -> dict[str, Any]:
    errors: list[str] = []
    with tempfile.TemporaryDirectory(prefix="galapagos_v9_28_smoke_") as tmp:
        ...
        errors.extend(_run_v9_28_tests(extract_root, env))
...
def _import_modules(extract_root: Path, env: dict[str, str]) -> list[str]:
    ...
    for module in [
        ...
        "galapagos.data.aggtrades_post_v9_bad_day_repair_v9_28_validation",
    ]:
...
def _run_v9_28_tests(extract_root: Path, env: dict[str, str]) -> list[str]:
    completed = _run(
        [
            "python",
            "-m",
    ...
    if completed["returncode"] != 0:
        return [f"V9.28 tests failed: {completed['stdout'][-1000:]} {completed['stderr'][-1000:]}"]
    return []
...
def _check_reports(extract_root: Path) -> list[str]:
    ...
    if report.get("version") != VERSION:
        errors.append("V9.28 report version mismatch")
    if report.get("decision") == "bad_day_repaired_and_remaining_window_completed":
        if summary.get("local_file_coverage_start") != "2024-05-05" or summary.get("local_file_coverage_end") != "2026-05-05":
            errors.append("V9.28 completed smoke expects full target coverage")
        if summary.get("repair_applied") is not True:
            errors.append("V9.28 completed smoke expects repair_applied=true")
    else:
        if summary.get("complete_collection_reached") is True:
            errors.append("V9.28 partial smoke cannot claim complete collection")
    return errors


def _run_self_audit(extract_root: Path, zip_path: Path, env: dict[str, str]) -> list[str]:
    completed = _run(["python", "scripts/audit_audit_lite_zip_v9_28.py", "--zip", str(zip_path)], extract_root, env, AUDIT_TIMEOUT_SECONDS)
    if completed["returncode"] != 0:
...
def _write_reports(result: dict[str, Any]) -> None:
    ...
    (report_dir / "zip_smoke_v9_28.json").write_text(json.dumps(result, indent=2, ensure_ascii=False) + "\n", encoding="utf-8")
    (report_dir / "zip_smoke_v9_28.md").write_text(
        "# Smoke ZIP V9.28\n\n"
        ...
        f"- Timeout tests V9.28 : `{TEST_TIMEOUT_SECONDS}` secondes.\n"
