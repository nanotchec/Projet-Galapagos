...
def main() -> None:
    ...
    with tempfile.TemporaryDirectory(prefix="galapagos_v3_1_6_smoke_") as tmp:
        tmp_path = Path(tmp)
        ...
        validator_errors, validator_timings = _run_validators(base_extract, validator_runs, smoke_logs)
    ...
    payload = {
        ...
        "smoke_logs_outside_validated_roots": True,
        "smoke_log_leaks_in_validated_roots": bool(leak_errors),
        "errors": errors,
    }
    Path("reports").mkdir(exist_ok=True)
    Path("reports/zip_smoke_test_v3_1_6.json").write_text(
        json.dumps(payload, indent=2, ensure_ascii=False) + "\n",
        encoding="utf-8",
    )
    Path("reports/zip_smoke_test_v3_1_6.md").write_text(
        "# Smoke ZIP V3.1.6\n\n"
        ...
        f"- Logs hors roots valides : `{payload['smoke_logs_outside_validated_roots']}`\n"
...
def _validate_state_and_safety(root: Path) -> list[str]:
    ...
    if manifest.get("correction_version") is None or report.get("correction_version") is None:
        ...
        if VERSION not in summary or VERSION not in project_state:
            errors.append("V3.1.6 must be documented in PROJECT_STATE/latest_summary when correction_version is absent")
    ...
    for flag in [
        ...
        if safety.get(flag) is not False:
            errors.append(f"unsafe V3.1.6 flag active: {flag}")
    if safety.get("labels_enabled") is not True:
        errors.append("labels_enabled must be true for V3.1.6 labels context")
    return errors


def _run_validators(base_extract: Path, validator_runs: Path, smoke_logs: Path) -> tuple[list[str], dict[str, float]]:
    errors: list[str] = []
    ...
    for name, script, timeout_seconds in VALIDATORS:
        ...
        if validator_root.exists():
            shutil.rmtree(validator_root)
        shutil.copytree(base_extract, validator_root)
        env = os.environ.copy()
...
def _validate_no_smoke_log_leaks(base_extract: Path, validator_runs: Path) -> list[str]:
    leaks = list(base_extract.rglob(".smoke-*")) + list(validator_runs.rglob(".smoke-*"))
