...
REQUIRED_FILES = [
    ...
    "scripts/run_aggtrades_post_v9_collection_v9_18.py",
    "scripts/validate_aggtrades_post_v9_collection_v9_18.py",
    "scripts/release_audit_lite_zip_v9_18.py",
    "scripts/audit_audit_lite_zip_v9_18.py",
    "scripts/smoke_audit_lite_zip_v9_18.py",
    "src/galapagos/data/aggtrades_post_v9_collection_v9_18.py",
    "src/galapagos/data/aggtrades_post_v9_collection_v9_18_validation.py",
    "tests/data/test_aggtrades_post_v9_collection_v9_18.py",
    "tests/validation/test_aggtrades_post_v9_collection_v9_18_validator.py",
    "reports/manifests/aggtrades_post_v9_collection_v9_18_manifest.json",
    "reports/data/aggtrades_post_v9_collection_v9_18.json",
    "reports/data/aggtrades_post_v9_collection_v9_18.md",
    "docs/aggtrades_post_v9_collection_v9_18.md",
    "reports/audit_lite/v9_18_command_results.json",
    "reports/audit_lite/v9_18_command_results.md",
    "reports/audit_lite/v9_18_full_local_validation_attestation.json",
    "reports/audit_lite/v9_18_full_local_validation_attestation.md",
    "reports/audit_lite/v9_18_artifact_inventory.json",
    "reports/audit_lite/v9_18_artifact_inventory.md",
    "reports/audit_lite/zip_size_report_v9_18.json",
    "reports/audit_lite/zip_size_report_v9_18.md",
    "reports/audit_lite/zip_audit_v9_18.json",
    "reports/audit_lite/zip_audit_v9_18.md",
    "reports/audit_lite/zip_smoke_v9_18.json",
    "reports/audit_lite/zip_smoke_v9_18.md",
...
def audit_zip(zip_path: Path) -> list[str]:
    ...
    errors: list[str] = []
    with tempfile.TemporaryDirectory(prefix="galapagos_v9_18_audit_") as tmp:
...
def _check_inventory(extract_root: Path, names: list[str]) -> list[str]:
    errors: list[str] = []
    inventory = _read_json(extract_root / "reports/audit_lite/v9_18_artifact_inventory.json")
    size_report = _read_json(extract_root / "reports/audit_lite/zip_size_report_v9_18.json")
...
def _check_reports(extract_root: Path) -> list[str]:
    errors: list[str] = []
    report = _read_json(extract_root / "reports/data/aggtrades_post_v9_collection_v9_18.json")
    manifest = _read_json(extract_root / "reports/manifests/aggtrades_post_v9_collection_v9_18_manifest.json")
    attestation = _read_json(extract_root / "reports/audit_lite/v9_18_full_local_validation_attestation.json")
    if report.get("mode") != "dry-run":
        errors.append("V9.18 audit-lite must be the dry-run pack")
    if report.get("v9_18_decision", {}).get("decision") != "aggtrades_post_v9_collection_pack_ready_dry_run_only":
        errors.append("V9.18 decision mismatch")
    coverage = report.get("coverage_summary", {})
    if coverage.get("days_expected") != 772:
        errors.append("V9.18 expected days mismatch")
    if report.get("collection_executed") is not False:
        errors.append("V9.18 dry-run must not execute collection")
    for key in ["network_used", "new_data_downloaded", "ingestion_executed", "features_created", "labels_created", "dataset_created", "ml_executed", "walk_forward_executed", "backtest_executed"]:
        if report.get(key) is not False:
            errors.append(f"V9.18 report must keep {key}=false")
    source = report.get("source_public_target", {})
    if source.get("host") != "data.binance.vision":
        errors.append("V9.18 source host mismatch")
...
def _check_state(extract_root: Path) -> list[str]:
    ...
    for name, payload in {"PROJECT_STATE": state, "latest_metrics": metrics}.items():
        if payload.get("last_validated_version") != "V9.17":
        ...
        if payload.get("direction") != "aggtrades_post_v9_collection_pack":
...
def _write_reports(result: dict[str, Any]) -> None:
    ...
    (report_dir / "zip_audit_v9_18.json").write_text(json.dumps(result, indent=2, ensure_ascii=False) + "\n", encoding="utf-8")
    (report_dir / "zip_audit_v9_18.md").write_text(
        "# Audit ZIP V9.18\n\n"
