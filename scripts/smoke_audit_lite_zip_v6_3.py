...
VERSION = "V6.3"
REPORT_JSON = Path("reports/audit_lite/zip_smoke_v6_3.json")
REPORT_MD = Path("reports/audit_lite/zip_smoke_v6_3.md")
...
def smoke_zip(zip_path: Path) -> dict[str, Any]:
    ...
    with tempfile.TemporaryDirectory(prefix="galapagos-v6-3-smoke-") as tmp:
        ...
        try:
            imports = {
                "galapagos.ml.advanced_ohlcv_robustness": "run_advanced_ohlcv_ml_robustness_v6_3",
                "galapagos.ml.advanced_ohlcv_robustness_validation": "validate_advanced_ohlcv_ml_robustness_v6_3",
                "galapagos.ml.schemas": "ML_SCORE_COLUMNS_V6_2",
            }
        ...
        inventory = _read_json(extract_root / "reports/audit_lite/v6_3_artifact_inventory.json")
        attestation = _read_json(extract_root / "reports/audit_lite/v6_3_full_local_validation_attestation.json")
        if manifest != report:
            errors.append("V6.3 manifest/report mismatch inside audit-lite ZIP")
        for analysis in [
            "baseline_delta",
            "split_stability",
            "timeframe_stability",
            ...
            if analysis not in manifest.get("analyses", {}):
                errors.append(f"missing V6.3 analysis in smoke: {analysis}")
        ...
        for key in ["authentication_used", "api_key_used", "private_endpoint_used", "orders_enabled", "paper_live_enabled", "trading_enabled", "backtest_enabled", "strategy_enabled", "execution_enabled"]:
            if safety.get(key) is not False:
                errors.append(f"V6.3 safety flag must be false: {key}")
        for key in ["public_read_only", "ml_enabled", "labels_enabled", "dataset_enabled"]:
            if safety.get(key) is not True:
                errors.append(f"V6.3 safety flag must be true: {key}")
        ...
        for key in [
            ...
            if findings.get(key) is not False:
                errors.append(f"V6.3 finding must be false: {key}")
        ...
        if collect_result["returncode"] != 0:
            errors.append("V6.3 audit-lite pytest collect-only failed")
...
def _render_markdown(result: dict[str, Any]) -> str:
    ...
    return f"""# Smoke ZIP audit-lite V6.3
