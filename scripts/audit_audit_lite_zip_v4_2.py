...
VERSION = "V4.2"
REPORT_JSON = Path("reports/audit_lite/zip_audit_v4_2.json")
REPORT_MD = Path("reports/audit_lite/zip_audit_v4_2.md")
REQUIRED_FILES = {
    "README.md",
    "pyproject.toml",
    "reports/manifests/one_year_public_market_data_v4_2_manifest.json",
    "reports/data_quality/one_year_public_market_data_v4_2.json",
    "reports/data_quality/one_year_public_market_data_v4_2.md",
    "reports/audit_lite/v4_2_artifact_inventory.json",
    "reports/audit_lite/v4_2_artifact_inventory.md",
    "reports/audit_lite/v4_2_parquet_summary.json",
    "reports/audit_lite/v4_2_full_local_validation_attestation.json",
    "reports/audit_lite/v4_2_full_local_validation_attestation.md",
...
REQUIRED_SAMPLES = {
    "data/audit_lite/v4_2/ohlcv/timeframe=1m/sample.parquet",
    "data/audit_lite/v4_2/ohlcv/timeframe=5m/sample.parquet",
    "data/audit_lite/v4_2/ohlcv/timeframe=15m/sample.parquet",
    "data/audit_lite/v4_2/ohlcv/timeframe=1h/sample.parquet",
}
FORBIDDEN_PREFIXES = [
...
def audit_zip(zip_path: Path) -> dict[str, Any]:
    ...
    with tempfile.TemporaryDirectory(prefix="galapagos-v4-2-audit-") as tmp:
        extract_root = Path(tmp)
        with zipfile.ZipFile(zip_path) as archive:
            archive.extractall(extract_root)
        manifest = _read_json(extract_root / "reports/manifests/one_year_public_market_data_v4_2_manifest.json")
        report = _read_json(extract_root / "reports/data_quality/one_year_public_market_data_v4_2.json")
        inventory = _read_json(extract_root / "reports/audit_lite/v4_2_artifact_inventory.json")
        attestation = _read_json(extract_root / "reports/audit_lite/v4_2_full_local_validation_attestation.json")
        if manifest != report:
            errors.append("V4.2 manifest/report mismatch in ZIP")
        if manifest.get("version") != VERSION:
            errors.append("V4.2 manifest version mismatch in ZIP")
        if len(inventory.get("raw_files_excluded", [])) != 366:
            errors.append("V4.2 raw inventory must represent 366 raw zips")
        ...
        for flag in ["validator_passed", "tests_passed", "audit_lite_passed", "smoke_audit_lite_passed", "no_trading", "no_backtest", "no_orders"]:
            if attestation.get(flag) is not True:
                errors.append(f"V4.2 attestation flag must be true: {flag}")
...
def _render_markdown(result: dict[str, Any]) -> str:
    ...
    return f"""# Audit ZIP audit-lite V4.2
