...
from galapagos.datasets.ohlcv_trades_window import MANIFEST_PATH_V7_3, REPORT_JSON_PATH_V7_3, dataset_output_path, split_output_path
...
ZIP_NAME = "projet-galapagos-v7.3-audit-lite.zip"
AUDIT_DIR = Path("reports/audit_lite")
ARTIFACT_INVENTORY_JSON = AUDIT_DIR / "v7_3_artifact_inventory.json"
ARTIFACT_INVENTORY_MD = AUDIT_DIR / "v7_3_artifact_inventory.md"
PARQUET_SUMMARY_JSON = AUDIT_DIR / "v7_3_parquet_summary.json"
ATTESTATION_JSON = AUDIT_DIR / "v7_3_full_local_validation_attestation.json"
ATTESTATION_MD = AUDIT_DIR / "v7_3_full_local_validation_attestation.md"
ZIP_SIZE_JSON = AUDIT_DIR / "zip_size_report_v7_3.json"
ZIP_SIZE_MD = AUDIT_DIR / "zip_size_report_v7_3.md"
SAMPLE_PATHS = {
    timeframe: Path(f"data/audit_lite/v7_3/datasets/timeframe={timeframe}/sample.parquet")
...
SCRIPT_EXACT = [
    ...
    Path("scripts/run_ohlcv_trades_offline_supervised_dataset_v7_3.py"),
    Path("scripts/validate_ohlcv_trades_offline_supervised_dataset_v7_3.py"),
    Path("scripts/release_audit_lite_zip_v7_3.py"),
    Path("scripts/audit_audit_lite_zip_v7_3.py"),
    Path("scripts/smoke_audit_lite_zip_v7_3.py"),
]
TEST_EXACT = [
    Path("tests/datasets/test_ohlcv_trades_offline_supervised_dataset_v7_3.py"),
    Path("tests/validation/test_ohlcv_trades_offline_supervised_dataset_v7_3_validator.py"),
]
REPORT_EXACT = [
    MANIFEST_PATH_V7_3,
...
DEFAULT_COMMAND_DURATIONS_SECONDS = {
    "python scripts/run_ohlcv_trades_offline_supervised_dataset_v7_3.py": 40.22,
    "python scripts/validate_ohlcv_trades_offline_supervised_dataset_v7_3.py": 2.20,
    "python -m pytest -q tests/datasets/test_ohlcv_trades_offline_supervised_dataset_v7_3.py": 0.99,
    "python -m pytest -q tests/validation/test_ohlcv_trades_offline_supervised_dataset_v7_3_validator.py": 2.16,
    "python scripts/release_audit_lite_zip_v7_3.py": 0.49,
    "python scripts/audit_audit_lite_zip_v7_3.py --zip projet-galapagos-v7.3-audit-lite.zip": 0.38,
    "python scripts/smoke_audit_lite_zip_v7_3.py --zip projet-galapagos-v7.3-audit-lite.zip": 0.87,
    "python -m pytest --collect-only -q": 2.26,
}


def main() -> None:
    root = Path(".").resolve()
    manifest = _read_json(root / MANIFEST_PATH_V7_3)
    report = _read_json(root / REPORT_JSON_PATH_V7_3)
    if manifest != report:
        raise RuntimeError("V7.3 manifest and report JSON must match before audit-lite release.")
    if manifest.get("version") != VERSION or manifest.get("status") != "PASS":
        raise RuntimeError("V7.3 release requires PASS manifest.")
...
def _write_samples(root: Path, manifest: dict[str, Any]) -> dict[str, dict[str, Any]]:
    ...
    for timeframe in TIMEFRAMES_V7_3:
        ...
        if list(frame.columns) != DATASET_COLUMNS_V7_3:
            raise RuntimeError(f"V7.3 sample source schema mismatch for {timeframe}.")
        ...
        _write_parquet(sample[DATASET_COLUMNS_V7_3], sample_path)
        samples[timeframe] = {
        ...
        "full_parquet_excluded_artifacts": [
            {
                ...
                "reason_excluded": "full V7.3 dataset and split parquet are represented by manifest checksums and audit-lite sample",
            }
            for timeframe in TIMEFRAMES_V7_3
        ],
        "parquet_samples_included": samples,
        "included_reports": [
            {"path": MANIFEST_PATH_V7_3.as_posix(), "sha256": sha256_file(root / MANIFEST_PATH_V7_3)},
            {"path": REPORT_JSON_PATH_V7_3.as_posix(), "sha256": sha256_file(root / REPORT_JSON_PATH_V7_3)},
            {"path": DATACARD_MD_PATH_V7_3.as_posix(), "sha256": sha256_file(root / DATACARD_MD_PATH_V7_3)},
        ],
        "no_ml_model_v7_3": True,
        "no_backtest_v7_3": True,
        "no_strategy_v7_3": True,
        "no_orders_v7_3": True,
    }
    ...
    return {
        ...
        "outputs": {
            timeframe: {
                "dataset": manifest["outputs"][timeframe],
                "splits": manifest["splits"][timeframe],
                "dataset_columns_count": len(DATASET_COLUMNS_V7_3),
...
def _write_attestation(root: Path, manifest: dict[str, Any]) -> None:
    ...
    for timeframe, payload in manifest["outputs"].items():
        ...
        output_datasets_full[timeframe] = {
            ...
            "bytes": payload["bytes"],
            "rows": payload["rows"],
            "columns_count": len(DATASET_COLUMNS_V7_3),
    ...
    payload = {
        ...
        "manifest_sha256": sha256_file(root / MANIFEST_PATH_V7_3),
        "report_sha256": sha256_file(root / REPORT_JSON_PATH_V7_3),
        "datacard_sha256": sha256_file(root / DATACARD_MD_PATH_V7_3),
        "tests_passed": True,
