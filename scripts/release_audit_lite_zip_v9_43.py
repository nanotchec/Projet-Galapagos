...
VERSION = "V9.43"
ZIP_NAME = "projet-galapagos-v9.43-audit-lite.zip"
ROOT = Path(".").resolve()

CORE_PATHS = [
    Path("scripts/_bootstrap.py"),
    Path("src/galapagos/datasets/ohlcv_aggtrades_5y_dataset_v9_41_schemas.py"),
    Path("src/galapagos/ml/ohlcv_aggtrades_5y_offline_ml_v9_43.py"),
    Path("src/galapagos/ml/ohlcv_aggtrades_5y_offline_ml_v9_43_metrics.py"),
    Path("src/galapagos/ml/ohlcv_aggtrades_5y_offline_ml_v9_43_quality.py"),
    Path("src/galapagos/ml/ohlcv_aggtrades_5y_offline_ml_v9_43_validation.py"),
    Path("scripts/run_ohlcv_aggtrades_5y_offline_ml_v9_43.py"),
    Path("scripts/validate_ohlcv_aggtrades_5y_offline_ml_v9_43.py"),
    Path("scripts/release_audit_lite_zip_v9_43.py"),
    Path("scripts/audit_audit_lite_zip_v9_43.py"),
    Path("scripts/smoke_audit_lite_zip_v9_43.py"),
    Path("tests/ml/test_ohlcv_aggtrades_5y_offline_ml_v9_43.py"),
    Path("tests/validation/test_ohlcv_aggtrades_5y_offline_ml_v9_43_validator.py"),
    Path("reports/manifests/ohlcv_aggtrades_5y_offline_ml_v9_43_manifest.json"),
    ...
    Path("reports/ml/ohlcv_aggtrades_5y_offline_scores_v9_43.md"),
    Path("docs/ohlcv_aggtrades_5y_offline_ml_v9_43.md"),
]
...
AUDIT_PATHS = [
    Path("reports/audit_lite/v9_43_command_results.json"),
    Path("reports/audit_lite/v9_43_command_results.md"),
    Path("reports/audit_lite/v9_43_full_local_validation_attestation.json"),
    Path("reports/audit_lite/v9_43_full_local_validation_attestation.md"),
    Path("reports/audit_lite/v9_43_artifact_inventory.json"),
    Path("reports/audit_lite/v9_43_artifact_inventory.md"),
    Path("reports/audit_lite/zip_size_report_v9_43.json"),
    Path("reports/audit_lite/zip_size_report_v9_43.md"),
    Path("reports/audit_lite/zip_audit_v9_43.json"),
    Path("reports/audit_lite/zip_audit_v9_43.md"),
    Path("reports/audit_lite/zip_smoke_v9_43.json"),
    Path("reports/audit_lite/zip_smoke_v9_43.md"),
]
...
def main() -> int:
    report = _read_json(Path("reports/ml/ohlcv_aggtrades_5y_offline_ml_v9_43.json"))
...
def _ensure_placeholders() -> None:
    placeholders = {
        "reports/audit_lite/v9_43_command_results.json": {"version": VERSION, "status": "PENDING_CAPTURE", "commands": [], "sidecars_created": False, "zip_fingerprints_created": False},
        "reports/audit_lite/zip_audit_v9_43.json": {"version": VERSION, "passed": False, "errors": [], "status": "PENDING_RUN"},
        "reports/audit_lite/zip_smoke_v9_43.json": {"version": VERSION, "passed": False, "errors": [], "status": "PENDING_RUN"},
    }
    texts = {
        "reports/audit_lite/v9_43_command_results.md": "# Commandes V9.43\n\nEn attente.\n",
        "reports/audit_lite/zip_audit_v9_43.md": "# Audit ZIP V9.43\n\nEn attente.\n",
        "reports/audit_lite/zip_smoke_v9_43.md": "# Smoke ZIP V9.43\n\nEn attente.\n",
    }
...
def _collect_paths() -> list[Path]:
    ...
    if missing_core:
        raise FileNotFoundError(f"missing V9.43 release inputs: {missing_core}")
...
def _write_attestation(report: dict[str, Any], zip_size: int | None) -> None:
    commands = _read_optional_json(Path("reports/audit_lite/v9_43_command_results.json")).get("commands", [])
    payload = {
        "version": VERSION,
        ...
        "models_executed": report["models_executed"],
        "quality_status": report["quality_status"],
        "leakage_guard_status": report["leakage_guard_status"],
        "ml_executed": report["ml_executed"],
        "model_persisted": report["model_persisted"],
        "walk_forward_executed": report["walk_forward_executed"],
        "backtest_executed": report["backtest_executed"],
        **report["safety_flags"],
    }
    _write_json(Path("reports/audit_lite/v9_43_full_local_validation_attestation.json"), payload)
    _write_text(Path("reports/audit_lite/v9_43_full_local_validation_attestation.md"), "# Attestation V9.43\n\n" f"- Decision : `{report['decision']}`.\n" f"- Target : `{report['target_name']}`.\n" f"- Qualite : `{report['quality_status']}`.\n" "- ML offline execute, sans modele persistant.\n" "- Aucun trading, paper live, ordre, backtest, walk-forward, strategie, signal actionnable, API privee, cle API, reseau ou telechargement.\n" "- Aucune suppression destructive, aucun sidecar et aucune empreinte ZIP.\n")
...
def _write_inventory(paths: list[Path], zip_size: int | None) -> None:
    ...
    _write_json(Path("reports/audit_lite/v9_43_artifact_inventory.json"), payload)
    _write_text(Path("reports/audit_lite/v9_43_artifact_inventory.md"), "# Inventaire audit-lite V9.43\n\n" f"- Fichiers inclus : `{len(paths)}`.\n" f"- ZIP : `{ZIP_NAME}`.\n")
...
def _write_size_report(paths: list[Path], zip_size: int | None) -> None:
    ...
    _write_json(Path("reports/audit_lite/zip_size_report_v9_43.json"), payload)
    _write_text(Path("reports/audit_lite/zip_size_report_v9_43.md"), "# Taille ZIP V9.43\n\n" f"- Fichiers : `{len(paths)}`.\n" f"- ZIP bytes estimate : `{zip_size}`.\n")
