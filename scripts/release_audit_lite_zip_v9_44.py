...
CORE_PATHS = [
    ...
    Path("src/galapagos/research/ohlcv_aggtrades_5y_ml_diagnostic_v9_44.py"),
    Path("src/galapagos/research/ohlcv_aggtrades_5y_ml_diagnostic_v9_44_validation.py"),
    Path("scripts/run_ohlcv_aggtrades_5y_ml_diagnostic_v9_44.py"),
    Path("scripts/validate_ohlcv_aggtrades_5y_ml_diagnostic_v9_44.py"),
    Path("scripts/release_audit_lite_zip_v9_44.py"),
    Path("scripts/audit_audit_lite_zip_v9_44.py"),
    Path("scripts/smoke_audit_lite_zip_v9_44.py"),
    Path("tests/research/test_ohlcv_aggtrades_5y_ml_diagnostic_v9_44.py"),
    Path("tests/validation/test_ohlcv_aggtrades_5y_ml_diagnostic_v9_44_validator.py"),
    ...
    Path("docs/ohlcv_aggtrades_5y_ml_diagnostic_v9_44.md"),
]
PRIOR_REPORTS = [
...
AUDIT_PATHS = [
    Path("reports/audit_lite/v9_44_command_results.json"),
    Path("reports/audit_lite/v9_44_command_results.md"),
    Path("reports/audit_lite/v9_44_full_local_validation_attestation.json"),
    Path("reports/audit_lite/v9_44_full_local_validation_attestation.md"),
    Path("reports/audit_lite/v9_44_artifact_inventory.json"),
    Path("reports/audit_lite/v9_44_artifact_inventory.md"),
    Path("reports/audit_lite/zip_size_report_v9_44.json"),
    Path("reports/audit_lite/zip_size_report_v9_44.md"),
    Path("reports/audit_lite/zip_audit_v9_44.json"),
    Path("reports/audit_lite/zip_audit_v9_44.md"),
    Path("reports/audit_lite/zip_smoke_v9_44.json"),
    Path("reports/audit_lite/zip_smoke_v9_44.md"),
]
...
def main() -> int:
    report = _read_json(Path("reports/research_decisions/ohlcv_aggtrades_5y_ml_diagnostic_v9_44.json"))
...
def _ensure_placeholders() -> None:
    placeholders = {
        "reports/audit_lite/v9_44_command_results.json": {"version": VERSION, "status": "PENDING_CAPTURE", "commands": [], "sidecars_created": False, "zip_fingerprints_created": False},
        "reports/audit_lite/zip_audit_v9_44.json": {"version": VERSION, "passed": False, "errors": [], "status": "PENDING_RUN"},
        "reports/audit_lite/zip_smoke_v9_44.json": {"version": VERSION, "passed": False, "errors": [], "status": "PENDING_RUN"},
    }
    texts = {
        "reports/audit_lite/v9_44_command_results.md": "# Commandes V9.44\n\nEn attente.\n",
        "reports/audit_lite/zip_audit_v9_44.md": "# Audit ZIP V9.44\n\nEn attente.\n",
        "reports/audit_lite/zip_smoke_v9_44.md": "# Smoke ZIP V9.44\n\nEn attente.\n",
    }
...
def _collect_paths() -> list[Path]:
    ...
    if missing:
        raise FileNotFoundError(f"missing V9.44 release inputs: {missing}")
...
def _write_attestation(report: dict[str, Any], zip_size: int | None) -> None:
    commands = _read_optional_json(Path("reports/audit_lite/v9_44_command_results.json")).get("commands", [])
    payload = {
        "version": VERSION,
        ...
        "heavy_ml_executed": report["heavy_ml_executed"],
    ...
    _write_json(Path("reports/audit_lite/v9_44_full_local_validation_attestation.json"), payload)
    _write_text(
        Path("reports/audit_lite/v9_44_full_local_validation_attestation.md"),
        "# Attestation V9.44\n\n"
        f"- Decision : `{report['decision']}`.\n"
        f"- Recommandation : `{report['next_recommendation']}`.\n"
        "- Diagnostic-only, sans ML lourd et sans modele persistant.\n"
...
def _write_inventory(paths: list[Path], zip_size: int | None) -> None:
    ...
    _write_json(Path("reports/audit_lite/v9_44_artifact_inventory.json"), payload)
    _write_text(Path("reports/audit_lite/v9_44_artifact_inventory.md"), "# Inventaire audit-lite V9.44\n\n" f"- Fichiers inclus : `{len(paths)}`.\n" f"- ZIP : `{ZIP_NAME}`.\n")
...
def _write_size_report(paths: list[Path], zip_size: int | None) -> None:
    ...
    _write_json(Path("reports/audit_lite/zip_size_report_v9_44.json"), payload)
    _write_text(Path("reports/audit_lite/zip_size_report_v9_44.md"), "# Taille ZIP V9.44\n\n" f"- Fichiers : `{len(paths)}`.\n" f"- ZIP bytes estimate : `{zip_size}`.\n")
