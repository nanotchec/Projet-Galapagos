...
VERSION = "V7.7"
AUDIT_JSON = Path("reports/audit_lite/zip_audit_v7_7.json")
AUDIT_MD = Path("reports/audit_lite/zip_audit_v7_7.md")
REQUIRED_SOURCE_PREFIXES = ["src/galapagos/data/public_trades/"]
REQUIRED_ENTRIES = {
    "README.md",
    "pyproject.toml",
    "reports/data_quality/public_trades_90d_window_v7_7_discovery.json",
    "reports/data_quality/public_trades_90d_window_v7_7_discovery.md",
    "reports/manifests/public_trades_90d_window_v7_7_manifest.json",
    "reports/data_quality/public_trades_90d_window_v7_7.json",
    "reports/data_quality/public_trades_90d_window_v7_7.md",
    "docs/public_trades_90d_window_v7_7.md",
    "reports/audit_lite/v7_7_artifact_inventory.json",
    "reports/audit_lite/v7_7_artifact_inventory.md",
    "reports/audit_lite/v7_7_parquet_summary.json",
    "reports/audit_lite/v7_7_full_local_validation_attestation.json",
    "reports/audit_lite/v7_7_full_local_validation_attestation.md",
    "reports/audit_lite/zip_size_report_v7_7.json",
    "reports/audit_lite/zip_size_report_v7_7.md",
    "reports/PROJECT_STATE.json",
    "reports/PROJECT_STATE.md",
    "reports/current/latest_metrics.json",
    "reports/current/latest_metrics.md",
    "reports/current/latest_summary.md",
    "scripts/discover_public_trades_v7_7.py",
    "scripts/run_public_trades_90d_window_v7_7.py",
    "scripts/validate_public_trades_90d_window_v7_7.py",
    "scripts/release_audit_lite_zip_v7_7.py",
    "scripts/audit_audit_lite_zip_v7_7.py",
    "scripts/smoke_audit_lite_zip_v7_7.py",
    "tests/data/test_public_trades_90d_window_v7_7.py",
    "tests/validation/test_public_trades_90d_window_v7_7_validator.py",
}
SAMPLE_ENTRY = "data/audit_lite/v7_7/trades/sample.parquet"
FORBIDDEN_PREFIXES = (
    "data/raw/",
    "data/research/",
    "reports/backtests/",
...
def audit_zip(zip_path: Path) -> dict[str, Any]:
    ...
    with tempfile.TemporaryDirectory(prefix="galapagos-v7-7-audit-") as tmp:
        root = Path(tmp)
        with zipfile.ZipFile(zip_path) as archive:
            archive.extractall(root)
        manifest = _read_json(root / "reports/manifests/public_trades_90d_window_v7_7_manifest.json")
        report = _read_json(root / "reports/data_quality/public_trades_90d_window_v7_7.json")
        if manifest != report:
            errors.append("V7.7 manifest/report mismatch in ZIP")
        if manifest.get("version") != VERSION or manifest.get("status") != "PASS":
            errors.append("V7.7 manifest version/status mismatch in ZIP")
        ...
        for flag in ["trading_enabled", "orders_enabled", "paper_live_enabled", "backtest_enabled", "strategy_enabled", "execution_enabled", "ml_enabled", "labels_enabled", "dataset_enabled"]:
            if safety.get(flag) is not False:
                errors.append(f"V7.7 safety flag must be false: {flag}")
        sample = root / SAMPLE_ENTRY
        if sample.exists():
            frame = pd.read_parquet(sample, engine="pyarrow")
            if list(frame.columns) != AGG_TRADE_COLUMNS_V7_7:
                errors.append("V7.7 sample schema mismatch")
            if len(frame) == 0:
                errors.append("V7.7 sample must not be empty")
