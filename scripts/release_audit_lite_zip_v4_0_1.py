...
VERSION = "V4.0.1"
ZIP_NAME = "projet-galapagos-v4.0.1-audit-lite.zip"
...
def main() -> None:
    ...
    _write_text(root / ZIP_SIZE_MD, "# Rapport de taille ZIP audit-lite V4.0.1\n\n- Note : `audit-lite does not replace full local validation`\n")
    ...
    payload = {
        "version": VERSION,
        "correction_scope": "audit-lite source package completeness fix",
        "status": "PASS",
...
def _write_size_report(root: Path, *, zip_size_bytes: int, included: list[Path], inventory: dict[str, Any]) -> None:
    ...
    _write_text(
        root / ZIP_SIZE_MD,
        "# Rapport de taille ZIP audit-lite V4.0.1\n\n"
        f"- ZIP : `{ZIP_NAME}`\n"
...
def _render_inventory_markdown(inventory: dict[str, Any]) -> str:
    ...
    return f"""# Inventaire audit-lite V4.0.1

Ce rapport decrit les artefacts lourds exclus du ZIP `audit-lite`.

- Correction : completude des packages source requis pour auto-audit.
