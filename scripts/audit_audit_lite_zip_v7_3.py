...
from galapagos.datasets.schemas import DATASET_COLUMNS_V7_3, TIMEFRAMES_V7_3


VERSION = "V7.3"
AUDIT_JSON = Path("reports/audit_lite/zip_audit_v7_3.json")
AUDIT_MD = Path("reports/audit_lite/zip_audit_v7_3.md")
REQUIRED_SOURCE_PREFIXES = ["src/galapagos/datasets/"]
REQUIRED_ENTRIES = {
    "README.md",
    "pyproject.toml",
    "reports/manifests/ohlcv_trades_offline_supervised_dataset_v7_3_manifest.json",
    "reports/datasets/ohlcv_trades_offline_supervised_dataset_v7_3.json",
    "reports/datasets/ohlcv_trades_offline_supervised_dataset_v7_3.md",
    "reports/datasets/ohlcv_trades_offline_supervised_dataset_v7_3_datacard.md",
    "docs/ohlcv_trades_offline_supervised_dataset_v7_3.md",
    "reports/audit_lite/v7_3_artifact_inventory.json",
    "reports/audit_lite/v7_3_artifact_inventory.md",
    "reports/audit_lite/v7_3_parquet_summary.json",
    "reports/audit_lite/v7_3_full_local_validation_attestation.json",
    "reports/audit_lite/v7_3_full_local_validation_attestation.md",
    "reports/audit_lite/zip_size_report_v7_3.json",
    "reports/audit_lite/zip_size_report_v7_3.md",
    "reports/PROJECT_STATE.json",
    "reports/PROJECT_STATE.md",
    "reports/current/latest_metrics.json",
    "reports/current/latest_metrics.md",
    "reports/current/latest_summary.md",
    "scripts/run_ohlcv_trades_offline_supervised_dataset_v7_3.py",
    "scripts/validate_ohlcv_trades_offline_supervised_dataset_v7_3.py",
    "scripts/release_audit_lite_zip_v7_3.py",
    "scripts/audit_audit_lite_zip_v7_3.py",
    "scripts/smoke_audit_lite_zip_v7_3.py",
    "tests/datasets/test_ohlcv_trades_offline_supervised_dataset_v7_3.py",
    "tests/validation/test_ohlcv_trades_offline_supervised_dataset_v7_3_validator.py",
}
SAMPLE_ENTRIES = {
    f"data/audit_lite/v7_3/datasets/timeframe={timeframe}/sample.parquet" for timeframe in TIMEFRAMES_V7_3
}
FORBIDDEN_PREFIXES = (
    "data/raw/",
    "data/research/",
...
def audit_zip(zip_path: Path) -> dict[str, Any]:
    ...
    with tempfile.TemporaryDirectory(prefix="galapagos-v7-3-audit-") as tmp:
        root = Path(tmp)
        with zipfile.ZipFile(zip_path) as archive:
            archive.extractall(root)
        manifest = _read_json(root / "reports/manifests/ohlcv_trades_offline_supervised_dataset_v7_3_manifest.json")
        report = _read_json(root / "reports/datasets/ohlcv_trades_offline_supervised_dataset_v7_3.json")
        if manifest != report:
            errors.append("V7.3 manifest/report mismatch in ZIP")
        if manifest.get("version") != VERSION or manifest.get("status") != "PASS":
            errors.append("V7.3 manifest version/status mismatch in ZIP")
        ...
        for flag in ["trading_enabled", "orders_enabled", "paper_live_enabled", "backtest_enabled", "strategy_enabled", "execution_enabled", "ml_enabled"]:
            if safety.get(flag) is not False:
                errors.append(f"V7.3 safety flag must be false: {flag}")
