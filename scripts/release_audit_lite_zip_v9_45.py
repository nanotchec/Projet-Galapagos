from __future__ import annotations

import json
import zipfile
from datetime import UTC, datetime
from pathlib import Path
from typing import Any

from _bootstrap import bootstrap_src_path

bootstrap_src_path()


VERSION = "V9.45"
ZIP_NAME = "projet-galapagos-v9.45-audit-lite.zip"
ROOT = Path(".").resolve()

CORE_PATHS = [
    Path("scripts/_bootstrap.py"),
    Path("src/galapagos/features/aggtrades_exact_5y_feature_enrichment_v9_45.py"),
    Path("src/galapagos/features/aggtrades_exact_5y_feature_enrichment_v9_45_validation.py"),
    Path("src/galapagos/features/aggtrades_exact_5y_feature_enrichment_v9_45_schemas.py"),
    Path("scripts/run_aggtrades_exact_5y_feature_enrichment_v9_45.py"),
    Path("scripts/validate_aggtrades_exact_5y_feature_enrichment_v9_45.py"),
    Path("scripts/release_audit_lite_zip_v9_45.py"),
    Path("scripts/audit_audit_lite_zip_v9_45.py"),
    Path("scripts/smoke_audit_lite_zip_v9_45.py"),
    Path("tests/features/test_aggtrades_exact_5y_feature_enrichment_v9_45.py"),
    Path("tests/validation/test_aggtrades_exact_5y_feature_enrichment_v9_45_validator.py"),
    Path("reports/manifests/aggtrades_exact_5y_feature_enrichment_v9_45_manifest.json"),
    Path("reports/features/aggtrades_exact_5y_feature_enrichment_v9_45.json"),
    Path("reports/features/aggtrades_exact_5y_feature_enrichment_v9_45.md"),
    Path("docs/aggtrades_exact_5y_feature_enrichment_v9_45.md"),
]
for _tf in ["1m", "5m", "15m", "1h"]:
    CORE_PATHS.append(Path(f"reports/features/aggtrades_exact_5y_feature_enrichment_{_tf}_v9_45.json"))

PRIOR_REPORTS = [
    Path("reports/research_decisions/ohlcv_aggtrades_5y_ml_diagnostic_v9_44.json"),
    Path("reports/research_decisions/ohlcv_aggtrades_5y_ml_diagnostic_v9_44.md"),
    Path("reports/manifests/ohlcv_aggtrades_5y_ml_diagnostic_v9_44_manifest.json"),
    Path("reports/ml/ohlcv_aggtrades_5y_offline_ml_v9_43.json"),
    Path("reports/ml/ohlcv_aggtrades_5y_offline_scores_v9_43.json"),
    Path("reports/datasets/ohlcv_aggtrades_5y_dataset_validation_v9_42.json"),
    Path("reports/features/ohlcv_aggtrades_5y_feature_store_validation_v9_38.json"),
    Path("reports/data/aggtrades_5y_full_coverage_validation_v9_32.json"),
]
STATE_PATHS = [
    Path("reports/PROJECT_STATE.json"),
    Path("reports/PROJECT_STATE.md"),
    Path("reports/current/latest_metrics.json"),
    Path("reports/current/latest_metrics.md"),
    Path("reports/current/latest_summary.md"),
    Path("README.md"),
    Path("pyproject.toml"),
]
AUDIT_PATHS = [
    Path("reports/audit_lite/v9_45_command_results.json"),
    Path("reports/audit_lite/v9_45_command_results.md"),
    Path("reports/audit_lite/v9_45_full_local_validation_attestation.json"),
    Path("reports/audit_lite/v9_45_full_local_validation_attestation.md"),
    Path("reports/audit_lite/v9_45_artifact_inventory.json"),
    Path("reports/audit_lite/v9_45_artifact_inventory.md"),
    Path("reports/audit_lite/zip_size_report_v9_45.json"),
    Path("reports/audit_lite/zip_size_report_v9_45.md"),
    Path("reports/audit_lite/zip_audit_v9_45.json"),
    Path("reports/audit_lite/zip_audit_v9_45.md"),
    Path("reports/audit_lite/zip_smoke_v9_45.json"),
    Path("reports/audit_lite/zip_smoke_v9_45.md"),
]
FORBIDDEN_PREFIXES = ("data/raw/", "data/silver/", "data/research/", "data/gold/", "models/", "checkpoints/", "reports/backtests/", "reports/strategies/", "orders/", "execution/")
FORBIDDEN_SUFFIXES = {".pyc", ".pkl", ".pickle", ".joblib", ".onnx", ".pt", ".pth", ".ckpt", ".pem", ".key", ".zip", ".sha256.json", ".sha256.txt"}
FORBIDDEN_NAMES = {".DS_Store", ".env", "Icon", "Icon\r"}


def main() -> int:
    report = _read_json(Path("reports/features/aggtrades_exact_5y_feature_enrichment_v9_45.json"))
    _ensure_placeholders()
    zip_size: int | None = None
    paths: list[Path] = []
    for _ in range(20):
        _write_attestation(report, zip_size)
        paths = _collect_paths()
        _write_inventory(paths, zip_size)
        _write_size_report(paths, zip_size)
        paths = _collect_paths()
        _write_zip(paths)
        current_size = (ROOT / ZIP_NAME).stat().st_size
        if current_size == zip_size:
            break
        zip_size = current_size
    result = {"version": VERSION, "zip_name": ZIP_NAME, "zip_bytes_estimate": (ROOT / ZIP_NAME).stat().st_size, "zip_bytes_is_authoritative": False, "included_files": len(paths), "sidecars_created": False, "zip_fingerprints_created": False, "status": "PASS"}
    print(json.dumps(result, indent=2, ensure_ascii=False))
    return 0


def _ensure_placeholders() -> None:
    placeholders = {
        "reports/audit_lite/v9_45_command_results.json": {"version": VERSION, "status": "PENDING_CAPTURE", "commands": [], "sidecars_created": False, "zip_fingerprints_created": False},
        "reports/audit_lite/zip_audit_v9_45.json": {"version": VERSION, "passed": False, "errors": [], "status": "PENDING_RUN"},
        "reports/audit_lite/zip_smoke_v9_45.json": {"version": VERSION, "passed": False, "errors": [], "status": "PENDING_RUN"},
    }
    texts = {
        "reports/audit_lite/v9_45_command_results.md": "# Commandes V9.45\n\nEn attente.\n",
        "reports/audit_lite/zip_audit_v9_45.md": "# Audit ZIP V9.45\n\nEn attente.\n",
        "reports/audit_lite/zip_smoke_v9_45.md": "# Smoke ZIP V9.45\n\nEn attente.\n",
    }
    for raw, payload in placeholders.items():
        if not (ROOT / raw).exists():
            payload["created_at_utc"] = _utc_now()
            _write_json(Path(raw), payload)
    for raw, text in texts.items():
        if not (ROOT / raw).exists():
            _write_text(Path(raw), text)


def _collect_paths() -> list[Path]:
    explicit = [*CORE_PATHS, *PRIOR_REPORTS, *STATE_PATHS, *AUDIT_PATHS]
    missing = [path.as_posix() for path in CORE_PATHS if not (ROOT / path).is_file()]
    if missing:
        raise FileNotFoundError(f"missing V9.45 release inputs: {missing}")
    return sorted({path for path in explicit if (ROOT / path).is_file() and _allowed(path)}, key=lambda item: item.as_posix())


def _write_attestation(report: dict[str, Any], zip_size: int | None) -> None:
    commands = _read_optional_json(Path("reports/audit_lite/v9_45_command_results.json")).get("commands", [])
    payload = {
        "version": VERSION,
        "created_at_utc": _utc_now(),
        "zip_name": ZIP_NAME,
        "zip_bytes_estimate": zip_size,
        "zip_bytes_is_authoritative": False,
        "commands_executed": [item.get("command") for item in commands],
        "decision": report["decision"],
        "quality_status": report["quality_status"],
        "coverage_status": report["coverage_status"],
        "features_created": report["features_created"],
        "dataset_created": report["dataset_created"],
        "labels_created": report["labels_created"],
        "ml_executed": report["ml_executed"],
        "model_persisted": False,
        "walk_forward_executed": report["walk_forward_executed"],
        "backtest_executed": report["backtest_executed"],
        **report["safety_flags"],
    }
    _write_json(Path("reports/audit_lite/v9_45_full_local_validation_attestation.json"), payload)
    _write_text(Path("reports/audit_lite/v9_45_full_local_validation_attestation.md"), "# Attestation V9.45\n\n" f"- Decision : `{report['decision']}`.\n" f"- Qualite : `{report['quality_status']}`.\n" "- Feature-enrichment-only, sans label, dataset supervise, ML, modele persistant, backtest ou walk-forward.\n" "- Aucun trading, paper live, ordre, strategie, signal actionnable, API privee, cle API, reseau ou telechargement.\n" "- Aucune suppression destructive, aucun sidecar et aucune empreinte ZIP.\n")


def _write_inventory(paths: list[Path], zip_size: int | None) -> None:
    payload = {"version": VERSION, "created_at_utc": _utc_now(), "zip_name": ZIP_NAME, "zip_bytes_estimate": zip_size, "zip_bytes_is_authoritative": False, "files_count": len(paths), "files": [path.as_posix() for path in paths], "sidecars_created": False, "zip_fingerprints_created": False}
    _write_json(Path("reports/audit_lite/v9_45_artifact_inventory.json"), payload)
    _write_text(Path("reports/audit_lite/v9_45_artifact_inventory.md"), "# Inventaire audit-lite V9.45\n\n" f"- Fichiers inclus : `{len(paths)}`.\n" f"- ZIP : `{ZIP_NAME}`.\n")


def _write_size_report(paths: list[Path], zip_size: int | None) -> None:
    payload = {"version": VERSION, "created_at_utc": _utc_now(), "zip_name": ZIP_NAME, "zip_bytes_estimate": zip_size, "zip_bytes_is_authoritative": False, "files_count": len(paths), "largest_files": sorted([{"path": path.as_posix(), "bytes": (ROOT / path).stat().st_size} for path in paths], key=lambda item: item["bytes"], reverse=True)[:20], "sidecars_created": False, "zip_fingerprints_created": False}
    _write_json(Path("reports/audit_lite/zip_size_report_v9_45.json"), payload)
    _write_text(Path("reports/audit_lite/zip_size_report_v9_45.md"), "# Taille ZIP V9.45\n\n" f"- Fichiers : `{len(paths)}`.\n" f"- ZIP bytes estimate : `{zip_size}`.\n")


def _write_zip(paths: list[Path]) -> None:
    zip_path = ROOT / ZIP_NAME
    if zip_path.exists():
        zip_path.unlink()
    with zipfile.ZipFile(zip_path, "w", compression=zipfile.ZIP_DEFLATED) as archive:
        for path in paths:
            archive.write(ROOT / path, path.as_posix())


def _allowed(path: Path) -> bool:
    raw = path.as_posix()
    if any(raw.startswith(prefix) for prefix in FORBIDDEN_PREFIXES):
        return False
    if path.name in FORBIDDEN_NAMES:
        return False
    return not any(path.name.endswith(suffix) for suffix in FORBIDDEN_SUFFIXES)


def _read_json(path: Path) -> dict[str, Any]:
    return json.loads((ROOT / path).read_text(encoding="utf-8"))


def _read_optional_json(path: Path) -> dict[str, Any]:
    full = ROOT / path
    if not full.exists():
        return {}
    return json.loads(full.read_text(encoding="utf-8"))


def _write_json(path: Path, payload: dict[str, Any]) -> None:
    full = ROOT / path
    full.parent.mkdir(parents=True, exist_ok=True)
    full.write_text(json.dumps(payload, indent=2, ensure_ascii=False, sort_keys=True) + "\n", encoding="utf-8")


def _write_text(path: Path, text: str) -> None:
    full = ROOT / path
    full.parent.mkdir(parents=True, exist_ok=True)
    full.write_text(text, encoding="utf-8")


def _utc_now() -> str:
    return datetime.now(UTC).replace(microsecond=0).isoformat().replace("+00:00", "Z")


if __name__ == "__main__":
    raise SystemExit(main())
