...
def smoke_zip(zip_path: Path) -> dict[str, Any]:
    errors: list[str] = []
    with tempfile.TemporaryDirectory(prefix="galapagos_v9_17_smoke_") as tmp:
        ...
        errors.extend(_run_v9_17_tests(extract_root, env))
...
def _import_modules(extract_root: Path, env: dict[str, str]) -> list[str]:
    errors: list[str] = []
    for module in [
        "galapagos.research.derivatives_history_collection_plan_v9_17",
        "galapagos.research.derivatives_history_collection_plan_v9_17_validation",
    ]:
...
def _run_v9_17_tests(extract_root: Path, env: dict[str, str]) -> list[str]:
    completed = _run(
        [
            "python",
            "-m",
    ...
    if completed["returncode"] != 0:
        return [f"V9.17 tests failed: {completed['stdout'][-1000:]} {completed['stderr'][-1000:]}"]
    return []


def _run_self_audit(extract_root: Path, zip_path: Path, env: dict[str, str]) -> list[str]:
    completed = _run(["python", "scripts/audit_audit_lite_zip_v9_17.py", "--zip", str(zip_path)], extract_root, env, AUDIT_TIMEOUT_SECONDS)
    if completed["returncode"] != 0:
...
def _check_reports(extract_root: Path) -> list[str]:
    ...
    if report.get("features_created") is not False or report.get("dataset_created") is not False:
        errors.append("V9.17 must not create features or datasets")
    if report.get("walk_forward_executed") is not False or report.get("backtest_executed") is not False:
        errors.append("V9.17 must not execute walk-forward or backtest")
    if len(report.get("source_collection_candidates", [])) < 5:
        errors.append("V9.17 source collection candidates missing")
    ...
    if report.get("safety_flags", {}).get("network_used") is not False:
        errors.append("V9.17 must confirm no network")
    if report.get("safety_flags", {}).get("no_new_data_download") is not True:
        errors.append("V9.17 must confirm no new data download")
    if report.get("safety_flags", {}).get("no_ingestion_executed") is not True:
        errors.append("V9.17 must confirm no ingestion")
    return errors
...
def _write_reports(result: dict[str, Any]) -> None:
    ...
    (report_dir / "zip_smoke_v9_17.json").write_text(json.dumps(result, indent=2, ensure_ascii=False) + "\n", encoding="utf-8")
    (report_dir / "zip_smoke_v9_17.md").write_text(
        "# Smoke ZIP V9.17\n\n"
        ...
        f"- Timeout tests V9.17 : `{TEST_TIMEOUT_SECONDS}` secondes.\n"
