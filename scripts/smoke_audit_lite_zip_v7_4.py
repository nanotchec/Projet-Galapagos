...
VERSION = "V7.4"
SMOKE_JSON = Path("reports/audit_lite/zip_smoke_v7_4.json")
SMOKE_MD = Path("reports/audit_lite/zip_smoke_v7_4.md")
...
def smoke_zip(zip_path: Path) -> dict[str, Any]:
    ...
    with tempfile.TemporaryDirectory(prefix="galapagos_v7_4_smoke_") as tmp:
        target = Path(tmp)
...
def _check_imports(root: Path) -> list[str]:
    ...
    errors: list[str] = []
    for module in [
        "galapagos.ml.ohlcv_trades_window",
        "galapagos.ml.ohlcv_trades_window_metrics",
        "galapagos.ml.ohlcv_trades_window_quality",
        "galapagos.ml.ohlcv_trades_window_validation",
...
def _check_samples(root: Path) -> list[str]:
    ...
    expected_columns = schema_module.ML_SCORE_COLUMNS_V7_4
    forbidden = {"signal", "trading_signal", "order", "strategy", "pnl", "profit"}
    for timeframe in ["1m", "5m", "15m", "1h"]:
        path = root / "data" / "audit_lite" / "v7_4" / "ml_scores" / f"timeframe={timeframe}" / "sample.parquet"
        if not path.exists():
...
def _check_reports(root: Path) -> list[str]:
    errors: list[str] = []
    manifest_path = root / "reports/manifests/ohlcv_trades_offline_ml_research_v7_4_manifest.json"
    report_path = root / "reports/ml/ohlcv_trades_offline_ml_research_v7_4.json"
