...
VERSION = "V4.2"
REPORT_JSON = Path("reports/audit_lite/zip_smoke_v4_2.json")
REPORT_MD = Path("reports/audit_lite/zip_smoke_v4_2.md")
SAMPLES = [
...
def smoke_zip(zip_path: Path) -> dict[str, Any]:
    ...
    with tempfile.TemporaryDirectory(prefix="galapagos-v4-2-smoke-") as tmp:
            ...
            [
                ...
                "\n".join(
                    [
                        ...
                        "from galapagos.data.public_market.one_year_window_quality import EXPECTED_ROWS_V4_2",
        ...
        summary = _read_json(extract_root / "reports/audit_lite/v4_2_parquet_summary.json")
        if manifest != report:
            errors.append("V4.2 manifest/report mismatch inside audit-lite ZIP")
        if manifest.get("version") != VERSION or manifest.get("status") != "PASS":
            errors.append("V4.2 manifest status/version invalid inside audit-lite ZIP")
        ...
        if set(summary.get("outputs", {})) != {"1m", "5m", "15m", "1h"}:
            errors.append("V4.2 parquet summary outputs mismatch")
        ...
        if safety.get("public_read_only") is not True:
            errors.append("V4.2 public_read_only must be true")
        for key in [
            "authentication_used",
            "api_key_used",
            ...
            if safety.get(key) is not False:
                errors.append(f"V4.2 safety flag must be false: {key}")
...
def _render_markdown(result: dict[str, Any]) -> str:
    ...
    return f"""# Smoke ZIP audit-lite V4.2
