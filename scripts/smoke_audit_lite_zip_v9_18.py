...
def smoke_zip(zip_path: Path) -> dict[str, Any]:
    errors: list[str] = []
    with tempfile.TemporaryDirectory(prefix="galapagos_v9_18_smoke_") as tmp:
        ...
        errors.extend(_run_v9_18_tests(extract_root, env))
...
def _import_modules(extract_root: Path, env: dict[str, str]) -> list[str]:
    ...
    for module in [
        ...
        "galapagos.data.aggtrades_post_v9_collection_v9_18_validation",
    ]:
...
def _run_v9_18_tests(extract_root: Path, env: dict[str, str]) -> list[str]:
    completed = _run(
        [
            "python",
            "-m",
    ...
    if completed["returncode"] != 0:
        return [f"V9.18 tests failed: {completed['stdout'][-1000:]} {completed['stderr'][-1000:]}"]
    return []


def _run_self_audit(extract_root: Path, zip_path: Path, env: dict[str, str]) -> list[str]:
    completed = _run(["python", "scripts/audit_audit_lite_zip_v9_18.py", "--zip", str(zip_path)], extract_root, env, AUDIT_TIMEOUT_SECONDS)
    if completed["returncode"] != 0:
...
def _check_reports(extract_root: Path) -> list[str]:
    ...
    if report.get("mode") != "dry-run":
        errors.append("V9.18 smoke expects dry-run mode")
    ...
    if coverage.get("days_expected") != 772:
        errors.append("V9.18 expected day count mismatch")
    if report.get("collection_executed") is not False:
        errors.append("V9.18 must not execute collection in dry-run smoke")
    if report.get("network_used") is not False or report.get("new_data_downloaded") is not False or report.get("ingestion_executed") is not False:
        errors.append("V9.18 dry-run must not use network, download, or ingestion")
    if report.get("safety_flags", {}).get("api_key_used") is not False:
        errors.append("V9.18 must confirm no API key")
    if report.get("safety_flags", {}).get("websocket_live_used") is not False:
        errors.append("V9.18 must confirm no websocket live")
    return errors
...
def _write_reports(result: dict[str, Any]) -> None:
    ...
    (report_dir / "zip_smoke_v9_18.json").write_text(json.dumps(result, indent=2, ensure_ascii=False) + "\n", encoding="utf-8")
    (report_dir / "zip_smoke_v9_18.md").write_text(
        "# Smoke ZIP V9.18\n\n"
        ...
        f"- Timeout tests V9.18 : `{TEST_TIMEOUT_SECONDS}` secondes.\n"
