...
def smoke_zip(zip_path: Path) -> dict[str, Any]:
    errors: list[str] = []
    with tempfile.TemporaryDirectory(prefix="galapagos_v9_35_smoke_") as tmp:
        ...
        errors.extend(_run_v9_35_tests(extract_root, env))
...
def _check_parquet_engine(extract_root: Path, env: dict[str, str]) -> list[str]:
    ...
    if completed["returncode"] != 0:
        ...
        if fallback["returncode"] != 0:
            return ["pyarrow or fastparquet is required for V9.35 parquet tests; neither import succeeded"]
    return []


def _import_modules(extract_root: Path, env: dict[str, str]) -> list[str]:
    errors: list[str] = []
    for module in ["galapagos.data.ohlcv_from_aggtrades_5y_v9_35", "galapagos.data.ohlcv_from_aggtrades_5y_v9_35_validation"]:
...
def _run_pytest_collect(extract_root: Path, env: dict[str, str]) -> list[str]:
    completed = _run(["python", "-m", "pytest", "--collect-only", "-q", "tests/data/test_ohlcv_from_aggtrades_5y_v9_35.py", "tests/validation/test_ohlcv_from_aggtrades_5y_v9_35_validator.py"], extract_root, env, PYTEST_TIMEOUT_SECONDS)
    if completed["returncode"] != 0:
        return [f"pytest collect-only failed: {completed['stdout'][-1000:]} {completed['stderr'][-1000:]}"]
    return []


def _run_v9_35_tests(extract_root: Path, env: dict[str, str]) -> list[str]:
    completed = _run(["python", "-m", "pytest", "-q", "tests/data/test_ohlcv_from_aggtrades_5y_v9_35.py", "tests/validation/test_ohlcv_from_aggtrades_5y_v9_35_validator.py"], extract_root, env, TEST_TIMEOUT_SECONDS)
    if completed["returncode"] != 0:
        return [f"V9.35 tests failed: {completed['stdout'][-1000:]} {completed['stderr'][-1000:]}"]
    return []
...
def _check_reports(extract_root: Path) -> list[str]:
    ...
    if report.get("labels_created") is not False or report.get("dataset_created") is not False or report.get("ml_executed") is not False:
        errors.append("V9.35 smoke expects no labels, dataset or ML")
    return errors


def _run_self_audit(extract_root: Path, zip_path: Path, env: dict[str, str]) -> list[str]:
    completed = _run(["python", "scripts/audit_audit_lite_zip_v9_35.py", "--zip", str(zip_path)], extract_root, env, AUDIT_TIMEOUT_SECONDS)
    if completed["returncode"] != 0:
