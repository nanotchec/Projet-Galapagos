...
def main() -> None:
    ...
    with tempfile.TemporaryDirectory(prefix="galapagos_v2_8_2_smoke_") as tmp:
        tmp_path = Path(tmp)
        ...
        commands.extend(
            [
                ...
                [sys.executable, "scripts/validate_offline_ml_research_v2_8.py"],
            ]
        )
        ...
        script_check = (
            ...
            "assert manifest['correction_version']=='V2.8.2'\n"
        ...
        for command in commands:
            try:
                completed = subprocess.run(command, cwd=tmp_path, text=True, capture_output=True, timeout=120, env=env)
    ...
    Path("reports/zip_smoke_test_v2_8_2.json").write_text(json.dumps(payload, indent=2, ensure_ascii=False) + "\n", encoding="utf-8")
    Path("reports/zip_smoke_test_v2_8_2.md").write_text(
        "# Smoke ZIP V2.8.2\n\n"
