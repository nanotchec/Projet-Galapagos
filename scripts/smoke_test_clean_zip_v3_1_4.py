...
def main() -> None:
    ...
    with tempfile.TemporaryDirectory(prefix="galapagos_v3_1_4_smoke_") as tmp:
        tmp_path = Path(tmp)
    ...
    Path("reports/zip_smoke_test_v3_1_4.json").write_text(
        json.dumps(payload, indent=2, ensure_ascii=False) + "\n",
        encoding="utf-8",
    )
    Path("reports/zip_smoke_test_v3_1_4.md").write_text(
        "# Smoke ZIP V3.1.4\n\n"
...
def _validate_state_and_safety(root: Path) -> list[str]:
    ...
    if manifest.get("correction_version") is None or report.get("correction_version") is None:
        ...
        if VERSION not in summary or VERSION not in project_state:
            errors.append("V3.1.4 must be documented in PROJECT_STATE/latest_summary when correction_version is absent")
    ...
    for flag in [
        ...
        if safety.get(flag) is not False:
            errors.append(f"unsafe V3.1.4 flag active: {flag}")
    if safety.get("labels_enabled") is not True:
        errors.append("labels_enabled must be true for V3.1.4 labels context")
    return errors
