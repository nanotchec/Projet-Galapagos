...
VERSION = "V3.7"
ZIP_NAME = "projet-galapagos-v3.7-audit-lite.zip"
AUDIT_LITE_DIR = Path("reports/audit_lite")
ARTIFACT_INVENTORY_JSON = AUDIT_LITE_DIR / "v3_7_artifact_inventory.json"
ARTIFACT_INVENTORY_MD = AUDIT_LITE_DIR / "v3_7_artifact_inventory.md"
PARQUET_SUMMARY_JSON = AUDIT_LITE_DIR / "v3_7_parquet_summary.json"
ZIP_SIZE_JSON = AUDIT_LITE_DIR / "zip_size_report_v3_7.json"
ZIP_SIZE_MD = AUDIT_LITE_DIR / "zip_size_report_v3_7.md"
FULL_LOCAL_ATTESTATION_JSON = AUDIT_LITE_DIR / "v3_7_full_local_validation_attestation.json"
FULL_LOCAL_ATTESTATION_MD = AUDIT_LITE_DIR / "v3_7_full_local_validation_attestation.md"
...
V3_7_REPORT = Path("reports/labels/expanded_label_factory_v3_7.json")
...
def main() -> None:
    ...
    report_v3_7 = _read_json(root / V3_7_REPORT)
    if manifest_v3_7 != report_v3_7:
        raise RuntimeError("V3.7 manifest and report must match before audit-lite release.")

    raw_inventory = _build_raw_inventory(manifest_v3_5)
    parquet_summary, samples = _build_parquet_summary_and_samples(root, manifest_v3_7)
    full_parquet_excluded = _build_full_parquet_exclusions(root, manifest_v3_5, manifest_v3_6, manifest_v3_7)
    artifact_inventory = {
        "version": VERSION,
        ...
        "notes": [
            ...
            "V3.7 labels remain separate from V3.6 features.",
        ],
    }
    ...
    _write_text(root / ZIP_SIZE_MD, "# Rapport de taille ZIP audit-lite V3.7\n\n- Note : `audit-lite does not replace full local validation`\n")
...
def _build_parquet_summary_and_samples(root: Path, manifest_v3_7: dict[str, Any]) -> tuple[dict[str, Any], list[dict[str, Any]]]:
    summaries: dict[str, dict[str, Any]] = {}
    samples: list[dict[str, Any]] = []
    for timeframe in TIMEFRAMES:
        output = manifest_v3_7["outputs"][timeframe]
        ...
        summary = {
            ...
            "schema_strict": list(frame.columns) == LABEL_COLUMNS_V3_7,
        }
        summaries[timeframe] = summary
        sample = _sample_label_frame(frame)
        if list(sample.columns) != LABEL_COLUMNS_V3_7:
            raise RuntimeError(f"V3.7 audit-lite sample schema mismatch for {timeframe}")
        sample_path = root / "data/audit_lite/v3_7/labels" / f"timeframe={timeframe}" / "sample.parquet"
        write_parquet(sample[LABEL_COLUMNS_V3_7], sample_path)
        samples.append(
            {
                "timeframe": timeframe,
    ...
    return {
        "version": VERSION,
        "audit_lite_does_not_replace_full_validation": True,
        "label_schema": "LABEL_COLUMNS_V3_7",
        "labels": summaries,
    }, samples
...
def _build_full_parquet_exclusions(
    ...
    for label, manifest, section, schema in [
            ...
            excluded.append(
                {
                    ...
                    "reason_excluded": f"full {label} Parquet is represented by manifest metadata and/or deterministic samples in audit-lite",
                }
            )
    return excluded


def _sample_label_frame(frame: pd.DataFrame) -> pd.DataFrame:
    row_count = len(frame)
    indexes: set[int] = set()
    indexes.update(range(min(100, row_count)))
    indexes.update(range(max(0, row_count - 120), row_count))
    indexes.update(range(max(0, row_count - 10), row_count))
    middle = row_count // 2
...
def _allowed(relative: Path) -> bool:
    ...
    if relative.suffix.casefold() == ".zip":
        return False
    if relative.suffix.casefold() == ".parquet" and not name.startswith("data/audit_lite/v3_7/labels/"):
        return False
...
def _write_size_report(root: Path, *, zip_size_bytes: int, included: list[Path], artifact_inventory: dict[str, Any]) -> None:
    ...
    _write_text(
        root / ZIP_SIZE_MD,
        "# Rapport de taille ZIP audit-lite V3.7\n\n"
        f"- ZIP : `{ZIP_NAME}`\n"
...
def _render_inventory_markdown(inventory: dict[str, Any]) -> str:
    ...
    return f"""# Inventaire audit-lite V3.7
...
def _find_forbidden_columns(columns: pd.Index) -> list[str]:
    return sorted(str(column) for column in columns if any(term in str(column).casefold() for term in FORBIDDEN_LABEL_COLUMNS))
