...
VERSION = "V9.24"
ZIP_NAME = "projet-galapagos-v9.24-audit-lite.zip"

REQUIRED_FILES = [
    "scripts/_bootstrap.py",
    "scripts/run_aggtrades_post_v9_batch3_collection_v9_24.py",
    "scripts/validate_aggtrades_post_v9_batch3_collection_v9_24.py",
    "scripts/release_audit_lite_zip_v9_24.py",
    "scripts/audit_audit_lite_zip_v9_24.py",
    "scripts/smoke_audit_lite_zip_v9_24.py",
    "src/galapagos/data/aggtrades_post_v9_collection_v9_18.py",
    "src/galapagos/data/aggtrades_post_v9_batch3_collection_v9_24.py",
    "src/galapagos/data/aggtrades_post_v9_batch3_collection_v9_24_validation.py",
    "tests/data/test_aggtrades_post_v9_batch3_collection_v9_24.py",
    "tests/validation/test_aggtrades_post_v9_batch3_collection_v9_24_validator.py",
    ...
    "reports/data/aggtrades_post_v9_batch3_collection_v9_24.md",
    "docs/aggtrades_post_v9_batch3_collection_v9_24.md",
    "reports/audit_lite/v9_24_command_results.json",
    "reports/audit_lite/v9_24_command_results.md",
    "reports/audit_lite/v9_24_full_local_validation_attestation.json",
    "reports/audit_lite/v9_24_full_local_validation_attestation.md",
    "reports/audit_lite/v9_24_artifact_inventory.json",
    "reports/audit_lite/v9_24_artifact_inventory.md",
    "reports/audit_lite/zip_size_report_v9_24.json",
    "reports/audit_lite/zip_size_report_v9_24.md",
    "reports/audit_lite/zip_audit_v9_24.json",
    "reports/audit_lite/zip_audit_v9_24.md",
    "reports/audit_lite/zip_smoke_v9_24.json",
    "reports/audit_lite/zip_smoke_v9_24.md",
    "reports/data/aggtrades_post_v9_batch2_collection_v9_23.json",
    "reports/manifests/aggtrades_post_v9_batch2_collection_v9_23_manifest.json",
    ...
    "reports/manifests/aggtrades_post_v9_batch_expansion_v9_21_manifest.json",
    "reports/data/aggtrades_post_v9_batch_collection_v9_20.json",
    "reports/manifests/aggtrades_post_v9_batch_collection_v9_20_manifest.json",
...
def audit_zip(zip_path: Path) -> list[str]:
    ...
    errors: list[str] = []
    with tempfile.TemporaryDirectory(prefix="galapagos_v9_24_audit_") as tmp:
...
def _check_inventory(extract_root: Path, names: list[str]) -> list[str]:
    errors: list[str] = []
    inventory = _read_json(extract_root / "reports/audit_lite/v9_24_artifact_inventory.json")
    size_report = _read_json(extract_root / "reports/audit_lite/zip_size_report_v9_24.json")
...
def _check_reports(extract_root: Path) -> list[str]:
    errors: list[str] = []
    report = _read_json(extract_root / "reports/data/aggtrades_post_v9_batch3_collection_v9_24.json")
    manifest = _read_json(extract_root / "reports/manifests/aggtrades_post_v9_batch3_collection_v9_24_manifest.json")
    attestation = _read_json(extract_root / "reports/audit_lite/v9_24_full_local_validation_attestation.json")
    if report.get("mode") != "collect":
        errors.append("V9.24 audit-lite must describe the collect run")
    if report.get("v9_24_decision", {}).get("decision") != "aggtrades_post_v9_batch3_collection_success":
        errors.append("V9.24 decision mismatch")
    batch = report.get("batch_window", {})
    if batch.get("batch_id") != "V9.24_batch_03" or batch.get("start") != "2024-10-09" or batch.get("end") != "2024-12-07":
        errors.append("V9.24 batch window mismatch")
    if batch.get("max_downloads") != 60 or batch.get("days_requested") != 60:
        errors.append("V9.24 batch must be limited to 60 days")
    summary = report.get("batch_validation", {}).get("summary", {})
    if summary.get("days_requested") != 60 or summary.get("days_attempted") != 60:
        errors.append("V9.24 summary day counts mismatch")
    if summary.get("days_downloaded") != 60 or summary.get("days_normalized") != 60 or summary.get("days_complete") != 60:
        errors.append("V9.24 must report 60 downloaded/normalized/complete days")
    if summary.get("days_failed") != 0 or summary.get("days_quarantined") != 0:
        errors.append("V9.24 must report no failed or quarantined days")
    if summary.get("total_rows", 0) <= 0 or summary.get("raw_bytes_total", 0) <= 0 or summary.get("silver_bytes_total", 0) <= 0:
        errors.append("V9.24 batch metrics must be positive")
    if not isinstance(summary.get("aggregate_trade_id_gap_warnings"), list):
        errors.append("V9.24 aggregate_trade_id gap warnings must be present as a list")
    reported = report.get("reported_cumulative_coverage", {})
    local = report.get("local_file_coverage", {})
    if reported.get("reported_cumulative_coverage_start") != "2024-05-05" or reported.get("reported_cumulative_coverage_end") != "2024-12-07":
        errors.append("V9.24 reported cumulative coverage mismatch")
    if local.get("local_file_coverage_start") != "2024-05-05" or local.get("local_file_coverage_end") != "2024-12-07":
        errors.append("V9.24 local file coverage mismatch")
    if report.get("audit_lite_mode_handling", {}).get("avoid_contradictory_regeneration_without_full_data") is not True:
        errors.append("V9.24 must document audit-lite no-regeneration handling")
    if report.get("complete_collection_reached") is not False:
        errors.append("V9.24 must not claim complete future coverage")
    for key in ["collection_executed", "network_used", "new_data_downloaded", "ingestion_executed"]:
        if report.get(key) is not True:
            errors.append(f"V9.24 report must keep {key}=true")
    for key in ["features_created", "labels_created", "dataset_created", "ml_executed", "walk_forward_executed", "backtest_executed"]:
        if report.get(key) is not False:
            errors.append(f"V9.24 report must keep {key}=false")
    source = report.get("source_public_target", {})
    if source.get("host") != "data.binance.vision":
        errors.append("V9.24 source host mismatch")
    ...
    if attestation.get("network_used") is not True or attestation.get("no_new_data_download") is not False or attestation.get("no_ingestion_executed") is not False:
        errors.append("attestation must confirm public batch network, new download and ingestion")
...
def _check_command_results(extract_root: Path) -> list[str]:
    errors: list[str] = []
    payload = _read_json(extract_root / "reports/audit_lite/v9_24_command_results.json")
    ...
    required_needles = [
        ...
        "test_aggtrades_post_v9_batch3_collection_v9_24.py",
        "test_aggtrades_post_v9_batch3_collection_v9_24_validator.py",
        "run_aggtrades_post_v9_batch3_collection_v9_24.py",
        "validate_aggtrades_post_v9_batch3_collection_v9_24.py",
        "release_audit_lite_zip_v9_24.py",
    ]
...
def _check_state(extract_root: Path) -> list[str]:
    ...
    for name, payload in {"PROJECT_STATE": state, "latest_metrics": metrics}.items():
        if payload.get("last_validated_version") != "V9.23":
        ...
        if payload.get("direction") != "aggtrades_post_v9_batch3_collection":
        ...
        if payload.get("network_used") is not True or payload.get("no_new_data_download") is not False or payload.get("no_ingestion_executed") is not False:
            errors.append(f"{name} must confirm public batch network, new download and ingestion")
...
def _write_reports(result: dict[str, Any]) -> None:
    ...
    (report_dir / "zip_audit_v9_24.json").write_text(json.dumps(result, indent=2, ensure_ascii=False) + "\n", encoding="utf-8")
    (report_dir / "zip_audit_v9_24.md").write_text(
        "# Audit ZIP V9.24\n\n"
