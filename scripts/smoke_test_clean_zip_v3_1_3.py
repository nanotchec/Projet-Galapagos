...
VALIDATORS = [
    ("validate_public_market_ingestion_v2_3", "scripts/validate_public_market_ingestion_v2_3.py", 60),
    ("validate_ohlcv_resampling_v2_4", "scripts/validate_ohlcv_resampling_v2_4.py", 60),
    ("validate_causal_feature_store_v2_5", "scripts/validate_causal_feature_store_v2_5.py", 60),
    ("validate_label_factory_v2_6", "scripts/validate_clean_label_factory_v2_6.py", 60),
    ("validate_offline_supervised_dataset_v2_7", "scripts/validate_offline_supervised_dataset_v2_7.py", 60),
...
def main() -> None:
    ...
    with tempfile.TemporaryDirectory(prefix="galapagos_v3_1_3_smoke_") as tmp:
        tmp_path = Path(tmp)
        ...
        errors.extend(_validate_label_outputs(tmp_path))
        errors.extend(_validate_state_and_safety(tmp_path))
        validator_errors, validator_timings = _run_validators(tmp_path)
    ...
    Path("reports/zip_smoke_test_v3_1_3.json").write_text(
        json.dumps(payload, indent=2, ensure_ascii=False) + "\n",
        encoding="utf-8",
    )
    Path("reports/zip_smoke_test_v3_1_3.md").write_text(
        "# Smoke ZIP V3.1.3\n\n"
...
def _validate_state_and_safety(root: Path) -> list[str]:
    ...
    if manifest.get("correction_version") is None or report.get("correction_version") is None:
        ...
        if VERSION not in summary or VERSION not in project_state:
            errors.append("V3.1.3 must be documented in PROJECT_STATE/latest_summary when correction_version is absent")
    ...
    for flag in [
        ...
        if safety.get(flag) is not False:
            errors.append(f"unsafe V3.1.3 flag active: {flag}")
    if safety.get("labels_enabled") is not True:
        errors.append("labels_enabled must be true for V3.1.3 labels context")
    return errors


def _run_validators(root: Path) -> tuple[list[str], dict[str, float]]:
    errors: list[str] = []
    ...
    for name, script, timeout_seconds in VALIDATORS:
        ...
        log_path = root / f".smoke-{name}.log"
