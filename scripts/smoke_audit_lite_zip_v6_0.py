...
VERSION = "V6.0"
SMOKE_JSON = Path("reports/audit_lite/zip_smoke_v6_0.json")
SMOKE_MD = Path("reports/audit_lite/zip_smoke_v6_0.md")
...
def smoke_zip(zip_path: Path) -> dict[str, Any]:
    ...
    with tempfile.TemporaryDirectory(prefix="galapagos_v6_0_smoke_") as tmp:
        target = Path(tmp)
...
def _check_imports(root: Path) -> list[str]:
    ...
    errors: list[str] = []
    for module in [
        "galapagos.features.advanced_ohlcv",
        "galapagos.features.advanced_ohlcv_quality",
        "galapagos.features.advanced_ohlcv_schemas",
        "galapagos.features.advanced_ohlcv_validation",
    ]:
        try:
...
def _check_samples(root: Path) -> list[str]:
    errors: list[str] = []
    schema_module = importlib.import_module("galapagos.features.advanced_ohlcv_schemas")
    expected_columns = schema_module.ADVANCED_OHLCV_FEATURE_COLUMNS_V6_0
    for timeframe in ["1m", "5m", "15m", "1h"]:
        path = root / "data" / "audit_lite" / "v6_0" / "features" / f"timeframe={timeframe}" / "sample.parquet"
        if not path.exists():
            errors.append(f"missing V6.0 feature sample for {timeframe}")
            continue
        frame = pd.read_parquet(path, engine="pyarrow")
        if list(frame.columns) != expected_columns:
            errors.append(f"V6.0 feature sample schema mismatch for {timeframe}")
...
def _check_reports(root: Path) -> list[str]:
    errors: list[str] = []
    manifest_path = root / "reports/manifests/advanced_ohlcv_feature_store_v6_0_manifest.json"
    report_path = root / "reports/features/advanced_ohlcv_feature_store_v6_0.json"
    if manifest_path.exists() and report_path.exists():
        manifest = json.loads(manifest_path.read_text(encoding="utf-8"))
        report = json.loads(report_path.read_text(encoding="utf-8"))
        if manifest != report:
            errors.append("smoke V6.0 manifest/report mismatch")
        safety = manifest.get("safety", {})
        for key in FORBIDDEN_SAFETY_TRUE:
            if safety.get(key) is not False:
                errors.append(f"smoke V6.0 safety flag must be false: {key}")
    return errors
