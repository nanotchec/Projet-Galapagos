...
VERSION = "V7.7"
SMOKE_JSON = Path("reports/audit_lite/zip_smoke_v7_7.json")
SMOKE_MD = Path("reports/audit_lite/zip_smoke_v7_7.md")
...
def smoke_zip(zip_path: Path) -> dict[str, Any]:
    ...
    with tempfile.TemporaryDirectory(prefix="galapagos_v7_7_smoke_") as tmp:
        target = Path(tmp)
...
def _check_imports() -> list[str]:
    ...
    for module in [
        ...
        "galapagos.data.public_trades.ninety_day_window",
        "galapagos.data.public_trades.ninety_day_window_quality",
        "galapagos.data.public_trades.ninety_day_window_validation",
    ]:
        try:
...
def _check_reports(root: Path) -> list[str]:
    errors: list[str] = []
    manifest_path = root / "reports/manifests/public_trades_90d_window_v7_7_manifest.json"
    report_path = root / "reports/data_quality/public_trades_90d_window_v7_7.json"
    discovery_path = root / "reports/data_quality/public_trades_90d_window_v7_7_discovery.json"
    for path in [manifest_path, report_path, discovery_path]:
        if not path.exists():
            errors.append(f"missing V7.7 report in ZIP: {path.relative_to(root).as_posix()}")
    if manifest_path.exists() and report_path.exists():
        manifest = json.loads(manifest_path.read_text(encoding="utf-8"))
        report = json.loads(report_path.read_text(encoding="utf-8"))
        if manifest != report:
            errors.append("smoke V7.7 manifest/report mismatch")
        if manifest.get("version") != VERSION or manifest.get("status") != "PASS":
            errors.append("smoke V7.7 manifest version/status mismatch")
        safety = manifest.get("safety", {})
        for key in FORBIDDEN_SAFETY_TRUE:
            if safety.get(key) is not False:
                errors.append(f"smoke V7.7 safety flag must be false: {key}")
    return errors
...
def _check_sample(root: Path) -> list[str]:
    ...
    if not path.exists():
        return [f"missing V7.7 trades sample: {SAMPLE_ENTRY}"]
    frame = pd.read_parquet(path, engine="pyarrow")
    if list(frame.columns) != schema_module.AGG_TRADE_COLUMNS_V7_7:
        errors.append("V7.7 trades sample schema mismatch")
    if len(frame) == 0:
        errors.append("V7.7 trades sample is empty")
