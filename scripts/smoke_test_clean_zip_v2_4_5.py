...
def main() -> None:
    ...
    errors: list[str] = []
    commands = []
    with tempfile.TemporaryDirectory(prefix="galapagos_v2_4_5_smoke_") as tmp:
        tmp_path = Path(tmp)
        ...
        commands.append(
            [
                ...
                (
                    ...
                    "OHLCV_COLUMNS = ['source', 'venue', 'market_type', 'symbol', 'timeframe', 'event_ts', 'close_ts', 'available_ts', 'decision_ts', 'ingested_at_ts', 'open', 'high', 'low', 'close', 'volume', 'quote_volume', 'trade_count', 'taker_buy_base_volume', 'taker_buy_quote_volume', 'source_open_time_raw', 'source_close_time_raw', 'source_timestamp_unit', 'raw_file_sha256', 'ingestion_run_id']\n"
                    f"paths={PARQUETS!r}\n"
                    ...
                    "assert manifest['correction_version']=='V2.4.5'\n"
                    "assert report['correction_version']=='V2.4.5'\n"
    ...
    payload = {
        ...
        "bounded_smoke_for_v2_4_5": True,
        "errors": errors,
    }
    Path("reports").mkdir(exist_ok=True)
    Path("reports/zip_smoke_test_v2_4_5.json").write_text(json.dumps(payload, indent=2, ensure_ascii=False) + "\n", encoding="utf-8")
    Path("reports/zip_smoke_test_v2_4_5.md").write_text(
        "# Smoke ZIP V2.4.5\n\n"
