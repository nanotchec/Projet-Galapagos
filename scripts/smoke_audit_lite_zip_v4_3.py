...
VERSION = "V4.3"
REPORT_JSON = Path("reports/audit_lite/zip_smoke_v4_3.json")
REPORT_MD = Path("reports/audit_lite/zip_smoke_v4_3.md")
SAMPLES = [
...
def smoke_zip(zip_path: Path) -> dict[str, Any]:
    ...
    with tempfile.TemporaryDirectory(prefix="galapagos-v4-3-smoke-") as tmp:
            ...
            [
                ...
                "\n".join(
                    [
                        ...
                        "assert EXPECTED_ROWS_V4_3['1m'] == 527040",
                        "print('imports_ok')",
                    ]
                ),
            ],
        ...
        if manifest != report:
            errors.append("V4.3 manifest/report mismatch inside audit-lite ZIP")
        if manifest.get("version") != VERSION or manifest.get("status") != "PASS":
            errors.append("V4.3 manifest status/version invalid inside audit-lite ZIP")
        ...
        if set(summary.get("features", {})) != {"1m", "5m", "15m", "1h"}:
            errors.append("V4.3 parquet summary features mismatch")
        ...
        if safety.get("public_read_only") is not True:
            errors.append("V4.3 public_read_only must be true")
        for key in [
            "authentication_used",
            "api_key_used",
            ...
            if safety.get(key) is not False:
                errors.append(f"V4.3 safety flag must be false: {key}")
...
def _render_markdown(result: dict[str, Any]) -> str:
    ...
    return f"""# Smoke ZIP audit-lite V4.3
