...
def main() -> None:
    ...
    with tempfile.TemporaryDirectory(prefix="galapagos_v3_1_2_smoke_") as tmp:
        tmp_path = Path(tmp)
        ...
        for item in essential:
            if item not in names:
                errors.append(f"missing essential entry: {item}")
        ...
        try:
            ...
            validators = [
                ...
                ("validate_label_factory_v2_6", validate_label_factory_v2_6),
                ...
                ("validate_multi_day_public_market_data_v2_9", validate_multi_day_public_market_data_v2_9),
                ("validate_multi_day_causal_feature_store_v3_0", validate_multi_day_causal_feature_store_v3_0),
                ("validate_multi_day_label_factory_v3_1", validate_multi_day_label_factory_v3_1),
            ]
            for name, validator in validators:
                ...
                validator_timings[name] = round(elapsed, 3)
                print(f"passed {name} in {elapsed:.3f}s", flush=True)
                validators_run += 1
                if not result["passed"]:
                    errors.append(f"{name} failed: {result['errors']}")
                if elapsed > step_timeout_seconds:
                    errors.append(f"{name} exceeded {step_timeout_seconds:.0f}s: {elapsed:.3f}s")
            ...
            for flag in ["trading_enabled", "backtest_enabled", "orders_enabled", "strategy_enabled", "execution_enabled", "dataset_enabled", "ml_enabled"]:
                if safety.get(flag) is not False:
                    errors.append(f"unsafe V3.1.2 flag active: {flag}")
            if safety.get("labels_enabled") is not True:
                errors.append("labels_enabled must be true for V3.1.2 labels context")
    ...
    Path("reports/zip_smoke_test_v3_1_2.json").write_text(json.dumps(payload, indent=2, ensure_ascii=False) + "\n", encoding="utf-8")
    Path("reports/zip_smoke_test_v3_1_2.md").write_text(
        "# Smoke ZIP V3.1.2\n\n"
