...
def smoke_zip(zip_path: Path) -> dict[str, Any]:
    ...
    with tempfile.TemporaryDirectory(prefix="galapagos_v9_39_smoke_") as tmp:
        ...
        errors.extend(_run_v9_39_tests(extract_root, env))
...
def _import_modules(extract_root: Path, env: dict[str, str]) -> list[str]:
    errors: list[str] = []
    for module in ["galapagos.datasets.ohlcv_aggtrades_5y_dataset_v9_39", "galapagos.datasets.ohlcv_aggtrades_5y_dataset_v9_39_validation"]:
...
def _run_pytest_collect(extract_root: Path, env: dict[str, str]) -> list[str]:
    completed = _run(["python", "-m", "pytest", "--collect-only", "-q", "tests/datasets/test_ohlcv_aggtrades_5y_dataset_v9_39.py", "tests/validation/test_ohlcv_aggtrades_5y_dataset_v9_39_validator.py"], extract_root, env, PYTEST_TIMEOUT_SECONDS)
    if completed["returncode"] != 0:
        return [f"pytest collect-only failed: {completed['stdout'][-1000:]} {completed['stderr'][-1000:]}"]
    return []


def _run_v9_39_tests(extract_root: Path, env: dict[str, str]) -> list[str]:
    completed = _run(["python", "-m", "pytest", "-q", "tests/datasets/test_ohlcv_aggtrades_5y_dataset_v9_39.py", "tests/validation/test_ohlcv_aggtrades_5y_dataset_v9_39_validator.py"], extract_root, env, TEST_TIMEOUT_SECONDS)
    if completed["returncode"] != 0:
        return [f"V9.39 tests failed: {completed['stdout'][-1000:]} {completed['stderr'][-1000:]}"]
    return []
...
def _check_reports(extract_root: Path) -> list[str]:
    ...
    if report.get("version") != VERSION:
        errors.append("V9.39 report version mismatch")
    if report.get("dataset_created") is not False:
        errors.append("V9.39 smoke expects no dataset when labels are missing")
    if report.get("network_used") is not False or report.get("new_data_downloaded") is not False:
        errors.append("V9.39 smoke expects no network and no downloads")
    if report.get("ml_executed") is not False or report.get("backtest_executed") is not False:
        errors.append("V9.39 smoke expects no ML or backtest")
    return errors


def _run_self_audit(extract_root: Path, zip_path: Path, env: dict[str, str]) -> list[str]:
    completed = _run(["python", "scripts/audit_audit_lite_zip_v9_39.py", "--zip", str(zip_path)], extract_root, env, AUDIT_TIMEOUT_SECONDS)
    if completed["returncode"] != 0:
