...
VERSION = "V7.2"
SMOKE_JSON = Path("reports/audit_lite/zip_smoke_v7_2.json")
SMOKE_MD = Path("reports/audit_lite/zip_smoke_v7_2.md")
SAMPLE_ENTRIES = {
    "data/audit_lite/v7_2/features/timeframe=1m/sample.parquet",
    "data/audit_lite/v7_2/features/timeframe=5m/sample.parquet",
    "data/audit_lite/v7_2/features/timeframe=15m/sample.parquet",
    "data/audit_lite/v7_2/features/timeframe=1h/sample.parquet",
}
...
def smoke_zip(zip_path: Path) -> dict[str, Any]:
    ...
    with tempfile.TemporaryDirectory(prefix="galapagos_v7_2_smoke_") as tmp:
        target = Path(tmp)
...
def _check_imports() -> list[str]:
    errors: list[str] = []
    for module in [
        "galapagos.features.ohlcv_trades",
        "galapagos.features.ohlcv_trades_quality",
        "galapagos.features.ohlcv_trades_schemas",
        "galapagos.features.ohlcv_trades_validation",
    ]:
        try:
...
def _check_reports(root: Path) -> list[str]:
    errors: list[str] = []
    manifest_path = root / "reports/manifests/ohlcv_trades_feature_store_v7_2_manifest.json"
    report_path = root / "reports/features/ohlcv_trades_feature_store_v7_2.json"
    for path in [manifest_path, report_path]:
        if not path.exists():
            errors.append(f"missing V7.2 report in ZIP: {path.relative_to(root).as_posix()}")
    if manifest_path.exists() and report_path.exists():
        manifest = json.loads(manifest_path.read_text(encoding="utf-8"))
        report = json.loads(report_path.read_text(encoding="utf-8"))
        if manifest != report:
            errors.append("smoke V7.2 manifest/report mismatch")
        if manifest.get("version") != VERSION or manifest.get("status") != "PASS":
            errors.append("smoke V7.2 manifest version/status mismatch")
        safety = manifest.get("safety", {})
        for key in FORBIDDEN_SAFETY_TRUE:
            if safety.get(key) is not False:
                errors.append(f"smoke V7.2 safety flag must be false: {key}")
    return errors


def _check_samples(root: Path) -> list[str]:
    errors: list[str] = []
    schema_module = importlib.import_module("galapagos.features.ohlcv_trades_schemas")
    for sample_entry in SAMPLE_ENTRIES:
        ...
        if list(frame.columns) != schema_module.OHLCV_TRADES_FEATURE_COLUMNS_V7_2:
