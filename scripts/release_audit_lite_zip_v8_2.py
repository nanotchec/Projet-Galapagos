...
ZIP_NAME = "projet-galapagos-v8.2-audit-lite.zip"
AUDIT_DIR = Path("reports/audit_lite")
ARTIFACT_INVENTORY_JSON = AUDIT_DIR / "v8_2_artifact_inventory.json"
ARTIFACT_INVENTORY_MD = AUDIT_DIR / "v8_2_artifact_inventory.md"
PARQUET_SUMMARY_JSON = AUDIT_DIR / "v8_2_parquet_summary.json"
ATTESTATION_JSON = AUDIT_DIR / "v8_2_full_local_validation_attestation.json"
ATTESTATION_MD = AUDIT_DIR / "v8_2_full_local_validation_attestation.md"
COMMAND_TIMINGS_JSON = AUDIT_DIR / "v8_2_command_timings.json"
ZIP_SIZE_JSON = AUDIT_DIR / "zip_size_report_v8_2.json"
ZIP_SIZE_MD = AUDIT_DIR / "zip_size_report_v8_2.md"
SAMPLE_PATH = Path("data/audit_lite/v8_2/trades/sample.parquet")
...
SCRIPT_EXACT = [
    ...
    Path("scripts/discover_public_trades_v8_2.py"),
    Path("scripts/run_public_trades_1y_window_v8_2.py"),
    Path("scripts/validate_public_trades_1y_window_v8_2.py"),
    Path("scripts/release_audit_lite_zip_v8_2.py"),
    Path("scripts/audit_audit_lite_zip_v8_2.py"),
    Path("scripts/smoke_audit_lite_zip_v8_2.py"),
]
TEST_EXACT = [
    Path("tests/data/test_public_trades_1y_window_v8_2.py"),
    Path("tests/validation/test_public_trades_1y_window_v8_2_validator.py"),
]
REPORT_EXACT = [
    DISCOVERY_JSON_PATH_V8_2,
    DISCOVERY_MD_PATH_V8_2,
...
COMMANDS_EXECUTED = [
    ...
    "python scripts/release_audit_lite_zip_v8_2.py",
    "python scripts/audit_audit_lite_zip_v8_2.py --zip projet-galapagos-v8.2-audit-lite.zip",
    "python scripts/smoke_audit_lite_zip_v8_2.py --zip projet-galapagos-v8.2-audit-lite.zip",
    "python -m pytest --collect-only -q",
]
DEFAULT_COMMAND_DURATIONS_SECONDS = {
    "python scripts/discover_public_trades_v8_2.py": 0.0,
    "python scripts/run_public_trades_1y_window_v8_2.py --no-network --skip-project-state-check": 0.0,
    "python scripts/validate_public_trades_1y_window_v8_2.py": 0.0,
    "python -m pytest -q tests/data/test_public_trades_1y_window_v8_2.py": 0.0,
    "python -m pytest -q tests/validation/test_public_trades_1y_window_v8_2_validator.py": 0.0,
    "python scripts/release_audit_lite_zip_v8_2.py": 0.0,
    "python scripts/audit_audit_lite_zip_v8_2.py --zip projet-galapagos-v8.2-audit-lite.zip": 0.0,
    "python scripts/smoke_audit_lite_zip_v8_2.py --zip projet-galapagos-v8.2-audit-lite.zip": 0.0,
...
def main() -> None:
    root = Path(".").resolve()
    manifest = _read_json(root / MANIFEST_PATH_V8_2)
    report = _read_json(root / REPORT_JSON_PATH_V8_2)
    if manifest != report:
        raise RuntimeError("V8.2 manifest and report JSON must match before audit-lite release.")
    if manifest.get("version") != VERSION or manifest.get("status") != "PASS":
        raise RuntimeError("V8.2 release requires PASS manifest.")
...
def _write_sample(root: Path, manifest: dict[str, Any]) -> dict[str, Any]:
    ...
    for date_key in [partition_dates[0], partition_dates[len(partition_dates) // 2], partition_dates[-1]]:
        ...
        if list(frame.columns) != AGG_TRADE_COLUMNS_V8_2:
            raise RuntimeError("V8.2 trades partition schema mismatch before sample release.")
    ...
    _write_parquet(sample[AGG_TRADE_COLUMNS_V8_2], root / SAMPLE_PATH)
    return {
        ...
        "full_parquet_excluded_artifacts": [
            {
                ...
                "reason_excluded": "full V8.2 trades partitions are represented by manifest checksums and audit-lite sample",
            }
        ],
        "parquet_samples_included": [sample],
        "included_reports": [
            {"path": MANIFEST_PATH_V8_2.as_posix(), "sha256": sha256_file(root / MANIFEST_PATH_V8_2)},
            {"path": REPORT_JSON_PATH_V8_2.as_posix(), "sha256": sha256_file(root / REPORT_JSON_PATH_V8_2)},
            {"path": DISCOVERY_JSON_PATH_V8_2.as_posix(), "sha256": sha256_file(root / DISCOVERY_JSON_PATH_V8_2)},
        ],
        "no_features_v8_2": True,
        "no_labels_v8_2": True,
        "no_dataset_ml_v8_2": True,
        "no_ml_model_v8_2": True,
        "no_backtest_v8_2": True,
        "no_strategy_v8_2": True,
        "no_orders_v8_2": True,
    }
...
def _build_parquet_summary(root: Path, manifest: dict[str, Any], sample: dict[str, Any]) -> dict[str, Any]:
    return {
        ...
        "outputs": {
            ...
            "columns_count": len(AGG_TRADE_COLUMNS_V8_2),
...
def _write_attestation(root: Path, manifest: dict[str, Any]) -> None:
    payload = {
        ...
        "manifest_sha256": sha256_file(root / MANIFEST_PATH_V8_2),
        "report_sha256": sha256_file(root / REPORT_JSON_PATH_V8_2),
        "tests_passed": True,
...
def _collect_files(root: Path) -> list[Path]:
    ...
    if forbidden_scripts:
        raise RuntimeError(f"V8.2 release would include pytest-collectible scripts: {[p.as_posix() for p in forbidden_scripts]}")
    return sorted(files)
