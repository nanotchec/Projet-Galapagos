...
VERSION = "V3.9"
REPORT_JSON = Path("reports/audit_lite/zip_smoke_v3_9.json")
REPORT_MD = Path("reports/audit_lite/zip_smoke_v3_9.md")
...
def smoke_zip(zip_path: Path) -> dict[str, Any]:
    ...
    with tempfile.TemporaryDirectory(prefix="galapagos-v3-9-smoke-") as tmp:
        ...
        for entry in entries:
            ...
            if entry.endswith(".parquet") and not entry.startswith("data/audit_lite/v3_9/ml_scores/"):
        ...
        report = _read_json(extract_root / "reports/ml/expanded_offline_ml_research_v3_9.json")
        parquet_summary = _read_json(extract_root / "reports/audit_lite/v3_9_parquet_summary.json")
        attestation = _read_json(extract_root / "reports/audit_lite/v3_9_full_local_validation_attestation.json")
        if manifest != report:
            errors.append("V3.9 manifest/report mismatch inside audit-lite ZIP")
        ...
        for key in expected_false:
            if safety.get(key) is not False:
                errors.append(f"V3.9 safety flag must be false: {key}")
        for key in ["public_read_only", "ml_enabled", "labels_enabled", "dataset_enabled"]:
            if safety.get(key) is not True:
                errors.append(f"V3.9 safety flag must be true: {key}")
        ...
        for timeframe in ["1m", "5m", "15m", "1h"]:
            sample = read_parquet(extract_root / f"data/audit_lite/v3_9/ml_scores/timeframe={timeframe}/sample.parquet")
...
def _render_markdown(result: dict[str, Any]) -> str:
    ...
    return f"""# Smoke ZIP audit-lite V3.9
