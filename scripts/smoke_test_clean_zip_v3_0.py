...
OUTPUTS = {
    timeframe: f"data/research/v3_0/features/ohlcv/source=binance_archive/market_type=spot/symbol=BTCUSDT/timeframe={timeframe}/window=2024-01-15_2024-01-21/features.parquet"
...
def main() -> None:
    ...
    errors: list[str] = []
    validators_run = 0
    with tempfile.TemporaryDirectory(prefix="galapagos_v3_0_smoke_") as tmp:
        tmp_path = Path(tmp)
        ...
        try:
            ...
            manifest = json.loads(Path("reports/manifests/multi_day_causal_feature_store_v3_0_manifest.json").read_text(encoding="utf-8"))
            for timeframe, path in OUTPUTS.items():
                frame = pd.read_parquet(path)
                if list(frame.columns) != FEATURE_COLUMNS_V3_0:
                    errors.append(f"feature schema mismatch: {timeframe}")
            ...
            for flag in ["trading_enabled", "backtest_enabled", "orders_enabled", "strategy_enabled", "execution_enabled", "labels_enabled", "dataset_enabled", "ml_enabled"]:
                if safety.get(flag) is not False:
                    errors.append(f"unsafe V3.0 flag active: {flag}")
    ...
    Path("reports/zip_smoke_test_v3_0.json").write_text(json.dumps(payload, indent=2, ensure_ascii=False) + "\n", encoding="utf-8")
    Path("reports/zip_smoke_test_v3_0.md").write_text(
        "# Smoke ZIP V3.0\n\n"
