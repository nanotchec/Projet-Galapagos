...
from galapagos.ml.ohlcv_trades_90d_window import score_output_path
...
VERSION = "V8.0"
ZIP_NAME = "projet-galapagos-v8.0-audit-lite.zip"
AUDIT_DIR = Path("reports/audit_lite")
ARTIFACT_INVENTORY_JSON = AUDIT_DIR / "v8_0_artifact_inventory.json"
ARTIFACT_INVENTORY_MD = AUDIT_DIR / "v8_0_artifact_inventory.md"
PARQUET_SUMMARY_JSON = AUDIT_DIR / "v8_0_parquet_summary.json"
ATTESTATION_JSON = AUDIT_DIR / "v8_0_full_local_validation_attestation.json"
ATTESTATION_MD = AUDIT_DIR / "v8_0_full_local_validation_attestation.md"
ZIP_SIZE_JSON = AUDIT_DIR / "zip_size_report_v8_0.json"
ZIP_SIZE_MD = AUDIT_DIR / "zip_size_report_v8_0.md"
ZIP_AUDIT_JSON = AUDIT_DIR / "zip_audit_v8_0.json"
ZIP_AUDIT_MD = AUDIT_DIR / "zip_audit_v8_0.md"
ZIP_SMOKE_JSON = AUDIT_DIR / "zip_smoke_v8_0.json"
ZIP_SMOKE_MD = AUDIT_DIR / "zip_smoke_v8_0.md"
COMMAND_TIMINGS_JSON = AUDIT_DIR / "v8_0_command_timings.json"
SAMPLE_ROOT = Path("data/audit_lite/v8_0/ml_scores")
SOURCE_PREFIXES = [
...
SCRIPT_EXACT = [
    ...
    Path("scripts/run_ohlcv_trades_90d_offline_ml_research_v8_0.py"),
    Path("scripts/validate_ohlcv_trades_90d_offline_ml_research_v8_0.py"),
    Path("scripts/run_ohlcv_trades_90d_ml_robustness_v8_0.py"),
    Path("scripts/validate_ohlcv_trades_90d_ml_robustness_v8_0.py"),
    Path("scripts/release_audit_lite_zip_v8_0.py"),
    Path("scripts/audit_audit_lite_zip_v8_0.py"),
    Path("scripts/smoke_audit_lite_zip_v8_0.py"),
]
TEST_EXACT = [
    Path("tests/ml/test_ohlcv_trades_90d_offline_ml_research_v8_0.py"),
    Path("tests/ml/test_ohlcv_trades_90d_ml_robustness_v8_0.py"),
    Path("tests/validation/test_ohlcv_trades_90d_offline_ml_research_v8_0_validator.py"),
    Path("tests/validation/test_ohlcv_trades_90d_ml_robustness_v8_0_validator.py"),
]
REPORT_EXACT = [
    MANIFEST_PATH_V8_0,
    REPORT_JSON_PATH_V8_0,
...
def main() -> None:
    root = Path(".").resolve()
    manifest = _read_json(root / MANIFEST_PATH_V8_0)
    report = _read_json(root / REPORT_JSON_PATH_V8_0)
    robustness_manifest = _read_json(root / ROBUSTNESS_MANIFEST_PATH_V8_0)
    robustness_report = _read_json(root / ROBUSTNESS_REPORT_JSON_PATH_V8_0)
    if manifest != report:
        raise RuntimeError("V8.0 manifest and report JSON must match before audit-lite release.")
    if robustness_manifest != robustness_report:
        raise RuntimeError("V8.0 robustness manifest and report JSON must match before audit-lite release.")
    samples = _write_samples(root, manifest)
    inventory = _build_inventory(root, manifest, robustness_manifest, samples)
    ...
    _write_attestation(root, manifest, robustness_manifest)
...
def _write_samples(root: Path, manifest: dict[str, Any]) -> list[dict[str, Any]]:
    ...
    for timeframe in TIMEFRAMES_V8_0:
        ...
        if list(scores.columns) != ML_SCORE_COLUMNS_V8_0:
            raise RuntimeError(f"V8.0 full score schema mismatch for {timeframe}")
        ...
        write_parquet(sample[ML_SCORE_COLUMNS_V8_0], root / sample_path)
        samples.append(
            {
                "timeframe": timeframe,
...
def _build_inventory(root: Path, manifest: dict[str, Any], robustness_manifest: dict[str, Any], samples: list[dict[str, Any]]) -> dict[str, Any]:
    return {
        "version": VERSION,
        "audit_lite_does_not_replace_full_validation": True,
        "raw_zips_excluded": True,
        ...
        "robustness_manifest": {
            "path": ROBUSTNESS_MANIFEST_PATH_V8_0.as_posix(),
            "sha256": sha256_file(root / ROBUSTNESS_MANIFEST_PATH_V8_0),
            "status": robustness_manifest["status"],
        },
        "input_datasets_full_parquet_excluded": [
            {"timeframe": timeframe, **payload, "reason_excluded": "full V7.9 dataset Parquet is represented by source hashes"}
            for timeframe, payload in sorted(manifest["input_datasets"].items())
        ],
        "input_splits_full_parquet_excluded": [
            {"timeframe": timeframe, **payload, "reason_excluded": "full V7.9 split Parquet is represented by source hashes"}
            for timeframe, payload in sorted(manifest["input_splits"].items())
        ],
        "full_parquet_excluded": [
            {"timeframe": timeframe, **payload, "reason_excluded": "full V8.0 score Parquet is represented by manifest checksums and deterministic samples"}
            for timeframe, payload in sorted(manifest["outputs"].items())
        ],
        "score_samples_included": samples,
        "included_reports": [
            {"path": MANIFEST_PATH_V8_0.as_posix(), "sha256": sha256_file(root / MANIFEST_PATH_V8_0)},
            {"path": REPORT_JSON_PATH_V8_0.as_posix(), "sha256": sha256_file(root / REPORT_JSON_PATH_V8_0)},
            {"path": SCORES_JSON_PATH_V8_0.as_posix(), "sha256": sha256_file(root / SCORES_JSON_PATH_V8_0)},
            {"path": ROBUSTNESS_MANIFEST_PATH_V8_0.as_posix(), "sha256": sha256_file(root / ROBUSTNESS_MANIFEST_PATH_V8_0)},
            {"path": ROBUSTNESS_REPORT_JSON_PATH_V8_0.as_posix(), "sha256": sha256_file(root / ROBUSTNESS_REPORT_JSON_PATH_V8_0)},
        ],
    }
...
def _build_parquet_summary(root: Path, manifest: dict[str, Any]) -> dict[str, Any]:
    ...
    for timeframe, payload in sorted(manifest["outputs"].items()):
        ...
        summary["scores"][timeframe] = {
            ...
            "schema_strict": list(frame.columns) == ML_SCORE_COLUMNS_V8_0,
        }
    return summary


def _write_attestation(root: Path, manifest: dict[str, Any], robustness_manifest: dict[str, Any]) -> None:
    scores = []
    for timeframe, payload in sorted(manifest["outputs"].items()):
    ...
    commands = [
        ...
        "python -m pytest -q tests/ml/test_ohlcv_trades_90d_ml_robustness_v8_0.py",
        "python -m pytest -q tests/validation/test_ohlcv_trades_90d_offline_ml_research_v8_0_validator.py",
        "python -m pytest -q tests/validation/test_ohlcv_trades_90d_ml_robustness_v8_0_validator.py",
        "python scripts/release_audit_lite_zip_v8_0.py",
        "python scripts/audit_audit_lite_zip_v8_0.py --zip projet-galapagos-v8.0-audit-lite.zip",
        "python scripts/smoke_audit_lite_zip_v8_0.py --zip projet-galapagos-v8.0-audit-lite.zip",
    ...
    payload = {
        ...
        "input_total_days": manifest["input_dataset_manifest"]["total_days"],
        "feature_columns_count": manifest["feature_columns_count"],
        "ml_manifest_sha256": sha256_file(root / MANIFEST_PATH_V8_0),
        "ml_report_sha256": sha256_file(root / REPORT_JSON_PATH_V8_0),
        "robustness_manifest_sha256": sha256_file(root / ROBUSTNESS_MANIFEST_PATH_V8_0),
        "robustness_report_sha256": sha256_file(root / ROBUSTNESS_REPORT_JSON_PATH_V8_0),
        "tests_passed": True,
        "ml_validator_passed": True,
        "robustness_validator_passed": True,
        "audit_lite_passed": True,
        "smoke_audit_lite_passed": True,
        "safety_flags": manifest["safety"],
        "robustness_findings": robustness_manifest["findings"],
        "no_trading": True,
        "no_backtest": True,
        "no_orders": True,
...
def _collect_files(root: Path) -> list[Path]:
    files: set[Path] = set()
    sample_exact = [SAMPLE_ROOT / f"timeframe={timeframe}" / "sample.parquet" for timeframe in TIMEFRAMES_V8_0]
...
def _allowed_member(relative: Path) -> bool:
    ...
    for prefix in FORBIDDEN_PREFIXES:
        if text.startswith(prefix) and not text.startswith("data/audit_lite/v8_0/"):
            return False
    return True
