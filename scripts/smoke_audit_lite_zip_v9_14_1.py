...
def smoke_zip(zip_path: Path) -> dict[str, Any]:
    errors: list[str] = []
    with tempfile.TemporaryDirectory(prefix="galapagos_v9_14_1_smoke_") as tmp:
        ...
        errors.extend(_run_v9_14_1_tests(extract_root, env))
...
def _import_modules(extract_root: Path, env: dict[str, str]) -> list[str]:
    errors: list[str] = []
    for module in [
        "galapagos.research.feature_label_separability_v9_14_1",
        "galapagos.research.feature_label_separability_v9_14_1_validation",
    ]:
...
def _run_v9_14_1_tests(extract_root: Path, env: dict[str, str]) -> list[str]:
    completed = _run(
        [
            "python",
            "-m",
    ...
    if completed["returncode"] != 0:
        return [f"V9.14.1 tests failed: {completed['stdout'][-1000:]} {completed['stderr'][-1000:]}"]
    return []


def _run_self_audit(extract_root: Path, zip_path: Path, env: dict[str, str]) -> list[str]:
    completed = _run(["python", "scripts/audit_audit_lite_zip_v9_14_1.py", "--zip", str(zip_path)], extract_root, env, AUDIT_TIMEOUT_SECONDS)
    if completed["returncode"] != 0:
...
def _check_reports(extract_root: Path) -> list[str]:
    ...
    if report.get("source_version") != "V9.14":
        errors.append("V9.14.1 source_version mismatch")
    if report.get("previous_v9_14_decision") != "feature_first_before_more_labels":
        errors.append("V9.14.1 previous decision mismatch")
    if report.get("corrected_decision") != "data_extension_first_before_more_labels":
        errors.append("V9.14.1 corrected decision mismatch")
    if len(report.get("data_source_inventory", [])) != 11:
        errors.append("V9.14.1 inventory must contain 11 sources")
    if {item.get("id") for item in report.get("hypothesis_ranking", [])} != {f"H{index}" for index in range(1, 12)}:
        errors.append("V9.14.1 hypotheses H1-H11 must be complete")
    if report.get("safety_flags", {}).get("no_walk_forward") is not True:
        errors.append("V9.14.1 must confirm no walk-forward")
    if report.get("safety_flags", {}).get("no_sidecars") is not True:
        errors.append("V9.14.1 must confirm no sidecars")
    return errors
...
def _write_reports(result: dict[str, Any]) -> None:
    ...
    (report_dir / "zip_smoke_v9_14_1.json").write_text(json.dumps(result, indent=2, ensure_ascii=False) + "\n", encoding="utf-8")
    (report_dir / "zip_smoke_v9_14_1.md").write_text(
        "# Smoke ZIP V9.14.1\n\n"
        ...
        f"- Timeout tests V9.14.1 : `{TEST_TIMEOUT_SECONDS}` secondes.\n"
