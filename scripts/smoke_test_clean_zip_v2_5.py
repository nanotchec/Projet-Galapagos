...
def main() -> None:
    ...
    errors: list[str] = []
    commands = []
    with tempfile.TemporaryDirectory(prefix="galapagos_v2_5_smoke_") as tmp:
        tmp_path = Path(tmp)
        ...
        commands.append(
            [
                ...
                (
                    ...
                    "manifest_keys={'version','status','created_at_utc','feature_run_id','input_ohlcv','outputs','feature_schema_version','feature_columns','quality','public_read_only','authentication_used','api_key_used','private_endpoint_used','orders_enabled','paper_live_enabled','trading_enabled','ml_enabled','labels_enabled','backtest_enabled','limitations'}\n"
                    "report_keys={'version','status','created_at_utc','feature_run_id','input_ohlcv','outputs','feature_schema_version','feature_columns','quality','safety','limitations'}\n"
    ...
    Path("reports/zip_smoke_test_v2_5.json").write_text(json.dumps(payload, indent=2, ensure_ascii=False) + "\n", encoding="utf-8")
    Path("reports/zip_smoke_test_v2_5.md").write_text(
        "# Smoke ZIP V2.5\n\n"
