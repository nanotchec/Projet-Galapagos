...
def main():
    ...
    parser.add_argument("--version", default="v1.81.1")
    ...
    audit_engine = CorrectiveAudit()
    audit_res = audit_engine.audit_v1_81_1_state(current_state)
    
    # 4. Final Summary
    final_verdict = "V1_81_1_CORRECTIVE_APPROVAL_INTAKE_HARDENING_PASSED"
    if not safety_res["safety_check_passed"] or not audit_res["corrective_audit_passed"] or not approval_res["approval_phrase_match"]:
        final_verdict = "V1_81_1_FAILED"
    ...
    summary_data = {
        ...
        "scope_drift_detected": audit_res["scope_drift_detected"],
        "v1_82_execution_attempted": audit_res["v1_82_execution_attempted"],
        "data_contract_dryrun_executed": audit_res["data_contract_dryrun_executed"],
    ...
    # Documentation
    doc_p = root / f"docs/microstructure_data_contract_approval_intake_corrective_v1_81_1.md"
    with open(doc_p, "w") as f:
        f.write(f"# Galapagos {args.version.upper()} - Corrective Hardening\n\n")
        f.write(f"Verdict Final: {summary_data['final_verdict']}\n\n")
        f.write(f"Negative Coverage: {summary_data['negative_test_coverage_complete']}\n\n")
        f.write(f"Invariants couverts: {summary_data['covered_negative_invariants_count']}/33\n\n")

    print(f"DONE: Generated V1.81.1 reports for {args.version}")
