...
REQUIRED_FILES = [
    ...
    "scripts/run_aggtrades_post_v9_multi_batch_plan_v9_22.py",
    "scripts/validate_aggtrades_post_v9_multi_batch_plan_v9_22.py",
    "scripts/release_audit_lite_zip_v9_22.py",
    "scripts/audit_audit_lite_zip_v9_22.py",
    "scripts/smoke_audit_lite_zip_v9_22.py",
    "src/galapagos/data/aggtrades_post_v9_collection_v9_18.py",
    "src/galapagos/data/aggtrades_post_v9_multi_batch_plan_v9_22.py",
    "src/galapagos/data/aggtrades_post_v9_multi_batch_plan_v9_22_validation.py",
    ...
    "reports/audit_lite/v9_22_command_results.json",
    "reports/audit_lite/v9_22_command_results.md",
    "reports/audit_lite/v9_22_full_local_validation_attestation.json",
    "reports/audit_lite/v9_22_full_local_validation_attestation.md",
    "reports/audit_lite/v9_22_artifact_inventory.json",
    "reports/audit_lite/v9_22_artifact_inventory.md",
    "reports/audit_lite/zip_size_report_v9_22.json",
    "reports/audit_lite/zip_size_report_v9_22.md",
    "reports/audit_lite/zip_audit_v9_22.json",
    "reports/audit_lite/zip_audit_v9_22.md",
    "reports/audit_lite/zip_smoke_v9_22.json",
    "reports/audit_lite/zip_smoke_v9_22.md",
...
def audit_zip(zip_path: Path) -> list[str]:
    ...
    errors: list[str] = []
    with tempfile.TemporaryDirectory(prefix="galapagos_v9_22_audit_") as tmp:
...
def _check_inventory(extract_root: Path, names: list[str]) -> list[str]:
    errors: list[str] = []
    inventory = _read_json(extract_root / "reports/audit_lite/v9_22_artifact_inventory.json")
    size_report = _read_json(extract_root / "reports/audit_lite/zip_size_report_v9_22.json")
...
def _check_reports(extract_root: Path) -> list[str]:
    ...
    manifest = _read_json(extract_root / "reports/manifests/aggtrades_post_v9_multi_batch_plan_v9_22_manifest.json")
    attestation = _read_json(extract_root / "reports/audit_lite/v9_22_full_local_validation_attestation.json")
    if report.get("mode") != "plan-only":
        errors.append("V9.22 audit-lite must describe the plan-only run")
    ...
    if coverage.get("gaps_detected") != []:
        errors.append("V9.22 must report no current coverage gaps")
    ...
    if len(batches) != 11:
        errors.append("V9.22 must include 11 proposed batches")
    if batches and (batches[0].get("start_date") != "2024-08-10" or batches[0].get("end_date") != "2024-10-08"):
        errors.append("V9.22 first proposed batch mismatch")
    if batches and batches[-1].get("end_date") != "2026-05-05":
        errors.append("V9.22 final proposed batch mismatch")
    if any(batch.get("expected_days", 0) > 60 or batch.get("max_downloads", 0) > 60 for batch in batches):
        errors.append("V9.22 proposed batches must not exceed 60 days")
    metrics = report.get("cumulative_metrics", {})
    if metrics.get("days_collected_total") != 97 or metrics.get("rows_collected_total", 0) <= 0:
        errors.append("V9.22 cumulative metrics mismatch")
    ...
    if estimates.get("estimated_remaining_raw_bytes", 0) <= 0 or estimates.get("estimated_remaining_silver_bytes", 0) <= 0:
        errors.append("V9.22 remaining estimates must be positive")
    if report.get("complete_collection_reached") is not False:
        errors.append("V9.22 must not claim complete future coverage")
    ...
    if attestation.get("network_used") is not False or attestation.get("no_new_data_download") is not True or attestation.get("no_ingestion_executed") is not True:
        errors.append("attestation must confirm no network, no new download and no ingestion")
...
def _check_command_results(extract_root: Path) -> list[str]:
    errors: list[str] = []
    payload = _read_json(extract_root / "reports/audit_lite/v9_22_command_results.json")
    ...
    required_needles = [
        ...
        "test_aggtrades_post_v9_multi_batch_plan_v9_22.py",
        "test_aggtrades_post_v9_multi_batch_plan_v9_22_validator.py",
        "run_aggtrades_post_v9_multi_batch_plan_v9_22.py",
        "validate_aggtrades_post_v9_multi_batch_plan_v9_22.py",
        "release_audit_lite_zip_v9_22.py",
    ]
...
def _check_state(extract_root: Path) -> list[str]:
    ...
    for name, payload in {"PROJECT_STATE": state, "latest_metrics": metrics}.items():
        if payload.get("last_validated_version") != "V9.21":
        ...
        if payload.get("direction") != "aggtrades_post_v9_multi_batch_completion_plan":
        ...
        if payload.get("network_used") is not False or payload.get("no_new_data_download") is not True or payload.get("no_ingestion_executed") is not True:
            errors.append(f"{name} must confirm no network, no new download and no ingestion")
...
def _write_reports(result: dict[str, Any]) -> None:
    ...
    (report_dir / "zip_audit_v9_22.json").write_text(json.dumps(result, indent=2, ensure_ascii=False) + "\n", encoding="utf-8")
    (report_dir / "zip_audit_v9_22.md").write_text(
        "# Audit ZIP V9.22\n\n"
