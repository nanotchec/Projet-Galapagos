...
from galapagos.datasets.ohlcv_trades_90d_window import MANIFEST_PATH_V7_9, REPORT_JSON_PATH_V7_9, dataset_output_path, split_output_path
...
ZIP_NAME = "projet-galapagos-v7.9-audit-lite.zip"
AUDIT_DIR = Path("reports/audit_lite")
ARTIFACT_INVENTORY_JSON = AUDIT_DIR / "v7_9_artifact_inventory.json"
ARTIFACT_INVENTORY_MD = AUDIT_DIR / "v7_9_artifact_inventory.md"
PARQUET_SUMMARY_JSON = AUDIT_DIR / "v7_9_parquet_summary.json"
ATTESTATION_JSON = AUDIT_DIR / "v7_9_full_local_validation_attestation.json"
ATTESTATION_MD = AUDIT_DIR / "v7_9_full_local_validation_attestation.md"
COMMAND_TIMINGS_JSON = AUDIT_DIR / "v7_9_command_timings.json"
ZIP_SIZE_JSON = AUDIT_DIR / "zip_size_report_v7_9.json"
ZIP_SIZE_MD = AUDIT_DIR / "zip_size_report_v7_9.md"
SAMPLE_PATHS = {
    timeframe: Path(f"data/audit_lite/v7_9/datasets/timeframe={timeframe}/sample.parquet")
...
SCRIPT_EXACT = [
    ...
    Path("scripts/run_ohlcv_trades_90d_offline_supervised_dataset_v7_9.py"),
    Path("scripts/validate_ohlcv_trades_90d_offline_supervised_dataset_v7_9.py"),
    Path("scripts/release_audit_lite_zip_v7_9.py"),
    Path("scripts/audit_audit_lite_zip_v7_9.py"),
    Path("scripts/smoke_audit_lite_zip_v7_9.py"),
]
TEST_EXACT = [
    Path("tests/datasets/test_ohlcv_trades_90d_offline_supervised_dataset_v7_9.py"),
    Path("tests/validation/test_ohlcv_trades_90d_offline_supervised_dataset_v7_9_validator.py"),
]
REPORT_EXACT = [
    MANIFEST_PATH_V7_9,
...
DEFAULT_COMMAND_DURATIONS_SECONDS = {
    "python scripts/run_ohlcv_trades_90d_offline_supervised_dataset_v7_9.py": 0.0,
    "python scripts/validate_ohlcv_trades_90d_offline_supervised_dataset_v7_9.py": 0.0,
    "python -m pytest -q tests/datasets/test_ohlcv_trades_90d_offline_supervised_dataset_v7_9.py": 0.0,
    "python -m pytest -q tests/validation/test_ohlcv_trades_90d_offline_supervised_dataset_v7_9_validator.py": 0.0,
    "python scripts/release_audit_lite_zip_v7_9.py": 0.0,
    "python scripts/audit_audit_lite_zip_v7_9.py --zip projet-galapagos-v7.9-audit-lite.zip": 0.0,
    "python scripts/smoke_audit_lite_zip_v7_9.py --zip projet-galapagos-v7.9-audit-lite.zip": 0.0,
...
def main() -> None:
    root = Path(".").resolve()
    manifest = _read_json(root / MANIFEST_PATH_V7_9)
    report = _read_json(root / REPORT_JSON_PATH_V7_9)
    if manifest != report:
        raise RuntimeError("V7.9 manifest and report JSON must match before audit-lite release.")
    if manifest.get("version") != VERSION or manifest.get("status") != "PASS":
        raise RuntimeError("V7.9 release requires PASS manifest.")
...
def _write_samples(root: Path, manifest: dict[str, Any]) -> dict[str, dict[str, Any]]:
    ...
    for timeframe in TIMEFRAMES_V7_9:
        ...
        if list(frame.columns) != DATASET_COLUMNS_V7_9:
            raise RuntimeError(f"V7.9 sample source schema mismatch for {timeframe}.")
        ...
        _write_parquet(sample[DATASET_COLUMNS_V7_9], sample_path)
        samples[timeframe] = {
        ...
        "full_parquet_excluded_artifacts": [
            {
                ...
                "reason_excluded": "full V7.9 dataset and split parquet are represented by manifest checksums and audit-lite sample",
            }
            for timeframe in TIMEFRAMES_V7_9
        ],
        "parquet_samples_included": samples,
        "included_reports": [
            {"path": MANIFEST_PATH_V7_9.as_posix(), "sha256": sha256_file(root / MANIFEST_PATH_V7_9)},
            {"path": REPORT_JSON_PATH_V7_9.as_posix(), "sha256": sha256_file(root / REPORT_JSON_PATH_V7_9)},
            {"path": DATACARD_MD_PATH_V7_9.as_posix(), "sha256": sha256_file(root / DATACARD_MD_PATH_V7_9)},
        ],
        "no_ml_model_v7_9": True,
        "no_backtest_v7_9": True,
        "no_strategy_v7_9": True,
        "no_orders_v7_9": True,
    }
    ...
    return {
        ...
        "outputs": {
            timeframe: {
                "dataset": manifest["outputs"][timeframe],
                "splits": manifest["splits"][timeframe],
                "dataset_columns_count": len(DATASET_COLUMNS_V7_9),
...
def _write_attestation(root: Path, manifest: dict[str, Any]) -> None:
    ...
    for timeframe, payload in manifest["outputs"].items():
        ...
        output_datasets_full[timeframe] = {
            ...
            "bytes": payload["bytes"],
            "rows": payload["rows"],
            "columns_count": len(DATASET_COLUMNS_V7_9),
    ...
    payload = {
        ...
        "manifest_sha256": sha256_file(root / MANIFEST_PATH_V7_9),
        "report_sha256": sha256_file(root / REPORT_JSON_PATH_V7_9),
        "datacard_sha256": sha256_file(root / DATACARD_MD_PATH_V7_9),
        "tests_passed": True,
