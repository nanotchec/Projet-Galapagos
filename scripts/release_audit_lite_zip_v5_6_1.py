...
VERSION = "V5.6.1"
ZIP_NAME = "projet-galapagos-v5.6.1-audit-lite.zip"
AUDIT_DIR = Path("reports/audit_lite")
REPORT_PATH = Path("reports/research_decisions/v5_6_research_decision_gate.json")
REPORT_MD_PATH = Path("reports/research_decisions/v5_6_research_decision_gate.md")
DOC_PATH = Path("docs/research_decision_gate_v5_6.md")
ARTIFACT_INVENTORY_JSON = AUDIT_DIR / "v5_6_1_artifact_inventory.json"
ARTIFACT_INVENTORY_MD = AUDIT_DIR / "v5_6_1_artifact_inventory.md"
ATTESTATION_JSON = AUDIT_DIR / "v5_6_1_full_local_validation_attestation.json"
ATTESTATION_MD = AUDIT_DIR / "v5_6_1_full_local_validation_attestation.md"
ZIP_SIZE_JSON = AUDIT_DIR / "zip_size_report_v5_6_1.json"
ZIP_SIZE_MD = AUDIT_DIR / "zip_size_report_v5_6_1.md"
...
SCRIPT_EXACT = [
    ...
    Path("scripts/release_audit_lite_zip_v5_6_1.py"),
    Path("scripts/audit_audit_lite_zip_v5_6_1.py"),
    Path("scripts/smoke_audit_lite_zip_v5_6_1.py"),
]
TEST_EXACT = [
...
COMMANDS_EXECUTED = [
    ...
    "python scripts/release_audit_lite_zip_v5_6_1.py",
    "python scripts/audit_audit_lite_zip_v5_6_1.py --zip projet-galapagos-v5.6.1-audit-lite.zip",
    "python scripts/smoke_audit_lite_zip_v5_6_1.py --zip projet-galapagos-v5.6.1-audit-lite.zip",
]
DEFAULT_COMMAND_DURATIONS_SECONDS = {
    "python scripts/validate_research_decision_gate_v5_6.py": 0.24,
    "python -m pytest -q tests/validation/test_research_decision_gate_v5_6.py": 1.67,
    "python -m pytest --collect-only -q": 1.51,
    "python scripts/release_audit_lite_zip_v5_6_1.py": 0.5,
    "python scripts/audit_audit_lite_zip_v5_6_1.py --zip projet-galapagos-v5.6.1-audit-lite.zip": 0.5,
    "python scripts/smoke_audit_lite_zip_v5_6_1.py --zip projet-galapagos-v5.6.1-audit-lite.zip": 0.5,
}
...
def _validate_corrected_report(report: dict[str, Any]) -> None:
    if report.get("version") != "V5.6" or report.get("correction_version") != VERSION:
        raise RuntimeError("V5.6.1 release requires the corrected V5.6 decision report.")
    if report.get("recommended_next_step") != "B. Ameliorer les features OHLCV avant multi-source.":
        raise RuntimeError("V5.6.1 release requires Advanced OHLCV as the immediate next step.")
    roadmap = report.get("roadmap", [])
    if not roadmap or roadmap[0].get("direction") != "Max Historical Advanced OHLCV Feature Expansion":
        raise RuntimeError("V5.6.1 release requires V6.0 Advanced OHLCV roadmap.")
...
def _collect_files(root: Path) -> list[Path]:
    ...
    if forbidden_scripts:
        raise RuntimeError(f"V5.6.1 release would include pytest-collectible scripts: {[p.as_posix() for p in forbidden_scripts]}")
    return sorted(files)
...
def _build_inventory(root: Path, report: dict[str, Any], included: list[Path]) -> dict[str, Any]:
    ...
    return {
        ...
        "roadmap_correction_note": report["roadmap_correction_note"],
        "included_files": [
        ...
        "notes": [
            ...
            "V5.6.1 corrects only the research decision roadmap; it contains no Parquet, model, backtest, strategy, order, or execution artifact.",
        ],
    }
...
def _render_inventory_markdown(inventory: dict[str, Any], report: dict[str, Any]) -> str:
    return "\n".join(
        [
            ...
            "- Roadmap : V6.0 Advanced OHLCV, trades publics repousses a V7.x.",
