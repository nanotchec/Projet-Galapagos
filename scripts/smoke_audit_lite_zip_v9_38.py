...
def smoke_zip(zip_path: Path) -> dict[str, Any]:
    ...
    with tempfile.TemporaryDirectory(prefix="galapagos_v9_38_smoke_") as tmp:
        ...
        parquet_errors = _check_parquet_engine(extract_root, env)
        parquet_engine_available = not parquet_errors
        ...
        errors.extend(_run_v9_38_tests(extract_root, env))
...
def _check_parquet_engine(extract_root: Path, env: dict[str, str]) -> list[str]:
    ...
    if completed["returncode"] != 0:
        ...
        if fallback["returncode"] != 0:
            return ["pyarrow or fastparquet is required for V9.38 parquet validation tests; neither import succeeded"]
    return []


def _import_modules(extract_root: Path, env: dict[str, str]) -> list[str]:
    errors: list[str] = []
    for module in ["galapagos.features.ohlcv_aggtrades_5y_feature_store_validation_v9_38", "galapagos.features.ohlcv_aggtrades_5y_feature_store_validation_v9_38_validation"]:
...
def _run_pytest_collect(extract_root: Path, env: dict[str, str]) -> list[str]:
    completed = _run(["python", "-m", "pytest", "--collect-only", "-q", "tests/features/test_ohlcv_aggtrades_5y_feature_store_validation_v9_38.py", "tests/validation/test_ohlcv_aggtrades_5y_feature_store_validation_v9_38_validator.py"], extract_root, env, PYTEST_TIMEOUT_SECONDS)
    if completed["returncode"] != 0:
        return [f"pytest collect-only failed: {completed['stdout'][-1000:]} {completed['stderr'][-1000:]}"]
    return []


def _run_v9_38_tests(extract_root: Path, env: dict[str, str]) -> list[str]:
    completed = _run(["python", "-m", "pytest", "-q", "tests/features/test_ohlcv_aggtrades_5y_feature_store_validation_v9_38.py", "tests/validation/test_ohlcv_aggtrades_5y_feature_store_validation_v9_38_validator.py"], extract_root, env, TEST_TIMEOUT_SECONDS)
    if completed["returncode"] != 0:
        return [f"V9.38 tests failed: {completed['stdout'][-1000:]} {completed['stderr'][-1000:]}"]
    return []
...
def _check_reports(extract_root: Path) -> list[str]:
    ...
    if report.get("version") != VERSION:
        errors.append("V9.38 report version mismatch")
    if report.get("network_used") is not False or report.get("new_data_downloaded") is not False:
        errors.append("V9.38 smoke expects no network and no downloads")
    if report.get("labels_created") is not False or report.get("dataset_created") is not False or report.get("ml_executed") is not False:
        errors.append("V9.38 smoke expects no labels, dataset or ML")
    return errors


def _run_self_audit(extract_root: Path, zip_path: Path, env: dict[str, str]) -> list[str]:
    completed = _run(["python", "scripts/audit_audit_lite_zip_v9_38.py", "--zip", str(zip_path)], extract_root, env, AUDIT_TIMEOUT_SECONDS)
    if completed["returncode"] != 0:
