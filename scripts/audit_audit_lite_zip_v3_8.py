...
VERSION = "V3.8"
REPORT_JSON = Path("reports/audit_lite/zip_audit_v3_8.json")
REPORT_MD = Path("reports/audit_lite/zip_audit_v3_8.md")
REQUIRED_FILES = {
    "README.md",
    "pyproject.toml",
    ...
    "reports/audit_lite/v3_8_artifact_inventory.json",
    "reports/audit_lite/v3_8_parquet_summary.json",
    "reports/audit_lite/v3_8_full_local_validation_attestation.json",
    "reports/audit_lite/v3_8_full_local_validation_attestation.md",
...
def audit_zip(zip_path: Path) -> dict[str, Any]:
    ...
    for entry in entries:
        ...
        if path.suffix.casefold() == ".parquet" and not entry.startswith("data/audit_lite/v3_8/datasets/"):
            errors.append(f"full Parquet must not be included in audit-lite ZIP: {entry}")
    sample_entries = [entry for entry in entries if entry.startswith("data/audit_lite/v3_8/datasets/") and entry.endswith(".parquet")]
    if len(sample_entries) < 8:
        errors.append("missing V3.8 dataset/split sample Parquet files")
    with tempfile.TemporaryDirectory(prefix="galapagos-v3-8-audit-") as tmp:
        extract_root = Path(tmp)
        with zipfile.ZipFile(zip_path) as archive:
            archive.extractall(extract_root)
        for sample in sample_entries:
            frame = read_parquet(extract_root / sample)
            expected_columns = SPLIT_COLUMNS_V3_8 if sample.endswith("splits_sample.parquet") else DATASET_COLUMNS_V3_8
        ...
        if manifest.get("version") != VERSION:
            errors.append("V3.8 manifest version mismatch in ZIP")
...
def _render_markdown(result: dict[str, Any]) -> str:
    ...
    return f"""# Audit ZIP audit-lite V3.8
