...
VERSION = "V5.2"
REPORT_JSON = Path("reports/audit_lite/zip_smoke_v5_2.json")
REPORT_MD = Path("reports/audit_lite/zip_smoke_v5_2.md")
SAMPLES = [
...
def smoke_zip(zip_path: Path) -> dict[str, Any]:
    ...
    with tempfile.TemporaryDirectory(prefix="galapagos-v5-2-smoke-") as tmp:
        ...
        import_probe = subprocess.run(
            [
                ...
                "\n".join(
                    [
                        "from galapagos.labels.max_history_window import TIMEFRAMES_V5_2, output_path",
                        ...
                        "from galapagos.labels.schemas import LABEL_COLUMNS_V5_2",
                        "assert LABEL_COLUMNS_V5_2[0] == 'source'",
                        "assert TIMEFRAMES_V5_2 == ['1m', '5m', '15m', '1h']",
                        "print('imports_ok')",
                    ]
                ),
            ],
        ...
        if manifest != report:
            errors.append("V5.2 manifest/report mismatch inside audit-lite ZIP")
        if manifest.get("version") != VERSION or manifest.get("status") != "PASS":
            errors.append("V5.2 manifest status/version invalid inside audit-lite ZIP")
        ...
        if safety.get("public_read_only") is not True or safety.get("labels_enabled") is not True:
            errors.append("V5.2 public_read_only and labels_enabled must be true")
        for key in [
            "authentication_used",
            "api_key_used",
            ...
            if safety.get(key) is not False:
                errors.append(f"V5.2 safety flag must be false: {key}")
...
def _render_markdown(result: dict[str, Any]) -> str:
    ...
    return f"""# Smoke ZIP audit-lite V5.2
