...
ZIP_NAME = "projet-galapagos-v8.3-audit-lite.zip"
AUDIT_DIR = Path("reports/audit_lite")
ARTIFACT_INVENTORY_JSON = AUDIT_DIR / "v8_3_artifact_inventory.json"
ARTIFACT_INVENTORY_MD = AUDIT_DIR / "v8_3_artifact_inventory.md"
PARQUET_SUMMARY_JSON = AUDIT_DIR / "v8_3_parquet_summary.json"
ATTESTATION_JSON = AUDIT_DIR / "v8_3_full_local_validation_attestation.json"
ATTESTATION_MD = AUDIT_DIR / "v8_3_full_local_validation_attestation.md"
COMMAND_TIMINGS_JSON = AUDIT_DIR / "v8_3_command_timings.json"
ZIP_SIZE_JSON = AUDIT_DIR / "zip_size_report_v8_3.json"
ZIP_SIZE_MD = AUDIT_DIR / "zip_size_report_v8_3.md"
SAMPLE_PATHS = {
    timeframe: Path(f"data/audit_lite/v8_3/features/timeframe={timeframe}/sample.parquet")
...
SCRIPT_EXACT = [
    ...
    Path("scripts/run_ohlcv_trades_1y_feature_store_v8_3.py"),
    Path("scripts/validate_ohlcv_trades_1y_feature_store_v8_3.py"),
    Path("scripts/release_audit_lite_zip_v8_3.py"),
    Path("scripts/audit_audit_lite_zip_v8_3.py"),
    Path("scripts/smoke_audit_lite_zip_v8_3.py"),
]
TEST_EXACT = [
    Path("tests/features/test_ohlcv_trades_1y_features_v8_3.py"),
    Path("tests/validation/test_ohlcv_trades_1y_feature_store_v8_3_validator.py"),
]
REPORT_EXACT = [
    MANIFEST_PATH_V8_3,
    REPORT_JSON_PATH_V8_3,
...
DEFAULT_COMMAND_DURATIONS_SECONDS = {
    "python scripts/run_ohlcv_trades_1y_feature_store_v8_3.py": 0.0,
    "python scripts/validate_ohlcv_trades_1y_feature_store_v8_3.py": 0.0,
    "python -m pytest -q tests/features/test_ohlcv_trades_1y_features_v8_3.py": 0.0,
    "python -m pytest -q tests/validation/test_ohlcv_trades_1y_feature_store_v8_3_validator.py": 0.0,
    "python scripts/release_audit_lite_zip_v8_3.py": 0.0,
    "python scripts/audit_audit_lite_zip_v8_3.py --zip projet-galapagos-v8.3-audit-lite.zip": 0.0,
    "python scripts/smoke_audit_lite_zip_v8_3.py --zip projet-galapagos-v8.3-audit-lite.zip": 0.0,
...
def main() -> None:
    root = Path(".").resolve()
    manifest = _read_json(root / MANIFEST_PATH_V8_3)
    report = _read_json(root / REPORT_JSON_PATH_V8_3)
    if manifest != report:
        raise RuntimeError("V8.3 manifest and report JSON must match before audit-lite release.")
    if manifest.get("version") != VERSION or manifest.get("status") != "PASS":
        raise RuntimeError("V8.3 release requires PASS manifest.")
...
def _write_samples(root: Path, manifest: dict[str, Any]) -> dict[str, dict[str, Any]]:
    ...
    for timeframe in TIMEFRAMES_V8_3:
        ...
        if list(frame.columns) != OHLCV_TRADES_FEATURE_COLUMNS_V8_3:
            raise RuntimeError(f"V8.3 sample source schema mismatch for {timeframe}.")
        ...
        _write_parquet(sample[OHLCV_TRADES_FEATURE_COLUMNS_V8_3], sample_path)
        samples[timeframe] = {
        ...
        "full_parquet_excluded_artifacts": [
            {
                ...
                "rows": payload["rows"],
                "bytes": payload["bytes"],
                "reason_excluded": "full V8.3 feature parquet is represented by manifest checksums and audit-lite sample",
            }
            for timeframe, payload in manifest["outputs"].items()
        ],
        "parquet_samples_included": samples,
        "included_reports": [
            {"path": MANIFEST_PATH_V8_3.as_posix(), "sha256": sha256_file(root / MANIFEST_PATH_V8_3)},
            {"path": REPORT_JSON_PATH_V8_3.as_posix(), "sha256": sha256_file(root / REPORT_JSON_PATH_V8_3)},
        ],
        "no_labels_v8_3": True,
        "no_dataset_ml_v8_3": True,
        "no_ml_model_v8_3": True,
        "no_backtest_v8_3": True,
        "no_strategy_v8_3": True,
        "no_orders_v8_3": True,
    }
...
def _build_parquet_summary(manifest: dict[str, Any], samples: dict[str, dict[str, Any]]) -> dict[str, Any]:
    return {
        ...
        "outputs": {
            timeframe: {
                **payload,
                "columns": OHLCV_TRADES_FEATURE_COLUMNS_V8_3,
                "columns_count": len(OHLCV_TRADES_FEATURE_COLUMNS_V8_3),
...
def _write_attestation(root: Path, manifest: dict[str, Any]) -> None:
    ...
    payload = {
        ...
        "manifest_sha256": sha256_file(root / MANIFEST_PATH_V8_3),
        "report_sha256": sha256_file(root / REPORT_JSON_PATH_V8_3),
        "tests_passed": True,
