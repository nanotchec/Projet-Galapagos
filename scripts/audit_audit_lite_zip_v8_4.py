...
VERSION = "V8.4"
AUDIT_JSON = Path("reports/audit_lite/zip_audit_v8_4.json")
AUDIT_MD = Path("reports/audit_lite/zip_audit_v8_4.md")
...
REQUIRED_ENTRIES = {
    ...
    "reports/datasets/ohlcv_trades_1y_offline_supervised_dataset_v8_4.json",
    "reports/datasets/ohlcv_trades_1y_offline_supervised_dataset_v8_4.md",
    "reports/datasets/ohlcv_trades_1y_offline_supervised_dataset_v8_4_datacard.md",
    "docs/ohlcv_trades_1y_offline_supervised_dataset_v8_4.md",
    "reports/audit_lite/v8_4_artifact_inventory.json",
    "reports/audit_lite/v8_4_artifact_inventory.md",
    "reports/audit_lite/v8_4_parquet_summary.json",
    "reports/audit_lite/v8_4_full_local_validation_attestation.json",
    "reports/audit_lite/v8_4_full_local_validation_attestation.md",
    "reports/audit_lite/zip_size_report_v8_4.json",
    "reports/audit_lite/zip_size_report_v8_4.md",
    "reports/PROJECT_STATE.json",
    "reports/PROJECT_STATE.md",
    "reports/current/latest_metrics.json",
    "reports/current/latest_metrics.md",
    "reports/current/latest_summary.md",
    "scripts/run_ohlcv_trades_1y_offline_supervised_dataset_v8_4.py",
    "scripts/validate_ohlcv_trades_1y_offline_supervised_dataset_v8_4.py",
    "scripts/release_audit_lite_zip_v8_4.py",
    "scripts/audit_audit_lite_zip_v8_4.py",
    "scripts/smoke_audit_lite_zip_v8_4.py",
    "tests/datasets/test_ohlcv_trades_1y_offline_supervised_dataset_v8_4.py",
    "tests/validation/test_ohlcv_trades_1y_offline_supervised_dataset_v8_4_validator.py",
}
SAMPLE_ENTRIES = {
    f"data/audit_lite/v8_4/datasets/timeframe={timeframe}/sample.parquet" for timeframe in TIMEFRAMES_V8_4
}
FORBIDDEN_PREFIXES = (
    "data/raw/",
    "data/research/",
...
def audit_zip(zip_path: Path) -> dict[str, Any]:
    ...
    with tempfile.TemporaryDirectory(prefix="galapagos-v8-4-audit-") as tmp:
        root = Path(tmp)
        with zipfile.ZipFile(zip_path) as archive:
            archive.extractall(root)
        manifest = _read_json(root / "reports/manifests/ohlcv_trades_1y_offline_supervised_dataset_v8_4_manifest.json")
        report = _read_json(root / "reports/datasets/ohlcv_trades_1y_offline_supervised_dataset_v8_4.json")
        if manifest != report:
            errors.append("V8.4 manifest/report mismatch in ZIP")
        if manifest.get("version") != VERSION or manifest.get("status") != "PASS":
            errors.append("V8.4 manifest version/status mismatch in ZIP")
        ...
        for flag in ["trading_enabled", "orders_enabled", "paper_live_enabled", "backtest_enabled", "strategy_enabled", "execution_enabled", "ml_enabled"]:
            if safety.get(flag) is not False:
                errors.append(f"V8.4 safety flag must be false: {flag}")
        for sample_entry in SAMPLE_ENTRIES:
            sample_path = root / sample_entry
            if not sample_path.exists():
                errors.append(f"missing V8.4 dataset sample: {sample_entry}")
                continue
            frame = pd.read_parquet(sample_path, engine="pyarrow")
            if list(frame.columns) != DATASET_COLUMNS_V8_4:
                errors.append(f"V8.4 dataset sample schema mismatch: {sample_entry}")
            if len(frame) == 0:
                errors.append(f"V8.4 dataset sample is empty: {sample_entry}")
