...
VERSION = "V4.5"
REPORT_JSON = Path("reports/audit_lite/zip_audit_v4_5.json")
REPORT_MD = Path("reports/audit_lite/zip_audit_v4_5.md")
REQUIRED_FILES = {
    "README.md",
    "pyproject.toml",
    ...
    "reports/datasets/one_year_offline_supervised_dataset_v4_5.md",
    "reports/datasets/one_year_offline_supervised_dataset_v4_5_datacard.md",
    "reports/audit_lite/v4_5_artifact_inventory.json",
    "reports/audit_lite/v4_5_artifact_inventory.md",
    "reports/audit_lite/v4_5_parquet_summary.json",
    "reports/audit_lite/v4_5_full_local_validation_attestation.json",
    "reports/audit_lite/v4_5_full_local_validation_attestation.md",
...
def audit_zip(zip_path: Path) -> dict[str, Any]:
    ...
    missing = sorted((REQUIRED_FILES | allowed_parquet) - entry_set)
    if missing:
    ...
    with tempfile.TemporaryDirectory(prefix="galapagos-v4-5-audit-") as tmp:
        ...
        attestation = _read_json(extract_root / "reports/audit_lite/v4_5_full_local_validation_attestation.json")
        if manifest != report:
            errors.append("V4.5 manifest/report mismatch in ZIP")
        if manifest.get("version") != VERSION:
            errors.append("V4.5 manifest version mismatch in ZIP")
        ...
        if set(summary.get("datasets", {})) != {"1m", "5m", "15m", "1h"}:
            errors.append("V4.5 parquet summary must include all dataset timeframes")
        ...
        for flag in ["validator_passed", "tests_passed", "audit_lite_passed", "smoke_audit_lite_passed", "no_trading", "no_backtest", "no_orders"]:
            if attestation.get(flag) is not True:
                errors.append(f"V4.5 attestation flag must be true: {flag}")
...
def _render_markdown(result: dict[str, Any]) -> str:
    ...
    return f"""# Audit ZIP audit-lite V4.5
