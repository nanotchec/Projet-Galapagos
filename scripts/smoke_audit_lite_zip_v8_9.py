...
VERSION = "V8.9"
SMOKE_JSON = Path("reports/audit_lite/zip_smoke_v8_9.json")
SMOKE_MD = Path("reports/audit_lite/zip_smoke_v8_9.md")
...
def smoke_zip(zip_path: Path) -> dict[str, Any]:
    ...
    with tempfile.TemporaryDirectory(prefix="galapagos_v8_9_smoke_") as tmp:
        target = Path(tmp)
...
def _check_reports(root: Path) -> list[str]:
    ...
    if manifest_path.exists() and report_path.exists():
        ...
        if manifest.get("findings", {}).get("feature_set_validated_for_trading") is not False:
            errors.append("smoke feature_set_validated_for_trading must be false")
    if selection_path.exists():
        selection = json.loads(selection_path.read_text(encoding="utf-8"))
        if not selection.get("candidate_refined_feature_set", {}).get("selected_features"):
