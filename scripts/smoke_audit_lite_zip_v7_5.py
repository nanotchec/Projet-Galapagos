...
VERSION = "V7.5"
REPORT_JSON = Path("reports/audit_lite/zip_smoke_v7_5.json")
REPORT_MD = Path("reports/audit_lite/zip_smoke_v7_5.md")
...
def smoke_zip(zip_path: Path) -> dict[str, Any]:
    ...
    with tempfile.TemporaryDirectory(prefix="galapagos-v7-5-smoke-") as tmp:
        ...
        try:
            imports = {
                "galapagos.ml.ohlcv_trades_robustness": "run_ohlcv_trades_ml_robustness_v7_5",
                "galapagos.ml.ohlcv_trades_robustness_validation": "validate_ohlcv_trades_ml_robustness_v7_5",
                "galapagos.ml.schemas": "ML_SCORE_COLUMNS_V7_4",
            }
        ...
        inventory = _read_json(extract_root / "reports/audit_lite/v7_5_artifact_inventory.json")
        attestation = _read_json(extract_root / "reports/audit_lite/v7_5_full_local_validation_attestation.json")
        if manifest != report:
            errors.append("V7.5 manifest/report mismatch inside audit-lite ZIP")
        for analysis in [
            "baseline_delta",
            "split_stability",
            "timeframe_stability",
            ...
            if analysis not in manifest.get("analyses", {}):
                errors.append(f"missing V7.5 analysis in smoke: {analysis}")
        ...
        for key in ["authentication_used", "api_key_used", "private_endpoint_used", "orders_enabled", "paper_live_enabled", "trading_enabled", "backtest_enabled", "strategy_enabled", "execution_enabled"]:
            if safety.get(key) is not False:
                errors.append(f"V7.5 safety flag must be false: {key}")
        for key in ["public_read_only", "ml_enabled", "labels_enabled", "dataset_enabled"]:
            if safety.get(key) is not True:
                errors.append(f"V7.5 safety flag must be true: {key}")
        ...
        for key in [
            ...
            if findings.get(key) is not False:
                errors.append(f"V7.5 finding must be false: {key}")
        ...
        if collect_result["returncode"] != 0:
            errors.append("V7.5 audit-lite pytest collect-only failed")
...
def _render_markdown(result: dict[str, Any]) -> str:
    ...
    return f"""# Smoke ZIP audit-lite V7.5
