...
VERSION = "V5.6"
REPORT_JSON = Path("reports/audit_lite/zip_audit_v5_6.json")
REPORT_MD = Path("reports/audit_lite/zip_audit_v5_6.md")
REQUIRED_FILES = {
    "README.md",
    "pyproject.toml",
    ...
    "reports/audit_lite/v5_6_artifact_inventory.json",
    "reports/audit_lite/v5_6_artifact_inventory.md",
    "reports/audit_lite/zip_size_report_v5_6.json",
    "reports/audit_lite/zip_size_report_v5_6.md",
    "reports/audit_lite/v5_6_full_local_validation_attestation.json",
    "reports/audit_lite/v5_6_full_local_validation_attestation.md",
    "reports/PROJECT_STATE.json",
    "reports/current/latest_summary.md",
    "scripts/validate_research_decision_gate_v5_6.py",
    "scripts/release_audit_lite_zip_v5_6.py",
    "scripts/audit_audit_lite_zip_v5_6.py",
    "scripts/smoke_audit_lite_zip_v5_6.py",
...
def audit_zip(zip_path: Path) -> dict[str, Any]:
    ...
    with tempfile.TemporaryDirectory(prefix="galapagos-v5-6-audit-") as tmp:
        ...
        if report.get("version") != VERSION:
            errors.append("V5.6 report version mismatch in ZIP")
        if report.get("status") != "PASS":
            errors.append("V5.6 report status must be PASS in ZIP")
        if report.get("decision_gate_type") != "research_only":
            errors.append("V5.6 decision gate must be research_only in ZIP")
...
def _render_markdown(result: dict[str, Any]) -> str:
    ...
    return f"""# Audit ZIP audit-lite V5.6
