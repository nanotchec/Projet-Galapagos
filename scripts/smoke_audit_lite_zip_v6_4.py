...
VERSION = "V6.4"
REPORT_JSON = Path("reports/audit_lite/zip_smoke_v6_4.json")
REPORT_MD = Path("reports/audit_lite/zip_smoke_v6_4.md")
...
def smoke_zip(zip_path: Path) -> dict[str, Any]:
    ...
    with tempfile.TemporaryDirectory(prefix="galapagos-v6-4-smoke-") as tmp:
        ...
        try:
            module = importlib.import_module("validate_research_decision_gate_v6_4")
            result = module.validate_research_decision_gate_v6_4(extract_root)
            if result.get("passed") is not True:
                errors.append(f"V6.4 embedded validator failed: {result.get('errors')}")
        ...
        inventory = _read_json(extract_root / "reports/audit_lite/v6_4_artifact_inventory.json")
        ...
        if report.get("version") != VERSION or report.get("status") != "PASS":
            errors.append("V6.4 decision report version/status mismatch inside audit-lite ZIP")
        ...
        if collect_result["returncode"] != 0:
            errors.append("V6.4 audit-lite pytest collect-only failed")
...
def _render_markdown(result: dict[str, Any]) -> str:
    ...
    return f"""# Smoke ZIP audit-lite V6.4
