...
REQUIRED_FILES = [
    ...
    "src/galapagos/features/ohlcv_aggtrades_5y_feature_store_validation_v9_38.py",
    "src/galapagos/features/ohlcv_aggtrades_5y_feature_store_validation_v9_38_validation.py",
    "src/galapagos/features/ohlcv_aggtrades_5y_feature_store_v9_37_schemas.py",
    "scripts/run_ohlcv_aggtrades_5y_feature_store_validation_v9_38.py",
    "scripts/validate_ohlcv_aggtrades_5y_feature_store_validation_v9_38.py",
    "scripts/release_audit_lite_zip_v9_38.py",
    "scripts/audit_audit_lite_zip_v9_38.py",
    "scripts/smoke_audit_lite_zip_v9_38.py",
    "tests/features/test_ohlcv_aggtrades_5y_feature_store_validation_v9_38.py",
    "tests/validation/test_ohlcv_aggtrades_5y_feature_store_validation_v9_38_validator.py",
    ...
    "reports/features/ohlcv_aggtrades_5y_feature_store_validation_v9_38.md",
    "docs/ohlcv_aggtrades_5y_feature_store_validation_v9_38.md",
    "reports/audit_lite/v9_38_command_results.json",
    "reports/audit_lite/v9_38_command_results.md",
    "reports/audit_lite/v9_38_full_local_validation_attestation.json",
    "reports/audit_lite/v9_38_full_local_validation_attestation.md",
    "reports/audit_lite/v9_38_artifact_inventory.json",
    "reports/audit_lite/v9_38_artifact_inventory.md",
    "reports/audit_lite/zip_size_report_v9_38.json",
    "reports/audit_lite/zip_size_report_v9_38.md",
    "reports/audit_lite/zip_audit_v9_38.json",
    "reports/audit_lite/zip_audit_v9_38.md",
    "reports/audit_lite/zip_smoke_v9_38.json",
    "reports/audit_lite/zip_smoke_v9_38.md",
...
def audit_zip(zip_path: Path) -> list[str]:
    ...
    errors: list[str] = []
    with tempfile.TemporaryDirectory(prefix="galapagos_v9_38_audit_") as tmp:
...
def _check_inventory(extract_root: Path, names: list[str]) -> list[str]:
    errors: list[str] = []
    inventory = _read_json(extract_root / "reports/audit_lite/v9_38_artifact_inventory.json")
    size_report = _read_json(extract_root / "reports/audit_lite/zip_size_report_v9_38.json")
...
def _check_reports(extract_root: Path) -> list[str]:
    errors: list[str] = []
    report = _read_json(extract_root / "reports/features/ohlcv_aggtrades_5y_feature_store_validation_v9_38.json")
    manifest = _read_json(extract_root / "reports/manifests/ohlcv_aggtrades_5y_feature_store_validation_v9_38_manifest.json")
    attestation = _read_json(extract_root / "reports/audit_lite/v9_38_full_local_validation_attestation.json")
    if report.get("version") != VERSION or report.get("source_version") != "V9.37":
        errors.append("V9.38 report version/source mismatch")
    if manifest.get("decision") != report.get("decision") or manifest.get("safety_flags") != report.get("safety_flags"):
        errors.append("V9.38 manifest mismatch")
    if report.get("network_used") is not False or report.get("new_data_downloaded") is not False:
        errors.append("V9.38 must not use network or download data")
    ...
    if report.get("quality_status") != "PASS" or report.get("coverage_status") != "target_5y_feature_window_complete":
        errors.append("V9.38 must pass quality and complete coverage")
...
def _check_command_results(extract_root: Path) -> list[str]:
    payload = _read_json(extract_root / "reports/audit_lite/v9_38_command_results.json")
    ...
    for needle in ["pytest --collect-only -q", "test_ohlcv_aggtrades_5y_feature_store_validation_v9_38.py", "test_ohlcv_aggtrades_5y_feature_store_validation_v9_38_validator.py", "run_ohlcv_aggtrades_5y_feature_store_validation_v9_38.py", "validate_ohlcv_aggtrades_5y_feature_store_validation_v9_38.py", "release_audit_lite_zip_v9_38.py"]:
...
def _check_state(extract_root: Path) -> list[str]:
    ...
    errors: list[str] = []
    if metrics.get("last_validated_version") != "V9.37":
    ...
    if metrics.get("direction") != "ohlcv_aggtrades_5y_feature_store_validation":
...
def _write_reports(result: dict[str, Any]) -> None:
    ...
    (report_dir / "zip_audit_v9_38.json").write_text(json.dumps(result, indent=2, ensure_ascii=False) + "\n", encoding="utf-8")
    (report_dir / "zip_audit_v9_38.md").write_text("# Audit ZIP V9.38\n\n" f"- Resultat : `{'PASS' if result['passed'] else 'FAIL'}`.\n" f"- Erreurs : `{result['errors']}`.\n" "- Aucun sidecar et aucune empreinte ZIP.\n", encoding="utf-8")
