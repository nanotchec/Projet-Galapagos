...
REQUIRED_FILES = [
    ...
    "scripts/run_aggtrades_post_v9_batch_collection_v9_20.py",
    "scripts/validate_aggtrades_post_v9_batch_collection_v9_20.py",
    "scripts/release_audit_lite_zip_v9_20.py",
    "scripts/audit_audit_lite_zip_v9_20.py",
    "scripts/smoke_audit_lite_zip_v9_20.py",
    "src/galapagos/data/aggtrades_post_v9_collection_v9_18.py",
    "src/galapagos/data/aggtrades_post_v9_batch_collection_v9_20.py",
    "src/galapagos/data/aggtrades_post_v9_batch_collection_v9_20_validation.py",
    "tests/data/test_aggtrades_post_v9_batch_collection_v9_20.py",
    "tests/validation/test_aggtrades_post_v9_batch_collection_v9_20_validator.py",
    ...
    "reports/data/aggtrades_post_v9_batch_collection_v9_20.md",
    "docs/aggtrades_post_v9_batch_collection_v9_20.md",
    "reports/audit_lite/v9_20_command_results.json",
    "reports/audit_lite/v9_20_command_results.md",
    "reports/audit_lite/v9_20_full_local_validation_attestation.json",
    "reports/audit_lite/v9_20_full_local_validation_attestation.md",
    "reports/audit_lite/v9_20_artifact_inventory.json",
    "reports/audit_lite/v9_20_artifact_inventory.md",
    "reports/audit_lite/zip_size_report_v9_20.json",
    "reports/audit_lite/zip_size_report_v9_20.md",
    "reports/audit_lite/zip_audit_v9_20.json",
    "reports/audit_lite/zip_audit_v9_20.md",
    "reports/audit_lite/zip_smoke_v9_20.json",
    "reports/audit_lite/zip_smoke_v9_20.md",
...
def audit_zip(zip_path: Path) -> list[str]:
    ...
    errors: list[str] = []
    with tempfile.TemporaryDirectory(prefix="galapagos_v9_20_audit_") as tmp:
...
def _check_inventory(extract_root: Path, names: list[str]) -> list[str]:
    errors: list[str] = []
    inventory = _read_json(extract_root / "reports/audit_lite/v9_20_artifact_inventory.json")
    size_report = _read_json(extract_root / "reports/audit_lite/zip_size_report_v9_20.json")
...
def _check_reports(extract_root: Path) -> list[str]:
    errors: list[str] = []
    report = _read_json(extract_root / "reports/data/aggtrades_post_v9_batch_collection_v9_20.json")
    manifest = _read_json(extract_root / "reports/manifests/aggtrades_post_v9_batch_collection_v9_20_manifest.json")
    attestation = _read_json(extract_root / "reports/audit_lite/v9_20_full_local_validation_attestation.json")
    if report.get("mode") != "collect":
        errors.append("V9.20 audit-lite must describe the collect batch")
    if report.get("v9_20_decision", {}).get("decision") != "aggtrades_post_v9_batch_collection_success":
        errors.append("V9.20 decision mismatch")
    batch = report.get("batch_window", {})
    if batch.get("start") != "2024-05-12" or batch.get("end") != "2024-06-10" or batch.get("days_requested") != 30:
        errors.append("V9.20 batch window mismatch")
    if batch.get("max_downloads") != 30:
        errors.append("V9.20 batch max_downloads mismatch")
    summary = report.get("batch_validation", {}).get("summary", {})
    for key in ["days_requested", "days_attempted", "days_normalized", "days_complete"]:
        if summary.get(key) != 30:
            errors.append(f"V9.20 {key} must equal 30")
    if summary.get("days_skipped_existing") not in {0, None}:
        errors.append("V9.20 first batch audit expects no skipped existing days inside the new batch")
    if summary.get("days_failed") != 0 or summary.get("days_quarantined") != 0:
        errors.append("V9.20 batch must have no failed or quarantined days")
    if summary.get("total_rows", 0) <= 0 or summary.get("raw_bytes_total", 0) <= 0 or summary.get("silver_bytes_total", 0) <= 0:
        errors.append("V9.20 batch row/byte totals must be positive")
    if report.get("complete_collection_reached") is not False or summary.get("future_full_coverage_complete") is not False:
        errors.append("V9.20 must not claim complete future coverage")
    for key in ["features_created", "labels_created", "dataset_created", "ml_executed", "walk_forward_executed", "backtest_executed"]:
        if report.get(key) is not False:
            errors.append(f"V9.20 report must keep {key}=false")
    source = report.get("source_public_target", {})
    if source.get("host") != "data.binance.vision":
        errors.append("V9.20 source host mismatch")
...
def _check_command_results(extract_root: Path) -> list[str]:
    errors: list[str] = []
    payload = _read_json(extract_root / "reports/audit_lite/v9_20_command_results.json")
    ...
    required_needles = [
        ...
        "test_aggtrades_post_v9_batch_collection_v9_20.py",
        "test_aggtrades_post_v9_batch_collection_v9_20_validator.py",
        "run_aggtrades_post_v9_batch_collection_v9_20.py --mode collect --start-date 2024-05-12 --end-date 2024-06-10 --max-downloads 30",
        "validate_aggtrades_post_v9_batch_collection_v9_20.py",
        "release_audit_lite_zip_v9_20.py",
    ]
...
def _check_state(extract_root: Path) -> list[str]:
    ...
    for name, payload in {"PROJECT_STATE": state, "latest_metrics": metrics}.items():
        if payload.get("last_validated_version") != "V9.19":
        ...
        if payload.get("direction") != "aggtrades_post_v9_controlled_batch_collection":
...
def _write_reports(result: dict[str, Any]) -> None:
    ...
    (report_dir / "zip_audit_v9_20.json").write_text(json.dumps(result, indent=2, ensure_ascii=False) + "\n", encoding="utf-8")
    (report_dir / "zip_audit_v9_20.md").write_text(
        "# Audit ZIP V9.20\n\n"
