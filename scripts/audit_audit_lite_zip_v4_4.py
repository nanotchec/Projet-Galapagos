...
VERSION = "V4.4"
REPORT_JSON = Path("reports/audit_lite/zip_audit_v4_4.json")
REPORT_MD = Path("reports/audit_lite/zip_audit_v4_4.md")
REQUIRED_FILES = {
    "README.md",
    "pyproject.toml",
    "reports/manifests/one_year_label_factory_v4_4_manifest.json",
    "reports/labels/one_year_label_factory_v4_4.json",
    "reports/labels/one_year_label_factory_v4_4.md",
    "reports/audit_lite/v4_4_artifact_inventory.json",
    "reports/audit_lite/v4_4_artifact_inventory.md",
    "reports/audit_lite/v4_4_parquet_summary.json",
    "reports/audit_lite/v4_4_full_local_validation_attestation.json",
    "reports/audit_lite/v4_4_full_local_validation_attestation.md",
...
def audit_zip(zip_path: Path) -> dict[str, Any]:
    ...
    with tempfile.TemporaryDirectory(prefix="galapagos-v4-4-audit-") as tmp:
        ...
        attestation = _read_json(extract_root / "reports/audit_lite/v4_4_full_local_validation_attestation.json")
        if manifest != report:
            errors.append("V4.4 manifest/report mismatch in ZIP")
        if manifest.get("version") != VERSION:
            errors.append("V4.4 manifest version mismatch in ZIP")
        ...
        if set(summary.get("labels", {})) != {"1m", "5m", "15m", "1h"}:
            errors.append("V4.4 parquet summary must include all label timeframes")
        ...
        for flag in ["validator_passed", "tests_passed", "audit_lite_passed", "smoke_audit_lite_passed", "no_trading", "no_backtest", "no_orders"]:
            if attestation.get(flag) is not True:
                errors.append(f"V4.4 attestation flag must be true: {flag}")
...
def _render_markdown(result: dict[str, Any]) -> str:
    ...
    return f"""# Audit ZIP audit-lite V4.4
