...
REQUIRED_FILES = [
    ...
    "scripts/run_ohlcv_aggtrades_5y_feature_store_v9_33.py",
    "scripts/validate_ohlcv_aggtrades_5y_feature_store_v9_33.py",
    "scripts/release_audit_lite_zip_v9_33.py",
    "scripts/audit_audit_lite_zip_v9_33.py",
    "scripts/smoke_audit_lite_zip_v9_33.py",
    "src/galapagos/features/ohlcv_aggtrades_5y_feature_store_v9_33_schemas.py",
    "src/galapagos/features/ohlcv_aggtrades_5y_feature_store_v9_33.py",
    "src/galapagos/features/ohlcv_aggtrades_5y_feature_store_v9_33_validation.py",
    ...
    "reports/manifests/ohlcv_aggtrades_5y_feature_store_v9_33_manifest.json",
    "reports/features/ohlcv_aggtrades_5y_feature_store_v9_33.json",
    "reports/features/ohlcv_aggtrades_5y_feature_store_v9_33.md",
    "docs/ohlcv_aggtrades_5y_feature_store_v9_33.md",
    "reports/audit_lite/v9_33_command_results.json",
    "reports/audit_lite/v9_33_command_results.md",
    "reports/audit_lite/v9_33_full_local_validation_attestation.json",
    "reports/audit_lite/v9_33_full_local_validation_attestation.md",
    "reports/audit_lite/v9_33_artifact_inventory.json",
    "reports/audit_lite/v9_33_artifact_inventory.md",
    "reports/audit_lite/zip_size_report_v9_33.json",
    "reports/audit_lite/zip_size_report_v9_33.md",
    "reports/audit_lite/zip_audit_v9_33.json",
    "reports/audit_lite/zip_audit_v9_33.md",
    "reports/audit_lite/zip_smoke_v9_33.json",
    "reports/audit_lite/zip_smoke_v9_33.md",
...
def audit_zip(zip_path: Path) -> list[str]:
    ...
    errors: list[str] = []
    with tempfile.TemporaryDirectory(prefix="galapagos_v9_33_audit_") as tmp:
...
def _check_inventory(extract_root: Path, names: list[str]) -> list[str]:
    errors: list[str] = []
    inventory = _read_json(extract_root / "reports/audit_lite/v9_33_artifact_inventory.json")
    size_report = _read_json(extract_root / "reports/audit_lite/zip_size_report_v9_33.json")
...
def _check_reports(extract_root: Path) -> list[str]:
    ...
    manifest = _read_json(extract_root / "reports/manifests/ohlcv_aggtrades_5y_feature_store_v9_33_manifest.json")
    attestation = _read_json(extract_root / "reports/audit_lite/v9_33_full_local_validation_attestation.json")
    if report.get("version") != VERSION or report.get("source_version") != "V9.32":
        errors.append("V9.33 report version/source mismatch")
    if report.get("decision") not in ALLOWED_DECISIONS:
        errors.append("V9.33 decision is not allowed")
    if manifest.get("decision") != report.get("decision") or manifest.get("safety_flags") != report.get("safety_flags"):
        errors.append("V9.33 manifest mismatch")
    if report.get("decision") == "ohlcv_5y_extension_required_before_feature_store":
        for key, expected in {
            ...
            if report.get(key) != expected or attestation.get(key) != expected:
                errors.append(f"V9.33 not-created report mismatch for {key}")
        if report.get("ohlcv_readiness", {}).get("ohlcv_5y_ready") is not False:
            errors.append("V9.33 expected incomplete OHLCV readiness for extension-required decision")
...
def _check_flags(report: dict[str, Any], attestation: dict[str, Any]) -> list[str]:
    ...
    for key in ["api_key_used", "private_endpoint_used", "exchange_auth_used", "websocket_live_used", "network_used", "ingestion_executed"]:
...
def _check_command_results(extract_root: Path) -> list[str]:
    payload = _read_json(extract_root / "reports/audit_lite/v9_33_command_results.json")
    ...
    for needle in [
        ...
        "test_ohlcv_aggtrades_5y_feature_store_v9_33.py",
        "test_ohlcv_aggtrades_5y_feature_store_v9_33_validator.py",
        "run_ohlcv_aggtrades_5y_feature_store_v9_33.py",
        "validate_ohlcv_aggtrades_5y_feature_store_v9_33.py",
        "release_audit_lite_zip_v9_33.py",
    ]:
...
def _check_state(extract_root: Path) -> list[str]:
    ...
    errors: list[str] = []
    if metrics.get("last_validated_version") != "V9.32":
    ...
    if metrics.get("direction") != "ohlcv_aggtrades_5y_feature_store_readiness":
...
def _write_reports(result: dict[str, Any]) -> None:
    ...
    (report_dir / "zip_audit_v9_33.json").write_text(json.dumps(result, indent=2, ensure_ascii=False) + "\n", encoding="utf-8")
    (report_dir / "zip_audit_v9_33.md").write_text(
        "# Audit ZIP V9.33\n\n"
