...
def smoke_zip(zip_path: Path) -> dict[str, Any]:
    errors: list[str] = []
    with tempfile.TemporaryDirectory(prefix="galapagos_v9_29_smoke_") as tmp:
        ...
        errors.extend(_run_v9_29_tests(extract_root, env))
...
def _import_modules(extract_root: Path, env: dict[str, str]) -> list[str]:
    ...
    for module in [
        ...
        "galapagos.data.aggtrades_post_v9_full_coverage_validation_v9_29",
        "galapagos.data.aggtrades_post_v9_full_coverage_validation_v9_29_validation",
    ]:
...
def _run_v9_29_tests(extract_root: Path, env: dict[str, str]) -> list[str]:
    completed = _run(
        [
            "python",
            "-m",
    ...
    if completed["returncode"] != 0:
        return [f"V9.29 tests failed: {completed['stdout'][-1000:]} {completed['stderr'][-1000:]}"]
    return []
...
def _check_reports(extract_root: Path) -> list[str]:
    ...
    if report.get("version") != VERSION:
        errors.append("V9.29 report version mismatch")
    if report.get("decision", "").startswith("aggtrades_full_coverage_validated"):
        if report.get("local_file_coverage_start") != "2024-05-05" or report.get("local_file_coverage_end") != "2026-05-05":
            errors.append("V9.29 validated smoke expects full target coverage")
        if report.get("complete_collection_reached") is not True:
            errors.append("V9.29 validated smoke expects complete_collection_reached=true")
        if report.get("network_used") is not False or report.get("new_data_downloaded") is not False:
            errors.append("V9.29 smoke expects no network and no new download")
    else:
        if report.get("complete_collection_reached") is True:
            errors.append("V9.29 blocked smoke cannot claim complete collection")
    return errors


def _run_self_audit(extract_root: Path, zip_path: Path, env: dict[str, str]) -> list[str]:
    completed = _run(["python", "scripts/audit_audit_lite_zip_v9_29.py", "--zip", str(zip_path)], extract_root, env, AUDIT_TIMEOUT_SECONDS)
    if completed["returncode"] != 0:
...
def _write_reports(result: dict[str, Any]) -> None:
    ...
    (report_dir / "zip_smoke_v9_29.json").write_text(json.dumps(result, indent=2, ensure_ascii=False) + "\n", encoding="utf-8")
    (report_dir / "zip_smoke_v9_29.md").write_text(
        "# Smoke ZIP V9.29\n\n"
        ...
        f"- Timeout tests V9.29 : `{TEST_TIMEOUT_SECONDS}` secondes.\n"
