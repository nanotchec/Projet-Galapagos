...
REQUIRED_FILES = [
    "src/galapagos/ml/ohlcv_aggtrades_5y_offline_ml_v9_43.py",
    "src/galapagos/ml/ohlcv_aggtrades_5y_offline_ml_v9_43_metrics.py",
    "src/galapagos/ml/ohlcv_aggtrades_5y_offline_ml_v9_43_quality.py",
    "src/galapagos/ml/ohlcv_aggtrades_5y_offline_ml_v9_43_validation.py",
    "scripts/run_ohlcv_aggtrades_5y_offline_ml_v9_43.py",
    "scripts/validate_ohlcv_aggtrades_5y_offline_ml_v9_43.py",
    "scripts/release_audit_lite_zip_v9_43.py",
    "scripts/audit_audit_lite_zip_v9_43.py",
    "scripts/smoke_audit_lite_zip_v9_43.py",
    "tests/ml/test_ohlcv_aggtrades_5y_offline_ml_v9_43.py",
    "tests/validation/test_ohlcv_aggtrades_5y_offline_ml_v9_43_validator.py",
    "reports/manifests/ohlcv_aggtrades_5y_offline_ml_v9_43_manifest.json",
    "reports/ml/ohlcv_aggtrades_5y_offline_ml_v9_43.json",
    "reports/ml/ohlcv_aggtrades_5y_offline_ml_v9_43.md",
    "reports/ml/ohlcv_aggtrades_5y_offline_scores_v9_43.json",
    "reports/ml/ohlcv_aggtrades_5y_offline_scores_v9_43.md",
    "docs/ohlcv_aggtrades_5y_offline_ml_v9_43.md",
    "reports/audit_lite/v9_43_command_results.json",
    "reports/audit_lite/v9_43_command_results.md",
    "reports/audit_lite/v9_43_full_local_validation_attestation.json",
    "reports/audit_lite/v9_43_full_local_validation_attestation.md",
    "reports/audit_lite/v9_43_artifact_inventory.json",
    "reports/audit_lite/v9_43_artifact_inventory.md",
    "reports/audit_lite/zip_size_report_v9_43.json",
    "reports/audit_lite/zip_size_report_v9_43.md",
    "reports/audit_lite/zip_audit_v9_43.json",
    "reports/audit_lite/zip_audit_v9_43.md",
    "reports/audit_lite/zip_smoke_v9_43.json",
    "reports/audit_lite/zip_smoke_v9_43.md",
]
...
def audit_zip(zip_path: Path) -> list[str]:
    ...
    errors: list[str] = []
    with tempfile.TemporaryDirectory(prefix="galapagos_v9_43_audit_") as tmp:
...
def _check_inventory(extract_root: Path, names: list[str]) -> list[str]:
    errors: list[str] = []
    inventory = _read_json(extract_root / "reports/audit_lite/v9_43_artifact_inventory.json")
    size_report = _read_json(extract_root / "reports/audit_lite/zip_size_report_v9_43.json")
...
def _check_reports(extract_root: Path) -> list[str]:
    errors: list[str] = []
    report = _read_json(extract_root / "reports/ml/ohlcv_aggtrades_5y_offline_ml_v9_43.json")
    manifest = _read_json(extract_root / "reports/manifests/ohlcv_aggtrades_5y_offline_ml_v9_43_manifest.json")
    scores = _read_json(extract_root / "reports/ml/ohlcv_aggtrades_5y_offline_scores_v9_43.json")
    attestation = _read_json(extract_root / "reports/audit_lite/v9_43_full_local_validation_attestation.json")
    if report.get("version") != VERSION or report.get("source_version") != "V9.42":
        errors.append("V9.43 report version/source mismatch")
    if report.get("ml_executed") is not True or report.get("model_persisted") is not False:
        errors.append("V9.43 must execute ML without persisting models")
    ...
    if scores.get("contains_predictions") is not False or scores.get("contains_actionable_signal") is not False:
        errors.append("V9.43 scores must be aggregate-only")
    if manifest.get("decision") != report.get("decision") or manifest.get("safety_flags") != report.get("safety_flags"):
        errors.append("V9.43 manifest mismatch")
    errors.extend(_check_flags(report, attestation))
    for payload_name, payload in {"report": report, "manifest": manifest, "scores": scores, "attestation": attestation}.items():
        if _contains_forbidden_zip_field(payload):
            errors.append(f"{payload_name} contains forbidden ZIP fingerprint or sidecar field")
    return errors
...
def _check_flags(report: dict[str, Any], attestation: dict[str, Any]) -> list[str]:
    errors: list[str] = []
    for key in ["no_trading", "no_paper_live", "no_orders", "no_backtest", "no_walk_forward", "no_strategy", "no_actionable_signal", "no_persistent_model", "no_new_data_download", "no_destructive_cleanup", "no_sidecars", "no_zip_fingerprints"]:
...
def _check_command_results(extract_root: Path) -> list[str]:
    payload = _read_json(extract_root / "reports/audit_lite/v9_43_command_results.json")
    ...
    for needle in ["pytest --collect-only -q", "test_ohlcv_aggtrades_5y_offline_ml_v9_43.py", "test_ohlcv_aggtrades_5y_offline_ml_v9_43_validator.py", "run_ohlcv_aggtrades_5y_offline_ml_v9_43.py", "validate_ohlcv_aggtrades_5y_offline_ml_v9_43.py", "release_audit_lite_zip_v9_43.py"]:
...
def _write_reports(result: dict[str, Any]) -> None:
    ...
    (report_dir / "zip_audit_v9_43.json").write_text(json.dumps(result, indent=2, ensure_ascii=False) + "\n", encoding="utf-8")
    (report_dir / "zip_audit_v9_43.md").write_text("# Audit ZIP V9.43\n\n" f"- Resultat : `{'PASS' if result['passed'] else 'FAIL'}`.\n" f"- Erreurs : `{result['errors']}`.\n" "- Aucun sidecar et aucune empreinte ZIP.\n", encoding="utf-8")
