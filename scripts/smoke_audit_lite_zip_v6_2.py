...
VERSION = "V6.2"
SMOKE_JSON = Path("reports/audit_lite/zip_smoke_v6_2.json")
SMOKE_MD = Path("reports/audit_lite/zip_smoke_v6_2.md")
...
def smoke_zip(zip_path: Path) -> dict[str, Any]:
    ...
    with tempfile.TemporaryDirectory(prefix="galapagos_v6_2_smoke_") as tmp:
        target = Path(tmp)
...
def _check_imports(root: Path) -> list[str]:
    ...
    errors: list[str] = []
    for module in [
        "galapagos.ml.advanced_ohlcv_window",
        "galapagos.ml.advanced_ohlcv_window_metrics",
        "galapagos.ml.advanced_ohlcv_window_quality",
        "galapagos.ml.advanced_ohlcv_window_validation",
...
def _check_samples(root: Path) -> list[str]:
    ...
    expected_columns = schema_module.ML_SCORE_COLUMNS_V6_2
    forbidden = {"signal", "trading_signal", "order", "strategy", "pnl", "profit"}
    for timeframe in ["1m", "5m", "15m", "1h"]:
        path = root / "data" / "audit_lite" / "v6_2" / "ml_scores" / f"timeframe={timeframe}" / "sample.parquet"
        if not path.exists():
...
def _check_reports(root: Path) -> list[str]:
    errors: list[str] = []
    manifest_path = root / "reports/manifests/advanced_ohlcv_offline_ml_research_v6_2_manifest.json"
    report_path = root / "reports/ml/advanced_ohlcv_offline_ml_research_v6_2.json"
