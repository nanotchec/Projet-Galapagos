...
ESSENTIAL_ENTRIES = [
    ...
    "src/galapagos/features/__init__.py",
...
def main() -> None:
    ...
    with tempfile.TemporaryDirectory(prefix="galapagos_v3_5_2_smoke_") as tmp:
        tmp_path = Path(tmp)
    ...
    Path("reports/zip_smoke_test_v3_5_2.json").write_text(json.dumps(payload, indent=2, ensure_ascii=False) + "\n", encoding="utf-8")
    Path("reports/zip_smoke_test_v3_5_2.md").write_text(
        "# Smoke ZIP V3.5.2\n\n"
...
def _validate_manifest_state_and_safety(root: Path) -> list[str]:
    ...
    if manifest.get("version") != MANIFEST_VERSION or report.get("version") != MANIFEST_VERSION:
    ...
    if project_state.get("candidate_version") != VERSION and VERSION not in latest_summary:
        errors.append("PROJECT_STATE/latest_summary must identify V3.5.2 as candidate")
