...
VERSION = "V5.3"
REPORT_JSON = Path("reports/audit_lite/zip_audit_v5_3.json")
REPORT_MD = Path("reports/audit_lite/zip_audit_v5_3.md")
REQUIRED_FILES = {
    "README.md",
    "pyproject.toml",
    "reports/manifests/max_history_offline_supervised_dataset_v5_3_manifest.json",
    "reports/datasets/max_history_offline_supervised_dataset_v5_3.json",
    "reports/datasets/max_history_offline_supervised_dataset_v5_3.md",
    "reports/datasets/max_history_offline_supervised_dataset_v5_3_datacard.md",
    "reports/audit_lite/v5_3_artifact_inventory.json",
    "reports/audit_lite/v5_3_artifact_inventory.md",
    "reports/audit_lite/v5_3_parquet_summary.json",
    "reports/audit_lite/v5_3_full_local_validation_attestation.json",
    "reports/audit_lite/v5_3_full_local_validation_attestation.md",
    "reports/PROJECT_STATE.json",
    "reports/current/latest_summary.md",
    "docs/max_history_offline_supervised_dataset_v5_3.md",
    "scripts/run_max_history_offline_supervised_dataset_v5_3.py",
    "scripts/validate_max_history_offline_supervised_dataset_v5_3.py",
    "scripts/release_audit_lite_zip_v5_3.py",
    "scripts/audit_audit_lite_zip_v5_3.py",
    "scripts/smoke_audit_lite_zip_v5_3.py",
    "tests/datasets/test_max_history_offline_supervised_dataset_v5_3.py",
    "tests/validation/test_max_history_offline_supervised_dataset_v5_3_validator.py",
}
...
def audit_zip(zip_path: Path) -> dict[str, Any]:
    ...
    missing = sorted((REQUIRED_FILES | REQUIRED_DATASET_SAMPLES | REQUIRED_SPLIT_SAMPLES) - entry_set)
    if missing:
    ...
    allowed_parquet = REQUIRED_DATASET_SAMPLES | REQUIRED_SPLIT_SAMPLES
    for entry in entries:
        ...
        if path.suffix.casefold() == ".parquet" and entry not in allowed_parquet:
            errors.append(f"unexpected Parquet in audit-lite ZIP: {entry}")

    with tempfile.TemporaryDirectory(prefix="galapagos-v5-3-audit-") as tmp:
        ...
        attestation = _read_json(extract_root / "reports/audit_lite/v5_3_full_local_validation_attestation.json")
        if manifest != report:
            errors.append("V5.3 manifest/report mismatch in ZIP")
        if manifest.get("version") != VERSION or manifest.get("status") != "PASS":
            errors.append("V5.3 manifest version/status mismatch in ZIP")
        ...
        if len(inventory.get("full_parquet_excluded", [])) != 8:
            errors.append("V5.3 full parquet exclusion count mismatch")
        ...
        for sample in REQUIRED_DATASET_SAMPLES:
        ...
        for sample in REQUIRED_SPLIT_SAMPLES:
            frame = pd.read_parquet(extract_root / sample, engine="pyarrow")
        ...
        for flag in [
            ...
            if attestation.get(flag) is not True:
                errors.append(f"V5.3 attestation flag must be true: {flag}")
...
def _render_markdown(result: dict[str, Any]) -> str:
    ...
    return f"""# Audit ZIP audit-lite V5.3
