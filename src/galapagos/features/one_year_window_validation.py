...
from galapagos.features.one_year_window_quality import assess_one_year_feature_quality
...
RUN_ID_PATTERN_V4_3 = re.compile(r"^v4_3_[0-9]{8}T[0-9]{6}Z_[0-9a-f]{8}$")
MANIFEST_KEYS = {
    "version",
    "status",
    "created_at_utc",
...
FORBIDDEN_V4_3_PATHS = [
    "data/research/v4_3/labels",
    "data/research/v4_3/datasets",
    "data/research/v4_3/ml",
    "data/research/v4_3/backtests",
    "data/research/v4_3/strategies",
    "reports/backtests",
    "reports/strategies",
    "reports/signals",
    "reports/predictions",
...
def validate_one_year_causal_feature_store_v4_3(root: Path = Path(".")) -> dict[str, Any]:
    root = root.resolve()
    errors: list[str] = []
    manifest_path = root / MANIFEST_PATH_V4_3
    report_path = root / REPORT_JSON_PATH_V4_3
    if not manifest_path.exists():
        return _result([f"missing V4.3 manifest: {MANIFEST_PATH_V4_3}"])
    if not report_path.exists():
        return _result([f"missing V4.3 feature report: {REPORT_JSON_PATH_V4_3}"])

    manifest = load_json(manifest_path)
    report = load_json(report_path)
    errors.extend(_validate_manifest_structure(root, manifest))
    errors.extend(scan_payload_for_forbidden_claims(manifest, "V4.3 manifest"))
    ...
    for timeframe in TIMEFRAMES_V4_3:
        ...
        if not input_path.exists():
            errors.append(f"missing V4.3 input OHLCV: {input_path.relative_to(root)}")
            continue
        if not feature_path.exists():
            errors.append(f"missing V4.3 output features: {feature_path.relative_to(root)}")
            continue
    ...
    errors.extend(scan_payload_for_forbidden_claims(report, "V4.3 feature report"))
    errors.extend(_validate_markdown(root))
    errors.extend(_find_forbidden_v4_3_artifacts(root))
    return _result(errors, manifest=manifest)


def _validate_manifest_structure(root: Path, manifest: dict[str, Any]) -> list[str]:
    errors: list[str] = []
    errors.extend(validate_exact_keys(manifest, MANIFEST_KEYS, "V4.3 manifest"))
    if manifest.get("version") != VERSION_V4_3:
        errors.append("V4.3 manifest version mismatch")
    if manifest.get("status") != "PASS":
        errors.append("V4.3 manifest status must be PASS")
    if not isinstance(manifest.get("created_at_utc"), str) or not manifest["created_at_utc"].endswith("Z"):
        errors.append("V4.3 manifest created_at_utc invalid")
    run_id = manifest.get("feature_run_id")
    if not isinstance(run_id, str) or RUN_ID_PATTERN_V4_3.fullmatch(run_id) is None:
        errors.append("V4.3 manifest feature_run_id invalid")
    if manifest.get("feature_schema_version") != FEATURE_SCHEMA_VERSION_V4_3:
        errors.append("V4.3 manifest feature_schema_version mismatch")
    if manifest.get("feature_columns") != FEATURE_COLUMNS_V4_3:
        errors.append("V4.3 manifest feature_columns mismatch")
    if manifest.get("limitations") != EXPECTED_LIMITATIONS_V4_3:
        errors.append("V4.3 manifest limitations mismatch")
    for section, keys in [("input_ohlcv", INPUT_KEYS), ("outputs", OUTPUT_KEYS), ("quality", QUALITY_KEYS)]:
        payload = manifest.get(section, {})
        errors.extend(validate_exact_keys(payload, set(TIMEFRAMES_V4_3), f"V4.3 manifest {section}"))
        for timeframe, block in payload.items():
            errors.extend(validate_exact_keys(block, keys, f"V4.3 manifest {section}.{timeframe}"))
    errors.extend(validate_exact_keys(manifest.get("safety"), SAFETY_KEYS, "V4.3 manifest safety"))
    ...
    for timeframe in TIMEFRAMES_V4_3:
        ...
        if manifest.get("input_ohlcv", {}).get(timeframe, {}).get("path") != str(input_path.relative_to(root)):
            errors.append(f"V4.3 manifest input path mismatch for {timeframe}")
        if manifest.get("outputs", {}).get(timeframe, {}).get("path") != str(feature_path.relative_to(root)):
            errors.append(f"V4.3 manifest output path mismatch for {timeframe}")
    return errors
...
def _validate_safety(safety: dict[str, Any]) -> list[str]:
    ...
    if safety.get("public_read_only") is not True:
        errors.append("V4.3 safety flag public_read_only must be True")
    for flag in sorted(SAFETY_KEYS - {"public_read_only"}):
        if safety.get(flag) is not False:
            errors.append(f"V4.3 safety flag {flag} must be False")
    return errors
...
def _validate_input_entry(
    ...
    for field, value in expected.items():
        if payload.get(field) != value:
            errors.append(f"V4.3 manifest input mismatch for {timeframe}.{field}")
    return errors
...
def _validate_output_entry(root: Path, manifest: dict[str, Any], timeframe: str, path: Path, frame: pd.DataFrame) -> list[str]:
    ...
    for field, value in expected.items():
        if payload.get(field) != value:
            errors.append(f"V4.3 manifest output mismatch for {timeframe}.{field}")
    return errors
...
def _validate_feature_frame(
    ...
    errors: list[str] = []
    if list(frame.columns) != FEATURE_COLUMNS_V4_3:
        errors.append(f"V4.3 feature schema mismatch for {timeframe}")
    forbidden = _forbidden_columns(frame)
    if forbidden:
        errors.append(f"V4.3 forbidden feature columns for {timeframe}: {forbidden}")
    if len(frame) != len(input_frame):
        errors.append(f"V4.3 feature row count mismatch for {timeframe}")
    ...
    if not source_hashes_valid:
        errors.append(f"V4.3 source_ohlcv_sha256 mismatch for {timeframe}")
    if "feature_run_id" in frame.columns and feature_run_id is not None:
        if set(frame["feature_run_id"].astype(str).unique()) != {str(feature_run_id)}:
            errors.append(f"V4.3 feature_run_id mismatch for {timeframe}")
    if "feature_schema_version" in frame.columns:
        if set(frame["feature_schema_version"].astype(str).unique()) != {FEATURE_SCHEMA_VERSION_V4_3}:
            errors.append(f"V4.3 feature_schema_version mismatch for {timeframe}")
    if {"event_ts", "close_ts", "available_ts", "decision_ts"}.issubset(frame.columns) and {
        ...
        try:
            ...
            errors.append(f"V4.3 feature metadata mismatch with input OHLCV for {timeframe}")
    if {"feature_available_ts", "available_ts"}.issubset(frame.columns):
        ...
        if not bool((feature_available == available).all()):
            errors.append(f"V4.3 feature_available_ts must equal available_ts for {timeframe}")

    quality = assess_one_year_feature_quality(frame, timeframe)
    quality["source_hashes_valid"] = source_hashes_valid
    errors.extend(f"V4.3 physical quality error for {timeframe}: {error}" for error in quality["errors"])
...
def _validate_manifest_quality(manifest: dict[str, Any], physical_quality: dict[str, dict[str, Any]]) -> list[str]:
    ...
    for timeframe in TIMEFRAMES_V4_3:
        ...
        if not isinstance(declared, dict):
            errors.append(f"V4.3 manifest quality missing for {timeframe}")
            continue
        if not isinstance(physical, dict):
            errors.append(f"V4.3 physical quality unavailable for {timeframe}")
            continue
        for field in sorted(QUALITY_KEYS - {"warnings", "errors"}):
            if declared.get(field) != physical.get(field):
                errors.append(f"V4.3 manifest quality mismatch for {timeframe}.{field}")
        if declared.get("errors") != physical.get("errors"):
            errors.append(f"V4.3 manifest quality errors mismatch for {timeframe}")
        if declared.get("warnings") != physical.get("warnings"):
            errors.append(f"V4.3 manifest quality warnings mismatch for {timeframe}")
    return errors


def _validate_report(manifest: dict[str, Any], report: dict[str, Any]) -> list[str]:
    errors: list[str] = []
    errors.extend(validate_exact_keys(report, REPORT_KEYS, "V4.3 feature report"))
    expected = {key: manifest.get(key) for key in REPORT_KEYS}
    for field, value in expected.items():
        if report.get(field) != value:
            errors.append(f"V4.3 feature report {field} mismatch")
    errors.extend(_validate_safety(report.get("safety", {})))
    return errors


def _validate_markdown(root: Path) -> list[str]:
    path = root / REPORT_MD_PATH_V4_3
    if not path.exists():
        return [f"missing V4.3 feature markdown: {REPORT_MD_PATH_V4_3}"]
    text = path.read_text(encoding="utf-8")
    errors = validate_markdown_forbidden_claims(text, "V4.3 feature markdown")
    required = [
    ...
    for clause in required:
        if clause.replace("’", "'") not in normalized_text:
            errors.append(f"V4.3 feature markdown missing safety clause: {clause}")
    return errors


def _find_forbidden_v4_3_artifacts(root: Path) -> list[str]:
    errors: list[str] = []
    for relative in FORBIDDEN_V4_3_PATHS:
        path = root / relative
        if path.exists():
            errors.append(f"Forbidden V4.3 artifact detected: {relative}")
            for child in sorted(path.rglob("*")):
                if child.is_file():
                    errors.append(f"Forbidden V4.3 artifact detected: {child.relative_to(root).as_posix()}")
    for path in root.rglob("*"):
        if ".git" in path.parts or ".venv" in path.parts or not path.is_file():
            continue
        if path.suffix.casefold() in FORBIDDEN_MODEL_SUFFIXES:
            errors.append(f"Forbidden V4.3 artifact detected: {path.relative_to(root).as_posix()}")
    return sorted(set(errors))


def _result(errors: list[str], *, manifest: dict[str, Any] | None = None) -> dict[str, Any]:
    return {"version": VERSION_V4_3, "passed": not errors, "errors": errors, "manifest": manifest}
