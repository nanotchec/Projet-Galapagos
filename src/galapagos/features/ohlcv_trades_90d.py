...
from galapagos.features.ohlcv_trades_90d_quality import assess_ohlcv_trades_feature_quality_v7_8
from galapagos.features.ohlcv_trades_90d_schemas import (
    OHLCV_TRADES_FEATURE_COLUMNS_V7_8,
    OHLCV_TRADES_VALUE_COLUMNS_V7_8,
)


VERSION_V7_8 = "V7.8"
FEATURE_SCHEMA_VERSION_V7_8 = "OHLCV_TRADES_FEATURE_COLUMNS_V7_8"
TIMEFRAMES_V7_8 = ["1m", "5m", "15m", "1h"]
EXPECTED_ROWS_V7_8 = {"1m": 129600, "5m": 25920, "15m": 8640, "1h": 2160}
TIMEFRAME_MINUTES_V7_8 = {"1m": 1, "5m": 5, "15m": 15, "1h": 60}
V7_7_TRADES_MANIFEST_PATH = Path("reports/manifests/public_trades_90d_window_v7_7_manifest.json")
MANIFEST_PATH_V7_8 = Path("reports/manifests/ohlcv_trades_90d_feature_store_v7_8_manifest.json")
REPORT_JSON_PATH_V7_8 = Path("reports/features/ohlcv_trades_90d_feature_store_v7_8.json")
REPORT_MD_PATH_V7_8 = Path("reports/features/ohlcv_trades_90d_feature_store_v7_8.md")
DOC_PATH_V7_8 = Path("docs/ohlcv_trades_90d_feature_store_v7_8.md")
WINDOW_START_V7_8 = "2023-03-25"
WINDOW_END_V7_8 = "2023-06-22"
TOTAL_DAYS_V7_8 = 90
LARGE_TRADE_QUOTE_THRESHOLD_V7_8 = 100_000.0
...
def run_ohlcv_trades_90d_feature_store_v7_8(
    root: Path = Path("."),
    *,
    ...
    if validate_inputs:
        ...
        if not v5_validation["passed"]:
            raise RuntimeError(f"V5.0 validation failed before V7.8: {v5_validation['errors']}")
        v7_7_validation = validate_public_trades_90d_window_v7_7(project_root)
        if not v7_7_validation["passed"]:
            raise RuntimeError(f"V7.7 validation failed before V7.8: {v7_7_validation['errors']}")
    ...
    feature_run_id = f"v7_8_{datetime.now(UTC).strftime('%Y%m%dT%H%M%SZ')}_{uuid.uuid4().hex[:8]}"
    trades_manifest_sha = sha256_file(project_root / V7_7_TRADES_MANIFEST_PATH)
    ...
    for timeframe in TIMEFRAMES_V7_8:
        ...
        ohlcv_window = filter_ohlcv_to_v7_8_window(ohlcv)
        feature_frame = build_ohlcv_trades_90d_features_v7_8(
            ohlcv_window,
        ...
        output = output_path(project_root, timeframe, WINDOW_START_V7_8, WINDOW_END_V7_8)
        write_parquet(feature_frame[OHLCV_TRADES_FEATURE_COLUMNS_V7_8], output)
        ...
        quality[timeframe] = assess_ohlcv_trades_feature_quality_v7_8(
            feature_frame,
            timeframe,
    ...
    manifest = {
        ...
        "input_trades_manifest": {
            "path": V7_7_TRADES_MANIFEST_PATH.as_posix(),
        ...
        "window": {
            "window_start": WINDOW_START_V7_8,
            "window_end": WINDOW_END_V7_8,
            "total_days": TOTAL_DAYS_V7_8,
        ...
        "outputs": outputs,
        "feature_schema_version": FEATURE_SCHEMA_VERSION_V7_8,
        "feature_columns": OHLCV_TRADES_FEATURE_COLUMNS_V7_8,
        "quality": quality,
        "safety": safety_flags_v7_8(),
        "limitations": EXPECTED_LIMITATIONS_V7_8,
    }
    report = build_report_v7_8(manifest)
    _write_json(project_root / MANIFEST_PATH_V7_8, manifest)
    _write_json(project_root / REPORT_JSON_PATH_V7_8, report)
    markdown = build_ohlcv_trades_feature_store_markdown_v7_8(report)
    _write_text(project_root / REPORT_MD_PATH_V7_8, markdown)
    _write_text(project_root / DOC_PATH_V7_8, markdown)
    update_project_state_v7_8(project_root, manifest)
    return manifest


def build_trade_aggregates_by_timeframe(root: Path, trades_manifest: dict[str, Any]) -> dict[str, pd.DataFrame]:
    aggregated: dict[str, list[pd.DataFrame]] = {timeframe: [] for timeframe in TIMEFRAMES_V7_8}
    for _date_key, payload in sorted(trades_manifest["outputs"]["partitions"].items()):
        ...
        partition["is_large_trade"] = partition["quote_quantity"] >= LARGE_TRADE_QUOTE_THRESHOLD_V7_8
        for timeframe in TIMEFRAMES_V7_8:
...
def build_ohlcv_trades_90d_features_v7_8(
    ohlcv: pd.DataFrame,
    ...
    *,
    ...
    features["feature_schema_version"] = FEATURE_SCHEMA_VERSION_V7_8
    features["feature_available_ts"] = features["available_ts"]

    audit_basis = [column for column in OHLCV_TRADES_VALUE_COLUMNS_V7_8 if column not in {"trades_feature_null_count", "trades_feature_error_count"}]
    ...
    return features[OHLCV_TRADES_FEATURE_COLUMNS_V7_8]


def filter_ohlcv_to_v7_8_window(ohlcv: pd.DataFrame) -> pd.DataFrame:
    frame = ohlcv.copy()
    event_ts = pd.to_datetime(frame["event_ts"], utc=True)
    start = pd.Timestamp(f"{WINDOW_START_V7_8}T00:00:00Z")
    end_exclusive = pd.Timestamp(f"{pd.Timestamp(WINDOW_END_V7_8) + pd.Timedelta(days=1):%Y-%m-%d}T00:00:00Z")
...
def load_v7_7_trades_manifest(root: Path) -> dict[str, Any]:
    return _read_json(root / V7_7_TRADES_MANIFEST_PATH)
...
def output_path(root: Path, timeframe: str, window_start: str, window_end: str) -> Path:
    return (
        root
        / "data/research/v7_8/features/ohlcv_trades"
...
def build_report_v7_8(manifest: dict[str, Any]) -> dict[str, Any]:
    return dict(manifest)


def build_ohlcv_trades_feature_store_markdown_v7_8(report: dict[str, Any]) -> str:
    output_rows = "\n".join(
    ...
    return f"""# Feature Store OHLCV + Trades V7.8
...
def safety_flags_v7_8() -> dict[str, bool]:
    return {
...
def update_project_state_v7_8(root: Path, manifest: dict[str, Any]) -> None:
    project_state_path = root / "reports/PROJECT_STATE.json"
    state = _read_json(project_state_path) if project_state_path.exists() else {}
    state.update(
        {
            "last_validated_version": "V7.7",
            "candidate_version": "V7.8",
            "candidate_status": "pending_external_audit",
            "direction": "OHLCV + public trades 90-day feature store preview",
            "ohlcv_trades_90d_features_v7_8_created": True,
            "labels_v7_8_created": False,
            "dataset_v7_8_created": False,
            "ml_v7_8_created": False,
            "model_v7_8_created": False,
            "backtest_v7_8_created": False,
            "strategy_v7_8_created": False,
            "orders_v7_8_created": False,
            "window_start_v7_8": manifest["window"]["window_start"],
            "window_end_v7_8": manifest["window"]["window_end"],
            "total_days_v7_8": manifest["window"]["total_days"],
            "trade_source_type_v7_8": manifest["input_trades_manifest"]["trade_source_type"],
            "feature_columns_v7_8_count": len(manifest["feature_columns"]),
            "output_rows_v7_8": {timeframe: payload["rows"] for timeframe, payload in manifest["outputs"].items()},
            "backtest_enabled": False,
            "strategy_enabled": False,
            "paper_live_enabled": False,
            "orders_enabled": False,
...
def _derive_feature_columns(frame: pd.DataFrame, timeframe: str) -> pd.DataFrame:
    ...
    minutes = TIMEFRAME_MINUTES_V7_8[timeframe]
...
def _validate_input_manifests(v5_manifest: dict[str, Any], trades_manifest: dict[str, Any]) -> None:
    ...
    if discovery["window_start"] != WINDOW_START_V7_8 or discovery["window_end"] != WINDOW_END_V7_8:
        raise ValueError("V7.8 window must exactly match V7.7.")
    if discovery["total_days"] != TOTAL_DAYS_V7_8:
        raise ValueError("V7.8 total_days must exactly match V7.7 total_days.")
    if trades_manifest["source"]["trade_source_type"] != "aggTrades":
        raise ValueError("V7.8 only accepts V7.7 aggTrades inputs.")
    if trades_manifest["source"]["market_type"] != "spot" or trades_manifest["source"]["symbol"] != "BTCUSDT":
        raise ValueError("V7.8 only accepts BTCUSDT spot trades.")
    v5_window_start = v5_manifest["discovery"]["window_start"]
    v5_window_end = v5_manifest["discovery"]["window_end"]
    if WINDOW_START_V7_8 < v5_window_start or WINDOW_END_V7_8 > v5_window_end:
        raise ValueError("V7.8 window must remain inside V5.0 OHLCV window.")
...
def _write_current_reports(root: Path, manifest: dict[str, Any]) -> None:
    ...
    _write_text(
        ...
        "\n".join(
            [
                "# Latest Summary V7.8",
                "",
                "V7.7 est la derniere version validee par audit externe.",
                "",
                "V7.8 est la candidate courante. Elle produit uniquement des features causales OHLCV + aggTrades sur la fenetre V7.7 de 90 jours.",
                "",
                ...
                f"Colonnes features : `{len(manifest['feature_columns'])}`.",
                "",
                "Aucun label V7.8, aucun dataset ML V7.8, aucun modele ML V7.8, aucun backtest, aucune strategie, aucun signal de trading, aucun paper live, aucun ordre et aucun trading reel.",
                "",
                "V7.8 reste `pending_external_audit`.",
            ]
        )
        + "\n",
    )
    _write_text(
        ...
        "\n".join(
            [
                "# Etat du Projet : V7.7 validee + candidat V7.8",
                "",
                "- **Derniere version validee** : V7.7.",
                "- **Version candidate** : V7.8.",
                ...
                f"- Colonnes features : `{len(manifest['feature_columns'])}`.",
                "- Aucun label V7.8.",
                "- Aucun dataset ML V7.8.",
                "- Aucun modele ML V7.8.",
                "- Aucun backtest, aucune strategie, aucun signal, aucun ordre, aucun trading reel.",
                "",
                "V7.8 reste non validee avant audit externe.",
            ]
        )
        + "\n",
    )
