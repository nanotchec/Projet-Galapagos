...
from galapagos.features.ohlcv_trades_1y_quality import assess_ohlcv_trades_feature_quality_v8_3
...
VERSION_V8_3 = "V8.3"
FEATURE_SCHEMA_VERSION_V8_3 = "OHLCV_TRADES_FEATURE_COLUMNS_V8_3"
TIMEFRAMES_V8_3 = ["1m", "5m", "15m", "1h"]
EXPECTED_ROWS_V8_3 = {"1m": 527040, "5m": 105408, "15m": 35136, "1h": 8784}
TIMEFRAME_MINUTES_V8_3 = {"1m": 1, "5m": 5, "15m": 15, "1h": 60}
V8_2_TRADES_MANIFEST_PATH = Path("reports/manifests/public_trades_1y_window_v8_2_manifest.json")
MANIFEST_PATH_V8_3 = Path("reports/manifests/ohlcv_trades_1y_feature_store_v8_3_manifest.json")
REPORT_JSON_PATH_V8_3 = Path("reports/features/ohlcv_trades_1y_feature_store_v8_3.json")
REPORT_MD_PATH_V8_3 = Path("reports/features/ohlcv_trades_1y_feature_store_v8_3.md")
DOC_PATH_V8_3 = Path("docs/ohlcv_trades_1y_feature_store_v8_3.md")
WINDOW_START_V8_3 = "2023-03-25"
WINDOW_END_V8_3 = "2024-03-24"
TOTAL_DAYS_V8_3 = 366
LARGE_TRADE_QUOTE_THRESHOLD_V8_3 = 100_000.0
...
def run_ohlcv_trades_1y_feature_store_v8_3(
    root: Path = Path("."),
    *,
    ...
    if validate_inputs:
        ...
        if not v5_validation["passed"]:
            raise RuntimeError(f"V5.0 validation failed before V8.3: {v5_validation['errors']}")
        v8_2_validation = validate_public_trades_1y_window_v8_2(project_root)
        if not v8_2_validation["passed"]:
            raise RuntimeError(f"V8.2 validation failed before V8.3: {v8_2_validation['errors']}")
    ...
    feature_run_id = f"v8_3_{datetime.now(UTC).strftime('%Y%m%dT%H%M%SZ')}_{uuid.uuid4().hex[:8]}"
    trades_manifest_sha = sha256_file(project_root / V8_2_TRADES_MANIFEST_PATH)
    ...
    for timeframe in TIMEFRAMES_V8_3:
        ...
        ohlcv_window = filter_ohlcv_to_v8_3_window(ohlcv)
        feature_frame = build_ohlcv_trades_1y_features_v8_3(
            ohlcv_window,
        ...
        output = output_path(project_root, timeframe, WINDOW_START_V8_3, WINDOW_END_V8_3)
        write_parquet(feature_frame[OHLCV_TRADES_FEATURE_COLUMNS_V8_3], output)
        ...
        quality[timeframe] = assess_ohlcv_trades_feature_quality_v8_3(
            feature_frame,
            timeframe,
    ...
    manifest = {
        ...
        "input_trades_manifest": {
            "path": V8_2_TRADES_MANIFEST_PATH.as_posix(),
        ...
        "window": {
            "window_start": WINDOW_START_V8_3,
            "window_end": WINDOW_END_V8_3,
            "total_days": TOTAL_DAYS_V8_3,
        ...
        "outputs": outputs,
        "feature_schema_version": FEATURE_SCHEMA_VERSION_V8_3,
        "feature_columns": OHLCV_TRADES_FEATURE_COLUMNS_V8_3,
        "quality": quality,
        "safety": safety_flags_v8_3(),
        "limitations": EXPECTED_LIMITATIONS_V8_3,
    }
    report = build_report_v8_3(manifest)
    _write_json(project_root / MANIFEST_PATH_V8_3, manifest)
    _write_json(project_root / REPORT_JSON_PATH_V8_3, report)
    markdown = build_ohlcv_trades_feature_store_markdown_v8_3(report)
    _write_text(project_root / REPORT_MD_PATH_V8_3, markdown)
    _write_text(project_root / DOC_PATH_V8_3, markdown)
    update_project_state_v8_3(project_root, manifest)
    return manifest


def build_trade_aggregates_by_timeframe(root: Path, trades_manifest: dict[str, Any]) -> dict[str, pd.DataFrame]:
    aggregated: dict[str, list[pd.DataFrame]] = {timeframe: [] for timeframe in TIMEFRAMES_V8_3}
    for _date_key, payload in sorted(trades_manifest["outputs"]["partitions"].items()):
        partition = pd.read_parquet(
            root / payload["path"],
            columns=["price", "quantity", "trade_ts", "is_buyer_maker"],
            engine="pyarrow",
        )
        ...
        partition["is_large_trade"] = partition["quote_quantity"] >= LARGE_TRADE_QUOTE_THRESHOLD_V8_3
        for timeframe in TIMEFRAMES_V8_3:
...
def build_ohlcv_trades_1y_features_v8_3(
    ohlcv: pd.DataFrame,
    ...
    *,
    ...
    features["feature_schema_version"] = FEATURE_SCHEMA_VERSION_V8_3
    features["feature_available_ts"] = features["available_ts"]

    audit_basis = [column for column in OHLCV_TRADES_VALUE_COLUMNS_V8_3 if column not in {"trades_feature_null_count", "trades_feature_error_count"}]
    ...
    return features[OHLCV_TRADES_FEATURE_COLUMNS_V8_3]


def filter_ohlcv_to_v8_3_window(ohlcv: pd.DataFrame) -> pd.DataFrame:
    frame = ohlcv.copy()
    event_ts = pd.to_datetime(frame["event_ts"], utc=True)
    start = pd.Timestamp(f"{WINDOW_START_V8_3}T00:00:00Z")
    end_exclusive = pd.Timestamp(f"{pd.Timestamp(WINDOW_END_V8_3) + pd.Timedelta(days=1):%Y-%m-%d}T00:00:00Z")
...
def load_v8_2_trades_manifest(root: Path) -> dict[str, Any]:
    return _read_json(root / V8_2_TRADES_MANIFEST_PATH)
...
def output_path(root: Path, timeframe: str, window_start: str, window_end: str) -> Path:
    return (
        root
        / "data/research/v8_3/features/ohlcv_trades"
...
def build_report_v8_3(manifest: dict[str, Any]) -> dict[str, Any]:
    return dict(manifest)


def build_ohlcv_trades_feature_store_markdown_v8_3(report: dict[str, Any]) -> str:
    output_rows = "\n".join(
    ...
    return f"""# Feature Store OHLCV + Trades V8.3
...
def safety_flags_v8_3() -> dict[str, bool]:
    return {
...
def update_project_state_v8_3(root: Path, manifest: dict[str, Any]) -> None:
    project_state_path = root / "reports/PROJECT_STATE.json"
    state = _read_json(project_state_path) if project_state_path.exists() else {}
    state.update(
        {
            "last_validated_version": "V8.2",
            "candidate_version": "V8.3",
            "candidate_status": "pending_external_audit",
            "direction": "OHLCV + public trades 1-year feature store preview",
            "ohlcv_trades_1y_features_v8_3_created": True,
            "labels_v8_3_created": False,
            "dataset_v8_3_created": False,
            "ml_v8_3_created": False,
            "model_v8_3_created": False,
            "backtest_v8_3_created": False,
            "strategy_v8_3_created": False,
            "orders_v8_3_created": False,
            "window_start_v8_3": manifest["window"]["window_start"],
            "window_end_v8_3": manifest["window"]["window_end"],
            "total_days_v8_3": manifest["window"]["total_days"],
            "trade_source_type_v8_3": manifest["input_trades_manifest"]["trade_source_type"],
            "feature_columns_v8_3_count": len(manifest["feature_columns"]),
            "output_rows_v8_3": {timeframe: payload["rows"] for timeframe, payload in manifest["outputs"].items()},
            "backtest_enabled": False,
            "strategy_enabled": False,
            "paper_live_enabled": False,
            "orders_enabled": False,
...
def _derive_feature_columns(frame: pd.DataFrame, timeframe: str) -> pd.DataFrame:
    ...
    minutes = TIMEFRAME_MINUTES_V8_3[timeframe]
...
def _validate_input_manifests(v5_manifest: dict[str, Any], trades_manifest: dict[str, Any]) -> None:
    ...
    if discovery["window_start"] != WINDOW_START_V8_3 or discovery["window_end"] != WINDOW_END_V8_3:
        raise ValueError("V8.3 window must exactly match V8.2.")
    if discovery["total_days"] != TOTAL_DAYS_V8_3:
        raise ValueError("V8.3 total_days must exactly match V8.2 total_days.")
    if trades_manifest["source"]["trade_source_type"] != "aggTrades":
        raise ValueError("V8.3 only accepts V8.2 aggTrades inputs.")
    if trades_manifest["source"]["market_type"] != "spot" or trades_manifest["source"]["symbol"] != "BTCUSDT":
        raise ValueError("V8.3 only accepts BTCUSDT spot trades.")
    v5_window_start = v5_manifest["discovery"]["window_start"]
    v5_window_end = v5_manifest["discovery"]["window_end"]
    if WINDOW_START_V8_3 < v5_window_start or WINDOW_END_V8_3 > v5_window_end:
        raise ValueError("V8.3 window must remain inside V5.0 OHLCV window.")
...
def _write_current_reports(root: Path, manifest: dict[str, Any]) -> None:
    ...
    _write_text(
        ...
        "\n".join(
            [
                "# Etat du Projet : V8.2 validee + candidat V8.3",
                "",
                "- **Derniere version validee** : V8.2.",
                "- **Version candidate** : V8.3.",
                ...
                f"- Colonnes features : `{len(manifest['feature_columns'])}`.",
                "- Aucun label V8.3.",
                "- Aucun dataset ML V8.3.",
                "- Aucun modele ML V8.3.",
                "- Aucun backtest, aucune strategie, aucun signal, aucun ordre, aucun trading reel.",
                "",
                "V8.3 reste non validee avant audit externe.",
            ]
        )
        + "\n",
    )
