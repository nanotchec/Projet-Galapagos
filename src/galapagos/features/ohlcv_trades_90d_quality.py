...
def assess_ohlcv_trades_feature_quality_v7_8(
    frame: pd.DataFrame,
    timeframe: str,
    *,
    expected_rows: int,
    ...
    if rows != expected_rows:
        errors.append(f"{timeframe} V7.8 rows mismatch: got {rows}, expected {expected_rows}")

    if list(frame.columns) != OHLCV_TRADES_FEATURE_COLUMNS_V7_8:
        errors.append(f"{timeframe} V7.8 feature schema mismatch")

    duplicate_rows = int(frame.duplicated(subset=["event_ts"]).sum()) if "event_ts" in frame.columns else 0
    if duplicate_rows:
        errors.append(f"{timeframe} V7.8 features contain duplicate event_ts rows: {duplicate_rows}")
    ...
    if rows and "event_ts" in frame.columns:
        ...
        if not timestamps_utc:
            errors.append(f"{timeframe} V7.8 event_ts must be UTC")
        if not monotonic_event_ts:
            errors.append(f"{timeframe} V7.8 event_ts is not monotonic increasing")
    ...
    if {"available_ts", "feature_available_ts", "decision_ts"}.issubset(frame.columns):
        ...
        if not feature_available_ts_valid:
            errors.append(f"{timeframe} V7.8 feature_available_ts < available_ts")
        if not decision_ts_valid:
            errors.append(f"{timeframe} V7.8 decision_ts < feature_available_ts")
    ...
    if rows >= 60 and warmup_rows < 60:
        errors.append(f"{timeframe} V7.8 warmup rows below expected rolling_60 minimum: {warmup_rows}")

    bars_without_trades = int((frame["agg_trade_count"].fillna(0) == 0).sum()) if "agg_trade_count" in frame.columns else 0
    forbidden_columns = forbidden_columns_present_v7_8(frame)
    if forbidden_columns:
        errors.append(f"{timeframe} V7.8 forbidden columns present: {forbidden_columns}")
    ...
    if median_volume_relative_diff is not None:
        if median_volume_relative_diff > 0.50:
            errors.append(f"{timeframe} V7.8 median volume relative diff above 0.50: {median_volume_relative_diff}")
        elif median_volume_relative_diff > 0.10:
            warnings.append(f"{timeframe} V7.8 median volume relative diff above 0.10: {median_volume_relative_diff}")
    if median_quote_volume_relative_diff is not None:
        if median_quote_volume_relative_diff > 0.50:
            errors.append(f"{timeframe} V7.8 median quote volume relative diff above 0.50: {median_quote_volume_relative_diff}")
        elif median_quote_volume_relative_diff > 0.10:
            warnings.append(
                f"{timeframe} V7.8 median quote volume relative diff above 0.10: {median_quote_volume_relative_diff}"
            )

    null_counts: dict[str, int] = {}
    inf_counts: dict[str, int] = {}
    for column in OHLCV_TRADES_VALUE_COLUMNS_V7_8:
        ...
        if pd.api.types.is_numeric_dtype(series):
            ...
            if inf_counts[column]:
                errors.append(f"{timeframe} V7.8 feature column {column} contains infinities")
        else:
            inf_counts[column] = 0
...
def forbidden_columns_present_v7_8(frame: pd.DataFrame) -> list[str]:
    forbidden: list[str] = []
    for column in frame.columns:
        if column in OHLCV_TRADES_FEATURE_COLUMNS_V7_8:
            continue
        lowered = str(column).casefold()
        if lowered in FORBIDDEN_OHLCV_TRADES_FEATURE_COLUMNS_V7_8:
