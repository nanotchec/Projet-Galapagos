...
def validate_redesigned_5y_label_factory_v9_64(root: Path = Path(".")) -> list[str]:
    root = root.resolve()
    ...
    errors = validate_report_payload_v9_64(report)
    if not manifest_path.is_file():
        errors.append(f"missing manifest: {MANIFEST_PATH}")
    else:
        manifest = json.loads(manifest_path.read_text(encoding="utf-8"))
        errors.extend(validate_manifest_payload_v9_64(manifest, report))
    return errors


def validate_report_payload_v9_64(report: dict[str, Any]) -> list[str]:
    errors: list[str] = []
    ...
    if report.get("selected_primary_label") != SELECTED_PRIMARY_LABEL:
        errors.append("selected primary label mismatch")
    if report.get("labels_created") is not True:
        errors.append("labels_created must be true")
    if report.get("dataset_created") is not False:
        errors.append("dataset_created must be false")
...
def validate_manifest_payload_v9_64(manifest: dict[str, Any], report: dict[str, Any]) -> list[str]:
    errors: list[str] = []
