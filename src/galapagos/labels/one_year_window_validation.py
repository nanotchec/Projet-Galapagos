...
from galapagos.labels.one_year_window_quality import assess_one_year_label_quality
...
RUN_ID_PATTERN_V4_4 = re.compile(r"^v4_4_[0-9]{8}T[0-9]{6}Z_[0-9a-f]{8}$")
MANIFEST_KEYS = {
    "version",
    "status",
    "created_at_utc",
...
FORBIDDEN_V4_4_PATHS = [
    "data/research/v4_4/datasets",
    "data/research/v4_4/ml",
    "data/research/v4_4/backtests",
    "data/research/v4_4/strategies",
    "reports/backtests",
    "reports/strategies",
    "reports/signals",
    "reports/predictions",
...
def validate_one_year_label_factory_v4_4(root: Path = Path(".")) -> dict[str, Any]:
    root = root.resolve()
    errors: list[str] = []
    manifest_path = root / MANIFEST_PATH_V4_4
    report_path = root / REPORT_JSON_PATH_V4_4
    if not manifest_path.exists():
        return _result([f"missing V4.4 manifest: {MANIFEST_PATH_V4_4}"])
    if not report_path.exists():
        return _result([f"missing V4.4 label report: {REPORT_JSON_PATH_V4_4}"])

    manifest = load_json(manifest_path)
    report = load_json(report_path)
    errors.extend(_validate_manifest_structure(root, manifest))
    errors.extend(scan_payload_for_forbidden_claims(manifest, "V4.4 manifest"))
    ...
    for timeframe in TIMEFRAMES_V4_4:
        ...
        if not input_path.exists():
            errors.append(f"missing V4.4 input OHLCV: {input_path.relative_to(root)}")
            continue
        if not label_path.exists():
            errors.append(f"missing V4.4 output labels: {label_path.relative_to(root)}")
            continue
    ...
    errors.extend(scan_payload_for_forbidden_claims(report, "V4.4 label report"))
    errors.extend(_validate_markdown(root))
    errors.extend(_validate_no_label_leakage_in_v4_3_features(root))
    errors.extend(_find_forbidden_v4_4_artifacts(root))
    return _result(errors, manifest=manifest)


def _validate_manifest_structure(root: Path, manifest: dict[str, Any]) -> list[str]:
    errors: list[str] = []
    errors.extend(validate_exact_keys(manifest, MANIFEST_KEYS, "V4.4 manifest"))
    if manifest.get("version") != VERSION_V4_4:
        errors.append("V4.4 manifest version mismatch")
    if manifest.get("status") != "PASS":
        errors.append("V4.4 manifest status must be PASS")
    if not isinstance(manifest.get("created_at_utc"), str) or not manifest["created_at_utc"].endswith("Z"):
        errors.append("V4.4 manifest created_at_utc invalid")
    run_id = manifest.get("label_run_id")
    if not isinstance(run_id, str) or RUN_ID_PATTERN_V4_4.fullmatch(run_id) is None:
        errors.append("V4.4 manifest label_run_id invalid")
    if manifest.get("label_schema_version") != LABEL_SCHEMA_VERSION_V4_4:
        errors.append("V4.4 manifest label_schema_version mismatch")
    if manifest.get("label_columns") != LABEL_COLUMNS_V4_4:
        errors.append("V4.4 manifest label_columns mismatch")
    if manifest.get("horizons") != HORIZONS:
        errors.append("V4.4 manifest horizons mismatch")
    if manifest.get("threshold") != THRESHOLD:
        errors.append("V4.4 manifest threshold mismatch")
    if manifest.get("limitations") != EXPECTED_LIMITATIONS_V4_4:
        errors.append("V4.4 manifest limitations mismatch")
    for section, keys in [("input_ohlcv", INPUT_KEYS), ("outputs", OUTPUT_KEYS), ("quality", QUALITY_KEYS)]:
        payload = manifest.get(section, {})
        errors.extend(validate_exact_keys(payload, set(TIMEFRAMES_V4_4), f"V4.4 manifest {section}"))
        for timeframe, block in payload.items():
            errors.extend(validate_exact_keys(block, keys, f"V4.4 manifest {section}.{timeframe}"))
    errors.extend(validate_exact_keys(manifest.get("safety"), SAFETY_KEYS, "V4.4 manifest safety"))
    ...
    for timeframe in TIMEFRAMES_V4_4:
        ...
        if manifest.get("input_ohlcv", {}).get(timeframe, {}).get("path") != str(input_path.relative_to(root)):
            errors.append(f"V4.4 manifest input path mismatch for {timeframe}")
        if manifest.get("outputs", {}).get(timeframe, {}).get("path") != str(label_path.relative_to(root)):
            errors.append(f"V4.4 manifest output path mismatch for {timeframe}")
    return errors
...
def _validate_safety(safety: dict[str, Any]) -> list[str]:
    ...
    if safety.get("public_read_only") is not True:
        errors.append("V4.4 safety flag public_read_only must be True")
    if safety.get("labels_enabled") is not True:
        errors.append("V4.4 safety flag labels_enabled must be True")
    for flag in sorted(SAFETY_KEYS - {"public_read_only", "labels_enabled"}):
        if safety.get(flag) is not False:
            errors.append(f"V4.4 safety flag {flag} must be False")
    return errors
...
def _validate_input_entry(root: Path, manifest: dict[str, Any], timeframe: str, path: Path, frame: pd.DataFrame) -> list[str]:
    ...
    return [f"V4.4 manifest input mismatch for {timeframe}.{field}" for field, value in expected.items() if payload.get(field) != value]
...
def _validate_output_entry(root: Path, manifest: dict[str, Any], timeframe: str, path: Path, frame: pd.DataFrame) -> list[str]:
    ...
    return [f"V4.4 manifest output mismatch for {timeframe}.{field}" for field, value in expected.items() if payload.get(field) != value]
...
def _validate_label_frame(
    ...
    if len(frame) != len(input_frame):
        errors.append(f"V4.4 label row count mismatch for {timeframe}")
    ...
    quality = assess_one_year_label_quality(frame, timeframe)
    quality["source_hashes_valid"] = source_hashes_valid
    errors.extend(f"V4.4 physical quality error for {timeframe}: {error}" for error in quality["errors"])
...
def _validate_label_schema(frame: pd.DataFrame, timeframe: str) -> list[str]:
    ...
    if list(frame.columns) != LABEL_COLUMNS_V4_4:
        errors.append(f"V4.4 label schema mismatch for {timeframe}")
    forbidden = _forbidden_columns(frame)
    if forbidden:
        errors.append(f"V4.4 forbidden label columns for {timeframe}: {forbidden}")
    return errors
...
def _validate_label_metadata(timeframe: str, frame: pd.DataFrame, label_run_id: str, source_ohlcv_sha256: str) -> list[str]:
    ...
    if not _source_hashes_valid(frame, source_ohlcv_sha256):
        errors.append(f"V4.4 source_ohlcv_sha256 mismatch for {timeframe}")
    if "label_run_id" in frame.columns and set(frame["label_run_id"].astype(str).unique()) != {str(label_run_id)}:
        errors.append(f"V4.4 label_run_id mismatch for {timeframe}")
    if "label_schema_version" in frame.columns:
        if set(frame["label_schema_version"].astype(str).unique()) != {LABEL_SCHEMA_VERSION_V4_4}:
            errors.append(f"V4.4 label_schema_version mismatch for {timeframe}")
    return errors
...
def _validate_label_values_against_ohlcv(
    ...
    *,
    ...
    if list(label_frame.columns) != LABEL_COLUMNS_V4_4:
        return []
...
def _compare_recomputed_labels(timeframe: str, frame: pd.DataFrame, expected: pd.DataFrame) -> list[str]:
    ...
    try:
        pdt.assert_frame_equal(
            frame[LABEL_COLUMNS_V4_4].reset_index(drop=True),
            expected[LABEL_COLUMNS_V4_4].reset_index(drop=True),
            check_dtype=False,
            check_exact=False,
            rtol=1e-12,
            atol=1e-12,
        ...
        if "future_close_h1" in text:
            errors.append(f"V4.4 future_close_h1 mismatch for {timeframe}")
        elif "future_log_return_h3" in text:
            errors.append(f"V4.4 future_log_return_h3 mismatch for {timeframe}")
        elif "direction_h5" in text:
            errors.append(f"V4.4 direction_h5 mismatch for {timeframe}")
        elif "up_down_flat_h1" in text:
            errors.append(f"V4.4 up_down_flat_h1 mismatch for {timeframe}")
        elif "label_valid" in text or "tail_row" in text:
            errors.append(f"V4.4 label_valid or tail_row mismatch for {timeframe}")
        else:
            errors.append(f"V4.4 recomputed labels mismatch for {timeframe}: {text.splitlines()[0]}")
    return errors
...
def _validate_temporal_label_rules(timeframe: str, frame: pd.DataFrame) -> list[str]:
    ...
    if valid_any.any():
        ...
        if (label_available <= decision).any():
            errors.append(f"V4.4 label_available_ts <= decision_ts for valid labels in {timeframe}")
    for h in HORIZONS:
        ...
        if list(valid.astype(bool)) != expected_tail_valid:
            errors.append(f"V4.4 label_valid_h{h} tail mismatch for {timeframe}")
        if (valid & frame[f"future_close_h{h}"].isna()).any():
            errors.append(f"V4.4 label_valid_h{h} true with null future_close in {timeframe}")
        if (valid & frame[f"label_end_ts_h{h}"].isna()).any():
            errors.append(f"V4.4 label_valid_h{h} true with null label_end_ts in {timeframe}")
    ...
    if not (frame["tail_row"].astype(bool).reset_index(drop=True) == expected_tail.reset_index(drop=True)).all():
        errors.append(f"V4.4 tail_row mismatch for {timeframe}")
    return errors
...
def _forbidden_columns(frame: pd.DataFrame) -> list[str]:
    ...
    for column in frame.columns:
        ...
        if any(term in lower for term in FORBIDDEN_COLUMNS_V4_4):
...
def _validate_manifest_quality(manifest: dict[str, Any], physical_quality: dict[str, dict[str, Any]]) -> list[str]:
    ...
    for timeframe in TIMEFRAMES_V4_4:
        ...
        if not isinstance(declared, dict):
            errors.append(f"V4.4 manifest quality missing for {timeframe}")
            continue
        if not isinstance(physical, dict):
            errors.append(f"V4.4 physical quality unavailable for {timeframe}")
            continue
        for field in sorted(QUALITY_KEYS - {"warnings", "errors"}):
            if declared.get(field) != physical.get(field):
                errors.append(f"V4.4 manifest quality mismatch for {timeframe}.{field}")
        if declared.get("errors") != physical.get("errors"):
            errors.append(f"V4.4 manifest quality errors mismatch for {timeframe}")
        if declared.get("warnings") != physical.get("warnings"):
            errors.append(f"V4.4 manifest quality warnings mismatch for {timeframe}")
    return errors


def _validate_report(manifest: dict[str, Any], report: dict[str, Any]) -> list[str]:
    errors: list[str] = []
    errors.extend(validate_exact_keys(report, REPORT_KEYS, "V4.4 label report"))
    for field in REPORT_KEYS:
        if report.get(field) != manifest.get(field):
            errors.append(f"V4.4 label report {field} mismatch")
    errors.extend(_validate_safety(report.get("safety", {})))
    return errors


def _validate_markdown(root: Path) -> list[str]:
    path = root / REPORT_MD_PATH_V4_4
    if not path.exists():
        return [f"missing V4.4 label markdown: {REPORT_MD_PATH_V4_4}"]
    text = path.read_text(encoding="utf-8")
    errors = validate_markdown_forbidden_claims(text, "V4.4 label markdown")
    ...
    for clause in required:
        if clause.replace("’", "'") not in normalized:
            errors.append(f"V4.4 label markdown missing safety clause: {clause}")
    return errors


def _validate_no_label_leakage_in_v4_3_features(root: Path) -> list[str]:
    errors: list[str] = []
    feature_root = root / "data/research/v4_3/features"
    ...
    for child in sorted(feature_root.rglob("*.parquet")):
        ...
        if leaked:
            errors.append(f"Forbidden V4.4 label leakage in V4.3 features: {child.relative_to(root)}")
    return errors


def _find_forbidden_v4_4_artifacts(root: Path) -> list[str]:
    errors: list[str] = []
    for prefix in FORBIDDEN_V4_4_PATHS:
        path = root / prefix
        if path.exists():
            if path.is_file():
                errors.append(f"Forbidden V4.4 artifact detected: {path.relative_to(root)}")
            else:
                children = sorted(child for child in path.rglob("*") if child.is_file())
                if children:
                    errors.extend(f"Forbidden V4.4 artifact detected: {child.relative_to(root)}" for child in children[:20])
                else:
                    errors.append(f"Forbidden V4.4 artifact detected: {path.relative_to(root)}")
    for child in sorted(root.rglob("*")):
        ...
        if child.is_file() and child.suffix.casefold() in FORBIDDEN_MODEL_SUFFIXES:
            errors.append(f"Forbidden V4.4 model artifact detected: {child.relative_to(root)}")
    return errors
