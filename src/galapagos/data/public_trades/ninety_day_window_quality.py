...
def assess_ninety_day_agg_trade_partitions_v7_7(
    root: Path,
    ...
    *,
    ...
    for date_key in sorted(partitions):
        ...
        if list(frame.columns) != AGG_TRADE_COLUMNS_V7_7:
            errors.append(f"{date_key}: AGG_TRADE_COLUMNS_V7_7 schema mismatch")
        ...
        forbidden_columns.update(column for column in frame.columns if column.casefold() in FORBIDDEN_TRADE_COLUMNS_V7_7)
    ...
    if missing_dates:
        errors.append(f"missing aggTrades dates in V7.7 window: {missing_dates}")
    if len(partitions) != expected_days - len(missing_dates):
        errors.append("V7.7 partition count does not match expected available days")
