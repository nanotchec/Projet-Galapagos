...
FORBIDDEN_ARTIFACT_PATHS = [
    Path("data/research/v7_7/features"),
    Path("data/research/v7_7/labels"),
    Path("data/research/v7_7/datasets"),
    Path("data/research/v7_7/ml"),
    Path("data/research/v7_7/backtests"),
    Path("data/research/v7_7/strategies"),
...
def validate_public_trades_90d_window_v7_7(root: Path = Path(".")) -> dict[str, Any]:
    project_root = root.resolve()
    errors: list[str] = []
    warnings: list[str] = []
    manifest_path = project_root / MANIFEST_PATH_V7_7
    report_path = project_root / REPORT_JSON_PATH_V7_7
    if not manifest_path.exists():
        return _result([f"missing V7.7 manifest: {MANIFEST_PATH_V7_7}"], warnings)
    if not report_path.exists():
        errors.append(f"missing V7.7 report JSON: {REPORT_JSON_PATH_V7_7}")
    manifest = _read_json(manifest_path)
    report = _read_json(report_path) if report_path.exists() else {}
    errors.extend(validate_manifest_payload_v7_7(manifest))
    if report != manifest:
        errors.append("V7.7 report JSON must be a deterministic projection of the manifest")
    errors.extend(_validate_exact_keys(report, REQUIRED_MANIFEST_KEYS, "V7.7 report JSON"))
    errors.extend(_validate_physical_files(project_root, manifest))
    errors.extend(_validate_markdown(project_root / REPORT_MD_PATH_V7_7, "V7.7 Markdown report"))
    errors.extend(_validate_markdown(project_root / DOC_PATH_V7_7, "V7.7 documentation"))
    errors.extend(_find_forbidden_artifacts(project_root))
    return _result(errors, warnings, manifest)


def validate_manifest_payload_v7_7(manifest: dict[str, Any]) -> list[str]:
    errors: list[str] = []
    errors.extend(_validate_exact_keys(manifest, REQUIRED_MANIFEST_KEYS, "V7.7 manifest"))
    if manifest.get("version") != VERSION_V7_7:
        errors.append("V7.7 manifest version mismatch")
    if manifest.get("status") != "PASS":
        errors.append("V7.7 manifest status must be PASS")
    if manifest.get("schema_version") != SCHEMA_VERSION_V7_7:
        errors.append("V7.7 schema_version mismatch")
    if manifest.get("trade_columns") != AGG_TRADE_COLUMNS_V7_7:
        errors.append("V7.7 trade_columns must match AGG_TRADE_COLUMNS_V7_7")
    errors.extend(_validate_exact_keys(manifest.get("source", {}), REQUIRED_SOURCE_KEYS, "V7.7 source"))
    source = manifest.get("source", {})
    if source.get("name") != "binance_public_archive":
        errors.append("V7.7 source name must be binance_public_archive")
    if source.get("venue") != "binance":
        errors.append("V7.7 venue must be binance")
    if source.get("market_type") != "spot":
        errors.append("V7.7 market_type must be spot")
    if source.get("symbol") != "BTCUSDT":
        errors.append("V7.7 symbol must be BTCUSDT")
    if source.get("trade_source_type") != "aggTrades":
        errors.append("V7.7 trade_source_type must be aggTrades")
    discovery = manifest.get("discovery", {})
    errors.extend(_validate_exact_keys(discovery, REQUIRED_DISCOVERY_KEYS, "V7.7 discovery"))
    if discovery.get("window_start") != "2023-03-25":
        errors.append("V7.7 window_start must be 2023-03-25")
    if discovery.get("window_end") != "2023-06-22":
        errors.append("V7.7 window_end must be 2023-06-22")
    if discovery.get("total_days") != 90:
        errors.append("V7.7 total_days must be 90")
    if discovery.get("documented_gaps_allowed") is not False:
        errors.append("V7.7 documented_gaps_allowed must be false")
    outputs = manifest.get("outputs", {})
    errors.extend(_validate_exact_keys(outputs, REQUIRED_OUTPUT_KEYS, "V7.7 outputs"))
    if outputs.get("format") != "partitioned_parquet":
        errors.append("V7.7 outputs format must be partitioned_parquet")
    for date_key, payload in outputs.get("partitions", {}).items():
        errors.extend(_validate_exact_keys(payload, REQUIRED_PARTITION_KEYS, f"V7.7 partition {date_key}"))
    safety = manifest.get("safety", {})
    if safety.get("public_read_only") is not True:
        errors.append("V7.7 public_read_only safety flag must be true")
    for key in [
        "authentication_used",
        "api_key_used",
        ...
        if safety.get(key) is not False:
            errors.append(f"V7.7 safety flag must be false: {key}")
    return errors
...
def _validate_physical_files(root: Path, manifest: dict[str, Any]) -> list[str]:
    ...
    if total_rows != int(outputs.get("total_rows", -1)):
        errors.append("V7.7 outputs total_rows mismatch")
    if total_bytes != int(outputs.get("total_bytes", -1)):
        errors.append("V7.7 outputs total_bytes mismatch")
    ...
    if quality != manifest.get("quality", {}):
        errors.append("V7.7 manifest quality does not match physical output")
    return errors
...
def _find_forbidden_artifacts(root: Path) -> list[str]:
    ...
    for relative in FORBIDDEN_ARTIFACT_PATHS:
        path = root / relative
        if path.exists():
            errors.append(f"Forbidden V7.7 artifact detected: {relative.as_posix()}")
            for child in sorted(path.rglob("*")):
                if child.is_file():
                    errors.append(f"Forbidden V7.7 artifact detected: {child.relative_to(root).as_posix()}")
