...
FORBIDDEN_ARTIFACT_PATHS = [
    Path("data/research/v8_2/features"),
    Path("data/research/v8_2/labels"),
    Path("data/research/v8_2/datasets"),
    Path("data/research/v8_2/ml"),
    Path("data/research/v8_2/backtests"),
    Path("data/research/v8_2/strategies"),
...
def validate_public_trades_1y_window_v8_2(root: Path = Path(".")) -> dict[str, Any]:
    project_root = root.resolve()
    errors: list[str] = []
    warnings: list[str] = []
    manifest_path = project_root / MANIFEST_PATH_V8_2
    report_path = project_root / REPORT_JSON_PATH_V8_2
    if not manifest_path.exists():
        return _result([f"missing V8.2 manifest: {MANIFEST_PATH_V8_2}"], warnings)
    if not report_path.exists():
        errors.append(f"missing V8.2 report JSON: {REPORT_JSON_PATH_V8_2}")
    manifest = _read_json(manifest_path)
    report = _read_json(report_path) if report_path.exists() else {}
    errors.extend(validate_manifest_payload_v8_2(manifest))
    if report != manifest:
        errors.append("V8.2 report JSON must be a deterministic projection of the manifest")
    errors.extend(_validate_exact_keys(report, REQUIRED_MANIFEST_KEYS, "V8.2 report JSON"))
    errors.extend(_validate_physical_files(project_root, manifest))
    errors.extend(_validate_markdown(project_root / REPORT_MD_PATH_V8_2, "V8.2 Markdown report"))
    errors.extend(_validate_markdown(project_root / DOC_PATH_V8_2, "V8.2 documentation"))
    errors.extend(_find_forbidden_artifacts(project_root))
    return _result(errors, warnings, manifest)


def validate_manifest_payload_v8_2(manifest: dict[str, Any]) -> list[str]:
    errors: list[str] = []
    errors.extend(_validate_exact_keys(manifest, REQUIRED_MANIFEST_KEYS, "V8.2 manifest"))
    if manifest.get("version") != VERSION_V8_2:
        errors.append("V8.2 manifest version mismatch")
    if manifest.get("status") != "PASS":
        errors.append("V8.2 manifest status must be PASS")
    if manifest.get("schema_version") != SCHEMA_VERSION_V8_2:
        errors.append("V8.2 schema_version mismatch")
    if manifest.get("trade_columns") != AGG_TRADE_COLUMNS_V8_2:
        errors.append("V8.2 trade_columns must match AGG_TRADE_COLUMNS_V8_2")
    errors.extend(_validate_exact_keys(manifest.get("source", {}), REQUIRED_SOURCE_KEYS, "V8.2 source"))
    source = manifest.get("source", {})
    if source.get("name") != "binance_public_archive":
        errors.append("V8.2 source name must be binance_public_archive")
    if source.get("venue") != "binance":
        errors.append("V8.2 venue must be binance")
    if source.get("market_type") != "spot":
        errors.append("V8.2 market_type must be spot")
    if source.get("symbol") != "BTCUSDT":
        errors.append("V8.2 symbol must be BTCUSDT")
    if source.get("trade_source_type") != "aggTrades":
        errors.append("V8.2 trade_source_type must be aggTrades")
    discovery = manifest.get("discovery", {})
    errors.extend(_validate_exact_keys(discovery, REQUIRED_DISCOVERY_KEYS, "V8.2 discovery"))
    if discovery.get("window_start") != "2023-03-25":
        errors.append("V8.2 window_start must be 2023-03-25")
    if discovery.get("window_end") != "2024-03-24":
        errors.append("V8.2 window_end must be 2024-03-24")
    if discovery.get("total_days") != 366:
        errors.append("V8.2 total_days must be 366")
    if discovery.get("documented_gaps_allowed") is not False:
        errors.append("V8.2 documented_gaps_allowed must be false")
    outputs = manifest.get("outputs", {})
    errors.extend(_validate_exact_keys(outputs, REQUIRED_OUTPUT_KEYS, "V8.2 outputs"))
    if outputs.get("format") != "partitioned_parquet":
        errors.append("V8.2 outputs format must be partitioned_parquet")
    for date_key, payload in outputs.get("partitions", {}).items():
        errors.extend(_validate_exact_keys(payload, REQUIRED_PARTITION_KEYS, f"V8.2 partition {date_key}"))
    safety = manifest.get("safety", {})
    if safety.get("public_read_only") is not True:
        errors.append("V8.2 public_read_only safety flag must be true")
    for key in [
        "authentication_used",
        "api_key_used",
        ...
        if safety.get(key) is not False:
            errors.append(f"V8.2 safety flag must be false: {key}")
    return errors
...
def _validate_physical_files(root: Path, manifest: dict[str, Any]) -> list[str]:
    ...
    if total_rows != int(outputs.get("total_rows", -1)):
        errors.append("V8.2 outputs total_rows mismatch")
    if total_bytes != int(outputs.get("total_bytes", -1)):
        errors.append("V8.2 outputs total_bytes mismatch")
    ...
    if quality != manifest.get("quality", {}):
        errors.append("V8.2 manifest quality does not match physical output")
    return errors
...
def _find_forbidden_artifacts(root: Path) -> list[str]:
    ...
    for relative in FORBIDDEN_ARTIFACT_PATHS:
        path = root / relative
        if path.exists():
            errors.append(f"Forbidden V8.2 artifact detected: {relative.as_posix()}")
            for child in sorted(path.rglob("*")):
                if child.is_file():
                    errors.append(f"Forbidden V8.2 artifact detected: {child.relative_to(root).as_posix()}")
