...
SOURCE_VERSION = "V9.22"
DIRECTION = "aggtrades_post_v9_batch2_collection"
BATCH_ID = "V9.23_batch_01"

BATCH_START = "2024-08-10"
BATCH_END = "2024-10-08"
MAX_BATCH_DOWNLOADS = 60
PREVIOUS_COVERAGE_START = "2024-05-05"
PREVIOUS_COVERAGE_END = "2024-08-09"

REPORT_JSON_PATH = Path("reports/data/aggtrades_post_v9_batch2_collection_v9_23.json")
REPORT_MD_PATH = Path("reports/data/aggtrades_post_v9_batch2_collection_v9_23.md")
MANIFEST_PATH = Path("reports/manifests/aggtrades_post_v9_batch2_collection_v9_23_manifest.json")
DOC_PATH = Path("docs/aggtrades_post_v9_batch2_collection_v9_23.md")

ALLOWED_MODES = {"dry-run", "collect", "validate-only"}
ALLOWED_DECISIONS = {
    "aggtrades_post_v9_batch2_collection_success",
    "aggtrades_post_v9_batch2_collection_partial",
    "aggtrades_post_v9_batch2_collection_failed_source_issue",
    "aggtrades_post_v9_batch2_collection_failed_quality",
    "aggtrades_post_v9_batch2_collection_not_executed",
...
def run_aggtrades_post_v9_batch2_collection_v9_23(
    root: Path = Path("."),
    *,
    mode: str = "dry-run",
    ...
    root = root.resolve()
    report = build_aggtrades_post_v9_batch_report_v9_23(
        root,
        mode=mode,
    ...
    markdown = build_markdown_v9_23(report)
    _write_text(root / REPORT_MD_PATH, markdown)
    _write_text(root / DOC_PATH, markdown)
    _write_json(root / MANIFEST_PATH, build_manifest_v9_23(report))
    update_state_surfaces_v9_23(root, report)
    return report


def build_aggtrades_post_v9_batch_report_v9_23(
    root: Path = Path("."),
    *,
    mode: str = "dry-run",
    start_date: str = BATCH_START,
    ...
    if mode not in ALLOWED_MODES:
        raise ValueError(f"unsupported V9.23 mode: {mode}")
    requested_dates = date_range_v9_23(start_date, end_date)
    validate_batch_request_v9_23(mode, requested_dates, max_downloads)
    root = root.resolve()
    started = time.monotonic()
    inputs = {name: _load_input(root, path) for name, path in INPUT_PATHS.items()}
    previous_coverage_check = build_previous_coverage_check_v9_23(root)
    day_plan_before = build_batch_day_plan_v9_23(root, requested_dates)
    collection_result = execute_batch_mode_v9_23(
        root,
        mode,
        requested_dates,
        max_downloads=max_downloads,
    )
    day_plan_after = build_batch_day_plan_v9_23(root, requested_dates)
    day_validation = [validate_batch_day_v9_23(root, day_value) for day_value in requested_dates]
    runtime_seconds = round(time.monotonic() - started, 3)
    batch_summary = summarize_batch_validation_v9_23(
        requested_dates,
        day_plan_before,
        collection_result,
        day_validation,
    ...
    reported_cumulative_coverage = build_reported_cumulative_coverage_v9_23(inputs, batch_summary)
    local_file_coverage = build_local_file_coverage_v9_23(root, FUNDING_FIRST_START, BATCH_END)
    ...
    safety_flags = safety_flags_for_batch_v9_23(collection_result)
    decision = decide_v9_23(collection_result, batch_summary)
    status = "PASS" if collection_result["status"] == "PASS" and not batch_summary["quality_errors"] else "FAIL"
    if mode != "collect" and decision["decision"] == "aggtrades_post_v9_batch2_collection_not_executed":
        status = "PASS"
    report = {
        "version": VERSION,
        ...
        "source_public_target": build_source_design_v9_23(),
        "global_target_window": {
            "start": TARGET_START,
            "end": TARGET_END,
            "days_expected": len(date_range_v9_23(TARGET_START, TARGET_END)),
            "complete_collection_reached": False,
        },
        "future_funding_first_window": {
            "start": FUNDING_FIRST_START,
            "end": FUNDING_FIRST_END,
            "days_expected": len(date_range_v9_23(FUNDING_FIRST_START, FUNDING_FIRST_END)),
            "complete_collection_reached": False,
        },
        "batch_window": {
            "batch_id": BATCH_ID,
        ...
        "audit_lite_mode_handling": {
            "repo_local_full_data_mode": "V9.23 runner inspects local raw/silver files and writes a full local report only in the local repository.",
            ...
            "avoid_v9_22_contradictory_regeneration": True,
        },
        "storage_convention": {
        ...
        "anti_leakage_plan": build_anti_leakage_plan_v9_23(),
        "v9_23_decision": decision,
        ...
        "warnings": build_warnings_v9_23(collection_result, batch_summary),
        "limitations": [
            "V9.23 ne collecte pas la fenetre complete de 772 jours.",
...
def validate_batch_request_v9_23(mode: str, requested_dates: list[str], max_downloads: int | None) -> None:
    if not requested_dates:
        raise ValueError("V9.23 batch window cannot be empty.")
    if len(requested_dates) > MAX_BATCH_DOWNLOADS:
        raise ValueError("V9.23 batch window is limited to 60 days.")
    if mode == "collect":
        if max_downloads is None:
            raise ValueError("V9.23 collect mode requires --max-downloads to prevent accidental bulk collection.")
        if max_downloads <= 0 or max_downloads > MAX_BATCH_DOWNLOADS:
            raise ValueError("V9.23 collect mode requires 1 <= --max-downloads <= 60.")
        if max_downloads > len(requested_dates):
            raise ValueError("V9.23 --max-downloads cannot exceed requested batch days.")


def build_source_design_v9_23() -> dict[str, Any]:
    return {
...
def build_batch_day_plan_v9_23(root: Path, requested_dates: list[str]) -> list[dict[str, Any]]:
...
def build_previous_coverage_check_v9_23(root: Path) -> dict[str, Any]:
    previous_dates = date_range_v9_23(PREVIOUS_COVERAGE_START, PREVIOUS_COVERAGE_END)
    day_plan = build_batch_day_plan_v9_23(root, previous_dates)
    ...
    return {
        ...
        "v9_23_can_run_independently": True,
        "reported_cumulative_coverage_allowed": v9_22_report_path.is_file(),
        "local_cumulative_coverage_claim_allowed": previous_complete,
        "note": "V9.23 peut s'executer independamment; la couverture cumulee declaree vient des rapports valides, tandis que la couverture locale reelle exige les fichiers raw/silver.",
    }


def execute_batch_mode_v9_23(
    root: Path,
    mode: str,
    ...
    *,
    ...
    return collect_batch_public_aggtrades_v9_23(root, requested_dates, max_downloads=max_downloads)


def collect_batch_public_aggtrades_v9_23(
    root: Path,
    requested_dates: list[str],
    *,
    max_downloads: int | None,
) -> dict[str, Any]:
    validate_batch_request_v9_23("collect", requested_dates, max_downloads)
    ...
    for day_value in requested_dates:
        ...
        try:
            ...
            download_public_archive_v9_23(build_public_archive_url_v9_18(day_value), raw_path)
            ...
            quarantine_path = quarantine_failed_raw_v9_23(root, day_value, raw_path)
    ...
    return {
        ...
        "new_data_download_scope": "public_historical_aggtrades_batch2_only",
        "ingestion_scope": "public_aggtrades_bronze_silver_batch2_only",
...
def download_public_archive_v9_23(url: str, destination: Path) -> None:
    from urllib.parse import urlparse
    from urllib.request import Request, urlopen

    parsed = urlparse(url)
    if parsed.scheme != "https" or parsed.netloc not in ALLOWED_PUBLIC_HOSTS:
        raise ValueError("V9.23 allows public read-only downloads from data.binance.vision only.")
    ...
    if tmp_path.exists():
        tmp_path.unlink()
    request = Request(url, headers={"User-Agent": "galapagos-v9.23-public-read-only"})
...
def validate_batch_day_v9_23(root: Path, day_value: str) -> dict[str, Any]:
    raw_path = root / raw_zip_path_for_date_v9_18(day_value)
    silver_path = root / silver_path_for_date_v9_18(day_value)
    errors: list[str] = []
    raw_bytes = raw_path.stat().st_size if raw_path.exists() else 0
...
def summarize_batch_validation_v9_23(
    requested_dates: list[str],
    ...
    target_days = len(date_range_v9_23(TARGET_START, TARGET_END))
...
def decide_v9_23(collection_result: dict[str, Any], batch_summary: dict[str, Any]) -> dict[str, Any]:
    if not collection_result["collection_executed"]:
        decision = "aggtrades_post_v9_batch2_collection_not_executed"
        recommendation = "V9.24 - AggTrades Batch 2 Correction."
        confidence = "high"
        justification = "Aucune collecte batche n'a ete executee."
    elif collection_result["status"] != "PASS":
        decision = "aggtrades_post_v9_batch2_collection_failed_source_issue"
        recommendation = "V9.24 - AggTrades Batch 2 Correction."
        confidence = "medium"
...
def build_reported_cumulative_coverage_v9_23(inputs: dict[str, dict[str, Any]], batch_summary: dict[str, Any]) -> dict[str, Any]:
    payload = inputs.get("v9_22_multi_batch_plan", {}).get("payload", {})
    coverage = payload.get("current_coverage", {}) if isinstance(payload, dict) else {}
    previous_start = coverage.get("current_coverage_start") or PREVIOUS_COVERAGE_START
    previous_end = coverage.get("current_coverage_end") or PREVIOUS_COVERAGE_END
    batch_complete = batch_summary["days_complete"] == batch_summary["days_requested"] and batch_summary["quality_status"] == "PASS"
    return {
        "source": "reports/data/aggtrades_post_v9_multi_batch_plan_v9_22.json plus V9.23 batch validation",
...
def build_local_file_coverage_v9_23(root: Path, start_date: str, end_date: str) -> dict[str, Any]:
    dates = date_range_v9_23(start_date, end_date)
...
def build_manifest_v9_23(report: dict[str, Any]) -> dict[str, Any]:
    ...
    return {
        ...
        "v9_23_decision": report["v9_23_decision"],
        "findings": report["findings"],
        "safety_flags": report["safety_flags"],
    }


def build_markdown_v9_23(report: dict[str, Any]) -> str:
    summary = report["batch_validation"]["summary"]
    decision = report["v9_23_decision"]
    ...
    lines = [
        ...
        f"- Decision V9.23 : `{decision['decision']}`.",
...
def update_state_surfaces_v9_23(root: Path, report: dict[str, Any]) -> None:
    summary = report["batch_validation"]["summary"]
    metrics = {
    ...
    text = (
        ...
        f"- Periode batche : `{report['batch_window']['start']}` -> `{report['batch_window']['end']}`.\n"
        f"- Decision V9.23 : `{report['v9_23_decision']['decision']}`.\n"
...
def safety_flags_for_batch_v9_23(collection_result: dict[str, Any]) -> dict[str, Any]:
...
def build_anti_leakage_plan_v9_23() -> dict[str, Any]:
    return {
        "rules": [
            "available_ts >= event_ts for every normalized batch row.",
            "V9.23 collects and normalizes aggTrades only; it creates no labels and no supervised dataset.",
            "Funding and open interest are not joined in V9.23.",
...
def build_warnings_v9_23(collection_result: dict[str, Any], batch_summary: dict[str, Any]) -> list[str]:
    warnings = [
        "V9.23 ne couvre pas la fenetre complete de 772 jours.",
...
def quarantine_failed_raw_v9_23(root: Path, day_value: str, raw_path: Path) -> Path | None:
    if not raw_path.exists():
        return None
...
def date_range_v9_23(start: str, end: str) -> list[str]:
