...
LAST_VALIDATED_VERSION = "V9.25.1"
DIRECTION = "aggtrades_post_v9_storage_resume_campaign"
...
REPORT_JSON_PATH = Path("reports/data/aggtrades_post_v9_storage_resume_campaign_v9_26.json")
REPORT_MD_PATH = Path("reports/data/aggtrades_post_v9_storage_resume_campaign_v9_26.md")
MANIFEST_PATH = Path("reports/manifests/aggtrades_post_v9_storage_resume_campaign_v9_26_manifest.json")
DOC_PATH = Path("docs/aggtrades_post_v9_storage_resume_campaign_v9_26.md")
...
class StorageResumeBatchSpec:
    ...
    def report_path(self) -> Path:
        ...
        return Path(f"reports/data/aggtrades_post_v9_storage_resume_batch{suffix}_v9_26.json")


def run_aggtrades_post_v9_storage_resume_campaign_v9_26(root: Path = Path(".")) -> dict[str, Any]:
    root = root.resolve()
    report = build_aggtrades_post_v9_storage_resume_campaign_v9_26(root)
    _write_json(root / REPORT_JSON_PATH, report)
    markdown = build_markdown_v9_26(report)
    _write_text(root / REPORT_MD_PATH, markdown)
    _write_text(root / DOC_PATH, markdown)
    _write_json(root / MANIFEST_PATH, build_manifest_v9_26(report))
    update_state_surfaces_v9_26(root, report)
    return report


def build_aggtrades_post_v9_storage_resume_campaign_v9_26(root: Path = Path(".")) -> dict[str, Any]:
    root = root.resolve()
    started = time.monotonic()
    inputs = {name: _load_input(root, path) for name, path in INPUT_PATHS.items()}
    canonical_before = reconcile_campaign_state_v9_26(root, inputs)
    disk_preflight = build_disk_preflight_v9_26(root, canonical_before)
    can_collect = (
    ...
    planned_batches = (
        build_storage_resume_batches_v9_26(canonical_before["first_missing_day"], TARGET_WINDOW_END, disk_preflight["batch_size_days"])
        if can_collect
        else []
    )
    batch_reports: list[dict[str, Any]] = []
    stop_reason = determine_initial_stop_reason_v9_26(canonical_before, disk_preflight)
    if can_collect:
        stop_reason = None
        for batch in planned_batches:
            fresh_preflight = build_disk_preflight_v9_26(root, build_local_coverage_inventory_v9_26(root, TARGET_WINDOW_START, TARGET_WINDOW_END))
            if not fresh_preflight["safe_to_continue_now"]:
                ...
                break
            batch_report = execute_storage_resume_batch_v9_26(root, batch, fresh_preflight)
            ...
            if batch_report["batch_summary"]["batch_success"] is not True:
                stop_reason = {
                    ...
                    "message": "storage resume campaign stopped after first non-complete internal batch",
                }
                break
    runtime = round(time.monotonic() - started, 3)
    canonical_after = build_local_coverage_inventory_v9_26(root, TARGET_WINDOW_START, TARGET_WINDOW_END)
    summary = build_storage_resume_summary_v9_26(
    ...
    decision = decide_v9_26(summary, canonical_before, disk_preflight, stop_reason)
    safety_flags = safety_flags_v9_26(summary)
    report = {
        "version": VERSION,
        ...
        "batches_planned": [batch_to_dict_v9_26(batch) for batch in planned_batches],
        ...
        "storage_resume_summary": summary,
        **summary,
        "decision": decision["decision"],
        "v9_26_decision": decision,
        ...
        "findings": FINDINGS_V9_26,
        "safety_flags": safety_flags,
        "blockers": build_blockers_v9_26(canonical_before, disk_preflight, stop_reason),
        "warnings": build_warnings_v9_26(summary, canonical_before, disk_preflight),
        "limitations": [
            "V9.26 mesure le volume reel du projet et de data avant toute reprise.",
...
def determine_initial_stop_reason_v9_26(canonical: dict[str, Any], disk: dict[str, Any]) -> dict[str, Any] | None:
    ...
    if canonical["first_missing_day"] is None:
        return {"type": "complete", "message": "target window already complete before V9.26"}
    if disk["storage_blocker"]:
        return {"type": "storage", "message": "free disk space is below V9.26 minimum threshold"}
    if not disk["resume_allowed_now"]:
        return {
            "type": "storage",
            "message": "V9.26 campaign requires more than 150 GiB free on the data volume before public collection resumes",
        }
    return None


def reconcile_campaign_state_v9_26(root: Path, inputs: dict[str, dict[str, Any]]) -> dict[str, Any]:
    local = build_local_coverage_inventory_v9_26(root, TARGET_WINDOW_START, TARGET_WINDOW_END)
    v9_25_1 = inputs.get("v9_25_1_campaign", {}).get("payload", {})
    v9_25_1_end = v9_25_1.get("local_file_coverage_end") if isinstance(v9_25_1, dict) else None
    coverage_more_advanced = bool(local["local_contiguous_coverage_end"] and v9_25_1_end and local["local_contiguous_coverage_end"] > v9_25_1_end)
    return {
        "target_window_start": TARGET_WINDOW_START,
        "target_window_end": TARGET_WINDOW_END,
        "source_reported_coverage_start": v9_25_1.get("local_file_coverage_start") if isinstance(v9_25_1, dict) else None,
        "source_reported_coverage_end": v9_25_1_end,
        "source_report_payload": v9_25_1 if isinstance(v9_25_1, dict) else {},
        "local_coverage_more_advanced_than_v9_25_1": coverage_more_advanced,
        **local,
        "state_reconciled": local["days_partial"] == 0 and len(gaps_before_first_missing_v9_26(local)) == 0,
        "reconciliation_basis": "local raw/silver file metadata plus V9.25.1, V9.25 and prior reports",
    }


def build_local_coverage_inventory_v9_26(root: Path, start: str, end: str) -> dict[str, Any]:
    dates = date_range_v9_26(start, end)
    complete: list[str] = []
    partial: list[str] = []
    missing: list[str] = []
    raw_bytes = 0
    ...
    quarantined = quarantine_dates_v9_26(root, set(dates))
    ...
    return {
        ...
        "last_complete_day_before_gap": previous_day_v9_26(first_gap) if first_gap else complete[-1] if complete else None,
        "days_complete": len(complete),
        "days_missing": len(missing),
        "days_partial": len(partial),
        "days_quarantined": len(quarantined),
        "gaps_detected": detect_gaps_v9_26(dates, complete_set),
...
def build_disk_preflight_v9_26(root: Path, canonical: dict[str, Any]) -> dict[str, Any]:
    data_root = root / "data"
    ...
    policy = classify_disk_policy_v9_26(effective_free)
    ...
    return {
        "project_mount_path": mount_path_for_v9_26(root),
        "data_mount_path": mount_path_for_v9_26(data_root if data_root.exists() else root),
        ...
        "raw_bytes_current": directory_size_bytes_v9_26(root / "data/raw"),
        "silver_bytes_current": directory_size_bytes_v9_26(root / "data/silver"),
        "quarantine_bytes_current": directory_size_bytes_v9_26(root / "data/quarantine"),
...
def classify_disk_policy_v9_26(free_bytes: int) -> dict[str, Any]:
    ...
    if free_bytes < MICRO_FREE_BYTES:
        return {
            ...
            "storage_warning": "free_disk_between_60gib_and_100gib_micro_batches_7_days_but_v9_26_campaign_resume_blocked_below_150gib",
            "batch_size_days": 7,
        }
    if free_bytes < COMFORT_FREE_BYTES:
        return {
            ...
            "completion_campaign_allowed_now": False,
            "storage_blocker": False,
            "storage_warning": "free_disk_between_100gib_and_150gib_batches_30_days_but_v9_26_campaign_resume_blocked_below_150gib",
            "batch_size_days": 30,
        }
    return {
        "safe_to_continue_now": True,
...
def build_storage_resume_batches_v9_26(start: str | None, end: str, batch_size_days: int) -> list[StorageResumeBatchSpec]:
    if start is None or batch_size_days <= 0:
        return []
    dates = date_range_v9_26(start, end)
    batches: list[StorageResumeBatchSpec] = []
    for index, offset in enumerate(range(0, len(dates), batch_size_days), start=1):
        chunk = dates[offset : offset + batch_size_days]
        batches.append(StorageResumeBatchSpec(f"V9.26_batch_{index:02d}", chunk[0], chunk[-1], len(chunk)))
    return batches


def execute_storage_resume_batch_v9_26(root: Path, batch: StorageResumeBatchSpec, preflight: dict[str, Any]) -> dict[str, Any]:
    requested_dates = date_range_v9_26(batch.start_date, batch.end_date)
    started = time.monotonic()
    collection_result = collect_storage_resume_batch_v9_26(root, batch, requested_dates, preflight)
    day_results = [validate_batch_day_v9_24(root, day_value) for day_value in requested_dates]
    summary = summarize_storage_resume_batch_v9_26(batch, requested_dates, collection_result, day_results, round(time.monotonic() - started, 3))
    return {
        "version": VERSION,
        "source_version": SOURCE_VERSION,
        ...
        "batch_id": batch.batch_id,
        "batch_spec": batch_to_dict_v9_26(batch),
...
def collect_storage_resume_batch_v9_26(root: Path, batch: StorageResumeBatchSpec, requested_dates: list[str], preflight: dict[str, Any]) -> dict[str, Any]:
    attempted: list[str] = []
    downloaded: list[str] = []
    normalized: list[str] = []
    ...
    for day_value in requested_dates:
        ...
        try:
            ...
            q_path = quarantine_failed_raw_v9_26(root, day_value, raw_path)
            if q_path:
    ...
    return {
        ...
        "new_data_download_scope": "public_historical_aggtrades_resume_campaign_only" if downloaded else None,
        "ingestion_scope": "public_aggtrades_bronze_silver_resume_campaign_only" if normalized else None,
...
def summarize_storage_resume_batch_v9_26(
    batch: StorageResumeBatchSpec,
    ...
    invalid_rows = sum_int_v9_26(day_results, "invalid_rows")
    duplicates = sum_int_v9_26(day_results, "duplicates")
    ...
    return {
        ...
        "total_rows_new": sum_int_v9_26(complete, "rows"),
        "raw_bytes_new": sum_int_v9_26(complete, "raw_bytes"),
        "silver_bytes_new": sum_int_v9_26(complete, "silver_bytes"),
...
def build_storage_resume_summary_v9_26(
    *,
    ...
    rows_before = _source_cumulative_int_v9_26(canonical_before, "total_rows_cumulative")
    return {
        "campaign_start": canonical_before["first_missing_day"],
        "campaign_end": TARGET_WINDOW_END,
        "target_window_start": TARGET_WINDOW_START,
        ...
        "days_requested_total": sum_int_v9_26(summaries, "days_requested"),
        "days_attempted_total": sum_int_v9_26(summaries, "days_attempted"),
        "days_downloaded_total": sum_int_v9_26(summaries, "days_downloaded"),
        "days_normalized_total": sum_int_v9_26(summaries, "days_normalized"),
        "days_complete_total": sum_int_v9_26(summaries, "days_complete"),
        "days_failed_total": sum_int_v9_26(summaries, "days_failed"),
        "days_quarantined_total": sum_int_v9_26(summaries, "days_quarantined"),
        "days_skipped_existing_total": sum_int_v9_26(summaries, "days_skipped_existing"),
        "total_rows_new": sum_int_v9_26(summaries, "total_rows_new"),
        "total_rows_cumulative": rows_before + sum_int_v9_26(summaries, "total_rows_new"),
        "raw_bytes_new": sum_int_v9_26(summaries, "raw_bytes_new"),
        "silver_bytes_new": sum_int_v9_26(summaries, "silver_bytes_new"),
        ...
        "restartability_status": "storage_resume_uses_first_missing_day_skips_existing_and_never_deletes_data",
        "storage_warning": disk_preflight["storage_warning"],
        "stop_reason": stop_reason,
    }


def decide_v9_26(summary: dict[str, Any], canonical: dict[str, Any], disk: dict[str, Any], stop_reason: dict[str, Any] | None) -> dict[str, Any]:
    if not canonical["state_reconciled"]:
        decision = "resume_collection_not_executed_state_not_reconciled"
        recommendation = "V9.27 - Manual Coverage Review Pack"
        ...
        recommendation = "V9.27 - Storage Cleanup / Compression Review"
        ...
        recommendation = "V9.27 - AggTrades Full Coverage Validation"
    elif stop_reason and stop_reason.get("type") == "source_or_quality":
        decision = "resume_collection_partial_quality_issue"
        recommendation = "V9.27 - Resume Collection Continuation"
    elif stop_reason and stop_reason.get("type") == "storage":
        decision = "resume_collection_partial_storage_warning" if summary["days_complete_total"] > 0 else "resume_collection_not_executed_storage_blocker"
        recommendation = "V9.27 - Storage Cleanup / Compression Review"
...
def safety_flags_v9_26(summary: dict[str, Any]) -> dict[str, Any]:
...
def build_manifest_v9_26(report: dict[str, Any]) -> dict[str, Any]:
    summary = report["storage_resume_summary"]
    return {
        "version": VERSION,
        "source_version": SOURCE_VERSION,
...
def build_markdown_v9_26(report: dict[str, Any]) -> str:
    summary = report["storage_resume_summary"]
    ...
    lines = [
        ...
        "",
        "## Resume",
        f"- Decision V9.26 : `{report['decision']}`.",
...
def update_state_surfaces_v9_26(root: Path, report: dict[str, Any]) -> None:
    summary = report["storage_resume_summary"]
    metrics = {
        ...
        "source_version": SOURCE_VERSION,
        "direction": DIRECTION,
        "v9_26_decision": report["decision"],
...
def date_range_v9_26(start: str, end: str) -> list[str]:
...
def previous_day_v9_26(value: str | None) -> str | None:
    if value is None:
        return None
    return (date.fromisoformat(value) - timedelta(days=1)).isoformat()


def detect_gaps_v9_26(dates: list[str], complete_set: set[str]) -> list[dict[str, Any]]:
...
def gaps_before_first_missing_v9_26(local: dict[str, Any]) -> list[dict[str, Any]]:
...
def quarantine_dates_v9_26(root: Path, target_dates: set[str]) -> list[str]:
...
def directory_size_bytes_v9_26(path: Path) -> int:
    if not path.exists():
        return 0
    total = 0
...
def mount_path_for_v9_26(path: Path) -> str:
...
def batch_to_dict_v9_26(batch: StorageResumeBatchSpec) -> dict[str, Any]:
    return {
        "batch_id": batch.batch_id,
        "start_date": batch.start_date,
        "end_date": batch.end_date,
        "max_downloads": batch.max_downloads,
        "expected_days": len(date_range_v9_26(batch.start_date, batch.end_date)),
    }


def sum_int_v9_26(items: list[dict[str, Any]], key: str) -> int:
    return sum(int(item.get(key) or 0) for item in items)


def _source_cumulative_int_v9_26(canonical: dict[str, Any], key: str) -> int:
    source = canonical.get("source_report_payload") or {}
    return int(source.get(key) or 0)


def quarantine_failed_raw_v9_26(root: Path, day_value: str, raw_path: Path) -> Path | None:
...
def build_blockers_v9_26(canonical: dict[str, Any], disk: dict[str, Any], stop_reason: dict[str, Any] | None) -> list[str]:
    blockers: list[str] = []
    if not canonical["state_reconciled"]:
        blockers.append("Etat local non reconciliable avant reprise.")
    if disk["storage_blocker"] or not disk["resume_allowed_now"]:
        blockers.append("Espace disque insuffisant pour la reprise V9.26 sur le volume data.")
    if stop_reason:
        blockers.append(str(stop_reason))
    return blockers


def build_warnings_v9_26(summary: dict[str, Any], canonical: dict[str, Any], disk: dict[str, Any]) -> list[str]:
    warnings: list[str] = []
    if canonical["local_coverage_more_advanced_than_v9_25_1"]:
        warnings.append("La couverture locale est plus avancee que le rapport V9.25.1.")
