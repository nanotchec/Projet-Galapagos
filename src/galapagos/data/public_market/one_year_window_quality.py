...
EXPECTED_ROWS_V4_2 = {"1m": 527040, "5m": 105408, "15m": 35136, "1h": 8784}
EXPECTED_MIN_EVENT_TS_V4_2 = "2024-01-01T00:00:00Z"
EXPECTED_MAX_EVENT_TS_V4_2 = {
...
def assess_one_year_timeframe(
    ...
    *,
    ...
    quality = assess_ohlcv_quality(
        frame,
        expected_rows=EXPECTED_ROWS_V4_2[timeframe],
        timeframe=timeframe,
    ).payload
    ...
    if quality["min_event_ts"] != EXPECTED_MIN_EVENT_TS_V4_2:
        quality["errors"].append(f"min_event_ts {quality['min_event_ts']} != {EXPECTED_MIN_EVENT_TS_V4_2}")
    if quality["max_event_ts"] != EXPECTED_MAX_EVENT_TS_V4_2[timeframe]:
        quality["errors"].append(
            f"max_event_ts {quality['max_event_ts']} != {EXPECTED_MAX_EVENT_TS_V4_2[timeframe]}"
        )
    return quality
...
def parent_child_consistent(frame_1m: pd.DataFrame, child: pd.DataFrame, timeframe: str) -> bool:
    try:
        expected = resample_one_year_ohlcv(frame_1m, target_timeframe=timeframe)
    except ValueError:
        return False
...
def resample_one_year_ohlcv(frame_1m: pd.DataFrame, *, target_timeframe: str) -> pd.DataFrame:
    if target_timeframe not in {"5m", "15m", "1h"}:
        raise ValueError("V4.2 supports target_timeframe=5m, 15m or 1h only.")
    ...
    expected_rows = EXPECTED_ROWS_V4_2[target_timeframe]
