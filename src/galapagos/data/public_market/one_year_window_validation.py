...
RUN_ID_PATTERN_V4_2 = re.compile(r"^v4_2_[0-9]{8}T[0-9]{6}Z_[0-9a-f]{8}$")
MANIFEST_KEYS = {
    "version",
    "status",
    "created_at_utc",
...
FORBIDDEN_V4_2_PATHS = [
    "data/research/v4_2/features",
    "data/research/v4_2/labels",
    "data/research/v4_2/datasets",
    "data/research/v4_2/ml",
    "data/research/v4_2/backtests",
    "data/research/v4_2/strategies",
    "reports/backtests",
    "reports/strategies",
    "reports/signals",
    "reports/predictions",
...
def validate_one_year_public_market_data_v4_2(root: Path = Path(".")) -> dict[str, Any]:
    root = root.resolve()
    errors: list[str] = []
    manifest_path = root / MANIFEST_PATH_V4_2
    report_path = root / REPORT_JSON_PATH_V4_2
    if not manifest_path.exists():
        return _result([f"missing manifest: {MANIFEST_PATH_V4_2}"])
    if not report_path.exists():
        return _result([f"missing quality report: {REPORT_JSON_PATH_V4_2}"])
    manifest = load_json(manifest_path)
    report = load_json(report_path)
    errors.extend(_validate_manifest_structure(manifest))
    errors.extend(scan_payload_for_forbidden_claims(manifest, "V4.2 manifest"))
    ...
    for timeframe in TIMEFRAMES_V4_2:
        ...
        if not path.exists():
            errors.append(f"missing V4.2 output parquet: {path.relative_to(root)}")
            continue
    ...
    if "1m" in frames:
        for timeframe in ["5m", "15m", "1h"]:
            try:
                expected_children[timeframe] = resample_one_year_ohlcv(frames["1m"], target_timeframe=timeframe)
            except ValueError as exc:
                errors.append(f"V4.2 parent-child resample failed for {timeframe}: {exc}")
    ...
    for timeframe, frame in frames.items():
        ...
        if timeframe != "1m" and not consistency:
            errors.append(f"V4.2 parent-child consistency mismatch for {timeframe}")
        quality = assess_one_year_timeframe(
            frame,
            timeframe=timeframe,
        ...
        for error in quality["errors"]:
            errors.append(f"V4.2 physical quality error for {timeframe}: {error}")
    errors.extend(_validate_manifest_quality(manifest, physical_quality))
    errors.extend(_validate_report(manifest, report))
    errors.extend(scan_payload_for_forbidden_claims(report, "V4.2 quality report"))
    errors.extend(_validate_markdown(root))
    errors.extend(_find_forbidden_v4_2_artifacts(root))
    return _result(errors, manifest=manifest)


def _validate_manifest_structure(manifest: dict[str, Any]) -> list[str]:
    errors: list[str] = []
    errors.extend(validate_exact_keys(manifest, MANIFEST_KEYS, "V4.2 manifest"))
    if manifest.get("version") != VERSION_V4_2:
        errors.append("V4.2 manifest version mismatch")
    if manifest.get("status") != "PASS":
        errors.append("V4.2 manifest status must be PASS")
    if not isinstance(manifest.get("created_at_utc"), str) or not manifest["created_at_utc"].endswith("Z"):
        errors.append("V4.2 manifest created_at_utc invalid")
    run_id = manifest.get("run_id")
    if not isinstance(run_id, str) or RUN_ID_PATTERN_V4_2.fullmatch(run_id) is None:
        errors.append("V4.2 manifest run_id invalid")
    errors.extend(validate_exact_keys(manifest.get("source"), SOURCE_KEYS, "V4.2 manifest source"))
    ...
    if source != expected_source:
        errors.append("V4.2 manifest source mismatch")
    if set(manifest.get("raw_files", {})) != set(DATES_V4_2):
        errors.append("V4.2 manifest raw_files dates mismatch")
    for current_date, payload in manifest.get("raw_files", {}).items():
        errors.extend(validate_exact_keys(payload, RAW_FILE_KEYS, f"V4.2 manifest raw_files.{current_date}"))
    if set(manifest.get("outputs", {})) != set(TIMEFRAMES_V4_2):
        errors.append("V4.2 manifest outputs mismatch")
    for timeframe, payload in manifest.get("outputs", {}).items():
        errors.extend(validate_exact_keys(payload, OUTPUT_KEYS, f"V4.2 manifest outputs.{timeframe}"))
    if manifest.get("expected_rows") != EXPECTED_ROWS_V4_2:
        errors.append("V4.2 manifest expected_rows mismatch")
    if set(manifest.get("quality", {})) != set(TIMEFRAMES_V4_2):
        errors.append("V4.2 manifest quality timeframes mismatch")
    for timeframe, payload in manifest.get("quality", {}).items():
        errors.extend(validate_exact_keys(payload, QUALITY_KEYS, f"V4.2 manifest quality.{timeframe}"))
    errors.extend(validate_exact_keys(manifest.get("safety"), SAFETY_KEYS, "V4.2 manifest safety"))
    errors.extend(_validate_safety(manifest.get("safety", {})))
    if manifest.get("limitations") != EXPECTED_LIMITATIONS_V4_2:
        errors.append("V4.2 limitations mismatch")
    return errors
...
def _validate_safety(safety: dict[str, Any]) -> list[str]:
    ...
    if safety.get("public_read_only") is not True:
        errors.append("V4.2 safety flag public_read_only must be True")
    for flag in sorted(SAFETY_KEYS - {"public_read_only"}):
        if safety.get(flag) is not False:
            errors.append(f"V4.2 safety flag {flag} must be False")
    return errors
...
def _validate_raw_files(root: Path, manifest: dict[str, Any]) -> tuple[list[str], dict[str, int]]:
    ...
    for current_date in DATES_V4_2:
        ...
        if raw_payload.get("path") != str(path.relative_to(root)):
            errors.append(f"V4.2 raw path mismatch for {current_date}")
        if raw_payload.get("sha256") != raw_sha:
            errors.append(f"V4.2 raw checksum mismatch for {current_date}")
        if raw_payload.get("bytes") != path.stat().st_size:
            errors.append(f"V4.2 raw bytes mismatch for {current_date}")
        try:
            raw_rows[current_date] = _count_binance_kline_zip_rows_fast(path)
        except Exception as exc:
            errors.append(f"V4.2 raw parse failed for {current_date}: {exc}")
            raw_rows[current_date] = -1
        if raw_payload.get("rows") != raw_rows[current_date]:
            errors.append(f"V4.2 raw rows mismatch for {current_date}")
        if raw_rows[current_date] != 1440:
            errors.append(f"V4.2 raw daily rows mismatch for {current_date}")
...
def _validate_output_entry(root: Path, manifest: dict[str, Any], timeframe: str, path: Path, frame: pd.DataFrame) -> list[str]:
    ...
    for field, value in expected.items():
        if payload.get(field) != value:
            errors.append(f"V4.2 manifest output mismatch for {timeframe}.{field}")
    return errors
...
def _validate_ohlcv_frame(timeframe: str, frame: pd.DataFrame) -> list[str]:
    ...
    if list(frame.columns) != OHLCV_COLUMNS:
        errors.append(f"V4.2 OHLCV schema mismatch for {timeframe}")
    forbidden_columns = sorted(column for column in frame.columns if str(column).casefold() in FORBIDDEN_OHLCV_COLUMNS_V4_2)
    if forbidden_columns:
        errors.append(f"V4.2 forbidden OHLCV columns for {timeframe}: {forbidden_columns}")
    quality = assess_one_year_timeframe(frame, timeframe=timeframe, parent_child_consistency=True)
    for error in quality["errors"]:
        errors.append(f"V4.2 physical quality error for {timeframe}: {error}")
    return errors
...
def _validate_raw_to_1m(manifest: dict[str, Any], frame_1m: pd.DataFrame, raw_rows: dict[str, int]) -> list[str]:
    ...
    for current_date in DATES_V4_2:
        ...
        if observed_sha_set != {expected_sha}:
            errors.append(f"V4.2 raw-to-1m checksum mismatch for {current_date}")
    return errors
...
def _validate_manifest_quality(manifest: dict[str, Any], physical_quality: dict[str, dict[str, Any]]) -> list[str]:
    ...
    for timeframe, expected in physical_quality.items():
        ...
        if observed != expected:
            errors.append(f"V4.2 manifest quality mismatch for {timeframe}")
    return errors


def _validate_report(manifest: dict[str, Any], report: dict[str, Any]) -> list[str]:
    if report != manifest:
        return ["V4.2 quality report mismatch"]
    return []


def _validate_markdown(root: Path) -> list[str]:
    path = root / REPORT_MD_PATH_V4_2
    if not path.exists():
        return [f"missing markdown report: {REPORT_MD_PATH_V4_2}"]
    return validate_markdown_forbidden_claims(path.read_text(encoding="utf-8"), "V4.2 Markdown report")


def _find_forbidden_v4_2_artifacts(root: Path) -> list[str]:
    errors: list[str] = []
    for relative in FORBIDDEN_V4_2_PATHS:
        path = root / relative
        if path.exists():
            errors.append(f"Forbidden V4.2 artifact detected: {relative}")
            for child in sorted(path.rglob("*")):
                if child.is_file():
                    errors.append(f"Forbidden V4.2 artifact detected: {child.relative_to(root).as_posix()}")
    for path in root.rglob("*"):
        if ".git" in path.parts or ".venv" in path.parts or not path.is_file():
            continue
        if path.suffix.casefold() in FORBIDDEN_MODEL_SUFFIXES:
            errors.append(f"Forbidden V4.2 artifact detected: {path.relative_to(root).as_posix()}")
...
def _assert_parent_child_equal(frame_1m: pd.DataFrame, child: pd.DataFrame, timeframe: str) -> None:
    if not parent_child_consistent(frame_1m, child, timeframe):
        raise AssertionError(f"V4.2 parent-child consistency mismatch for {timeframe}")
...
def _result(errors: list[str], *, manifest: dict[str, Any] | None = None) -> dict[str, Any]:
    return {"version": VERSION_V4_2, "passed": not errors, "errors": errors, "manifest": manifest}
