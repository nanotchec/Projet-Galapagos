...
VERSION = "V9.21"
LAST_VALIDATED_VERSION = "V9.20"
SOURCE_VERSION = "V9.20"
DIRECTION = "aggtrades_post_v9_batch_expansion"
...
REPORT_JSON_PATH = Path("reports/data/aggtrades_post_v9_batch_expansion_v9_21.json")
REPORT_MD_PATH = Path("reports/data/aggtrades_post_v9_batch_expansion_v9_21.md")
MANIFEST_PATH = Path("reports/manifests/aggtrades_post_v9_batch_expansion_v9_21_manifest.json")
DOC_PATH = Path("docs/aggtrades_post_v9_batch_expansion_v9_21.md")

ALLOWED_MODES = {"dry-run", "collect", "validate-only"}
ALLOWED_DECISIONS = {
    "aggtrades_post_v9_batch_expansion_success",
    "aggtrades_post_v9_batch_expansion_partial",
    "aggtrades_post_v9_batch_expansion_failed_source_issue",
    "aggtrades_post_v9_batch_expansion_failed_quality",
    "aggtrades_post_v9_batch_expansion_not_executed",
...
def run_aggtrades_post_v9_batch_expansion_v9_21(
    root: Path = Path("."),
    *,
    mode: str = "dry-run",
    ...
    root = root.resolve()
    report = build_aggtrades_post_v9_batch_report_v9_21(
        root,
        mode=mode,
    ...
    markdown = build_markdown_v9_21(report)
    _write_text(root / REPORT_MD_PATH, markdown)
    _write_text(root / DOC_PATH, markdown)
    _write_json(root / MANIFEST_PATH, build_manifest_v9_21(report))
    update_state_surfaces_v9_21(root, report)
    return report


def build_aggtrades_post_v9_batch_report_v9_21(
    root: Path = Path("."),
    *,
    mode: str = "dry-run",
    start_date: str = BATCH_START,
    ...
    if mode not in ALLOWED_MODES:
        raise ValueError(f"unsupported V9.21 mode: {mode}")
    requested_dates = date_range_v9_21(start_date, end_date)
    validate_batch_request_v9_21(mode, requested_dates, max_downloads)
    root = root.resolve()
    started = time.monotonic()
    inputs = {name: _load_input(root, path) for name, path in INPUT_PATHS.items()}
    v9_20_prerequisite = build_v9_20_prerequisite_check_v9_21(root)
    day_plan_before = build_batch_day_plan_v9_21(root, requested_dates)
    collection_result = execute_batch_mode_v9_21(
        root,
        mode,
        requested_dates,
        max_downloads=max_downloads,
    )
    day_plan_after = build_batch_day_plan_v9_21(root, requested_dates)
    day_validation = [validate_batch_day_v9_21(root, day_value) for day_value in requested_dates]
    runtime_seconds = round(time.monotonic() - started, 3)
    batch_summary = summarize_batch_validation_v9_21(
        requested_dates,
        day_plan_before,
        collection_result,
        day_validation,
    ...
    safety_flags = safety_flags_for_batch_v9_21(collection_result)
    ...
    if mode != "collect" and decision["decision"] == "aggtrades_post_v9_batch_expansion_not_executed":
        status = "PASS"
    report = {
        "version": VERSION,
        ...
        "source_public_target": build_source_design_v9_21(),
        "global_target_window": {
            "start": TARGET_START,
            "end": TARGET_END,
            "days_expected": len(date_range_v9_21(TARGET_START, TARGET_END)),
            "complete_collection_reached": False,
        },
        "future_funding_first_window": {
            "start": FUNDING_FIRST_START,
            "end": FUNDING_FIRST_END,
            "days_expected": len(date_range_v9_21(FUNDING_FIRST_START, FUNDING_FIRST_END)),
            "complete_collection_reached": False,
        },
        "batch_window": {
            "start": start_date,
        ...
        "v9_20_prerequisite_check": v9_20_prerequisite,
        "storage_convention": {
        ...
        "anti_leakage_plan": build_anti_leakage_plan_v9_21(),
        "v9_21_decision": decision,
        ...
        "warnings": build_warnings_v9_21(collection_result, batch_summary),
        "limitations": [
            "V9.21 ne collecte pas la fenetre complete de 772 jours.",
...
def validate_batch_request_v9_21(mode: str, requested_dates: list[str], max_downloads: int | None) -> None:
    if not requested_dates:
        raise ValueError("V9.21 batch window cannot be empty.")
    if len(requested_dates) > MAX_BATCH_DOWNLOADS:
        raise ValueError("V9.21 batch window is limited to 60 days.")
    if mode == "collect":
        if max_downloads is None:
            raise ValueError("V9.21 collect mode requires --max-downloads to prevent accidental bulk collection.")
        if max_downloads <= 0 or max_downloads > MAX_BATCH_DOWNLOADS:
            raise ValueError("V9.21 collect mode requires 1 <= --max-downloads <= 60.")
        if max_downloads > len(requested_dates):
            raise ValueError("V9.21 --max-downloads cannot exceed requested batch days.")


def build_source_design_v9_21() -> dict[str, Any]:
    return {
...
def build_batch_day_plan_v9_21(root: Path, requested_dates: list[str]) -> list[dict[str, Any]]:
...
def build_v9_20_prerequisite_check_v9_21(root: Path) -> dict[str, Any]:
    previous_dates = date_range_v9_21(PREVIOUS_COVERAGE_START, PREVIOUS_COVERAGE_END)
    day_plan = build_batch_day_plan_v9_21(root, previous_dates)
    ...
    return {
        ...
        "v9_19_v9_20_days_present_and_complete": previous_complete,
        "v9_21_can_run_independently": True,
        "cumulative_coverage_claim_allowed": previous_complete,
        "note": "V9.21 peut s'executer independamment; la couverture cumulee V9.19+V9.20+V9.21 n'est revendiquee que si les jours precedents sont presents localement.",
    }


def execute_batch_mode_v9_21(
    root: Path,
    mode: str,
    ...
    *,
    ...
    return collect_batch_public_aggtrades_v9_21(root, requested_dates, max_downloads=max_downloads)


def collect_batch_public_aggtrades_v9_21(
    root: Path,
    requested_dates: list[str],
    *,
    max_downloads: int | None,
) -> dict[str, Any]:
    validate_batch_request_v9_21("collect", requested_dates, max_downloads)
    ...
    for day_value in requested_dates:
        ...
        try:
            ...
            download_public_archive_v9_21(build_public_archive_url_v9_18(day_value), raw_path)
            ...
            quarantine_path = quarantine_failed_raw_v9_21(root, day_value, raw_path)
    ...
    return {
        ...
        "new_data_download_scope": "public_historical_aggtrades_batch_expansion_only",
        "ingestion_scope": "public_aggtrades_bronze_silver_batch_expansion_only",
...
def download_public_archive_v9_21(url: str, destination: Path) -> None:
    from urllib.parse import urlparse
    from urllib.request import Request, urlopen

    parsed = urlparse(url)
    if parsed.scheme != "https" or parsed.netloc not in ALLOWED_PUBLIC_HOSTS:
        raise ValueError("V9.21 allows public read-only downloads from data.binance.vision only.")
    ...
    if tmp_path.exists():
        tmp_path.unlink()
    request = Request(url, headers={"User-Agent": "galapagos-v9.21-public-read-only"})
...
def validate_batch_day_v9_21(root: Path, day_value: str) -> dict[str, Any]:
    raw_path = root / raw_zip_path_for_date_v9_18(day_value)
    silver_path = root / silver_path_for_date_v9_18(day_value)
    errors: list[str] = []
    raw_bytes = raw_path.stat().st_size if raw_path.exists() else 0
...
def summarize_batch_validation_v9_21(
    ...
    target_days = len(date_range_v9_21(TARGET_START, TARGET_END))
    ...
    cumulative_start = PREVIOUS_COVERAGE_START if v9_20_prerequisite["v9_19_v9_20_days_present_and_complete"] and len(complete_days) == len(requested_dates) else None
...
def decide_v9_21(collection_result: dict[str, Any], batch_summary: dict[str, Any]) -> dict[str, Any]:
    if not collection_result["collection_executed"]:
        decision = "aggtrades_post_v9_batch_expansion_not_executed"
        recommendation = "V9.22 - AggTrades Post-V9 Next Batch Expansion."
        confidence = "high"
        justification = "Aucune collecte batche n'a ete executee."
    elif collection_result["status"] != "PASS":
        decision = "aggtrades_post_v9_batch_expansion_failed_source_issue"
        recommendation = "V9.22 - AggTrades Batch Collection Correction."
        confidence = "medium"
        ...
        decision = "aggtrades_post_v9_batch_expansion_failed_quality"
        recommendation = "V9.22 - AggTrades Batch Collection Correction."
        confidence = "medium"
        ...
        recommendation = "V9.22 - AggTrades Post-V9 Multi-Batch Completion Plan."
        confidence = "high"
        justification = "Les jours batches demandes ont ete collectes, normalises et valides sans etendre la collecte complete."
    else:
        decision = "aggtrades_post_v9_batch_expansion_partial"
        recommendation = "V9.22 - AggTrades Batch Collection Correction."
        confidence = "medium"
...
def build_manifest_v9_21(report: dict[str, Any]) -> dict[str, Any]:
    ...
    return {
        ...
        "v9_21_decision": report["v9_21_decision"],
        "findings": report["findings"],
        "safety_flags": report["safety_flags"],
    }


def build_markdown_v9_21(report: dict[str, Any]) -> str:
    summary = report["batch_validation"]["summary"]
    decision = report["v9_21_decision"]
    ...
    lines = [
        ...
        f"- Decision V9.21 : `{decision['decision']}`.",
        ...
        f"- Couverture cumulee connue V9.19+V9.20+V9.21 : `{summary['cumulative_known_coverage_start']}` -> `{summary['cumulative_known_coverage_end']}`.",
        "",
...
def update_state_surfaces_v9_21(root: Path, report: dict[str, Any]) -> None:
    summary = report["batch_validation"]["summary"]
    metrics = {
        ...
        "direction": DIRECTION,
        "mode": report["mode"],
        "v9_21_decision": report["v9_21_decision"]["decision"],
    ...
    text = (
        "# Synthese courante - V9.21\n\n"
        "- Derniere version validee : `V9.20`.\n"
        "- Candidate : `V9.21`.\n"
        "- Statut : `pending_external_audit`.\n"
        "- Direction : expansion batche de collecte aggTrades post-V9.\n"
        f"- Periode batche : `{report['batch_window']['start']}` -> `{report['batch_window']['end']}`.\n"
        f"- Decision V9.21 : `{report['v9_21_decision']['decision']}`.\n"
    ...
    _write_text(
        ...
        "# Projet Galapagos\n\n"
        "- Derniere version validee : V9.20.\n"
        "- Candidate : V9.21, expansion batche de collecte aggTrades post-V9.\n"
        "- Batch limite : 2024-06-11 -> 2024-08-09, maximum 60 jours.\n"
...
def safety_flags_for_batch_v9_21(collection_result: dict[str, Any]) -> dict[str, Any]:
...
def build_anti_leakage_plan_v9_21() -> dict[str, Any]:
    return {
        "rules": [
            "available_ts >= event_ts for every normalized batch row.",
            "V9.21 collects and normalizes aggTrades only; it creates no labels and no supervised dataset.",
            "Funding and open interest are not joined in V9.21.",
...
def build_warnings_v9_21(collection_result: dict[str, Any], batch_summary: dict[str, Any]) -> list[str]:
    warnings = [
        "V9.21 ne couvre pas la fenetre complete de 772 jours.",
...
def quarantine_failed_raw_v9_21(root: Path, day_value: str, raw_path: Path) -> Path | None:
    if not raw_path.exists():
        return None
...
def date_range_v9_21(start: str, end: str) -> list[str]:
