    ...
    def get_plan(self) -> Dict[str, Any]:
        return {
            ...
            "plan_details": {
                ...
                "symbol": "BTCUSDT",
                "expected_raw_fields": ["id", "price", "qty", "quoteQty", "time", "isBuyerMaker", "isBestMatch"],
                "normalized_schema_candidate": [
                    "provider", "symbol", "trade_id", "price_decimal_string", 
                    "quantity_decimal_string", "quote_quantity_decimal_string", 
                    "exchange_timestamp_ms", "is_buyer_maker", "is_best_match", 
                    "request_index", "source_status_code", "collected_at_utc"
                ],
                "quality_checks_candidate": [
                    "status_code non-null", "status_code == 200", "required fields present",
                    "numeric strings parseable", "timestamp integer milliseconds",
                    "no duplicate provider/symbol/trade_id within preview",
                    "monotonicity check within request when applicable",
                    "no lookahead labels", "no strategy fields", "no target labels",
