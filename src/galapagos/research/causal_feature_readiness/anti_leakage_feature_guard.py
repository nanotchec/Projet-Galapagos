...
def _matches(value: str, term: str) -> bool:
    ...
    if normalized_term == "ev":
        return bool(re.search(r"(^|_)ev($|_)", normalized))
    if normalized_term == "win":
        return bool(re.search(r"(^|_)win($|_)", normalized))
    return normalized_term in normalized


def scan_forbidden_feature_terms(payloads: dict[str, Any]) -> dict[str, Any]:
                        ...
                        occurrences.append(
                            {
                                "file": source,
                                "json_path": key_path,
                                "offending_key_or_value": str(key),
                                "matched_term": term,
                            }
                        )
    ...
    return {
        ...
        "lookahead_detected": any(o["matched_term"] in {"future", "future_return", "next_return", "next_price"} for o in occurrences),
