...
def assess_refined_ohlcv_trades_dataset_quality_v9_1(
    frame: pd.DataFrame,
    ...
    *,
    ...
    if rows != expected_rows:
        errors.append(f"{timeframe} V9.1 dataset rows mismatch: got {rows}, expected {expected_rows}")
    ...
    if duplicate_rows:
        errors.append(f"{timeframe} V9.1 dataset duplicate join keys: {duplicate_rows}")
    ...
    expected_splits = EXPECTED_SPLIT_COUNTS_V9_1[timeframe]
    if split_counts != expected_splits:
        errors.append(f"{timeframe} V9.1 split counts mismatch: {split_counts}")
    ...
    if not walk_forward_group_counts:
        errors.append(f"{timeframe} V9.1 walk_forward_group missing")
    elif split_frame["walk_forward_group"].isna().any():
        errors.append(f"{timeframe} V9.1 walk_forward_group contains nulls")
    ...
    null_counts_by_column = {
        column: int(frame[column].isna().sum()) if column in frame.columns else expected_rows for column in DATASET_COLUMNS_V9_1
    }

    forbidden_columns_present = forbidden_dataset_columns_v9_1(frame)
    if forbidden_columns_present:
        errors.append(f"{timeframe} V9.1 dataset contains forbidden columns: {forbidden_columns_present}")

    timestamps_utc = _timestamps_utc(frame)
    if not timestamps_utc:
        errors.append(f"{timeframe} V9.1 dataset timestamps are not UTC")
    ...
    if "event_ts" in frame.columns and rows > 0:
        ...
        if not monotonic_event_ts:
            errors.append(f"{timeframe} V9.1 dataset event_ts is not monotonic")
    ...
    if {"feature_available_ts", "decision_ts"}.issubset(frame.columns):
        ...
        if not feature_available_ts_valid:
            errors.append(f"{timeframe} V9.1 feature_available_ts > decision_ts")

    label_available_ts_valid = _label_available_ts_valid(frame)
    if not label_available_ts_valid:
        errors.append(f"{timeframe} V9.1 label_available_ts <= decision_ts for valid labels")

    split_temporal_order_valid = _split_temporal_order_valid(frame)
    if not split_temporal_order_valid:
        errors.append(f"{timeframe} V9.1 split temporal order invalid")
    ...
    if {"source_features_sha256", "source_labels_sha256"}.issubset(frame.columns):
        ...
        if not source_hashes_valid:
            errors.append(f"{timeframe} V9.1 source hashes invalid")
...
def forbidden_dataset_columns_v9_1(frame: pd.DataFrame) -> list[str]:
    forbidden = set(FORBIDDEN_DATASET_COLUMNS_EXACT_V9_1)
