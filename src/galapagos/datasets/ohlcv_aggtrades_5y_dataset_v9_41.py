...
from galapagos.datasets.ohlcv_aggtrades_5y_dataset_v9_41_datacard import build_dataset_datacard_v9_41
...
def run_ohlcv_aggtrades_5y_dataset_v9_41(root: Path = Path(".")) -> dict[str, Any]:
    root = root.resolve()
    report = build_dataset_report_v9_41(root)
    _write_json(root / REPORT_JSON_PATH, report)
    markdown = build_markdown_v9_41(report)
    _write_text(root / REPORT_MD_PATH, markdown)
    _write_text(root / DATACARD_MD_PATH, build_dataset_datacard_v9_41(report))
    _write_text(root / DOC_PATH, markdown)
    _write_json(root / MANIFEST_PATH, build_manifest_v9_41(report))
    update_state_surfaces_v9_41(root, report)
    return report


def build_dataset_report_v9_41(root: Path) -> dict[str, Any]:
    started = time.monotonic()
    inputs = {name: _load_input(root, path) for name, path in INPUT_PATHS.items()}
    feature_readiness = assess_feature_readiness_v9_41(inputs)
    label_readiness = assess_label_readiness_v9_41(inputs)
    ...
    if feature_readiness["ready"] and label_readiness["ready"]:
        for timeframe in TIMEFRAMES:
            try:
                result = create_timeframe_dataset_v9_41(root, timeframe, dataset_run_id)
    ...
    leakage_guard = build_leakage_guard_v9_41(metrics_by_timeframe)
    forbidden_column_scan = build_forbidden_scan_v9_41(metrics_by_timeframe)
    ...
    quality_status = quality_status_v9_41(dataset_created, leakage_guard, forbidden_column_scan, warnings, errors)
    coverage_status = coverage_status_v9_41(dataset_created, row_counts)
    decision = decide_v9_41(dataset_created, feature_readiness, label_readiness, leakage_guard, quality_status, warnings, errors)

    report = {
        "version": VERSION,
        "source_version": SOURCE_VERSION,
        "status": "PASS" if decision in {"ohlcv_aggtrades_5y_dataset_created", "ohlcv_aggtrades_5y_dataset_created_with_warnings"} else "FAIL",
        ...
        "errors": errors,
        "limitations": [
            "V9.41 cree un dataset supervise offline, mais n'execute aucun ML.",
            "Le target H1 est un candidat de recherche et ne valide aucun edge, strategie ou signal.",
            "Le split est temporel simple 60/20/20 sans purge/embargo reel; V9.41 reste un dataset-only preview.",
        ],
        "decision": decision,
        "next_recommendation": next_recommendation_v9_41(decision),
    ...
    if report["decision"] not in ALLOWED_DECISIONS:
        raise RuntimeError(f"invalid V9.41 decision: {report['decision']}")
    return report


def assess_feature_readiness_v9_41(inputs: dict[str, dict[str, Any]]) -> dict[str, Any]:
    report = inputs["v9_38_feature_validation"].get("payload", {})
    errors: list[str] = []
    ...
    if report.get("coverage_status") != "target_5y_feature_window_complete":
        errors.append("V9.38 feature coverage is not complete")
    if report.get("quality_status") != "PASS":
        errors.append("V9.38 feature quality is not PASS")
    if report.get("leakage_guard_status") != "PASS":
        errors.append("V9.38 feature leakage guard is not PASS")
    if report.get("actual_rows") != EXPECTED_ROWS:
        errors.append(f"V9.38 feature row counts mismatch: {report.get('actual_rows')}")
    return {
        "ready": not errors,
        "errors": errors,
        "source_feature_store_version": SOURCE_FEATURE_STORE_VERSION,
        ...
        "row_counts": report.get("actual_rows"),
    }


def assess_label_readiness_v9_41(inputs: dict[str, dict[str, Any]]) -> dict[str, Any]:
...
def create_timeframe_dataset_v9_41(root: Path, timeframe: str, dataset_run_id: str) -> dict[str, Any]:
    ...
    validate_alignment_v9_41(features, labels, timeframe)
    dataset = assemble_dataset_frame_v9_41(features, labels, timeframe, dataset_run_id)
    output_path = root / dataset_output_path_v9_41(timeframe)
    output_path.parent.mkdir(parents=True, exist_ok=True)
    dataset.to_parquet(output_path, index=False, engine="pyarrow", compression="zstd")
    metrics = summarize_dataset_v9_41(dataset, timeframe, output_path)
    return {
        "output": {
            "created": True,
            "path": dataset_output_path_v9_41(timeframe).as_posix(),
...
def validate_alignment_v9_41(features: pd.DataFrame, labels: pd.DataFrame, timeframe: str) -> None:
    if len(features) != len(labels):
        raise ValueError(f"{timeframe}: feature/label row count mismatch {len(features)} != {len(labels)}")
    for column in ["source", "venue", "market_type", "symbol", "timeframe", "event_ts", "decision_ts"]:
        if not features[column].equals(labels[column]):
            raise ValueError(f"{timeframe}: feature/label alignment mismatch on {column}")


def assemble_dataset_frame_v9_41(features: pd.DataFrame, labels: pd.DataFrame, timeframe: str, dataset_run_id: str) -> pd.DataFrame:
    n = len(features)
    split = split_series_v9_41(n)
    target = labels[SELECTED_PRIMARY_LABEL]
    label_available_ts = selected_h1_label_available_ts_v9_41(features, timeframe)
    feature_ok = features["row_valid_for_features"].fillna(False).astype(bool)
    ...
    feature_errors = pd.to_numeric(features["feature_error_count"], errors="coerce").fillna(0).astype("int16")
    dataset_error_count = (feature_errors + target_missing + leakage_errors).astype("int16")
    dataset_null_count = (
        pd.to_numeric(features["feature_null_count"], errors="coerce").fillna(0).astype("int16")
    ...
    out["source_feature_validation_version"] = SOURCE_FEATURE_VALIDATION_VERSION
...
def selected_h1_label_available_ts_v9_41(features: pd.DataFrame, timeframe: str) -> pd.Series:
    horizon_bars = 60 // TIMEFRAME_MINUTES[timeframe]
    return features["close_ts"].shift(-horizon_bars) + pd.Timedelta(milliseconds=1)


def split_series_v9_41(n: int) -> np.ndarray:
...
def summarize_dataset_v9_41(dataset: pd.DataFrame, timeframe: str, output_path: Path) -> dict[str, Any]:
    valid = dataset.loc[dataset["row_valid_for_dataset"], SELECTED_PRIMARY_LABEL]
    target_distribution = distribution_stats_v9_41(value_counts_v9_41(valid))
    ...
    metrics = {
        ...
        "split_distribution": split_distribution_v9_41(dataset),
        "monthly_distribution": monthly_distribution_v9_41(dataset),
        "null_summary": null_summary_v9_41(dataset),
        ...
        "split_monotone": split_monotone_v9_41(dataset),
        "train_before_validation_before_test": train_before_validation_before_test_v9_41(dataset),
        "warnings": [],
    }
...
def value_counts_v9_41(series: pd.Series) -> dict[str, int]:
    return {str(int(key)): int(value) for key, value in series.dropna().value_counts().sort_index().items()}


def distribution_stats_v9_41(counts: dict[str, int]) -> dict[str, Any]:
...
def split_distribution_v9_41(dataset: pd.DataFrame) -> dict[str, Any]:
    result: dict[str, Any] = {}
    for split_name, chunk in dataset.loc[dataset["row_valid_for_dataset"]].groupby("split", sort=False):
        result[str(split_name)] = distribution_stats_v9_41(value_counts_v9_41(chunk[SELECTED_PRIMARY_LABEL]))
        result[str(split_name)]["rows"] = int(len(chunk))
    return result


def monthly_distribution_v9_41(dataset: pd.DataFrame) -> dict[str, Any]:
    ...
    for month, chunk in valid.groupby("walk_forward_group", sort=True):
        result[str(month)] = distribution_stats_v9_41(value_counts_v9_41(chunk[SELECTED_PRIMARY_LABEL]))
        result[str(month)]["rows"] = int(len(chunk))
    return result


def null_summary_v9_41(dataset: pd.DataFrame) -> dict[str, Any]:
...
def split_monotone_v9_41(dataset: pd.DataFrame) -> bool:
    ranks = dataset["split"].map({"train": 0, "validation": 1, "test": 2}).to_numpy()
    return bool(np.all(ranks[:-1] <= ranks[1:])) if len(ranks) > 1 else True


def train_before_validation_before_test_v9_41(dataset: pd.DataFrame) -> bool:
    ranges = {}
...
def build_leakage_guard_v9_41(metrics_by_timeframe: dict[str, Any]) -> dict[str, Any]:
...
def build_forbidden_scan_v9_41(metrics_by_timeframe: dict[str, Any]) -> dict[str, Any]:
...
def coverage_status_v9_41(dataset_created: bool, row_counts: dict[str, int]) -> str:
...
def decide_v9_41(
    ...
    if not label_readiness["ready"]:
        return "ohlcv_aggtrades_5y_dataset_blocked_by_label_quality"
    if leakage_guard["status"] != "PASS":
    ...
    if dataset_created:
        return "ohlcv_aggtrades_5y_dataset_created"
    if warnings:
        return "ohlcv_aggtrades_5y_dataset_partial"
    return "ohlcv_aggtrades_5y_dataset_blocked_by_label_quality"


def next_recommendation_v9_41(decision: str) -> str:
    if decision in {"ohlcv_aggtrades_5y_dataset_created", "ohlcv_aggtrades_5y_dataset_created_with_warnings"}:
        return "V9.42 - OHLCV + AggTrades 5Y Dataset Validation"
    if decision == "ohlcv_aggtrades_5y_dataset_blocked_by_leakage":
        return "V9.42 - 5Y Dataset Correction"
    if "feature" in decision or "label" in decision:
        return "V9.42 - Manual Dataset Review Pack"
    return "V9.42 - 5Y Dataset Correction"


def dataset_output_path_v9_41(timeframe: str) -> Path:
    return DATASET_BASE_PATH / f"timeframe={timeframe}" / f"window={TARGET_WINDOW_START}_{TARGET_WINDOW_END}" / "dataset.parquet"


def build_manifest_v9_41(report: dict[str, Any]) -> dict[str, Any]:
    return {
        "version": VERSION,
        "source_version": SOURCE_VERSION,
...
def build_markdown_v9_41(report: dict[str, Any]) -> str:
    lines = [
        "# V9.41 - OHLCV + AggTrades 5Y Dataset",
        "",
        "## Resume",
        f"- Decision V9.41 : `{report['decision']}`.",
        ...
        "",
        "## Readiness",
        f"- Features V9.37/V9.38 : `{report['feature_readiness']['ready']}`.",
...
def update_state_surfaces_v9_41(root: Path, report: dict[str, Any]) -> None:
    metrics = {
        ...
        "source_version": SOURCE_VERSION,
        "direction": DIRECTION,
        "v9_41_decision": report["decision"],
