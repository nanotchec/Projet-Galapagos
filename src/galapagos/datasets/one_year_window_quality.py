...
def assess_one_year_dataset_quality(
    frame: pd.DataFrame,
    *,
    expected_rows: int,
    timeframe: str,
    ...
    if rows != expected_rows:
        errors.append(f"{timeframe} V4.5 dataset rows mismatch: got {rows}, expected {expected_rows}")
    ...
    if duplicate_rows:
        errors.append(f"{timeframe} V4.5 dataset duplicate join keys: {duplicate_rows}")
    ...
    if split_counts != EXPECTED_SPLIT_COUNTS_V4_5[timeframe]:
        errors.append(f"{timeframe} V4.5 split counts mismatch: {split_counts}")
    ...
    null_counts_by_column = {
        ...
        for column in DATASET_COLUMNS_V4_5
    }
    ...
    if forbidden_columns_present:
        errors.append(f"{timeframe} V4.5 dataset contains forbidden columns: {forbidden_columns_present}")

    timestamps_utc = _timestamps_utc(frame)
    if not timestamps_utc:
        errors.append(f"{timeframe} V4.5 dataset timestamps are not UTC")
    ...
    if "event_ts" in frame.columns and rows > 0:
        ...
        if not monotonic_event_ts:
            errors.append(f"{timeframe} V4.5 dataset event_ts is not monotonic")
    ...
    if {"feature_available_ts", "decision_ts"}.issubset(frame.columns):
        ...
        if not feature_available_ts_valid:
            errors.append(f"{timeframe} V4.5 feature_available_ts > decision_ts")

    label_available_ts_valid = _label_available_ts_valid(frame)
    if not label_available_ts_valid:
        errors.append(f"{timeframe} V4.5 label_available_ts <= decision_ts for valid labels")

    split_temporal_order_valid = _split_temporal_order_valid(frame)
    if not split_temporal_order_valid:
        errors.append(f"{timeframe} V4.5 split temporal order invalid")
    ...
    if {"source_features_sha256", "source_labels_sha256"}.issubset(frame.columns):
        ...
        if not source_hashes_valid:
            errors.append(f"{timeframe} V4.5 source hashes invalid")
