...
def expected_split_counts_v5_3(rows: int) -> dict[str, int]:
    train = int(rows * SPLIT_POLICY_V5_3["train_ratio"])
    validation = int(rows * SPLIT_POLICY_V5_3["validation_ratio"])
    return {"train": train, "validation": validation, "test": rows - train - validation}


def assess_max_history_dataset_quality(
    frame: pd.DataFrame,
    ...
    *,
    ...
    if rows != expected_rows:
        errors.append(f"{timeframe} V5.3 dataset rows mismatch: got {rows}, expected {expected_rows}")
    ...
    if duplicate_rows:
        errors.append(f"{timeframe} V5.3 dataset duplicate join keys: {duplicate_rows}")
    ...
    expected_splits = expected_split_counts_v5_3(expected_rows)
    if split_counts != expected_splits:
        errors.append(f"{timeframe} V5.3 split counts mismatch: {split_counts}")
    ...
    if not walk_forward_group_counts:
        errors.append(f"{timeframe} V5.3 walk_forward_group missing")
    elif split_frame["walk_forward_group"].isna().any():
        errors.append(f"{timeframe} V5.3 walk_forward_group contains nulls")
    ...
    null_counts_by_column = {
        column: int(frame[column].isna().sum()) if column in frame.columns else expected_rows for column in DATASET_COLUMNS_V5_3
    }
    ...
    if forbidden_columns_present:
        errors.append(f"{timeframe} V5.3 dataset contains forbidden columns: {forbidden_columns_present}")

    timestamps_utc = _timestamps_utc(frame)
    if not timestamps_utc:
        errors.append(f"{timeframe} V5.3 dataset timestamps are not UTC")
    ...
    if "event_ts" in frame.columns and rows > 0:
        ...
        if not monotonic_event_ts:
            errors.append(f"{timeframe} V5.3 dataset event_ts is not monotonic")
    ...
    if {"feature_available_ts", "decision_ts"}.issubset(frame.columns):
        ...
        if not feature_available_ts_valid:
            errors.append(f"{timeframe} V5.3 feature_available_ts > decision_ts")

    label_available_ts_valid = _label_available_ts_valid(frame)
    if not label_available_ts_valid:
        errors.append(f"{timeframe} V5.3 label_available_ts <= decision_ts for valid labels")

    split_temporal_order_valid = _split_temporal_order_valid(frame)
    if not split_temporal_order_valid:
        errors.append(f"{timeframe} V5.3 split temporal order invalid")
    ...
    if {"source_features_sha256", "source_labels_sha256"}.issubset(frame.columns):
        ...
        if not source_hashes_valid:
            errors.append(f"{timeframe} V5.3 source hashes invalid")
