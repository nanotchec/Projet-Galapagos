...
def validate_v9_49_report(root: Path = Path("."), *, audit_lite: bool = False) -> dict[str, Any]:
    root = root.resolve()
    ...
    if not report_path.is_file():
        ...
        return result_v9_49(errors)
    if not manifest_path.is_file():
        errors.append(f"missing manifest: {MANIFEST_PATH.as_posix()}")
        return result_v9_49(errors)
    report = _read_json(report_path)
    manifest = _read_json(manifest_path)
    errors.extend(validate_report_payload_v9_49(report))
    errors.extend(validate_manifest_payload_v9_49(report, manifest))
    if not audit_lite:
        errors.extend(validate_outputs_v9_49(root, report))
    return result_v9_49(errors, report)


def validate_report_payload_v9_49(report: dict[str, Any]) -> list[str]:
    errors: list[str] = []
    if report.get("version") != VERSION or report.get("source_version") != "V9.48":
    ...
    if report.get("network_used") is not False or report.get("new_data_downloaded") is not False:
        errors.append("V9.49 must not use network or download data")
    if report.get("ml_executed") is not False or report.get("walk_forward_executed") is not False or report.get("backtest_executed") is not False:
        errors.append("V9.49 must not run ML, walk-forward or backtest")
    if report.get("signal_created") is not False or report.get("strategy_created") is not False:
        errors.append("V9.49 must not create signals or strategies")
    if report.get("decision") in {"combined_features_5y_dataset_created", "combined_features_5y_dataset_created_with_warnings"}:
    ...
    errors.extend(validate_safety_flags_v9_49(report))
    return errors


def validate_manifest_payload_v9_49(report: dict[str, Any], manifest: dict[str, Any]) -> list[str]:
    errors: list[str] = []
    for key in [
        "version",
        "source_version",
...
def validate_outputs_v9_49(root: Path, report: dict[str, Any]) -> list[str]:
    errors: list[str] = []
    if report.get("dataset_created") is not True:
        return errors
...
def validate_safety_flags_v9_49(report: dict[str, Any]) -> list[str]:
    errors: list[str] = []
...
def result_v9_49(errors: list[str], report: dict[str, Any] | None = None) -> dict[str, Any]:
    return {
        "version": VERSION,
