...
def validate_v9_50_report(root: Path = Path("."), mode: str = "audit-lite") -> dict[str, Any]:
    root = root.resolve()
    errors: list[str] = []
    ...
    errors.extend(validate_report_payload_v9_50(report))
    errors.extend(validate_manifest_payload_v9_50(report, manifest))
    if mode == "audit-lite":
        errors.extend(validate_audit_lite_samples_v9_50(root, samples))
    return result_v9_50(errors, report)


def validate_report_payload_v9_50(report: dict[str, Any]) -> list[str]:
    errors: list[str] = []
    if report.get("version") != VERSION or report.get("source_version") != "V9.49":
    ...
    if report.get("dataset_created") is not False:
        errors.append("V9.50 itself must not create a new full dataset")
    if report.get("network_used") is not False or report.get("new_data_downloaded") is not False:
        errors.append("V9.50 must not use network or download data")
    if report.get("ml_executed") is not False or report.get("walk_forward_executed") is not False or report.get("backtest_executed") is not False:
        errors.append("V9.50 must not run ML, walk-forward or backtest")
    if report.get("signal_created") is not False or report.get("strategy_created") is not False:
        errors.append("V9.50 must not create signals or strategies")
    ...
    if report.get("decision") in {"combined_features_5y_dataset_validated", "combined_features_5y_dataset_validated_with_warnings"}:
    ...
    errors.extend(validate_safety_flags_v9_50(report))
    return errors


def validate_manifest_payload_v9_50(report: dict[str, Any], manifest: dict[str, Any]) -> list[str]:
    errors: list[str] = []
...
def validate_audit_lite_samples_v9_50(root: Path, samples: dict[str, Any]) -> list[str]:
    errors: list[str] = []
...
def validate_safety_flags_v9_50(report: dict[str, Any]) -> list[str]:
    errors: list[str] = []
...
def result_v9_50(errors: list[str], report: dict[str, Any] | None = None) -> dict[str, Any]:
    return {
        "version": VERSION,
