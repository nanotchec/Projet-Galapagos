...
from galapagos.datasets.expanded_window_datacard import build_datacard_markdown_v3_8, build_quality_markdown_v3_8
...
from galapagos.features.expanded_window import output_path as v3_6_feature_path
from galapagos.features.expanded_window_validation import validate_expanded_causal_feature_store_v3_6
from galapagos.labels.expanded_window import output_path as v3_7_label_path
from galapagos.labels.expanded_window_validation import validate_expanded_label_factory_v3_7


def run_expanded_offline_supervised_dataset_v3_8(
    root: Path = Path("."),
    *,
    ...
    dataset_run_id = f"v3_8_{datetime.now(UTC).strftime('%Y%m%dT%H%M%SZ')}_{uuid.uuid4().hex[:8]}"
    ...
    for timeframe in TIMEFRAMES_V3_8:
        ...
        dataset = build_expanded_offline_supervised_dataset_v3_8(
            feature_frame,
            label_frame,
        ...
        split_frame = build_split_frame_v3_8(dataset)
        ...
        quality[timeframe] = assess_expanded_dataset_quality(
            dataset,
    ...
    manifest = {
        ...
        "outputs": outputs,
        "splits": splits,
        "dataset_schema_version": DATASET_SCHEMA_VERSION_V3_8,
        "dataset_columns": DATASET_COLUMNS_V3_8,
        "split_policy": SPLIT_POLICY_V3_8,
        "quality": quality,
        "safety": _safety(),
        "limitations": EXPECTED_LIMITATIONS_V3_8,
    }
    report = _build_report(manifest)
    _write_json(root / MANIFEST_PATH_V3_8, manifest)
    _write_json(root / REPORT_JSON_PATH_V3_8, report)
    quality_markdown = build_quality_markdown_v3_8(manifest)
    datacard = build_datacard_markdown_v3_8(manifest)
    _write_text(root / REPORT_MD_PATH_V3_8, quality_markdown)
    _write_text(root / DATACARD_MD_PATH_V3_8, datacard)
    _write_text(root / DOC_PATH_V3_8, quality_markdown)
    return manifest


def build_expanded_offline_supervised_dataset_v3_8(
    ...
    *,
    ...
    merged["dataset_schema_version"] = DATASET_SCHEMA_VERSION_V3_8
    merged["source_features_sha256"] = feature_sha256
    merged["source_labels_sha256"] = label_sha256
    merged = assign_temporal_splits_v3_8(merged)
    merged["dataset_error_count"] = 0
    null_columns = [column for column in DATASET_COLUMNS_V3_8 if column != "dataset_null_count"]
    merged["dataset_null_count"] = merged[null_columns].isna().sum(axis=1).astype(int)
    return merged[DATASET_COLUMNS_V3_8].copy()


def assign_temporal_splits_v3_8(frame: pd.DataFrame) -> pd.DataFrame:
    ordered = frame.sort_values("event_ts").reset_index(drop=True).copy()
    rows = len(ordered)
    train_end = int(rows * SPLIT_POLICY_V3_8["train_ratio"])
    validation_end = train_end + int(rows * SPLIT_POLICY_V3_8["validation_ratio"])
    ordered["split"] = "test"
    ordered.loc[: train_end - 1, "split"] = "train"
    ordered.loc[train_end : validation_end - 1, "split"] = "validation"
    ordered["split_order"] = range(rows)
    ordered["purge_embargo_group"] = SPLIT_POLICY_V3_8["purge_embargo"]
    return ordered


def build_split_frame_v3_8(dataset: pd.DataFrame) -> pd.DataFrame:
    _require_columns(dataset, SPLIT_COLUMNS_V3_8, "dataset")
    return dataset[SPLIT_COLUMNS_V3_8].copy()


def input_feature_path(root: Path, timeframe: str) -> Path:
    return v3_6_feature_path(root, timeframe)


def input_label_path(root: Path, timeframe: str) -> Path:
    return v3_7_label_path(root, timeframe)


def _validate_inputs(root: Path) -> None:
    validators = [
        ("V3.6 expanded features", validate_expanded_causal_feature_store_v3_6),
        ("V3.7 expanded labels", validate_expanded_label_factory_v3_7),
    ]
    for label, validator in validators:
        result = validator(root)
        if not result["passed"]:
            raise RuntimeError(f"{label} validation failed before V3.8: {result['errors']}")
