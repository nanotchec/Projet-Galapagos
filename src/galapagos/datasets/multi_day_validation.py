...
DATASET_RUN_ID_PATTERN = re.compile(r"^v3_2_[0-9]{8}T[0-9]{6}Z_[0-9a-f]{8}$")
TIMEFRAME_KEYS = set(TIMEFRAMES_V3_2)
...
def validate_multi_day_offline_supervised_dataset_v3_2(project_root: Path = Path(".")) -> dict[str, Any]:
    project_root = project_root.resolve()
    errors: list[str] = []
    warnings: list[str] = []

    manifest_path = project_root / MANIFEST_PATH_V3_2
    report_path = project_root / REPORT_JSON_PATH_V3_2
    if not manifest_path.exists():
        return _result([f"missing V3.2 manifest: {MANIFEST_PATH_V3_2}"], warnings)
    if not report_path.exists():
        return _result([f"missing V3.2 quality report: {REPORT_JSON_PATH_V3_2}"], warnings)

    manifest = _load_json(manifest_path)
    report = _load_json(report_path)
    errors.extend(_validate_manifest_structure(project_root, manifest))
    errors.extend(scan_payload_for_forbidden_claims(manifest, "V3.2 manifest"))
    errors.extend(_validate_report(manifest, report))
    errors.extend(scan_payload_for_forbidden_claims(report, "V3.2 quality report"))
    errors.extend(_validate_markdown(project_root))
    errors.extend(_find_forbidden_v3_2_artifacts(project_root))
...
def _validate_timeframe(
    ...
    dataset_path = get_dataset_v3_2_path(project_root, timeframe)
    split_path = get_split_v3_2_path(project_root, timeframe)
    for label, path in [("features", feature_path), ("labels", label_path), ("dataset", dataset_path), ("splits", split_path)]:
        if not path.exists():
            errors.append(f"missing V3.2 {label} file for {timeframe}: {path.relative_to(project_root)}")
    if errors:
        return errors
    ...
    errors.extend(_compare_io_block(manifest["input_features"][timeframe], feature_path, feature_sha, len(features), project_root, f"V3.2 manifest input_features.{timeframe}", include_bytes=False))
    errors.extend(_compare_io_block(manifest["input_labels"][timeframe], label_path, label_sha, len(labels), project_root, f"V3.2 manifest input_labels.{timeframe}", include_bytes=False))
    errors.extend(_compare_io_block(manifest["outputs"][timeframe], dataset_path, dataset_sha, len(dataset), project_root, f"V3.2 manifest outputs.{timeframe}", include_bytes=True))
    errors.extend(_compare_io_block(manifest["splits"][timeframe], split_path, split_sha, len(splits), project_root, f"V3.2 manifest splits.{timeframe}", include_bytes=True))
    errors.extend(_validate_dataset_schema(dataset, timeframe))
    if list(splits.columns) != SPLIT_COLUMNS_V3_2:
        errors.append(f"V3.2 split schema mismatch for {timeframe}")
    if len(dataset) != len(features) or len(dataset) != len(labels):
        errors.append(f"V3.2 dataset row count does not match sources for {timeframe}")
    ...
    expected_splits = build_split_frame_v3_2(dataset)
    try:
        ...
        errors.append(f"V3.2 split physical mismatch for {timeframe}: {str(exc).splitlines()[0]}")
    ...
    try:
        ...
        errors.append(f"V3.2 dataset feature source mismatch for {timeframe}: {str(exc).splitlines()[0]}")
    try:
        ...
        errors.append(f"V3.2 dataset label source mismatch for {timeframe}: {str(exc).splitlines()[0]}")

    quality = assess_multi_day_dataset_quality(
        dataset,
...
def _validate_dataset_schema(frame: pd.DataFrame, timeframe: str = "") -> list[str]:
    ...
    if list(frame.columns) != DATASET_COLUMNS_V3_2:
        errors.append(f"V3.2 dataset schema mismatch{label}")
    forbidden = [
        column
    ...
    if forbidden:
        errors.append(f"V3.2 dataset forbidden columns{label}: {forbidden}")
    return errors
...
def _validate_dataset_temporal_rules(frame: pd.DataFrame, timeframe: str = "") -> list[str]:
    ...
    quality = assess_multi_day_dataset_quality(frame, expected_rows=len(frame), timeframe=timeframe or "dataset", feature_sha256=feature_sha, label_sha256=label_sha)
    return [
...
def _validate_dataset_values_against_sources(
    ...
    errors: list[str] = []
    try:
        expected = build_multi_day_offline_supervised_dataset_v3_2(
            feature_frame,
            label_frame,
        ...
        errors.append(f"V3.2 dataset physical mismatch for {timeframe}: {str(exc).splitlines()[0]}")
    except Exception as exc:
        errors.append(f"V3.2 dataset value validation failed for {timeframe}: {exc}")
    return errors


def _validate_manifest_structure(project_root: Path, manifest: dict[str, Any]) -> list[str]:
    errors: list[str] = []
    errors.extend(validate_exact_keys(manifest, MANIFEST_KEYS, "V3.2 manifest"))
    if manifest.get("version") != VERSION_V3_2:
        errors.append("V3.2 manifest version mismatch")
    if manifest.get("status") != "PASS":
        errors.append("V3.2 manifest status must be PASS")
    if not _is_iso_utc(manifest.get("created_at_utc")):
        errors.append("V3.2 manifest created_at_utc invalid")
    if not isinstance(manifest.get("dataset_run_id"), str) or DATASET_RUN_ID_PATTERN.fullmatch(manifest["dataset_run_id"]) is None:
        errors.append("V3.2 manifest dataset_run_id invalid")
    if manifest.get("dataset_schema_version") != DATASET_SCHEMA_VERSION_V3_2:
        errors.append("V3.2 manifest dataset_schema_version mismatch")
    if manifest.get("dataset_columns") != DATASET_COLUMNS_V3_2:
        errors.append("V3.2 manifest dataset_columns mismatch")
    if manifest.get("split_policy") != SPLIT_POLICY_V3_2:
        errors.append("V3.2 manifest split_policy mismatch")
    if manifest.get("limitations") != EXPECTED_LIMITATIONS_V3_2:
        errors.append("V3.2 manifest limitations mismatch")
    for section in ["input_features", "input_labels", "outputs", "splits", "quality"]:
        errors.extend(validate_exact_keys(manifest.get(section, {}), TIMEFRAME_KEYS, f"V3.2 manifest {section}"))
    errors.extend(validate_exact_keys(manifest.get("split_policy", {}), SPLIT_POLICY_KEYS, "V3.2 manifest split_policy"))
    errors.extend(validate_exact_keys(manifest.get("safety", {}), SAFETY_KEYS, "V3.2 manifest safety"))
    for timeframe in TIMEFRAMES_V3_2:
        errors.extend(validate_exact_keys(manifest.get("input_features", {}).get(timeframe, {}), INPUT_KEYS, f"V3.2 manifest input_features.{timeframe}"))
        errors.extend(validate_exact_keys(manifest.get("input_labels", {}).get(timeframe, {}), INPUT_KEYS, f"V3.2 manifest input_labels.{timeframe}"))
        errors.extend(validate_exact_keys(manifest.get("outputs", {}).get(timeframe, {}), OUTPUT_KEYS, f"V3.2 manifest outputs.{timeframe}"))
        errors.extend(validate_exact_keys(manifest.get("splits", {}).get(timeframe, {}), OUTPUT_KEYS, f"V3.2 manifest splits.{timeframe}"))
        quality = manifest.get("quality", {}).get(timeframe, {})
        errors.extend(validate_exact_keys(quality, QUALITY_KEYS, f"V3.2 manifest quality.{timeframe}"))
        if isinstance(quality, dict):
            errors.extend(validate_exact_keys(quality.get("split_counts", {}), SPLIT_COUNT_KEYS, f"V3.2 manifest quality.{timeframe}.split_counts"))
            errors.extend(validate_exact_keys(quality.get("label_valid_counts_by_horizon", {}), HORIZON_KEYS, f"V3.2 manifest quality.{timeframe}.label_valid_counts_by_horizon"))
            errors.extend(validate_exact_keys(quality.get("null_counts_by_column", {}), set(DATASET_COLUMNS_V3_2), f"V3.2 manifest quality.{timeframe}.null_counts_by_column"))
    return errors


def _validate_report(manifest: dict[str, Any], report: dict[str, Any]) -> list[str]:
    errors = validate_exact_keys(report, MANIFEST_KEYS, "V3.2 quality report")
    if report != manifest:
        for path in _compare_nested(manifest, report, "V3.2 quality report"):
            errors.append(f"V3.2 quality report mismatch for {path}")
    return errors
...
def _compare_quality(manifest: dict[str, Any], physical_quality: dict[str, dict[str, Any]]) -> list[str]:
    ...
    for timeframe, quality in physical_quality.items():
        for path in _compare_nested(quality, manifest["quality"].get(timeframe), timeframe):
            errors.append(f"V3.2 manifest quality mismatch for {path}")
    return errors
...
def _validate_safety(safety: Any) -> list[str]:
    ...
    if not isinstance(safety, dict):
        return ["V3.2 manifest safety must be an object"]
    return [f"V3.2 safety flag {key} must be {value}" for key, value in expected.items() if safety.get(key) is not value]


def _validate_markdown(project_root: Path) -> list[str]:
    errors: list[str] = []
    for relative, label in [
        (REPORT_MD_PATH_V3_2, "V3.2 Markdown report"),
        (DATACARD_MD_PATH_V3_2, "V3.2 data card"),
    ]:
...
def _find_forbidden_v3_2_artifacts(project_root: Path) -> list[str]:
    forbidden_roots = [
        Path("models"),
        Path("checkpoints"),
        ...
        Path("orders"),
        Path("execution"),
        Path("data/research/v3_2/ml"),
        Path("data/research/v3_2/backtests"),
        Path("data/research/v3_2/strategies"),
    ]
    errors: list[str] = []
    ...
    for suffix in [".pkl", ".pickle", ".joblib", ".ckpt", ".pt", ".pth", ".onnx"]:
        ...
        for path in matches:
            errors.append(f"Forbidden V3.2 model artifact detected: {path.relative_to(project_root).as_posix()}")
    return errors
