...
def build_quality_markdown_v7_3(manifest: dict[str, Any]) -> str:
    inputs = "\n".join(
        "\n".join(
            [
                f"- `{timeframe}` features OHLCV+trades V7.2 : `{manifest['input_features'][timeframe]['path']}` ({manifest['input_features'][timeframe]['rows']} lignes)",
    ...
    return f"""# Rapport qualite - V7.3 Dataset supervise offline OHLCV + trades
...
- Nombre de colonnes dataset : `{len(DATASET_COLUMNS_V7_3)}`
...
{_non_usage_markdown('V7.3')}
"""


def build_datacard_markdown_v7_3(manifest: dict[str, Any]) -> str:
    limitations = "\n".join(f"- {item}" for item in manifest["limitations"])
    return f"""# Data Card - Galapagos V7.3 Dataset supervise offline OHLCV + trades
