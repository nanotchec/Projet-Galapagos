...
VERSION = "V9.50"
SOURCE_VERSION = "V9.49"
LAST_VALIDATED_VERSION = "V9.49"
DIRECTION = "ohlcv_aggtrades_exact_5y_dataset_validation"
REPORT_JSON_PATH = Path("reports/datasets/ohlcv_aggtrades_exact_5y_dataset_validation_v9_50.json")
REPORT_MD_PATH = Path("reports/datasets/ohlcv_aggtrades_exact_5y_dataset_validation_v9_50.md")
SAMPLES_JSON_PATH = Path("reports/datasets/ohlcv_aggtrades_exact_5y_dataset_validation_v9_50_samples.json")
MANIFEST_PATH = Path("reports/manifests/ohlcv_aggtrades_exact_5y_dataset_validation_v9_50_manifest.json")
DOC_PATH = Path("docs/ohlcv_aggtrades_exact_5y_dataset_validation_v9_50.md")
SAMPLE_BASE_PATH = Path("data/audit_samples/v9_50/ohlcv_aggtrades_exact_5y_dataset")
EXPECTED_SPLITS = {"train", "validation", "test"}
ALLOWED_DECISIONS = {
    "combined_features_5y_dataset_validated",
    "combined_features_5y_dataset_validated_with_warnings",
    "combined_features_5y_dataset_blocked_by_coverage",
    "combined_features_5y_dataset_blocked_by_schema",
    "combined_features_5y_dataset_blocked_by_quality",
...
def run_ohlcv_aggtrades_exact_5y_dataset_validation_v9_50(root: Path = Path("."), mode: str = "full-local") -> dict[str, Any]:
    root = root.resolve()
    started = time.monotonic()
    inputs = load_inputs_v9_50(root)
    readiness = assess_v9_49_readiness_v9_50(inputs)
    ...
    if mode not in {"full-local", "audit-lite"}:
        raise ValueError(f"unsupported V9.50 mode: {mode}")
    if not readiness["ready"]:
        errors.extend(readiness["errors"])
    elif mode == "full-local":
        for timeframe in TIMEFRAMES:
            try:
                result = validate_full_timeframe_v9_50(root, timeframe)
                timeframe_results[timeframe] = result
                warnings.extend(result["warnings"])
                sample_inventory[timeframe] = write_sample_v9_50(root, timeframe, result["sample_rows"])
        ...
        sample_inventory = inspect_samples_v9_50(root)
        warnings.append("audit-lite mode: full Parquet datasets are not required and were not revalidated")

    coverage_status = coverage_status_v9_50(timeframe_results, mode, errors)
    schema_status = schema_status_v9_50(timeframe_results, mode, errors)
    leakage_guard = leakage_guard_v9_50(timeframe_results, mode, errors)
    forbidden_scan = forbidden_scan_v9_50(timeframe_results, mode)
    quality_status = quality_status_v9_50(timeframe_results, mode, errors, warnings, leakage_guard, forbidden_scan)
    decision = decide_v9_50(coverage_status, schema_status, quality_status, leakage_guard, forbidden_scan, warnings, errors)
    report = {
        "version": VERSION,
        "source_version": SOURCE_VERSION,
        "status": "PASS" if decision in {"combined_features_5y_dataset_validated", "combined_features_5y_dataset_validated_with_warnings"} else "FAIL",
        ...
        "row_counts_by_timeframe": collect_metric_v9_50(timeframe_results, "row_count"),
        "valid_row_counts": collect_metric_v9_50(timeframe_results, "valid_row_count"),
        "invalid_row_counts": collect_metric_v9_50(timeframe_results, "invalid_row_count"),
        "invalid_reason_summary": collect_metric_v9_50(timeframe_results, "invalid_reason_summary"),
        "null_summary": collect_metric_v9_50(timeframe_results, "null_summary"),
        "feature_null_summary": collect_metric_v9_50(timeframe_results, "feature_null_summary"),
        "label_null_summary": collect_metric_v9_50(timeframe_results, "label_null_summary"),
        "split_distribution": collect_metric_v9_50(timeframe_results, "split_distribution"),
        "target_distribution": collect_metric_v9_50(timeframe_results, "target_distribution"),
        "target_distribution_by_split": collect_metric_v9_50(timeframe_results, "target_distribution_by_split"),
        "target_distribution_by_year": collect_metric_v9_50(timeframe_results, "target_distribution_by_year"),
        "target_distribution_by_month": collect_metric_v9_50(timeframe_results, "target_distribution_by_month"),
        "majority_class_ratio": collect_metric_v9_50(timeframe_results, "majority_class_ratio"),
        "flat_ratio": collect_metric_v9_50(timeframe_results, "flat_ratio"),
        "entropy": collect_metric_v9_50(timeframe_results, "entropy"),
        ...
        "errors": errors,
        "decision": decision,
        "next_recommendation": next_recommendation_v9_50(decision),
    ...
    if report["decision"] not in ALLOWED_DECISIONS:
        raise RuntimeError(f"invalid V9.50 decision: {report['decision']}")
    _write_json(root / REPORT_JSON_PATH, report)
    _write_text(root / REPORT_MD_PATH, build_markdown_v9_50(report))
    _write_text(root / DOC_PATH, build_markdown_v9_50(report))
    _write_json(root / SAMPLES_JSON_PATH, {"version": VERSION, "samples": sample_inventory})
    _write_json(root / MANIFEST_PATH, build_manifest_v9_50(report))
    update_state_surfaces_v9_50(root, report)
    return report


def validate_full_timeframe_v9_50(root: Path, timeframe: str) -> dict[str, Any]:
    path = dataset_path_v9_50(root, timeframe)
    if not path.is_file():
    ...
    errors.extend(validate_metadata_v9_50(frame, timeframe))
    errors.extend(validate_temporal_v9_50(frame))
    leakage_errors = leakage_errors_v9_50(frame)
    ...
    target_distribution = distribution_stats_v9_50(value_counts_v9_50(frame.loc[frame["row_valid_for_dataset"], SELECTED_PRIMARY_LABEL]))
    ...
    split_stats = target_distribution_by_split_v9_50(frame)
    if distribution_drift_warning_v9_50(split_stats):
        warnings.append(f"{timeframe}: target distribution drift between train/validation/test")
    year_stats = grouped_distribution_v9_50(frame, "%Y")
    if distribution_drift_warning_v9_50(year_stats):
    ...
    return {
        ...
        "source_versions": {
            "source_combined_feature_store_version": sorted(map(str, frame["source_combined_feature_store_version"].dropna().unique().tolist())),
            "source_combined_feature_validation_version": sorted(map(str, frame["source_combined_feature_validation_version"].dropna().unique().tolist())),
            "source_label_version": sorted(map(str, frame["source_label_version"].dropna().unique().tolist())),
        },
        "temporal_status": "PASS" if not validate_temporal_v9_50(frame) else "FAIL",
        ...
        "null_summary": null_summary_v9_50(frame),
        ...
        "target_distribution_by_month": grouped_distribution_v9_50(frame, "%Y-%m"),
        ...
        "errors": errors,
        "warnings": warnings,
        "sample_rows": sample_rows_v9_50(frame),
    }


def validate_metadata_v9_50(frame: pd.DataFrame, timeframe: str) -> list[str]:
    errors: list[str] = []
    expected_values = {
        "timeframe": timeframe,
        "target_name": SELECTED_PRIMARY_LABEL,
        "selected_primary_label": SELECTED_PRIMARY_LABEL,
        "source_combined_feature_store_version": "V9.47",
        "source_combined_feature_validation_version": "V9.48",
...
def validate_temporal_v9_50(frame: pd.DataFrame) -> list[str]:
    errors: list[str] = []
...
def leakage_errors_v9_50(frame: pd.DataFrame) -> dict[str, int]:
...
def value_counts_v9_50(series: pd.Series) -> dict[str, int]:
    return {str(int(key)): int(value) for key, value in series.dropna().value_counts().sort_index().items()}


def distribution_stats_v9_50(counts: dict[str, int]) -> dict[str, Any]:
...
def target_distribution_by_split_v9_50(frame: pd.DataFrame) -> dict[str, Any]:
    valid = frame.loc[frame["row_valid_for_dataset"], ["split", SELECTED_PRIMARY_LABEL]]
    return {str(key): distribution_stats_v9_50(value_counts_v9_50(chunk[SELECTED_PRIMARY_LABEL])) for key, chunk in valid.groupby("split", sort=False)}


def grouped_distribution_v9_50(frame: pd.DataFrame, fmt: str) -> dict[str, Any]:
    valid = frame.loc[frame["row_valid_for_dataset"], ["decision_ts", SELECTED_PRIMARY_LABEL]]
    group = valid["decision_ts"].dt.strftime(fmt)
    return {str(key): distribution_stats_v9_50(value_counts_v9_50(chunk[SELECTED_PRIMARY_LABEL])) for key, chunk in valid.groupby(group, sort=True)}


def distribution_drift_warning_v9_50(stats: dict[str, Any]) -> bool:
    if len(stats) < 2:
        return False
...
def null_summary_v9_50(frame: pd.DataFrame) -> dict[str, int]:
    return {
...
def sample_rows_v9_50(frame: pd.DataFrame) -> pd.DataFrame:
    columns = [
        "source",
        "venue",
        "market_type",
...
def write_sample_v9_50(root: Path, timeframe: str, sample: pd.DataFrame) -> dict[str, Any]:
...
def inspect_samples_v9_50(root: Path) -> dict[str, Any]:
...
def load_inputs_v9_50(root: Path) -> dict[str, dict[str, Any]]:
    paths = {
        "v9_49_report": Path("reports/datasets/ohlcv_aggtrades_exact_5y_dataset_v9_49.json"),
        "v9_49_manifest": Path("reports/manifests/ohlcv_aggtrades_exact_5y_dataset_v9_49_manifest.json"),
    ...
    return {name: load_input_v9_50(root, path) for name, path in paths.items()}


def load_input_v9_50(root: Path, path: Path) -> dict[str, Any]:
    full = root / path
    if not full.exists():
...
def assess_v9_49_readiness_v9_50(inputs: dict[str, dict[str, Any]]) -> dict[str, Any]:
    report = inputs["v9_49_report"].get("payload", {})
    errors: list[str] = []
    if not inputs["v9_49_report"].get("available"):
        errors.append("missing V9.49 dataset report")
    if not inputs["v9_49_manifest"].get("available"):
        errors.append("missing V9.49 manifest")
    if report.get("dataset_created") is not True:
        errors.append("V9.49 dataset_created is not true")
    if report.get("coverage_status") != "target_5y_dataset_window_complete":
        errors.append("V9.49 coverage status is not complete")
    if report.get("quality_status") not in {"PASS", "PASS_WITH_WARNINGS"}:
        errors.append("V9.49 quality status is not PASS/PASS_WITH_WARNINGS")
    if report.get("leakage_guard", {}).get("status") != "PASS":
        errors.append("V9.49 leakage guard is not PASS")
    if report.get("forbidden_column_scan", {}).get("status") != "PASS":
        errors.append("V9.49 forbidden column scan is not PASS")
    if report.get("row_counts") != EXPECTED_ROWS:
        errors.append("V9.49 row counts mismatch")
    return {"ready": not errors, "errors": errors, "v9_49_decision": report.get("decision"), "v9_49_quality_status": report.get("quality_status")}


def coverage_status_v9_50(results: dict[str, Any], mode: str, errors: list[str]) -> str:
    if mode == "audit-lite":
...
def schema_status_v9_50(results: dict[str, Any], mode: str, errors: list[str]) -> str:
    if mode == "audit-lite":
...
def leakage_guard_v9_50(results: dict[str, Any], mode: str, errors: list[str]) -> dict[str, Any]:
    if mode == "audit-lite":
        return {"status": "PASS", "mode": mode, "note": "validated from V9.49 report and optional samples; full Parquets not required"}
...
def forbidden_scan_v9_50(results: dict[str, Any], mode: str) -> dict[str, Any]:
    if mode == "audit-lite":
...
def quality_status_v9_50(results: dict[str, Any], mode: str, errors: list[str], warnings: list[str], leakage_guard: dict[str, Any], forbidden_scan: dict[str, Any]) -> str:
    if errors:
        return "FAIL"
...
def decide_v9_50(coverage: str, schema: str, quality: str, leakage_guard: dict[str, Any], forbidden_scan: dict[str, Any], warnings: list[str], errors: list[str]) -> str:
    if leakage_guard["status"] != "PASS":
        return "combined_features_5y_dataset_blocked_by_leakage"
    if schema == "FAIL" or forbidden_scan["status"] != "PASS":
        return "combined_features_5y_dataset_blocked_by_schema"
    if coverage == "target_5y_dataset_coverage_failed":
        return "combined_features_5y_dataset_blocked_by_coverage"
    if errors or quality.startswith("FAIL"):
        return "combined_features_5y_dataset_blocked_by_quality"
    if warnings or quality == "PASS_WITH_WARNINGS":
        return "combined_features_5y_dataset_validated_with_warnings"
    return "combined_features_5y_dataset_validated"


def next_recommendation_v9_50(decision: str) -> str:
    if decision in {"combined_features_5y_dataset_validated", "combined_features_5y_dataset_validated_with_warnings"}:
        return "V9.51 - Combined Features 5Y ML Offline"
    if decision.endswith("_leakage") or decision.endswith("_schema") or decision.endswith("_quality"):
        return "V9.51 - Dataset Correction"
    return "V9.51 - Manual Dataset Review Pack"


def collect_metric_v9_50(results: dict[str, Any], key: str) -> dict[str, Any]:
    return {timeframe: results.get(timeframe, {}).get(key) for timeframe in TIMEFRAMES}


def dataset_path_v9_50(root: Path, timeframe: str) -> Path:
    return root / DATASET_BASE_PATH / f"timeframe={timeframe}" / f"window={TARGET_WINDOW_START}_{TARGET_WINDOW_END}" / "dataset.parquet"


def build_manifest_v9_50(report: dict[str, Any]) -> dict[str, Any]:
    keys = [
        "version",
        "source_version",
        "status",
...
def build_markdown_v9_50(report: dict[str, Any]) -> str:
    lines = [
        "# V9.50 - OHLCV + AggTrades 5Y Dataset Validation",
        "",
        "## Resume",
        f"- Mode : `{report['validation_mode']}`.",
        f"- Decision V9.50 : `{report['decision']}`.",
...
def update_state_surfaces_v9_50(root: Path, report: dict[str, Any]) -> None:
    metrics = {
        ...
        "source_version": SOURCE_VERSION,
        "direction": DIRECTION,
        "v9_50_decision": report["decision"],
