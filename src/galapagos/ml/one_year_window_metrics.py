...
from galapagos.ml.schemas import TARGET_CLASSES_V4_6


def compute_one_year_classification_metrics_v4_6(scores: pd.DataFrame) -> dict[str, Any]:
    metrics: dict[str, Any] = {}
    split_counts_by_model = {
        (timeframe, model_name): {
            split: int(group[group["split"] == split].shape[0])
    ...
    for (timeframe, model_name, split), group in scores.groupby(["timeframe", "model_name", "split"], sort=True):
        ...
        metrics[key] = {
            ...
            "per_class_precision": _per_class(precision_score(y_true, y_pred, labels=TARGET_CLASSES_V4_6, average=None, zero_division=0)),
            "per_class_recall": _per_class(recall_score(y_true, y_pred, labels=TARGET_CLASSES_V4_6, average=None, zero_division=0)),
            "confusion_matrix": confusion_matrix(y_true, y_pred, labels=TARGET_CLASSES_V4_6).astype(int).tolist(),
...
def _distribution(values: pd.Series) -> dict[str, int]:
    return {label: int(values.eq(label).sum()) for label in TARGET_CLASSES_V4_6}


def _per_class(values: Any) -> dict[str, float]:
    return {label: float(values[index]) for index, label in enumerate(TARGET_CLASSES_V4_6)}
