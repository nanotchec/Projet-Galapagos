...
def assess_strict_walk_forward_quality_v8_7(
    ...
    forbidden_features = find_forbidden_feature_columns_v8_7(ALLOWED_FEATURE_COLUMNS_V8_7)
    forbidden_outputs = [column for column in scores.columns if column.casefold() in {item.casefold() for item in FORBIDDEN_OUTPUT_COLUMNS_EXACT_V8_7}]
    ...
    if folds.empty:
        errors.append("no strict walk-forward folds generated")
    if scores.empty:
        errors.append("no strict walk-forward scores generated")
    return {
        ...
        "fold_temporal_order_valid": fold_temporal_order_valid_v8_7(folds),
...
def find_forbidden_feature_columns_v8_7(columns: list[str]) -> list[str]:
    exact = {item.casefold() for item in FORBIDDEN_FEATURE_EXACT_V8_7}
    prefixes = tuple(item.casefold() for item in FORBIDDEN_FEATURE_PREFIXES_V8_7)
    forbidden: list[str] = []
...
def fold_temporal_order_valid_v8_7(folds: pd.DataFrame) -> bool:
    if folds.empty:
        return False
