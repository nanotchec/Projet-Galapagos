...
from galapagos.ml.max_history_window import input_dataset_path, score_output_path
...
VERSION_V5_5 = "V5.5"
ROBUSTNESS_RUN_ID_PREFIX_V5_5 = "v5_5"
MANIFEST_PATH_V5_5 = Path("reports/manifests/max_history_ml_robustness_v5_5_manifest.json")
REPORT_JSON_PATH_V5_5 = Path("reports/ml/max_history_ml_robustness_v5_5.json")
REPORT_MD_PATH_V5_5 = Path("reports/ml/max_history_ml_robustness_v5_5.md")
DOC_PATH_V5_5 = Path("docs/max_history_ml_robustness_v5_5.md")
ACCURACY_GAP_WARNING_THRESHOLD_V5_5 = 0.10
MACRO_F1_GAP_WARNING_THRESHOLD_V5_5 = 0.10
LABEL_SHUFFLE_RANDOM_SEED_V5_5 = 123
ROBUSTNESS_MODELS_V5_5 = ["logistic_regression", "decision_tree_depth_2"]
ROBUSTNESS_SPLITS_V5_5 = ["train", "validation", "test"]
EVALUATION_SPLITS_V5_5 = ["validation", "test"]
WALK_FORWARD_WEAK_ACCURACY_THRESHOLD_V5_5 = 0.34
WALK_FORWARD_WEAK_MACRO_F1_THRESHOLD_V5_5 = 0.20
FORBIDDEN_ROBUSTNESS_FEATURE_TERMS_V5_5 = [
    "future_",
    "label_",
    "direction_",
    "up_down_flat_",
...
def run_max_history_ml_robustness_v5_5(
    root: Path = Path("."),
    *,
    ...
    dataset_manifest = _read_json(root / MANIFEST_PATH_V5_3)
    ml_manifest = _read_json(root / MANIFEST_PATH_V5_4)
    ml_report = _read_json(root / REPORT_JSON_PATH_V5_4)
    scores_report = _read_json(root / SCORES_JSON_PATH_V5_4)
    ...
    analyses: dict[str, Any] = {
        "baseline_delta": compute_max_history_baseline_delta_v5_5(metrics),
        "split_stability": compute_max_history_split_stability_v5_5(metrics),
        "timeframe_stability": compute_max_history_timeframe_stability_v5_5(metrics),
        "walk_forward_stability": compute_max_history_walk_forward_stability_v5_5(walk_forward_metrics),
        "label_shuffle_falsification": compute_max_history_label_shuffle_falsification_v5_5(root, ml_manifest),
        "feature_leakage_scan": scan_max_history_feature_leakage_v5_5(ml_manifest["feature_columns"]),
    }
    analyses["metric_forbidden_scan"] = scan_max_history_metric_forbidden_terms_v5_5(
        {
    ...
    manifest = {
        ...
        "input_ml_manifest": {
            "path": MANIFEST_PATH_V5_4.as_posix(),
            "sha256": sha256_file(root / MANIFEST_PATH_V5_4),
            "window_start": window["window_start"],
            "window_end": window["window_end"],
            "total_days": int(window["total_days"]),
        },
        ...
        "analyses": analyses,
        "thresholds": {
            "accuracy_gap_warning_threshold": ACCURACY_GAP_WARNING_THRESHOLD_V5_5,
            "macro_f1_gap_warning_threshold": MACRO_F1_GAP_WARNING_THRESHOLD_V5_5,
            "random_seed": LABEL_SHUFFLE_RANDOM_SEED_V5_5,
        },
        "findings": {
        ...
        "safety": SAFETY_FLAGS_V5_5,
        "limitations": EXPECTED_LIMITATIONS_V5_5,
    }
    _write_json(root / MANIFEST_PATH_V5_5, manifest)
    _write_json(root / REPORT_JSON_PATH_V5_5, manifest)
    markdown = build_max_history_ml_robustness_markdown_v5_5(manifest)
    _write_text(root / REPORT_MD_PATH_V5_5, markdown)
    _write_text(root / DOC_PATH_V5_5, markdown)
    _update_project_state(root, manifest)
    return manifest


def compute_max_history_baseline_delta_v5_5(metrics: dict[str, Any]) -> dict[str, Any]:
...
def compute_max_history_split_stability_v5_5(metrics: dict[str, Any]) -> dict[str, Any]:
    stability: dict[str, Any] = {}
    models = sorted({metric["model_name"] for metric in metrics.values()})
    timeframes = sorted({metric["timeframe"] for metric in metrics.values()}, key=TIMEFRAMES_V5_4.index)
    for timeframe in timeframes:
        for model_name in models:
            split_metrics = {split: metrics.get(f"{timeframe}.{model_name}.{split}") for split in ROBUSTNESS_SPLITS_V5_5}
            ...
            overfit_warning = (
                train_validation_accuracy_gap > ACCURACY_GAP_WARNING_THRESHOLD_V5_5
                or train_validation_macro_f1_gap > MACRO_F1_GAP_WARNING_THRESHOLD_V5_5
            )
...
def compute_max_history_timeframe_stability_v5_5(metrics: dict[str, Any]) -> dict[str, Any]:
    stability: dict[str, Any] = {}
    models = sorted({metric["model_name"] for metric in metrics.values()})
    for model_name in models:
        test_metrics = {
        ...
        stability[model_name] = {
            ...
            "single_timeframe_concentration_warning": bool(sorted_accuracy[0][1] - second_accuracy > ACCURACY_GAP_WARNING_THRESHOLD_V5_5),
        }
    return stability


def compute_max_history_walk_forward_stability_v5_5(walk_forward_metrics: dict[str, Any]) -> dict[str, Any]:
    ...
    for (timeframe, model_name), records in sorted(grouped.items()):
        ...
        weak_groups = [
            ...
            if accuracy_by_group[group] < WALK_FORWARD_WEAK_ACCURACY_THRESHOLD_V5_5
            or macro_f1_by_group[group] < WALK_FORWARD_WEAK_MACRO_F1_THRESHOLD_V5_5
        ]
        unstable_groups = [
            group
            for group in accuracy_by_group
            if abs(accuracy_by_group[group] - mean_accuracy) > ACCURACY_GAP_WARNING_THRESHOLD_V5_5
            or abs(macro_f1_by_group[group] - mean_macro_f1) > MACRO_F1_GAP_WARNING_THRESHOLD_V5_5
        ]
        stability[f"{timeframe}.{model_name}"] = {
            "timeframe": timeframe,
            "model_name": model_name,
...
def compute_max_history_label_shuffle_falsification_v5_5(root: Path, ml_manifest: dict[str, Any]) -> dict[str, Any]:
    root = root.resolve()
    ...
    for timeframe in TIMEFRAMES_V5_4:
        rng = np.random.default_rng(LABEL_SHUFFLE_RANDOM_SEED_V5_5)
        ...
        slices = {split: ml_frame[ml_frame["split"] == split].copy() for split in ROBUSTNESS_SPLITS_V5_5}
        train = slices["train"]
        shuffled_train_target = pd.Series(rng.permutation(train[TARGET_NAME_V5_4].astype(str).to_numpy()), index=train.index)
        predict_frame = pd.concat([slices[split] for split in EVALUATION_SPLITS_V5_5], axis=0)
        for model_name in ROBUSTNESS_MODELS_V5_5:
            result = fit_predict_model(
                model_name,
                train[ALLOWED_FEATURE_COLUMNS_V5_4],
                shuffled_train_target,
                predict_frame[ALLOWED_FEATURE_COLUMNS_V5_4],
            )
            for split in EVALUATION_SPLITS_V5_5:
                split_frame = slices[split]
                y_true = split_frame[TARGET_NAME_V5_4].astype(str)
...
def scan_max_history_feature_leakage_v5_5(feature_columns: list[str]) -> dict[str, Any]:
    forbidden = [
        column
        for column in feature_columns
        if any(term in column.casefold() for term in FORBIDDEN_ROBUSTNESS_FEATURE_TERMS_V5_5)
    ]
    return {
...
def scan_max_history_metric_forbidden_terms_v5_5(payload: Any) -> dict[str, Any]:
    text = json.dumps(payload, ensure_ascii=False, sort_keys=True).casefold()
    present = [term for term in FORBIDDEN_ROBUSTNESS_METRIC_TERMS_V5_5 if term in text]
    return {
        "forbidden_terms": FORBIDDEN_ROBUSTNESS_METRIC_TERMS_V5_5,
        "forbidden_terms_present": present,
        "metric_forbidden_terms_detected": bool(present),
    }


def build_max_history_ml_robustness_markdown_v5_5(manifest: dict[str, Any]) -> str:
...
def _validate_inputs(root: Path) -> None:
    dataset_result = validate_max_history_offline_supervised_dataset_v5_3(root)
    if not dataset_result["passed"]:
        raise RuntimeError(f"V5.3 dataset validation failed before V5.5: {dataset_result['errors']}")
    ml_result = validate_max_history_offline_ml_research_v5_4(root)
    if not ml_result["passed"]:
        raise RuntimeError(f"V5.4 ML validation failed before V5.5: {ml_result['errors']}")
...
def _classification_summary(y_true: pd.Series, y_pred: pd.Series) -> dict[str, float]:
    return {
        ...
        "macro_f1": float(f1_score(y_true, y_pred, labels=TARGET_CLASSES_V5_4, average="macro", zero_division=0)),
    }
...
def _robustness_run_id() -> str:
    return f"{ROBUSTNESS_RUN_ID_PREFIX_V5_5}_{datetime.now(UTC).strftime('%Y%m%dT%H%M%SZ')}_{uuid.uuid4().hex[:8]}"
...
def _update_project_state(root: Path, manifest: dict[str, Any]) -> None:
    ...
    state.update(
        {
            "last_validated_version": "V5.4.1",
            "candidate_version": "V5.5",
            "candidate_status": "pending_external_audit",
            "direction": "max historical ML robustness and walk-forward falsification audit",
            "max_history_robustness_window_start_v5_5": manifest["input_ml_manifest"]["window_start"],
            "max_history_robustness_window_end_v5_5": manifest["input_ml_manifest"]["window_end"],
            "max_history_robustness_days_v5_5": manifest["input_ml_manifest"]["total_days"],
            "backtest_v5_5_created": False,
            "strategy_v5_5_created": False,
            "signal_v5_5_created": False,
            "orders_v5_5_created": False,
            "paper_live_v5_5_created": False,
            "trading_v5_5_created": False,
            "persistent_model_v5_5_created": False,
            "backtest_enabled": False,
            "strategy_enabled": False,
            "paper_live_enabled": False,
            "orders_enabled": False,
...
def _project_state_markdown(manifest: dict[str, Any]) -> str:
    return f"""# Etat du Projet : V5.4.1 validee + candidat V5.5
