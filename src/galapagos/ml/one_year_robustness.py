...
from galapagos.datasets.schemas import MANIFEST_PATH_V4_5, get_dataset_v4_5_path
...
VERSION_V4_7 = "V4.7"
ROBUSTNESS_RUN_ID_PREFIX_V4_7 = "v4_7"
MANIFEST_PATH_V4_7 = Path("reports/manifests/one_year_ml_robustness_v4_7_manifest.json")
REPORT_JSON_PATH_V4_7 = Path("reports/ml/one_year_ml_robustness_v4_7.json")
REPORT_MD_PATH_V4_7 = Path("reports/ml/one_year_ml_robustness_v4_7.md")
DOC_PATH_V4_7 = Path("docs/one_year_ml_robustness_v4_7.md")
ACCURACY_GAP_WARNING_THRESHOLD_V4_7 = 0.10
MACRO_F1_GAP_WARNING_THRESHOLD_V4_7 = 0.10
LABEL_SHUFFLE_RANDOM_SEED_V4_7 = 123
ROBUSTNESS_MODELS_V4_7 = ["logistic_regression", "decision_tree_depth_2"]
ROBUSTNESS_SPLITS_V4_7 = ["train", "validation", "test"]
EVALUATION_SPLITS_V4_7 = ["validation", "test"]
FORBIDDEN_ROBUSTNESS_FEATURE_TERMS_V4_7 = [
    "future_",
    "label_",
    "direction_",
    "up_down_flat_",
...
def run_one_year_ml_robustness_v4_7(
    root: Path = Path("."),
    *,
    ...
    ml_manifest = _read_json(root / MANIFEST_PATH_V4_6)
    ml_report = _read_json(root / REPORT_JSON_PATH_V4_6)
    scores_report = _read_json(root / SCORES_JSON_PATH_V4_6)
    scores_by_timeframe = {timeframe: read_parquet(get_one_year_ml_score_path_v4_6(root, timeframe)) for timeframe in TIMEFRAMES_V4_6}
    metrics = ml_manifest["metrics"]

    analyses = {
        "baseline_delta": compute_one_year_baseline_delta_v4_7(metrics),
        "split_stability": compute_one_year_split_stability_v4_7(metrics),
        "timeframe_stability": compute_one_year_timeframe_stability_v4_7(metrics),
        "label_shuffle_falsification": compute_one_year_label_shuffle_falsification_v4_7(root, metrics),
        "feature_leakage_scan": scan_one_year_feature_leakage_v4_7(ml_manifest["feature_columns"]),
        "metric_forbidden_scan": scan_one_year_metric_forbidden_terms_v4_7(
            {
    ...
    manifest = {
        "version": VERSION_V4_7,
        "status": status,
        "created_at_utc": utc_now_iso(),
        "robustness_run_id": _robustness_run_id(),
        "input_dataset_manifest": _path_sha_block(root, MANIFEST_PATH_V4_5),
        "input_ml_manifest": _path_sha_block(root, MANIFEST_PATH_V4_6),
        "input_score_files": _input_score_files(root, scores_by_timeframe),
        "analyses": analyses,
        "thresholds": {
            "accuracy_gap_warning_threshold": ACCURACY_GAP_WARNING_THRESHOLD_V4_7,
            "macro_f1_gap_warning_threshold": MACRO_F1_GAP_WARNING_THRESHOLD_V4_7,
            "random_seed": LABEL_SHUFFLE_RANDOM_SEED_V4_7,
        },
        "findings": {
        ...
        "safety": SAFETY_FLAGS_V4_7,
        "limitations": EXPECTED_LIMITATIONS_V4_7,
    }
    _write_json(root / MANIFEST_PATH_V4_7, manifest)
    _write_json(root / REPORT_JSON_PATH_V4_7, manifest)
    markdown = build_one_year_ml_robustness_markdown_v4_7(manifest)
    _write_text(root / REPORT_MD_PATH_V4_7, markdown)
    _write_text(root / DOC_PATH_V4_7, markdown)
    return manifest


def compute_one_year_baseline_delta_v4_7(metrics: dict[str, Any]) -> dict[str, Any]:
...
def compute_one_year_split_stability_v4_7(metrics: dict[str, Any]) -> dict[str, Any]:
    stability: dict[str, Any] = {}
    models = sorted({metric["model_name"] for metric in metrics.values()})
    timeframes = sorted({metric["timeframe"] for metric in metrics.values()}, key=TIMEFRAMES_V4_6.index)
    for timeframe in timeframes:
        for model_name in models:
            split_metrics = {split: metrics.get(f"{timeframe}.{model_name}.{split}") for split in ROBUSTNESS_SPLITS_V4_7}
            ...
            overfit_warning = (
                train_validation_accuracy_gap > ACCURACY_GAP_WARNING_THRESHOLD_V4_7
                or train_validation_macro_f1_gap > MACRO_F1_GAP_WARNING_THRESHOLD_V4_7
            )
...
def compute_one_year_timeframe_stability_v4_7(metrics: dict[str, Any]) -> dict[str, Any]:
    stability: dict[str, Any] = {}
    models = sorted({metric["model_name"] for metric in metrics.values()})
    for model_name in models:
        test_metrics = {
        ...
        stability[model_name] = {
            ...
            "single_timeframe_concentration_warning": bool(sorted_accuracy[0][1] - second_accuracy > ACCURACY_GAP_WARNING_THRESHOLD_V4_7),
        }
    return stability


def compute_one_year_label_shuffle_falsification_v4_7(root: Path, original_metrics: dict[str, Any]) -> dict[str, Any]:
    root = root.resolve()
    falsification: dict[str, Any] = {}
    for timeframe in TIMEFRAMES_V4_6:
        rng = np.random.default_rng(LABEL_SHUFFLE_RANDOM_SEED_V4_7)
        dataset = read_parquet(get_dataset_v4_5_path(root, timeframe))
        ml_frame = dataset[(dataset["label_valid_h1"] == True) & (dataset["warmup_row"] == False)].copy()  # noqa: E712
        slices = {split: ml_frame[ml_frame["split"] == split].copy() for split in ROBUSTNESS_SPLITS_V4_7}
        train = slices["train"]
        shuffled_train_target = pd.Series(rng.permutation(train[TARGET_NAME_V4_6].astype(str).to_numpy()), index=train.index)
        predict_frame = pd.concat([slices[split] for split in EVALUATION_SPLITS_V4_7], axis=0)
        for model_name in ROBUSTNESS_MODELS_V4_7:
            result = fit_predict_model(
                model_name,
                train[ALLOWED_FEATURE_COLUMNS_V4_6],
                shuffled_train_target,
                predict_frame[ALLOWED_FEATURE_COLUMNS_V4_6],
            )
            for split in EVALUATION_SPLITS_V4_7:
                split_frame = slices[split]
                y_true = split_frame[TARGET_NAME_V4_6].astype(str)
...
def scan_one_year_feature_leakage_v4_7(feature_columns: list[str]) -> dict[str, Any]:
    forbidden = [
        column
        for column in feature_columns
        if any(term in column.casefold() for term in FORBIDDEN_ROBUSTNESS_FEATURE_TERMS_V4_7)
    ]
    return {
...
def scan_one_year_metric_forbidden_terms_v4_7(payload: Any) -> dict[str, Any]:
    text = json.dumps(payload, ensure_ascii=False, sort_keys=True).casefold()
    present = [term for term in FORBIDDEN_ROBUSTNESS_METRIC_TERMS_V4_7 if term in text]
    return {
        "forbidden_terms": FORBIDDEN_ROBUSTNESS_METRIC_TERMS_V4_7,
        "forbidden_terms_present": present,
        "metric_forbidden_terms_detected": bool(present),
    }


def build_one_year_ml_robustness_markdown_v4_7(manifest: dict[str, Any]) -> str:
...
def _validate_inputs(root: Path) -> None:
    dataset_result = validate_one_year_offline_supervised_dataset_v4_5(root)
    if not dataset_result["passed"]:
        raise RuntimeError(f"V4.5 dataset validation failed before V4.7: {dataset_result['errors']}")
    ml_result = validate_one_year_offline_ml_research_v4_6(root)
    if not ml_result["passed"]:
        raise RuntimeError(f"V4.6 ML validation failed before V4.7: {ml_result['errors']}")
...
def _input_score_files(root: Path, scores_by_timeframe: dict[str, pd.DataFrame]) -> dict[str, dict[str, Any]]:
    ...
    for timeframe, scores in scores_by_timeframe.items():
        path = get_one_year_ml_score_path_v4_6(root, timeframe)
        blocks[timeframe] = {
...
def _classification_summary(y_true: pd.Series, y_pred: pd.Series) -> dict[str, float]:
    return {
        ...
        "macro_f1": float(f1_score(y_true, y_pred, labels=TARGET_CLASSES_V4_6, average="macro", zero_division=0)),
    }
...
def _robustness_run_id() -> str:
    return f"{ROBUSTNESS_RUN_ID_PREFIX_V4_7}_{datetime.now(UTC).strftime('%Y%m%dT%H%M%SZ')}_{uuid.uuid4().hex[:8]}"
