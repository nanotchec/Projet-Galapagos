...
from galapagos.ml.ohlcv_trades_window import input_dataset_path, score_output_path
from galapagos.ml.ohlcv_trades_window_validation import validate_ohlcv_trades_offline_ml_research_v7_4
...
VERSION_V7_5 = "V7.5"
ROBUSTNESS_RUN_ID_PREFIX_V7_5 = "v7_5"
MANIFEST_PATH_V7_5 = Path("reports/manifests/ohlcv_trades_ml_robustness_v7_5_manifest.json")
REPORT_JSON_PATH_V7_5 = Path("reports/ml/ohlcv_trades_ml_robustness_v7_5.json")
REPORT_MD_PATH_V7_5 = Path("reports/ml/ohlcv_trades_ml_robustness_v7_5.md")
DOC_PATH_V7_5 = Path("docs/ohlcv_trades_ml_robustness_v7_5.md")
ACCURACY_GAP_WARNING_THRESHOLD_V7_5 = 0.10
MACRO_F1_GAP_WARNING_THRESHOLD_V7_5 = 0.10
LABEL_SHUFFLE_RANDOM_SEED_V7_5 = 123
ROBUSTNESS_MODELS_V7_5 = ["logistic_regression", "decision_tree_depth_2"]
ROBUSTNESS_SPLITS_V7_5 = ["train", "validation", "test"]
EVALUATION_SPLITS_V7_5 = ["validation", "test"]
WALK_FORWARD_WEAK_ACCURACY_THRESHOLD_V7_5 = 0.34
WALK_FORWARD_WEAK_MACRO_F1_THRESHOLD_V7_5 = 0.20
FORBIDDEN_ROBUSTNESS_FEATURE_PREFIXES_V7_5 = [
    "future_",
    "label_",
    "direction_",
    "up_down_flat_",
]
FORBIDDEN_ROBUSTNESS_FEATURE_EXACT_V7_5 = [
    "target",
    "split",
    "walk_forward_group",
    "signal",
...
FORBIDDEN_ROBUSTNESS_METRIC_TERMS_V7_5 = [
    "sharpe",
    "drawdown",
    "pnl",
    "equity_curve",
...
def run_ohlcv_trades_ml_robustness_v7_5(
    root: Path = Path("."),
    *,
    ...
    dataset_manifest = _read_json(root / MANIFEST_PATH_V7_3)
    ml_manifest = _read_json(root / MANIFEST_PATH_V7_4)
    ml_report = _read_json(root / REPORT_JSON_PATH_V7_4)
    scores_report = _read_json(root / SCORES_JSON_PATH_V7_4)
    ...
    reference_reports = build_reference_reports_v7_5(root)

    analyses: dict[str, Any] = {
        "baseline_delta": compute_ohlcv_trades_baseline_delta_v7_5(metrics),
        "split_stability": compute_ohlcv_trades_split_stability_v7_5(metrics),
        "timeframe_stability": compute_ohlcv_trades_timeframe_stability_v7_5(metrics),
        "walk_forward_stability": compute_ohlcv_trades_walk_forward_stability_v7_5(walk_forward_metrics),
        "ohlcv_trades_vs_references_comparison": compute_ohlcv_trades_vs_references_comparison_v7_5(root, metrics, walk_forward_metrics),
        "label_shuffle_falsification": compute_ohlcv_trades_label_shuffle_falsification_v7_5(root, ml_manifest),
        "feature_leakage_scan": scan_ohlcv_trades_feature_leakage_v7_5(ml_manifest["feature_columns"]),
    }
    analyses["metric_forbidden_scan"] = scan_ohlcv_trades_metric_forbidden_terms_v7_5(
        {
    ...
    manifest = {
        "version": VERSION_V7_5,
        "status": status,
        "created_at_utc": utc_now_iso(),
        "robustness_run_id": _robustness_run_id(),
        "input_dataset_manifest": {
            "path": MANIFEST_PATH_V7_3.as_posix(),
            "sha256": sha256_file(root / MANIFEST_PATH_V7_3),
        ...
        "input_ml_manifest": {
            "path": MANIFEST_PATH_V7_4.as_posix(),
            "sha256": sha256_file(root / MANIFEST_PATH_V7_4),
            "window_start": window["window_start"],
            "window_end": window["window_end"],
            "total_days": int(window["total_days"]),
        ...
        "analyses": analyses,
        "thresholds": {
            "accuracy_gap_warning_threshold": ACCURACY_GAP_WARNING_THRESHOLD_V7_5,
            "macro_f1_gap_warning_threshold": MACRO_F1_GAP_WARNING_THRESHOLD_V7_5,
            "random_seed": LABEL_SHUFFLE_RANDOM_SEED_V7_5,
        },
        "findings": {
        ...
        "safety": SAFETY_FLAGS_V7_5,
        "limitations": EXPECTED_LIMITATIONS_V7_5,
    }
    _write_json(root / MANIFEST_PATH_V7_5, manifest)
    _write_json(root / REPORT_JSON_PATH_V7_5, manifest)
    markdown = build_ohlcv_trades_ml_robustness_markdown_v7_5(manifest)
    _write_text(root / REPORT_MD_PATH_V7_5, markdown)
    _write_text(root / DOC_PATH_V7_5, markdown)
    _update_project_state(root, manifest)
    return manifest


def compute_ohlcv_trades_baseline_delta_v7_5(metrics: dict[str, Any]) -> dict[str, Any]:
...
def compute_ohlcv_trades_split_stability_v7_5(metrics: dict[str, Any]) -> dict[str, Any]:
    stability: dict[str, Any] = {}
    models = sorted({metric["model_name"] for metric in metrics.values()})
    timeframes = sorted({metric["timeframe"] for metric in metrics.values()}, key=TIMEFRAMES_V7_4.index)
    for timeframe in timeframes:
        for model_name in models:
            split_metrics = {split: metrics.get(f"{timeframe}.{model_name}.{split}") for split in ROBUSTNESS_SPLITS_V7_5}
            ...
            overfit_warning = (
                train_validation_accuracy_gap > ACCURACY_GAP_WARNING_THRESHOLD_V7_5
                or train_validation_macro_f1_gap > MACRO_F1_GAP_WARNING_THRESHOLD_V7_5
            )
...
def compute_ohlcv_trades_timeframe_stability_v7_5(metrics: dict[str, Any]) -> dict[str, Any]:
    stability: dict[str, Any] = {}
    models = sorted({metric["model_name"] for metric in metrics.values()})
    for model_name in models:
        test_metrics = {
        ...
        stability[model_name] = {
            ...
            "single_timeframe_concentration_warning": bool(sorted_accuracy[0][1] - second_accuracy > ACCURACY_GAP_WARNING_THRESHOLD_V7_5),
        }
    return stability


def compute_ohlcv_trades_walk_forward_stability_v7_5(walk_forward_metrics: dict[str, Any]) -> dict[str, Any]:
    ...
    for (timeframe, model_name), records in sorted(grouped.items()):
        ...
        weak_groups = [
            ...
            if accuracy_by_group[group] < WALK_FORWARD_WEAK_ACCURACY_THRESHOLD_V7_5
            or macro_f1_by_group[group] < WALK_FORWARD_WEAK_MACRO_F1_THRESHOLD_V7_5
        ]
        unstable_groups = [
            group
            for group in accuracy_by_group
            if abs(accuracy_by_group[group] - mean_accuracy) > ACCURACY_GAP_WARNING_THRESHOLD_V7_5
            or abs(macro_f1_by_group[group] - mean_macro_f1) > MACRO_F1_GAP_WARNING_THRESHOLD_V7_5
        ]
        stability[f"{timeframe}.{model_name}"] = {
            "timeframe": timeframe,
            "model_name": model_name,
...
def build_reference_reports_v7_5(root: Path) -> dict[str, Any]:
    warnings: list[str] = []
    ...
    if v6_2_available or v5_4_available:
...
def compute_ohlcv_trades_vs_references_comparison_v7_5(
    root: Path,
    ...
    for reference_name, reference in references.items():
        compared[reference_name] = _compare_single_reference_v7_5(
            root,
            reference_name,
...
def _compare_single_reference_v7_5(
    root: Path,
    reference_name: str,
    reference_label: str,
...
def compute_ohlcv_trades_label_shuffle_falsification_v7_5(root: Path, ml_manifest: dict[str, Any]) -> dict[str, Any]:
    root = root.resolve()
    ...
    for timeframe in TIMEFRAMES_V7_4:
        rng = np.random.default_rng(LABEL_SHUFFLE_RANDOM_SEED_V7_5)
        ...
        slices = {split: ml_frame[ml_frame["split"] == split].copy() for split in ROBUSTNESS_SPLITS_V7_5}
        train = slices["train"]
        shuffled_train_target = pd.Series(rng.permutation(train[TARGET_NAME_V7_4].astype(str).to_numpy()), index=train.index)
        predict_frame = pd.concat([slices[split] for split in EVALUATION_SPLITS_V7_5], axis=0)
        for model_name in ROBUSTNESS_MODELS_V7_5:
            result = fit_predict_model(
                model_name,
                train[ALLOWED_FEATURE_COLUMNS_V7_4],
                shuffled_train_target,
                predict_frame[ALLOWED_FEATURE_COLUMNS_V7_4],
            )
            for split in EVALUATION_SPLITS_V7_5:
                split_frame = slices[split]
                y_true = split_frame[TARGET_NAME_V7_4].astype(str)
...
def scan_ohlcv_trades_feature_leakage_v7_5(feature_columns: list[str]) -> dict[str, Any]:
    exact_terms = {term.casefold() for term in FORBIDDEN_ROBUSTNESS_FEATURE_EXACT_V7_5}
    prefix_terms = tuple(term.casefold() for term in FORBIDDEN_ROBUSTNESS_FEATURE_PREFIXES_V7_5)
    forbidden: list[str] = []
...
def scan_ohlcv_trades_metric_forbidden_terms_v7_5(payload: Any) -> dict[str, Any]:
    text = json.dumps(payload, ensure_ascii=False, sort_keys=True).casefold()
    present = [term for term in FORBIDDEN_ROBUSTNESS_METRIC_TERMS_V7_5 if term in text]
    return {
        "forbidden_terms": FORBIDDEN_ROBUSTNESS_METRIC_TERMS_V7_5,
        "forbidden_terms_present": present,
        "metric_forbidden_terms_detected": bool(present),
    }


def build_ohlcv_trades_ml_robustness_markdown_v7_5(manifest: dict[str, Any]) -> str:
...
def _validate_inputs(root: Path) -> None:
    dataset_result = validate_ohlcv_trades_offline_supervised_dataset_v7_3(root)
    if not dataset_result["passed"]:
        raise RuntimeError(f"V7.3 dataset validation failed before V7.5: {dataset_result['errors']}")
    ml_result = validate_ohlcv_trades_offline_ml_research_v7_4(root)
    if not ml_result["passed"]:
        raise RuntimeError(f"V7.4 ML validation failed before V7.5: {ml_result['errors']}")
...
def _classification_summary(y_true: pd.Series, y_pred: pd.Series) -> dict[str, float]:
    return {
        ...
        "macro_f1": float(f1_score(y_true, y_pred, labels=TARGET_CLASSES_V7_4, average="macro", zero_division=0)),
    }
...
def _collect_warnings(analyses: dict[str, Any]) -> list[str]:
    ...
    warnings.extend(analyses["ohlcv_trades_vs_references_comparison"].get("warnings", []))
...
def _robustness_run_id() -> str:
    return f"{ROBUSTNESS_RUN_ID_PREFIX_V7_5}_{datetime.now(UTC).strftime('%Y%m%dT%H%M%SZ')}_{uuid.uuid4().hex[:8]}"
...
def _update_project_state(root: Path, manifest: dict[str, Any]) -> None:
    ...
    state.update(
        {
            "last_validated_version": "V7.4",
            "candidate_version": "V7.5",
            "candidate_status": "pending_external_audit",
            "direction": "OHLCV + trades robustness and walk-forward falsification audit",
            "ohlcv_trades_robustness_window_start_v7_5": manifest["input_ml_manifest"]["window_start"],
            "ohlcv_trades_robustness_window_end_v7_5": manifest["input_ml_manifest"]["window_end"],
            "ohlcv_trades_robustness_days_v7_5": manifest["input_ml_manifest"]["total_days"],
            "feature_columns_count_v7_5": manifest["input_ml_manifest"]["feature_columns_count"],
            "backtest_v7_5_created": False,
            "strategy_v7_5_created": False,
            "signal_v7_5_created": False,
            "orders_v7_5_created": False,
            "paper_live_v7_5_created": False,
            "trading_v7_5_created": False,
            "persistent_model_v7_5_created": False,
            "backtest_enabled": False,
            "strategy_enabled": False,
            "paper_live_enabled": False,
            "orders_enabled": False,
...
def _project_state_markdown(manifest: dict[str, Any]) -> str:
    return f"""# Etat du Projet : V7.4 validee + candidat V7.5
...
def _latest_summary_markdown(manifest: dict[str, Any]) -> str:
    return f"""# Latest Summary V7.5

V7.4 est la derniere version validee par audit externe.

V7.5 est la candidate courante. Elle audite uniquement la robustesse descriptive, les groupes walk-forward et la falsification par label shuffle des resultats ML offline V7.4.
...
V7.5 reste `pending_external_audit`.
"""
