...
from galapagos.ml.schemas import TARGET_CLASSES_V5_4


def compute_max_history_classification_metrics_v5_4(scores: pd.DataFrame) -> dict[str, Any]:
    ...
    for (timeframe, model_name, split), group in scores.groupby(["timeframe", "model_name", "split"], sort=True):
        ...
        metrics[key] = {
            ...
            "per_class_precision": _per_class(precision_score(y_true, y_pred, labels=TARGET_CLASSES_V5_4, average=None, zero_division=0)),
            "per_class_recall": _per_class(recall_score(y_true, y_pred, labels=TARGET_CLASSES_V5_4, average=None, zero_division=0)),
            "confusion_matrix": confusion_matrix(y_true, y_pred, labels=TARGET_CLASSES_V5_4).astype(int).tolist(),
...
def compute_max_history_walk_forward_metrics_v5_4(scores: pd.DataFrame) -> dict[str, Any]:
    metrics: dict[str, Any] = {}
...
def _distribution(values: pd.Series) -> dict[str, int]:
    return {label: int(values.eq(label).sum()) for label in TARGET_CLASSES_V5_4}


def _per_class(values: Any) -> dict[str, float]:
    return {label: float(values[index]) for index, label in enumerate(TARGET_CLASSES_V5_4)}
