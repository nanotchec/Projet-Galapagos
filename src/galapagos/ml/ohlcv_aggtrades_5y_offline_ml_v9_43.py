...
from galapagos.ml.ohlcv_aggtrades_5y_offline_ml_v9_43_metrics import classification_metrics_v9_43
from galapagos.ml.ohlcv_aggtrades_5y_offline_ml_v9_43_quality import (
    no_persistent_model_check_v9_43,
    scan_forbidden_features_v9_43,
    scan_forbidden_metrics_v9_43,
)


VERSION = "V9.43"
SOURCE_VERSION = "V9.42"
SOURCE_DATASET_VERSION = "V9.41"
...
REPORT_JSON_PATH = Path("reports/ml/ohlcv_aggtrades_5y_offline_ml_v9_43.json")
REPORT_MD_PATH = Path("reports/ml/ohlcv_aggtrades_5y_offline_ml_v9_43.md")
SCORES_JSON_PATH = Path("reports/ml/ohlcv_aggtrades_5y_offline_scores_v9_43.json")
SCORES_MD_PATH = Path("reports/ml/ohlcv_aggtrades_5y_offline_scores_v9_43.md")
MANIFEST_PATH = Path("reports/manifests/ohlcv_aggtrades_5y_offline_ml_v9_43_manifest.json")
DOC_PATH = Path("docs/ohlcv_aggtrades_5y_offline_ml_v9_43.md")

V9_42_REPORT_PATH = Path("reports/datasets/ohlcv_aggtrades_5y_dataset_validation_v9_42.json")
V9_41_REPORT_PATH = Path("reports/datasets/ohlcv_aggtrades_5y_dataset_v9_41.json")
V9_38_REPORT_PATH = Path("reports/features/ohlcv_aggtrades_5y_feature_store_validation_v9_38.json")
...
def run_offline_ml_v9_43(root: Path = Path(".")) -> dict[str, Any]:
    started = datetime.now(UTC)
    root = root.resolve()
    source_validation = _read_json(root / V9_42_REPORT_PATH)
    if source_validation.get("decision") != "ohlcv_aggtrades_5y_dataset_validated":
        report = _blocked_report("offline_ml_5y_blocked_by_dataset_issue", started, ["V9.42 dataset validation decision is not validated"])
        _write_outputs(root, report)
        return report

    ml_run_id = f"v9_43_ml_{started.strftime('%Y%m%dT%H%M%SZ')}_{uuid.uuid4().hex[:8]}"
    ...
    feature_scan = scan_forbidden_features_v9_43(list(FEATURE_COLUMNS))
    ...
    for timeframe in TIMEFRAMES:
        ...
        if frame.empty:
            errors.append(f"empty V9.43 ML frame for {timeframe}")
            continue
    ...
    forbidden_metric_scan = scan_forbidden_metrics_v9_43(
        {
    ...
    no_model_check = no_persistent_model_check_v9_43()
    ...
    decision = _decide(quality_status, leakage_guard_status, forbidden_metric_scan, baseline_comparison, no_clear_vs_shuffled_count)
    ...
    report = {
        ...
        "errors": errors,
        "limitations": [
            "V9.43 est un diagnostic ML offline classification-only.",
...
def _run_timeframe_models(timeframe: str, frame: pd.DataFrame) -> tuple[dict[str, Any], dict[str, Any], dict[str, Any]]:
    ...
    for model_name in MODEL_NAMES:
        result = _fit_predict_model_v9_43(model_name, train[list(FEATURE_COLUMNS)], train[TARGET_NAME].astype(str), frame[list(FEATURE_COLUMNS)])
        ...
        for split, split_frame in frame.groupby("split", sort=True, observed=True):
            ...
            metrics[key] = classification_metrics_v9_43(
                timeframe=timeframe,
    ...
    for model_name in LEARNED_MODEL_NAMES:
        shuffled = _fit_predict_model_v9_43(model_name, train[list(FEATURE_COLUMNS)], shuffled_target.astype(str), eval_frame[list(FEATURE_COLUMNS)])
        ...
        for split, split_frame in eval_frame.groupby("split", sort=True, observed=True):
            ...
            shuffle_metrics[shuffled_key] = classification_metrics_v9_43(
                timeframe=timeframe,
...
def _fit_predict_model_v9_43(
    ...
    if model_name == "logistic_regression":
        ...
        raise ValueError(f"unsupported V9.43 model: {model_name}")
...
def _decide(
    ...
    if leakage_guard_status != "PASS":
        return "offline_ml_5y_blocked_by_leakage"
    if forbidden_metric_scan["status"] != "PASS":
        return "offline_ml_5y_blocked_by_forbidden_metrics"
    if quality_status != "PASS":
        return "offline_ml_5y_blocked_by_dataset_issue"
    if no_clear_vs_shuffled_count > 0:
        return "offline_ml_5y_completed_but_close_to_shuffled_labels"
    if baseline_comparison["clear_wins_count"] < max(1, len(baseline_comparison["comparisons"]) // 2):
        return "offline_ml_5y_completed_but_weak_vs_baselines"
    return "offline_ml_5y_completed"


def _recommendation(decision: str) -> str:
    if decision in {"offline_ml_5y_completed_but_weak_vs_baselines", "offline_ml_5y_completed_but_close_to_shuffled_labels"}:
        return "V9.44 - 5Y ML Diagnostic / Feature Review"
    if decision == "offline_ml_5y_completed":
        return "V9.44 - OHLCV + AggTrades 5Y Strict Walk-Forward"
    return "V9.44 - Label Redesign"
...
def _blocked_report(decision: str, started: datetime, errors: list[str]) -> dict[str, Any]:
    return {
        ...
        "no_persistent_model_check": no_persistent_model_check_v9_43(),
        "ml_executed": False,
        "walk_forward_executed": False,
        "backtest_executed": False,
        "signal_created": False,
        ...
        "next_recommendation": "V9.44 - 5Y ML Diagnostic / Feature Review",
