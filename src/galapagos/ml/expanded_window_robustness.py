...
from galapagos.datasets.schemas import MANIFEST_PATH_V3_8, get_dataset_v3_8_path
...
VERSION_V4_0 = "V4.0"
ROBUSTNESS_RUN_ID_PREFIX_V4_0 = "v4_0"
MANIFEST_PATH_V4_0 = Path("reports/manifests/expanded_ml_robustness_v4_0_manifest.json")
REPORT_JSON_PATH_V4_0 = Path("reports/ml/expanded_ml_robustness_v4_0.json")
REPORT_MD_PATH_V4_0 = Path("reports/ml/expanded_ml_robustness_v4_0.md")
DOC_PATH_V4_0 = Path("docs/expanded_ml_robustness_v4_0.md")
ACCURACY_GAP_WARNING_THRESHOLD_V4_0 = 0.10
MACRO_F1_GAP_WARNING_THRESHOLD_V4_0 = 0.10
LABEL_SHUFFLE_RANDOM_SEED_V4_0 = 123
ROBUSTNESS_MODELS_V4_0 = ["logistic_regression", "decision_tree_depth_2"]
ROBUSTNESS_SPLITS_V4_0 = ["train", "validation", "test"]
EVALUATION_SPLITS_V4_0 = ["validation", "test"]
FORBIDDEN_ROBUSTNESS_FEATURE_TERMS_V4_0 = [
    "future_",
    "label_",
    "direction_",
    "up_down_flat_",
...
def run_expanded_ml_robustness_v4_0(
    root: Path = Path("."),
    *,
    ...
    ml_manifest = _read_json(root / MANIFEST_PATH_V3_9)
    ml_report = _read_json(root / REPORT_JSON_PATH_V3_9)
    scores_report = _read_json(root / SCORES_JSON_PATH_V3_9)
    scores_by_timeframe = {timeframe: read_parquet(get_expanded_ml_score_path_v3_9(root, timeframe)) for timeframe in TIMEFRAMES_V3_9}
    metrics = ml_manifest["metrics"]

    analyses = {
        "baseline_delta": compute_expanded_baseline_delta_v4_0(metrics),
        "split_stability": compute_expanded_split_stability_v4_0(metrics),
        "timeframe_stability": compute_expanded_timeframe_stability_v4_0(metrics),
        "label_shuffle_falsification": compute_expanded_label_shuffle_falsification_v4_0(root, metrics),
        "feature_leakage_scan": scan_expanded_feature_leakage_v4_0(ml_manifest["feature_columns"]),
        "metric_forbidden_scan": scan_expanded_metric_forbidden_terms_v4_0(
            {
    ...
    manifest = {
        "version": VERSION_V4_0,
        "status": status,
        "created_at_utc": utc_now_iso(),
        "robustness_run_id": _robustness_run_id(),
        "input_dataset_manifest": _path_sha_block(root, MANIFEST_PATH_V3_8),
        "input_ml_manifest": _path_sha_block(root, MANIFEST_PATH_V3_9),
        "input_score_files": _input_score_files(root, scores_by_timeframe),
        "analyses": analyses,
        "thresholds": {
            "accuracy_gap_warning_threshold": ACCURACY_GAP_WARNING_THRESHOLD_V4_0,
            "macro_f1_gap_warning_threshold": MACRO_F1_GAP_WARNING_THRESHOLD_V4_0,
            "random_seed": LABEL_SHUFFLE_RANDOM_SEED_V4_0,
        },
        "findings": {
        ...
        "safety": SAFETY_FLAGS_V4_0,
        "limitations": EXPECTED_LIMITATIONS_V4_0,
    }
    _write_json(root / MANIFEST_PATH_V4_0, manifest)
    _write_json(root / REPORT_JSON_PATH_V4_0, manifest)
    markdown = build_expanded_ml_robustness_markdown_v4_0(manifest)
    _write_text(root / REPORT_MD_PATH_V4_0, markdown)
    _write_text(root / DOC_PATH_V4_0, markdown)
    return manifest


def compute_expanded_baseline_delta_v4_0(metrics: dict[str, Any]) -> dict[str, Any]:
...
def compute_expanded_split_stability_v4_0(metrics: dict[str, Any]) -> dict[str, Any]:
    stability: dict[str, Any] = {}
    models = sorted({metric["model_name"] for metric in metrics.values()})
    timeframes = sorted({metric["timeframe"] for metric in metrics.values()}, key=TIMEFRAMES_V3_9.index)
    for timeframe in timeframes:
        for model_name in models:
            split_metrics = {split: metrics.get(f"{timeframe}.{model_name}.{split}") for split in ROBUSTNESS_SPLITS_V4_0}
            ...
            overfit_warning = (
                train_validation_accuracy_gap > ACCURACY_GAP_WARNING_THRESHOLD_V4_0
                or train_validation_macro_f1_gap > MACRO_F1_GAP_WARNING_THRESHOLD_V4_0
            )
...
def compute_expanded_timeframe_stability_v4_0(metrics: dict[str, Any]) -> dict[str, Any]:
    stability: dict[str, Any] = {}
    models = sorted({metric["model_name"] for metric in metrics.values()})
    for model_name in models:
        test_metrics = {
        ...
        stability[model_name] = {
            ...
            "single_timeframe_concentration_warning": bool(sorted_accuracy[0][1] - second_accuracy > ACCURACY_GAP_WARNING_THRESHOLD_V4_0),
        }
    return stability


def compute_expanded_label_shuffle_falsification_v4_0(root: Path, original_metrics: dict[str, Any]) -> dict[str, Any]:
    root = root.resolve()
    falsification: dict[str, Any] = {}
    rng = np.random.default_rng(LABEL_SHUFFLE_RANDOM_SEED_V4_0)
    for timeframe in TIMEFRAMES_V3_9:
        ...
        slices = {split: ml_frame[ml_frame["split"] == split].copy() for split in ROBUSTNESS_SPLITS_V4_0}
        train = slices["train"]
        shuffled_train_target = pd.Series(rng.permutation(train[TARGET_NAME_V3_9].astype(str).to_numpy()), index=train.index)
        for model_name in ROBUSTNESS_MODELS_V4_0:
            for split in EVALUATION_SPLITS_V4_0:
                split_frame = slices[split]
                result = fit_predict_model(
                    model_name,
                    train[ALLOWED_FEATURE_COLUMNS_V3_9],
                    shuffled_train_target,
                    split_frame[ALLOWED_FEATURE_COLUMNS_V3_9],
                )
                y_true = split_frame[TARGET_NAME_V3_9].astype(str)
...
def scan_expanded_feature_leakage_v4_0(feature_columns: list[str]) -> dict[str, Any]:
    forbidden = [
        column
        for column in feature_columns
        if any(term in column.casefold() for term in FORBIDDEN_ROBUSTNESS_FEATURE_TERMS_V4_0)
    ]
    return {
...
def scan_expanded_metric_forbidden_terms_v4_0(payload: Any) -> dict[str, Any]:
    text = json.dumps(payload, ensure_ascii=False, sort_keys=True).casefold()
    present = [term for term in FORBIDDEN_ROBUSTNESS_METRIC_TERMS_V4_0 if term in text]
    return {
        "forbidden_terms": FORBIDDEN_ROBUSTNESS_METRIC_TERMS_V4_0,
        "forbidden_terms_present": present,
        "metric_forbidden_terms_detected": bool(present),
    }


def build_expanded_ml_robustness_markdown_v4_0(manifest: dict[str, Any]) -> str:
...
def _validate_inputs(root: Path) -> None:
    dataset_result = validate_expanded_offline_supervised_dataset_v3_8(root)
    if not dataset_result["passed"]:
        raise RuntimeError(f"V3.8 dataset validation failed before V4.0: {dataset_result['errors']}")
    ml_result = validate_expanded_offline_ml_research_v3_9(root)
    if not ml_result["passed"]:
        raise RuntimeError(f"V3.9 ML validation failed before V4.0: {ml_result['errors']}")
...
def _input_score_files(root: Path, scores_by_timeframe: dict[str, pd.DataFrame]) -> dict[str, dict[str, Any]]:
    ...
    for timeframe, scores in scores_by_timeframe.items():
        path = get_expanded_ml_score_path_v3_9(root, timeframe)
        blocks[timeframe] = {
...
def _classification_summary(y_true: pd.Series, y_pred: pd.Series) -> dict[str, float]:
    return {
        ...
        "macro_f1": float(f1_score(y_true, y_pred, labels=TARGET_CLASSES_V3_9, average="macro", zero_division=0)),
    }
...
def _robustness_run_id() -> str:
    return f"{ROBUSTNESS_RUN_ID_PREFIX_V4_0}_{datetime.now(UTC).strftime('%Y%m%dT%H%M%SZ')}_{uuid.uuid4().hex[:8]}"
