...
from galapagos.datasets.schemas import MANIFEST_PATH_V7_3, SPLIT_COLUMNS_V7_3
...
def run_ohlcv_trades_offline_ml_research_v7_4(
    root: Path = Path("."),
    *,
    ...
    ml_run_id = f"v7_4_{datetime.now(UTC).strftime('%Y%m%dT%H%M%SZ')}_{uuid.uuid4().hex[:8]}"
    ...
    for timeframe in TIMEFRAMES_V7_4:
        ...
        dataset_for_ml = validate_split_alignment_v7_4(dataset, splits)
        dataset_sha = sha256_file(dataset_path)

        scores = build_ohlcv_trades_model_scores_v7_4(dataset_for_ml, dataset_sha256=dataset_sha, ml_run_id=ml_run_id)
        ...
        quality[timeframe] = assess_ohlcv_trades_ml_quality_v7_4(dataset_for_ml, scores, timeframe)
        if quality[timeframe]["errors"]:
            status = "FAIL"
        sanity_checks[timeframe] = _sanity_from_quality(dataset_for_ml, quality[timeframe])
        metrics.update(compute_ohlcv_trades_classification_metrics_v7_4(scores))
        walk_forward_metrics.update(compute_ohlcv_trades_walk_forward_metrics_v7_4(scores))

    comparison_to_references = build_comparison_to_references_v7_4(root, metrics)

    manifest = {
        "version": VERSION_V7_4,
        "status": status,
        ...
        "outputs": outputs,
        "target_name": TARGET_NAME_V7_4,
        "feature_columns": ALLOWED_FEATURE_COLUMNS_V7_4,
        "feature_columns_count": len(ALLOWED_FEATURE_COLUMNS_V7_4),
        "models": MODEL_NAMES_V7_4,
        "metrics": metrics,
        "walk_forward_metrics": walk_forward_metrics,
        "comparison_to_references": comparison_to_references,
        "sanity_checks": sanity_checks,
        "quality": quality,
        "safety": SAFETY_FLAGS_V7_4,
        "limitations": EXPECTED_LIMITATIONS_V7_4,
    }

    _write_json(root / MANIFEST_PATH_V7_4, manifest)
    _write_json(root / REPORT_JSON_PATH_V7_4, manifest)
    _write_json(root / SCORES_JSON_PATH_V7_4, _scores_report(manifest))
    markdown = build_ohlcv_trades_ml_markdown_v7_4(manifest)
    _write_text(root / REPORT_MD_PATH_V7_4, markdown)
    _write_text(root / SCORES_MD_PATH_V7_4, markdown)
    _write_text(root / DOC_PATH_V7_4, markdown)
    _update_project_state(root, manifest)
    return manifest


def prepare_ohlcv_trades_ml_frame_v7_4(dataset: pd.DataFrame) -> pd.DataFrame:
    frame = dataset[(dataset["label_valid_h1"] == True) & (dataset["warmup_row"] == False)].copy()  # noqa: E712
    return frame.reset_index(drop=True)


def validate_split_alignment_v7_4(dataset: pd.DataFrame, splits: pd.DataFrame) -> pd.DataFrame:
    missing_dataset = [column for column in SPLIT_COLUMNS_V7_3 if column not in dataset.columns]
    missing_splits = [column for column in SPLIT_COLUMNS_V7_3 if column not in splits.columns]
    if missing_dataset or missing_splits:
        raise ValueError(f"V7.4 split alignment missing dataset={missing_dataset}, missing_splits={missing_splits}")
    try:
        assert_frame_equal(
            dataset[SPLIT_COLUMNS_V7_3].reset_index(drop=True),
            splits[SPLIT_COLUMNS_V7_3].reset_index(drop=True),
            check_dtype=False,
        )
    except AssertionError as exc:
        raise ValueError("V7.4 dataset and split files are not aligned") from exc
    if dataset["walk_forward_group"].isna().any():
        raise ValueError("V7.4 dataset contains null walk_forward_group values")
    return dataset.copy()


def get_ohlcv_trades_training_slices_v7_4(frame: pd.DataFrame) -> dict[str, pd.DataFrame]:
    return {split: frame[frame["split"] == split].copy() for split in ["train", "validation", "test"]}


def build_ohlcv_trades_model_scores_v7_4(
    dataset: pd.DataFrame,
    *,
    dataset_sha256: str,
    ml_run_id: str,
) -> pd.DataFrame:
    ml_frame = prepare_ohlcv_trades_ml_frame_v7_4(dataset)
    slices = get_ohlcv_trades_training_slices_v7_4(ml_frame)
    train = slices["train"]
    if train.empty:
        return pd.DataFrame(columns=ML_SCORE_COLUMNS_V7_4)

    score_frames: list[pd.DataFrame] = []
    train_features = train[ALLOWED_FEATURE_COLUMNS_V7_4]
    train_target = train[TARGET_NAME_V7_4].astype(str)
    score_features = ml_frame[ALLOWED_FEATURE_COLUMNS_V7_4]
    feature_columns_sha256 = get_feature_columns_sha256_v7_4()

    for model_name in MODEL_NAMES_V7_4:
        ...
        scores["target_name"] = TARGET_NAME_V7_4
        scores["dataset_sha256"] = dataset_sha256
        scores["feature_columns_sha256"] = feature_columns_sha256
        scores["ml_schema_version"] = ML_SCHEMA_VERSION_V7_4
        scores["target_value"] = ml_frame[TARGET_NAME_V7_4].astype(str).to_numpy()
        ...
        score_frames.append(scores[ML_SCORE_COLUMNS_V7_4])

    return pd.concat(score_frames, ignore_index=True)


def build_ohlcv_trades_ml_markdown_v7_4(manifest: dict[str, Any]) -> str:
    lines = [
...
def load_v7_3_dataset_manifest(root: Path = Path(".")) -> dict[str, Any]:
    return json.loads((root.resolve() / MANIFEST_PATH_V7_3).read_text(encoding="utf-8"))


def input_dataset_path(root: Path, timeframe: str, manifest: dict[str, Any] | None = None) -> Path:
    manifest = manifest or load_v7_3_dataset_manifest(root)
...
def score_output_path(root: Path, timeframe: str, window_start: str | None = None, window_end: str | None = None) -> Path:
    if window_start is None or window_end is None:
        window = _input_window(load_v7_3_dataset_manifest(root))
        window_start = window["window_start"]
        window_end = window["window_end"]
    return get_ohlcv_trades_ml_score_path_v7_4(root.resolve(), timeframe, window_start, window_end)


def _validate_dataset_layer(root: Path) -> None:
    result = validate_ohlcv_trades_offline_supervised_dataset_v7_3(root)
    if not result["passed"]:
        raise RuntimeError(f"V7.3 dataset validation failed before V7.4: {result['errors']}")
...
def _target_classes(frame: pd.DataFrame, split: str) -> list[str]:
    return sorted(frame[frame["split"] == split][TARGET_NAME_V7_4].dropna().astype(str).unique().tolist())


def build_comparison_to_references_v7_4(root: Path, ohlcv_trades_metrics: dict[str, Any]) -> dict[str, Any]:
    references = {
...
def _build_reference_comparison(
    ...
    for key, current_payload in sorted(ohlcv_trades_metrics.items()):
        ...
        comparisons[key] = {
            ...
            "accuracy_delta_v7_4_minus_reference": _metric_delta(current_payload, reference_payload, "accuracy"),
            "balanced_accuracy_delta_v7_4_minus_reference": _metric_delta(current_payload, reference_payload, "balanced_accuracy"),
            "macro_f1_delta_v7_4_minus_reference": _metric_delta(current_payload, reference_payload, "macro_f1"),
        }
    return {
        "status": "PASS",
        "source_manifest_path": source_manifest_path.relative_to(root).as_posix(),
...
def _update_project_state(root: Path, manifest: dict[str, Any]) -> None:
    ...
    state.update(
        {
            "last_validated_version": "V7.3",
            "candidate_version": "V7.4",
            "candidate_status": "pending_external_audit",
            "direction": "OHLCV + public trades offline ML research baselines",
            "v7_4_candidate": True,
            "ohlcv_trades_offline_ml_research_v7_4_created": True,
            "ohlcv_trades_ml_window_start_v7_4": window["window_start"],
            "ohlcv_trades_ml_window_end_v7_4": window["window_end"],
            "ohlcv_trades_ml_days_v7_4": window["total_days"],
            "ohlcv_trades_ml_score_rows_v7_4": rows,
            "feature_columns_count_v7_4": manifest["feature_columns_count"],
            "ohlcv_trades_ml_models_v7_4": MODEL_NAMES_V7_4,
            "ohlcv_trades_ml_target_v7_4": TARGET_NAME_V7_4,
            "backtest_v7_4_created": False,
            "strategy_v7_4_created": False,
            "signal_v7_4_created": False,
            "orders_v7_4_created": False,
            "paper_live_v7_4_created": False,
            "trading_v7_4_created": False,
            "persistent_model_v7_4_created": False,
...
def _build_latest_metrics(manifest: dict[str, Any], state: dict[str, Any]) -> dict[str, Any]:
    ...
    return {
        ...
        "ohlcv_trades_ml_window_end_v7_4": window["window_end"],
        "ohlcv_trades_ml_days_v7_4": window["total_days"],
        "ohlcv_trades_ml_score_rows_v7_4": {timeframe: payload["rows"] for timeframe, payload in manifest["outputs"].items()},
        "feature_columns_count_v7_4": manifest["feature_columns_count"],
        "ohlcv_trades_ml_models_v7_4": manifest["models"],
        "ohlcv_trades_ml_target_v7_4": manifest["target_name"],
        "ml_schema_version_v7_4": ML_SCHEMA_VERSION_V7_4,
        "ml_score_columns_count_v7_4": len(ML_SCORE_COLUMNS_V7_4),
        "walk_forward_metrics_v7_4": True,
        "backtest_v7_4_created": False,
        "strategy_v7_4_created": False,
        "signal_v7_4_created": False,
        "orders_v7_4_created": False,
...
def _build_project_state_markdown(manifest: dict[str, Any]) -> str:
    window = manifest["input_dataset_manifest"]
    rows = {timeframe: payload["rows"] for timeframe, payload in manifest["outputs"].items()}
    return f"""# Etat du Projet : V7.3 validee + candidat V7.4
...
- Fenetre V7.3 utilisee : `{window['window_start']}` -> `{window['window_end']}`.
...
def _build_latest_summary_markdown(manifest: dict[str, Any]) -> str:
    window = manifest["input_dataset_manifest"]
    rows = {timeframe: payload["rows"] for timeframe, payload in manifest["outputs"].items()}
    return f"""# Latest Summary V7.4

V7.3 est la derniere version validee par audit externe.

V7.4 est la candidate courante. Elle entraine uniquement des baselines ML offline simples sur le dataset V7.3 avec OHLCV + public trades features, produit des scores de recherche `research_*`, calcule des metriques descriptives par split et par groupe walk-forward, et compare descriptivement V7.4 a V6.2/V5.4 si disponible.
...
V7.4 reste `pending_external_audit`.
"""
