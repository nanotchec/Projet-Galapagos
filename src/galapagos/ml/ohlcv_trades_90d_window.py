...
from galapagos.datasets.schemas import MANIFEST_PATH_V7_9, SPLIT_COLUMNS_V7_9
...
def run_ohlcv_trades_90d_offline_ml_research_v8_0(
    root: Path = Path("."),
    *,
    ...
    ml_run_id = f"v8_0_{datetime.now(UTC).strftime('%Y%m%dT%H%M%SZ')}_{uuid.uuid4().hex[:8]}"
    ...
    for timeframe in TIMEFRAMES_V8_0:
        ...
        dataset_for_ml = validate_split_alignment_v8_0(dataset, splits)
        dataset_sha = sha256_file(dataset_path)

        scores = build_ohlcv_trades_model_scores_v8_0(dataset_for_ml, dataset_sha256=dataset_sha, ml_run_id=ml_run_id)
        ...
        quality[timeframe] = assess_ohlcv_trades_ml_quality_v8_0(dataset_for_ml, scores, timeframe)
        if quality[timeframe]["errors"]:
            status = "FAIL"
        sanity_checks[timeframe] = _sanity_from_quality(dataset_for_ml, quality[timeframe])
        metrics.update(compute_ohlcv_trades_classification_metrics_v8_0(scores))
        walk_forward_metrics.update(compute_ohlcv_trades_walk_forward_metrics_v8_0(scores))

    comparison_to_references = build_comparison_to_references_v8_0(root, metrics)
    ...
    manifest = {
        ...
        "outputs": outputs,
        "target_name": TARGET_NAME_V8_0,
        "feature_columns": ALLOWED_FEATURE_COLUMNS_V8_0,
        "feature_columns_count": len(ALLOWED_FEATURE_COLUMNS_V8_0),
        "models": MODEL_NAMES_V8_0,
        "metrics": metrics,
    ...
    _write_json(root / MANIFEST_PATH_V8_0, manifest)
    _write_json(root / REPORT_JSON_PATH_V8_0, manifest)
    _write_json(root / SCORES_JSON_PATH_V8_0, _scores_report(manifest))
    markdown = build_ohlcv_trades_ml_markdown_v8_0(manifest)
    _write_text(root / REPORT_MD_PATH_V8_0, markdown)
    _write_text(root / SCORES_MD_PATH_V8_0, markdown)
    _write_text(root / DOC_PATH_V8_0, markdown)
    _update_project_state(root, manifest)
    return manifest


def prepare_ohlcv_trades_ml_frame_v8_0(dataset: pd.DataFrame) -> pd.DataFrame:
    frame = dataset[(dataset["label_valid_h1"] == True) & (dataset["warmup_row"] == False)].copy()  # noqa: E712
    return frame.reset_index(drop=True)


def validate_split_alignment_v8_0(dataset: pd.DataFrame, splits: pd.DataFrame) -> pd.DataFrame:
    missing_dataset = [column for column in SPLIT_COLUMNS_V7_9 if column not in dataset.columns]
    missing_splits = [column for column in SPLIT_COLUMNS_V7_9 if column not in splits.columns]
    if missing_dataset or missing_splits:
        raise ValueError(f"V8.0 split alignment missing dataset={missing_dataset}, missing_splits={missing_splits}")
    try:
        assert_frame_equal(
            dataset[SPLIT_COLUMNS_V7_9].reset_index(drop=True),
            splits[SPLIT_COLUMNS_V7_9].reset_index(drop=True),
            check_dtype=False,
        )
    except AssertionError as exc:
        raise ValueError("V8.0 dataset and split files are not aligned") from exc
    if dataset["walk_forward_group"].isna().any():
        raise ValueError("V8.0 dataset contains null walk_forward_group values")
    return dataset.copy()


def get_ohlcv_trades_training_slices_v8_0(frame: pd.DataFrame) -> dict[str, pd.DataFrame]:
    return {split: frame[frame["split"] == split].copy() for split in ["train", "validation", "test"]}


def build_ohlcv_trades_model_scores_v8_0(
    dataset: pd.DataFrame,
    *,
    dataset_sha256: str,
    ml_run_id: str,
) -> pd.DataFrame:
    ml_frame = prepare_ohlcv_trades_ml_frame_v8_0(dataset)
    slices = get_ohlcv_trades_training_slices_v8_0(ml_frame)
    train = slices["train"]
    if train.empty:
        return pd.DataFrame(columns=ML_SCORE_COLUMNS_V8_0)

    score_frames: list[pd.DataFrame] = []
    train_features = train[ALLOWED_FEATURE_COLUMNS_V8_0]
    train_target = train[TARGET_NAME_V8_0].astype(str)
    score_features = ml_frame[ALLOWED_FEATURE_COLUMNS_V8_0]
    feature_columns_sha256 = get_feature_columns_sha256_v8_0()

    for model_name in MODEL_NAMES_V8_0:
        ...
        scores["target_name"] = TARGET_NAME_V8_0
        scores["dataset_sha256"] = dataset_sha256
        scores["feature_columns_sha256"] = feature_columns_sha256
        scores["ml_schema_version"] = ML_SCHEMA_VERSION_V8_0
        scores["target_value"] = ml_frame[TARGET_NAME_V8_0].astype(str).to_numpy()
        ...
        score_frames.append(scores[ML_SCORE_COLUMNS_V8_0])

    return pd.concat(score_frames, ignore_index=True)


def build_ohlcv_trades_ml_markdown_v8_0(manifest: dict[str, Any]) -> str:
    lines = [
...
def load_v7_9_dataset_manifest(root: Path = Path(".")) -> dict[str, Any]:
    return json.loads((root.resolve() / MANIFEST_PATH_V7_9).read_text(encoding="utf-8"))


def input_dataset_path(root: Path, timeframe: str, manifest: dict[str, Any] | None = None) -> Path:
    manifest = manifest or load_v7_9_dataset_manifest(root)
...
def score_output_path(root: Path, timeframe: str, window_start: str | None = None, window_end: str | None = None) -> Path:
    if window_start is None or window_end is None:
        window = _input_window(load_v7_9_dataset_manifest(root))
        window_start = window["window_start"]
        window_end = window["window_end"]
    return get_ohlcv_trades_ml_score_path_v8_0(root.resolve(), timeframe, window_start, window_end)


def _validate_dataset_layer(root: Path) -> None:
    result = validate_ohlcv_trades_90d_offline_supervised_dataset_v7_9(root)
    if not result["passed"]:
        raise RuntimeError(f"V7.9 dataset validation failed before V8.0: {result['errors']}")
...
def _target_classes(frame: pd.DataFrame, split: str) -> list[str]:
    return sorted(frame[frame["split"] == split][TARGET_NAME_V8_0].dropna().astype(str).unique().tolist())


def build_comparison_to_references_v8_0(root: Path, ohlcv_trades_metrics: dict[str, Any]) -> dict[str, Any]:
    references = {
...
def _build_reference_comparison(
    ...
    for key, current_payload in sorted(ohlcv_trades_metrics.items()):
        ...
        comparisons[key] = {
            ...
            "accuracy_delta_v8_0_minus_reference": _metric_delta(current_payload, reference_payload, "accuracy"),
            "balanced_accuracy_delta_v8_0_minus_reference": _metric_delta(current_payload, reference_payload, "balanced_accuracy"),
            "macro_f1_delta_v8_0_minus_reference": _metric_delta(current_payload, reference_payload, "macro_f1"),
        }
    return {
        "status": "PASS",
        "source_manifest_path": source_manifest_path.relative_to(root).as_posix(),
...
def _update_project_state(root: Path, manifest: dict[str, Any]) -> None:
    ...
    state.update(
        ...
        "direction": "OHLCV + public trades 90-day ML offline and robustness",
            "v8_0_candidate": True,
            "ohlcv_trades_90d_offline_ml_research_v8_0_created": True,
            "ohlcv_trades_ml_window_start_v8_0": window["window_start"],
            "ohlcv_trades_ml_window_end_v8_0": window["window_end"],
            "ohlcv_trades_ml_days_v8_0": window["total_days"],
            "ohlcv_trades_ml_score_rows_v8_0": rows,
            "feature_columns_count_v8_0": manifest["feature_columns_count"],
            "ohlcv_trades_ml_models_v8_0": MODEL_NAMES_V8_0,
            "ohlcv_trades_ml_target_v8_0": TARGET_NAME_V8_0,
...
def _build_latest_metrics(manifest: dict[str, Any], state: dict[str, Any]) -> dict[str, Any]:
    ...
    return {
        ...
        "ohlcv_trades_ml_window_end_v8_0": window["window_end"],
        "ohlcv_trades_ml_days_v8_0": window["total_days"],
        "ohlcv_trades_ml_score_rows_v8_0": {timeframe: payload["rows"] for timeframe, payload in manifest["outputs"].items()},
        "feature_columns_count_v8_0": manifest["feature_columns_count"],
        "ohlcv_trades_ml_models_v8_0": manifest["models"],
        "ohlcv_trades_ml_target_v8_0": manifest["target_name"],
        "ml_schema_version_v8_0": ML_SCHEMA_VERSION_V8_0,
        "ml_score_columns_count_v8_0": len(ML_SCORE_COLUMNS_V8_0),
        "walk_forward_metrics_v8_0": True,
        "backtest_v8_0_created": False,
        "strategy_v8_0_created": False,
        "signal_v8_0_created": False,
        "orders_v8_0_created": False,
...
- Fenetre V7.9 utilisee : `{window['window_start']}` -> `{window['window_end']}`.
- Nombre de jours : `{window['total_days']}`.
- Row counts scores : `{rows}`.
- Schema : `ML_SCORE_COLUMNS_V8_0`.
...
- Comparaisons V8.0 vs V7.4/V6.2/V5.4 : descriptives uniquement, non actionnables et non directement comparables si les fenetres different.
...
V8.0 est la candidate courante. Elle entraine uniquement des baselines ML offline simples sur le dataset V7.9 avec OHLCV + public trades features, produit des scores de recherche `research_*`, calcule des metriques descriptives par split et par groupe walk-forward, et compare descriptivement V8.0 a V7.4/V6.2/V5.4 si disponible.
...
V8.0 entraine uniquement des baselines ML offline simples sur le dataset V7.9 OHLCV + aggTrades, avec scores de recherche `research_*` et metriques descriptives.
...
python -m pytest -q tests/ml/test_ohlcv_trades_90d_offline_ml_research_v8_0.py
python -m pytest -q tests/validation/test_ohlcv_trades_90d_offline_ml_research_v8_0_validator.py
python scripts/release_audit_lite_zip_v8_0.py
python scripts/audit_audit_lite_zip_v8_0.py --zip projet-galapagos-v7.4-audit-lite.zip
python scripts/smoke_audit_lite_zip_v8_0.py --zip projet-galapagos-v7.4-audit-lite.zip
