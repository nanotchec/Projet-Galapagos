...
def run_one_year_offline_ml_research_v4_6(
    root: Path = Path("."),
    *,
    ...
    ml_run_id = f"v4_6_{datetime.now(UTC).strftime('%Y%m%dT%H%M%SZ')}_{uuid.uuid4().hex[:8]}"
    ...
    for timeframe in TIMEFRAMES_V4_6:
        ...
        scores = build_one_year_model_scores_v4_6(dataset, dataset_sha256=dataset_sha, ml_run_id=ml_run_id)
        ...
        quality[timeframe] = assess_one_year_ml_quality_v4_6(dataset, scores, timeframe)
    ...
    metrics = compute_one_year_classification_metrics_v4_6(pd.concat(non_empty_scores, ignore_index=True))
    manifest = {
        "version": VERSION_V4_6,
        "status": status,
        ...
        "outputs": outputs,
        "target_name": TARGET_NAME_V4_6,
        "feature_columns": ALLOWED_FEATURE_COLUMNS_V4_6,
        "models": MODEL_NAMES_V4_6,
        "metrics": metrics,
        "sanity_checks": sanity_checks,
        "quality": quality,
        "safety": SAFETY_FLAGS_V4_6,
        "limitations": EXPECTED_LIMITATIONS_V4_6,
    }

    _write_json(root / MANIFEST_PATH_V4_6, manifest)
    _write_json(root / REPORT_JSON_PATH_V4_6, manifest)
    _write_json(root / SCORES_JSON_PATH_V4_6, _scores_report(manifest))
    markdown = build_one_year_ml_markdown_v4_6(manifest)
    _write_text(root / REPORT_MD_PATH_V4_6, markdown)
    _write_text(root / SCORES_MD_PATH_V4_6, markdown)
    _write_text(root / DOC_PATH_V4_6, markdown)
    return manifest


def prepare_one_year_ml_frame_v4_6(dataset: pd.DataFrame) -> pd.DataFrame:
    frame = dataset[(dataset["label_valid_h1"] == True) & (dataset["warmup_row"] == False)].copy()  # noqa: E712
    return frame.reset_index(drop=True)


def get_one_year_training_slices_v4_6(frame: pd.DataFrame) -> dict[str, pd.DataFrame]:
    return {split: frame[frame["split"] == split].copy() for split in ["train", "validation", "test"]}


def build_one_year_model_scores_v4_6(
    dataset: pd.DataFrame,
    *,
    dataset_sha256: str,
    ml_run_id: str,
) -> pd.DataFrame:
    ml_frame = prepare_one_year_ml_frame_v4_6(dataset)
    slices = get_one_year_training_slices_v4_6(ml_frame)
    train = slices["train"]
    if train.empty:
        return pd.DataFrame(columns=ML_SCORE_COLUMNS_V4_6)

    score_frames: list[pd.DataFrame] = []
    train_features = train[ALLOWED_FEATURE_COLUMNS_V4_6]
    train_target = train[TARGET_NAME_V4_6].astype(str)
    feature_columns_sha256 = get_feature_columns_sha256_v4_6()

    for model_name in MODEL_NAMES_V4_6:
        for split_name, split_frame in slices.items():
            ...
            result = fit_predict_model(
                ...
                train_features,
                train_target,
                split_frame[ALLOWED_FEATURE_COLUMNS_V4_6],
            )
            scores = split_frame[
            ...
            scores["target_name"] = TARGET_NAME_V4_6
            scores["dataset_sha256"] = dataset_sha256
            scores["feature_columns_sha256"] = feature_columns_sha256
            scores["ml_schema_version"] = ML_SCHEMA_VERSION_V4_6
            scores["target_value"] = split_frame[TARGET_NAME_V4_6].astype(str).to_numpy()
            ...
            score_frames.append(scores[ML_SCORE_COLUMNS_V4_6])

    return pd.concat(score_frames, ignore_index=True)


def build_one_year_ml_markdown_v4_6(manifest: dict[str, Any]) -> str:
    lines = [
...
def _validate_dataset_layer(root: Path) -> None:
    ...
    if not result["passed"]:
        raise RuntimeError(f"V4.5 dataset validation failed before V4.6: {result['errors']}")
...
def _sanity_from_quality(dataset: pd.DataFrame, quality: dict[str, Any]) -> dict[str, Any]:
    used = prepare_one_year_ml_frame_v4_6(dataset)
    return {
...
def _target_classes(frame: pd.DataFrame, split: str) -> list[str]:
    return sorted(frame[frame["split"] == split][TARGET_NAME_V4_6].dropna().astype(str).unique().tolist())
