...
def assess_ohlcv_trades_ml_quality_v8_5(dataset: pd.DataFrame, scores: pd.DataFrame, timeframe: str) -> dict[str, Any]:
    ...
    forbidden_feature_columns = find_forbidden_feature_columns_v8_5(ALLOWED_FEATURE_COLUMNS_V8_5)
    forbidden_output_columns = find_forbidden_output_columns_v8_5(scores.columns)
    errors: list[str] = []
    if forbidden_feature_columns:
        errors.append(f"V8.5 forbidden feature columns for {timeframe}: {forbidden_feature_columns}")
    if forbidden_output_columns:
        errors.append(f"V8.5 forbidden output columns for {timeframe}: {forbidden_output_columns}")
    if TARGET_NAME_V8_5 != "up_down_flat_h1":
        errors.append("V8.5 target name invalid")
    split_valid = _split_temporal_order_valid(used)
    if len(used) > 0 and not split_valid:
        errors.append(f"V8.5 split temporal order invalid for {timeframe}")
    if len(scores) > 0 and "walk_forward_group" not in scores.columns:
        errors.append(f"V8.5 walk_forward_group missing in scores for {timeframe}")
    if len(scores) > 0 and scores["walk_forward_group"].isna().any():
        errors.append(f"V8.5 walk_forward_group has null values in scores for {timeframe}")
    return {
...
def find_forbidden_feature_columns_v8_5(columns: list[str] | pd.Index) -> list[str]:
...
def find_forbidden_output_columns_v8_5(columns: list[str] | pd.Index) -> list[str]:
    forbidden_exact = {column.casefold() for column in FORBIDDEN_OUTPUT_COLUMNS_EXACT_V8_5}
