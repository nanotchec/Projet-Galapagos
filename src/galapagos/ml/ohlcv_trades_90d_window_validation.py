...
from galapagos.ml.ohlcv_trades_90d_window import (
    ...
    load_v7_9_dataset_manifest,
    prepare_ohlcv_trades_ml_frame_v8_0,
    score_output_path,
    validate_split_alignment_v8_0,
)
...
from galapagos.ml.ohlcv_trades_90d_window_quality import assess_ohlcv_trades_ml_quality_v8_0
from galapagos.ml.schemas import (
    ...
    MANIFEST_PATH_V8_0,
    ML_SCHEMA_VERSION_V8_0,
    ML_SCORE_COLUMNS_V8_0,
    MODEL_NAMES_V8_0,
    REPORT_JSON_PATH_V8_0,
    REPORT_MD_PATH_V8_0,
    SAFETY_FLAGS_V8_0,
    SCORES_JSON_PATH_V8_0,
    SCORES_MD_PATH_V8_0,
    TARGET_NAME_V8_0,
    TIMEFRAMES_V8_0,
    VERSION_V8_0,
    get_feature_columns_sha256_v8_0,
)
...
ML_RUN_ID_PATTERN_V8_0 = re.compile(r"^v8_0_[0-9]{8}T[0-9]{6}Z_[0-9a-f]{8}$")
...
def validate_ohlcv_trades_90d_offline_ml_research_v8_0(project_root: Path = Path(".")) -> dict[str, Any]:
    project_root = project_root.resolve()
    errors: list[str] = []
    warnings: list[str] = []

    dataset_validation = _validate_v7_9_dataset_layer(project_root)
    if dataset_validation:
        return _result(dataset_validation, warnings)

    manifest_path = project_root / MANIFEST_PATH_V8_0
    report_path = project_root / REPORT_JSON_PATH_V8_0
    scores_json_path = project_root / SCORES_JSON_PATH_V8_0
    if not manifest_path.exists():
        return _result([f"missing V8.0 manifest: {MANIFEST_PATH_V8_0}"], warnings)
    if not report_path.exists():
        return _result([f"missing V8.0 quality report: {REPORT_JSON_PATH_V8_0}"], warnings)
    if not scores_json_path.exists():
        return _result([f"missing V8.0 scores report: {SCORES_JSON_PATH_V8_0}"], warnings)

    dataset_manifest = load_v7_9_dataset_manifest(project_root)
    ...
    errors.extend(scan_payload_for_forbidden_claims(manifest, "V8.0 manifest"))
    errors.extend(_validate_report(manifest, report))
    errors.extend(scan_payload_for_forbidden_claims(report, "V8.0 quality report"))
    errors.extend(_validate_scores_report(manifest, scores_report))
    errors.extend(scan_payload_for_forbidden_claims(scores_report, "V8.0 scores report"))
    ...
    if recomputed_metrics != manifest.get("metrics"):
        errors.append("V8.0 manifest metrics mismatch")
    if recomputed_walk_forward_metrics != manifest.get("walk_forward_metrics"):
        errors.append("V8.0 manifest walk_forward_metrics mismatch")
...
def _validate_manifest_structure(root: Path, manifest: dict[str, Any], dataset_manifest: dict[str, Any]) -> list[str]:
    ...
    if not isinstance(manifest.get("ml_run_id"), str) or ML_RUN_ID_PATTERN_V8_0.fullmatch(manifest["ml_run_id"]) is None:
        errors.append("V8.0 manifest ml_run_id invalid")
    errors.extend(validate_exact_keys(manifest.get("input_dataset_manifest", {}), INPUT_MANIFEST_KEYS, "V8.0 manifest input_dataset_manifest"))
    expected_window = dataset_manifest["input_features_manifest"]
    input_manifest = manifest.get("input_dataset_manifest", {})
    if input_manifest.get("path") != MANIFEST_PATH_V7_9.as_posix():
        errors.append("V8.0 input_dataset_manifest.path mismatch")
    if input_manifest.get("sha256") != sha256_file(root / MANIFEST_PATH_V7_9):
        errors.append("V8.0 input_dataset_manifest.sha256 mismatch")
    for key in ["window_start", "window_end", "total_days"]:
        if input_manifest.get(key) != expected_window.get(key):
            errors.append(f"V8.0 input_dataset_manifest.{key} mismatch")
    ...
    if manifest.get("feature_columns_count") != len(ALLOWED_FEATURE_COLUMNS_V8_0):
        errors.append("V8.0 feature_columns_count mismatch")
    if manifest.get("target_name") != TARGET_NAME_V8_0:
        errors.append("V8.0 target_name mismatch")
    if manifest.get("feature_columns") != ALLOWED_FEATURE_COLUMNS_V8_0:
        errors.append("V8.0 feature_columns mismatch")
    errors.extend(_validate_feature_columns(manifest.get("feature_columns", [])))
    if manifest.get("models") != MODEL_NAMES_V8_0:
        errors.append("V8.0 models mismatch")
    expected_comparison = build_comparison_to_references_v8_0(root, manifest.get("metrics", {}))
    if manifest.get("comparison_to_references") != expected_comparison:
        errors.append("V8.0 comparison_to_references mismatch")
    ...
    for section in ["input_datasets", "input_splits", "outputs", "quality", "sanity_checks"]:
        errors.extend(validate_exact_keys(manifest.get(section, {}), TIMEFRAME_KEYS, f"V8.0 manifest {section}"))
    for timeframe in TIMEFRAMES_V8_0:
        errors.extend(validate_exact_keys(manifest.get("input_datasets", {}).get(timeframe, {}), INPUT_KEYS, f"V8.0 manifest input_datasets.{timeframe}"))
        errors.extend(validate_exact_keys(manifest.get("input_splits", {}).get(timeframe, {}), INPUT_KEYS, f"V8.0 manifest input_splits.{timeframe}"))
        errors.extend(validate_exact_keys(manifest.get("outputs", {}).get(timeframe, {}), OUTPUT_KEYS, f"V8.0 manifest outputs.{timeframe}"))
        errors.extend(validate_exact_keys(manifest.get("quality", {}).get(timeframe, {}), QUALITY_KEYS, f"V8.0 manifest quality.{timeframe}"))
        errors.extend(validate_exact_keys(manifest.get("sanity_checks", {}).get(timeframe, {}), SANITY_KEYS, f"V8.0 manifest sanity_checks.{timeframe}"))
    errors.extend(validate_exact_keys(manifest.get("safety", {}), set(SAFETY_FLAGS_V8_0), "V8.0 manifest safety"))
    return errors


def _validate_feature_columns(columns: Any) -> list[str]:
    if not isinstance(columns, list):
        return ["V8.0 feature_columns must be list"]
    ...
    for column in columns:
        ...
        if folded in forbidden_exact or folded.startswith(forbidden_prefixes):
            forbidden.append(column)
    return [f"V8.0 forbidden feature columns: {forbidden}"] if forbidden else []


def _validate_report(manifest: dict[str, Any], report: dict[str, Any]) -> list[str]:
    errors = validate_exact_keys(report, MANIFEST_KEYS, "V8.0 quality report")
    if report != manifest:
        errors.append("V8.0 quality report mismatch")
    return errors
...
def _validate_scores_report(manifest: dict[str, Any], report: dict[str, Any]) -> list[str]:
    ...
    if report != expected:
        errors.append("V8.0 scores report mismatch")
    return errors
...
def _validate_timeframe(
    ...
    for label, path in [("dataset", dataset_path), ("split", split_path), ("scores", score_path)]:
        if not path.exists():
            errors.append(f"missing V8.0 {label} file for {timeframe}: {path.relative_to(root)}")
    if errors:
        return errors

    dataset = read_parquet(dataset_path)
    split = read_parquet(split_path)
    dataset_for_ml = validate_split_alignment_v8_0(dataset, split)
    scores = read_parquet(score_path)
    dataset_sha = sha256_file(dataset_path)
    errors.extend(_compare_input_block(manifest["input_datasets"][timeframe], dataset_path, dataset_sha, len(dataset), root, f"V8.0 manifest input_datasets.{timeframe}"))
    errors.extend(_compare_input_block(manifest["input_splits"][timeframe], split_path, sha256_file(split_path), len(split), root, f"V8.0 manifest input_splits.{timeframe}"))
    errors.extend(_compare_output_block(manifest["outputs"][timeframe], score_path, sha256_file(score_path), len(scores), root, f"V8.0 manifest outputs.{timeframe}"))
    ...
    quality = assess_ohlcv_trades_ml_quality_v8_0(dataset_for_ml, scores, timeframe)
    physical_quality[timeframe] = quality
    errors.extend(quality["errors"])
    recomputed_metrics.update(compute_ohlcv_trades_classification_metrics_v8_0(scores))
    recomputed_walk_forward_metrics.update(compute_ohlcv_trades_walk_forward_metrics_v8_0(scores))
    return errors
...
def _validate_score_frame_schema_only(scores: pd.DataFrame, timeframe: str = "") -> list[str]:
    ...
    if list(scores.columns) != ML_SCORE_COLUMNS_V8_0:
        errors.append(f"V8.0 score schema mismatch{label}")
    forbidden_output = [
        column for column in scores.columns if str(column).casefold() in {item.casefold() for item in FORBIDDEN_OUTPUT_COLUMNS_EXACT_V8_0}
    ]
    if forbidden_output:
        errors.append(f"V8.0 score forbidden columns{label}: {forbidden_output}")
    if "walk_forward_group" not in scores.columns:
        errors.append(f"V8.0 score walk_forward_group missing{label}")
    if {"prediction_available_ts", "decision_ts"}.issubset(scores.columns) and not scores["prediction_available_ts"].ge(scores["decision_ts"]).all():
        errors.append(f"V8.0 prediction_available_ts invalid{label}")
    if len(scores) > 0:
        if "model_name" in scores.columns and set(scores["model_name"].unique()) != set(MODEL_NAMES_V8_0):
            errors.append(f"V8.0 score models mismatch{label}")
        if "target_name" in scores.columns and set(scores["target_name"].unique()) != {TARGET_NAME_V8_0}:
            errors.append(f"V8.0 score target mismatch{label}")
        if "feature_columns_sha256" in scores.columns and set(scores["feature_columns_sha256"].unique()) != {get_feature_columns_sha256_v8_0()}:
            errors.append(f"V8.0 score feature_columns_sha256 mismatch{label}")
        if "ml_schema_version" in scores.columns and set(scores["ml_schema_version"].unique()) != {ML_SCHEMA_VERSION_V8_0}:
            errors.append(f"V8.0 score schema version mismatch{label}")
    return errors


def _validate_score_values(dataset: pd.DataFrame, scores: pd.DataFrame, timeframe: str, dataset_sha: str) -> list[str]:
    errors: list[str] = []
    used = prepare_ohlcv_trades_ml_frame_v8_0(dataset)
    expected_rows = len(used) * len(MODEL_NAMES_V8_0)
    if len(scores) != expected_rows:
        errors.append(f"V8.0 score row count mismatch for {timeframe}")
    if len(scores) and set(scores["dataset_sha256"].unique()) != {dataset_sha}:
        errors.append(f"V8.0 score dataset_sha256 mismatch for {timeframe}")
    if len(scores) and not scores["row_valid_for_ml"].eq(True).all():
        errors.append(f"V8.0 score row_valid_for_ml mismatch for {timeframe}")
    if len(scores) and set(scores["target_value"].dropna().astype(str).unique()) - {"DOWN", "FLAT", "UP"}:
        errors.append(f"V8.0 score target classes invalid for {timeframe}")
    if len(scores) and scores["walk_forward_group"].isna().any():
        errors.append(f"V8.0 score walk_forward_group null for {timeframe}")
    if len(scores):
        ...
        if not score_groups.issubset(expected_groups):
            errors.append(f"V8.0 score walk_forward_group unexpected for {timeframe}")
    return errors
...
def _compare_quality(manifest: dict[str, Any], physical_quality: dict[str, dict[str, Any]]) -> list[str]:
    ...
    for timeframe, quality in physical_quality.items():
        if manifest["quality"].get(timeframe) != quality:
            errors.append(f"V8.0 manifest quality mismatch for {timeframe}")
    return errors


def _validate_safety(safety: Any) -> list[str]:
    if not isinstance(safety, dict):
        return ["V8.0 manifest safety must be object"]
...
def _find_forbidden_v8_0_artifacts(root: Path) -> list[str]:
    ...
    data_v8_0_ml = root / "data/research/v8_0/ml"
    if data_v8_0_ml.exists():
        for child in sorted(data_v8_0_ml.rglob("*")):
            if not child.is_file():
                continue
            relative = child.relative_to(root)
            if _is_allowed_v8_0_ml_score_path(relative):
                continue
            errors.append(f"Forbidden V8.0 artifact detected: {relative.as_posix()}")
...
def _is_allowed_v8_0_ml_score_path(relative: Path) -> bool:
    ...
    return (
        ...
        and parts[2] == "v8_0"
        and parts[3] == "ml"
        and parts[4] == "offline_research_ohlcv_trades_90d"
        and parts[5] == "source=binance_archive"
        and parts[6] == "market_type=spot"
        and parts[7] == "symbol=BTCUSDT"
        and parts[8] in {f"timeframe={timeframe}" for timeframe in TIMEFRAMES_V8_0}
...
def _validate_markdown(root: Path) -> list[str]:
    ...
    for relative, label in [
        (REPORT_MD_PATH_V8_0, "V8.0 Markdown report"),
        (SCORES_MD_PATH_V8_0, "V8.0 scores Markdown"),
        (DOC_PATH_V8_0, "V8.0 documentation"),
    ]:
        path = root / relative
        if not path.exists():
...
def _scan_metrics_for_forbidden_terms(metrics: Any, label: str) -> list[str]:
    ...
    return [f"V8.0 {label} contain forbidden trading metric: {term}" for term in FORBIDDEN_METRIC_TERMS_V8_0 if term in text]


def _validate_metric_bounds(metrics: Any, label: str) -> list[str]:
    errors: list[str] = []
    if not isinstance(metrics, dict):
        return [f"V8.0 {label} must be object"]
    ...
    for metric_key, payload in metrics.items():
        if not isinstance(payload, dict):
            errors.append(f"V8.0 {label}.{metric_key} must be object")
            continue
        for name in bounded_names:
            value = payload.get(name)
            if not isinstance(value, (int, float)) or not 0 <= float(value) <= 1:
                errors.append(f"V8.0 {label}.{metric_key}.{name} out of bounds")
        ...
        if not isinstance(rows, int) or rows < 0:
            errors.append(f"V8.0 {label}.{metric_key}.rows invalid")
    return errors


def _validate_comparison_bounds(comparison: Any) -> list[str]:
    if not isinstance(comparison, dict):
        return ["V8.0 comparison_to_references must be object"]
    errors: list[str] = []
    delta_names = {
        "accuracy_delta_v8_0_minus_reference",
        "balanced_accuracy_delta_v8_0_minus_reference",
        "macro_f1_delta_v8_0_minus_reference",
    }
    for reference_name, reference_payload in comparison.items():
        if not isinstance(reference_payload, dict):
            errors.append(f"V8.0 comparison_to_references.{reference_name} must be object")
            continue
        comparisons = reference_payload.get("comparisons", {})
        if not isinstance(comparisons, dict):
            errors.append(f"V8.0 comparison_to_references.{reference_name}.comparisons must be object")
            continue
        for key, payload in comparisons.items():
            if not isinstance(payload, dict):
                errors.append(f"V8.0 comparison_to_references.{reference_name}.{key} must be object")
                continue
            for name in delta_names:
                value = payload.get(name)
                if value is not None and (not isinstance(value, (int, float)) or not -1 <= float(value) <= 1):
                    errors.append(f"V8.0 comparison_to_references.{reference_name}.{key}.{name} out of bounds")
    return errors


def _validate_v7_9_dataset_layer(root: Path) -> list[str]:
    ...
    if result["passed"]:
        return []
    return [f"V7.9 dataset validation failed before V8.0: {result['errors']}"]
