...
from galapagos.ml.ohlcv_trades_robustness import (
    ACCURACY_GAP_WARNING_THRESHOLD_V7_5,
    DOC_PATH_V7_5,
    EXPECTED_LIMITATIONS_V7_5,
    FORBIDDEN_ROBUSTNESS_METRIC_TERMS_V7_5,
    LABEL_SHUFFLE_RANDOM_SEED_V7_5,
    MACRO_F1_GAP_WARNING_THRESHOLD_V7_5,
    MANIFEST_PATH_V7_5,
    REPORT_JSON_PATH_V7_5,
    REPORT_MD_PATH_V7_5,
    SAFETY_FLAGS_V7_5,
    VERSION_V7_5,
)
from galapagos.ml.schemas import (
    ...
    MANIFEST_PATH_V7_4,
    TIMEFRAMES_V7_4,
    get_ohlcv_trades_ml_score_path_v7_4,
)
from galapagos.validation.safety import validate_exact_keys, validate_markdown_forbidden_claims


ROBUSTNESS_RUN_ID_PATTERN_V7_5 = re.compile(r"^v7_5_[0-9]{8}T[0-9]{6}Z_[0-9a-f]{8}$")
...
ANALYSES_KEYS = {
    ...
    "ohlcv_trades_vs_references_comparison",
...
FORBIDDEN_ARTIFACT_ROOTS = [
    ...
    Path("orders"),
    Path("execution"),
    Path("data/research/v7_5/backtests"),
    Path("data/research/v7_5/strategies"),
    Path("data/research/v7_5/orders"),
    Path("data/research/v7_5/execution"),
    Path("data/research/v7_5/models"),
    Path("data/research/v7_5/checkpoints"),
]
...
def validate_ohlcv_trades_ml_robustness_v7_5(project_root: Path = Path(".")) -> dict[str, Any]:
    project_root = project_root.resolve()
    errors: list[str] = []
    warnings: list[str] = []

    manifest_path = project_root / MANIFEST_PATH_V7_5
    report_path = project_root / REPORT_JSON_PATH_V7_5
    if not manifest_path.exists():
        return _result([f"missing V7.5 manifest: {MANIFEST_PATH_V7_5}"], warnings)
    if not report_path.exists():
        return _result([f"missing V7.5 report: {REPORT_JSON_PATH_V7_5}"], warnings)
    ...
    errors.extend(_find_forbidden_v7_5_artifacts(project_root))
    return _result(errors, warnings, manifest)


def _validate_manifest_structure(manifest: dict[str, Any]) -> list[str]:
    errors: list[str] = []
    errors.extend(validate_exact_keys(manifest, MANIFEST_KEYS, "V7.5 manifest"))
    if manifest.get("version") != VERSION_V7_5:
        errors.append("V7.5 manifest version mismatch")
    if manifest.get("status") != "PASS":
        errors.append("V7.5 manifest status must be PASS")
    if not _is_iso_utc(manifest.get("created_at_utc")):
        errors.append("V7.5 manifest created_at_utc invalid")
    if not isinstance(manifest.get("robustness_run_id"), str) or ROBUSTNESS_RUN_ID_PATTERN_V7_5.fullmatch(manifest["robustness_run_id"]) is None:
        errors.append("V7.5 robustness_run_id invalid")
    errors.extend(validate_exact_keys(manifest.get("input_dataset_manifest", {}), INPUT_MANIFEST_KEYS, "V7.5 input_dataset_manifest"))
    errors.extend(validate_exact_keys(manifest.get("input_ml_manifest", {}), INPUT_MANIFEST_KEYS, "V7.5 input_ml_manifest"))
    errors.extend(validate_exact_keys(manifest.get("input_score_files", {}), TIMEFRAME_KEYS, "V7.5 input_score_files"))
    errors.extend(validate_exact_keys(manifest.get("reference_reports", {}), REFERENCE_REPORT_KEYS, "V7.5 reference_reports"))
    for timeframe in TIMEFRAMES_V7_4:
        errors.extend(validate_exact_keys(manifest.get("input_score_files", {}).get(timeframe, {}), SCORE_FILE_KEYS, f"V7.5 input_score_files.{timeframe}"))
    errors.extend(validate_exact_keys(manifest.get("analyses", {}), ANALYSES_KEYS, "V7.5 analyses"))
    errors.extend(validate_exact_keys(manifest.get("thresholds", {}), THRESHOLD_KEYS, "V7.5 thresholds"))
    errors.extend(validate_exact_keys(manifest.get("findings", {}), FINDINGS_KEYS, "V7.5 findings"))
    errors.extend(validate_exact_keys(manifest.get("safety", {}), set(SAFETY_FLAGS_V7_5), "V7.5 safety"))
    if manifest.get("thresholds", {}).get("accuracy_gap_warning_threshold") != ACCURACY_GAP_WARNING_THRESHOLD_V7_5:
        errors.append("V7.5 accuracy threshold mismatch")
    if manifest.get("thresholds", {}).get("macro_f1_gap_warning_threshold") != MACRO_F1_GAP_WARNING_THRESHOLD_V7_5:
        errors.append("V7.5 macro F1 threshold mismatch")
    if manifest.get("thresholds", {}).get("random_seed") != LABEL_SHUFFLE_RANDOM_SEED_V7_5:
        errors.append("V7.5 random seed mismatch")
    if manifest.get("limitations") != EXPECTED_LIMITATIONS_V7_5:
        errors.append("V7.5 limitations mismatch")
    analyses = manifest.get("analyses", {})
    for key in ANALYSES_KEYS:
        if not analyses.get(key):
            errors.append(f"V7.5 analysis missing or empty: {key}")
    feature_scan = analyses.get("feature_leakage_scan", {})
    if feature_scan.get("feature_leakage_detected") is not False:
        errors.append("V7.5 feature leakage scan failed")
    metric_scan = analyses.get("metric_forbidden_scan", {})
    if metric_scan.get("metric_forbidden_terms_detected") is not False:
        errors.append("V7.5 metric forbidden scan failed")
    return errors


def _validate_report(manifest: dict[str, Any], report: dict[str, Any]) -> list[str]:
    errors = validate_exact_keys(report, MANIFEST_KEYS, "V7.5 report")
    if report != manifest:
        errors.append("V7.5 report JSON mismatch")
    return errors


def _validate_input_hashes(root: Path, manifest: dict[str, Any]) -> list[str]:
    errors: list[str] = []
    errors.extend(_validate_path_sha(root, manifest.get("input_dataset_manifest", {}), MANIFEST_PATH_V7_3, "V7.5 input_dataset_manifest"))
    errors.extend(_validate_path_sha(root, manifest.get("input_ml_manifest", {}), MANIFEST_PATH_V7_4, "V7.5 input_ml_manifest"))
    dataset_manifest_path = root / MANIFEST_PATH_V7_3
    ml_manifest_path = root / MANIFEST_PATH_V7_4
    if dataset_manifest_path.exists():
        ...
        if manifest.get("input_dataset_manifest", {}).get("feature_columns_count") != dataset_manifest.get("feature_columns_count"):
            errors.append("V7.5 input_dataset_manifest.feature_columns_count mismatch")
    if ml_manifest_path.exists():
        ...
        if manifest.get("input_ml_manifest", {}).get("feature_columns_count") != ml_manifest.get("feature_columns_count"):
            errors.append("V7.5 input_ml_manifest.feature_columns_count mismatch")
...
def _validate_reference_reports(root: Path, block: Any) -> list[str]:
    if not isinstance(block, dict):
        return ["V7.5 reference_reports must be object"]
    errors: list[str] = []
    if not isinstance(block.get("v6_2_available"), bool):
        errors.append("V7.5 reference_reports.v6_2_available must be bool")
    if not isinstance(block.get("v5_4_available"), bool):
        errors.append("V7.5 reference_reports.v5_4_available must be bool")
    if not isinstance(block.get("warnings"), list):
        errors.append("V7.5 reference_reports.warnings must be list")
    for version_key, expected_path in [("v6_2", MANIFEST_PATH_V6_2), ("v5_4", MANIFEST_PATH_V5_4)]:
        ...
        if block.get(path_key) != expected_path.as_posix():
            errors.append(f"V7.5 reference_reports.{path_key} mismatch")
        path = root / expected_path
        if block.get(available_key) is True:
            if not path.exists():
                errors.append(f"V7.5 reference_reports missing {version_key} manifest")
            elif block.get(sha_key) != sha256_file(path):
                errors.append(f"V7.5 reference_reports.{sha_key} mismatch")
        elif block.get(sha_key) is not None:
            errors.append(f"V7.5 reference_reports.{sha_key} must be null when unavailable")
    return errors
...
def _validate_score_files(root: Path, manifest: dict[str, Any]) -> list[str]:
    ...
    for timeframe in TIMEFRAMES_V7_4:
        expected_path = get_ohlcv_trades_ml_score_path_v7_4(root, timeframe, window_start, window_end)
        block = manifest.get("input_score_files", {}).get(timeframe, {})
        if (root / Path(str(block.get("path", "")))).resolve() != expected_path.resolve():
            errors.append(f"V7.5 input_score_files.{timeframe}.path mismatch")
            continue
        if not expected_path.exists():
            errors.append(f"missing V7.5 input score file: {expected_path.relative_to(root)}")
            continue
        if block.get("sha256") != sha256_file(expected_path):
            errors.append(f"V7.5 input_score_files.{timeframe}.sha256 mismatch")
        scores = read_parquet(expected_path)
        if block.get("rows") != len(scores):
            errors.append(f"V7.5 input_score_files.{timeframe}.rows mismatch")
    return errors


def _validate_findings(findings: Any) -> list[str]:
    if not isinstance(findings, dict):
        return ["V7.5 findings must be object"]
    errors: list[str] = []
    for key in [
        "robust_edge_claimed",
        "strategy_validated",
        ...
        if findings.get(key) is not False:
            errors.append(f"V7.5 finding {key} must be False")
    if not isinstance(findings.get("warnings"), list):
        errors.append("V7.5 findings warnings must be list")
    return errors


def _validate_safety(safety: Any) -> list[str]:
    if not isinstance(safety, dict):
        return ["V7.5 safety must be object"]
    return [f"V7.5 safety flag {key} must be {value}" for key, value in SAFETY_FLAGS_V7_5.items() if safety.get(key) is not value]
...
def _scan_metrics_for_forbidden_terms(payload: Any) -> list[str]:
    ...
    return [f"V7.5 metrics contain forbidden trading metric: {term}" for term in FORBIDDEN_ROBUSTNESS_METRIC_TERMS_V7_5 if term in text]
...
def _validate_metric_field(field: str, value: Any, path: str, errors: list[str]) -> None:
    if field in EXPECTED_BOOL_FIELDS and not isinstance(value, bool):
        errors.append(f"V7.5 metric bound violation: {path} = {value}")
        return
...
def _validate_markdown(root: Path) -> list[str]:
    errors: list[str] = []
    for relative, label in [(REPORT_MD_PATH_V7_5, "V7.5 Markdown report"), (DOC_PATH_V7_5, "V7.5 documentation")]:
        path = root / relative
        if not path.exists():
...
def _find_forbidden_v7_5_artifacts(root: Path) -> list[str]:
    errors: list[str] = []
    for relative in FORBIDDEN_ARTIFACT_ROOTS:
        path = root / relative
        if path.exists():
            errors.append(f"Forbidden V7.5 artifact detected: {relative.as_posix()}")
            for child in sorted(path.rglob("*")):
                if child.is_file():
                    errors.append(f"Forbidden V7.5 artifact detected: {child.relative_to(root).as_posix()}")
    for path in root.rglob("*"):
        if ".git" in path.parts or ".venv" in path.parts or not path.is_file():
            continue
        if path.suffix.casefold() in PERSISTENT_MODEL_SUFFIXES:
            errors.append(f"Forbidden V7.5 artifact detected: {path.relative_to(root).as_posix()}")
...
def _result(errors: list[str], warnings: list[str], manifest: dict[str, Any] | None = None) -> dict[str, Any]:
    return {"version": VERSION_V7_5, "passed": not errors, "errors": errors, "warnings": warnings, "manifest": manifest}
