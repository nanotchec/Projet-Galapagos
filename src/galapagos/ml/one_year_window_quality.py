...
def assess_one_year_ml_quality_v4_6(dataset: pd.DataFrame, scores: pd.DataFrame, timeframe: str) -> dict[str, Any]:
    ...
    forbidden_feature_columns = [
        column
        for column in ALLOWED_FEATURE_COLUMNS_V4_6
        if any(term in column.casefold() for term in FORBIDDEN_FEATURE_TERMS_V4_6)
    ]
    forbidden_output_columns = [
        column
        for column in scores.columns
        if any(term in column.casefold() for term in FORBIDDEN_OUTPUT_TERMS_V4_6)
    ]
    errors: list[str] = []
    if forbidden_feature_columns:
        errors.append(f"V4.6 forbidden feature columns for {timeframe}: {forbidden_feature_columns}")
    if forbidden_output_columns:
        errors.append(f"V4.6 forbidden output columns for {timeframe}: {forbidden_output_columns}")
    if TARGET_NAME_V4_6 != "up_down_flat_h1":
        errors.append("V4.6 target name invalid")
    split_valid = _split_temporal_order_valid(used)
    if len(used) > 0 and not split_valid:
        errors.append(f"V4.6 split temporal order invalid for {timeframe}")
    return {
